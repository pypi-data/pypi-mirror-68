...
class WMLEngine(BaseEngine):
    ...
    def _initialize_wml_pipeline_metadata(self) -> None:
                        ...
                        'parameters': {
                            ...
                            'optimization': {
                                ...
                                'train_sample_rows_test_size':
                                    self._auto_pipelines_parameters.get('train_sample_rows_test_size'),
        ...
        if not self._wml_client.ICP:
            self._wml_pipeline_metadata[
                self._wml_client.pipelines.ConfigurationMetaNames.DOCUMENT]['runtimes'][0]['version'] = self._auto_pipelines_parameters.get('autoai_pod_version', '0.1.418')
    ...
    def summary(self) -> 'DataFrame':
        ...
        details = self._wml_client.training.get_details(training_uid=self._current_run_id)
        columns = (['Pipeline Name', 'Number of enhancements'] +
            ...
            if pipeline['context']['phase'] == 'global_output':
                ...
                values.append(
                    ([f"Pipeline_{pipeline['context']['intermediate_model']['name'].split('P')[-1]}"] +
                     ...
                     [metric for metric in pipeline['ml_metrics'].values()]
                     ))
        ...
        try:
            if (MetricsToDirections[self._auto_pipelines_parameters['scoring'].upper()].value ==
                    Directions.ASCENDING):
                return pipelines.sort_values(
                    by=[f"training_{self._auto_pipelines_parameters['scoring']}"], ascending=False).rename(
                    {
                        f"training_{self._auto_pipelines_parameters['scoring']}":
                            f"training_{self._auto_pipelines_parameters['scoring']}_(optimized)"
                    }, axis='columns')

            else:
                return pipelines.sort_values(by=[f"training_{self._auto_pipelines_parameters['scoring']}"]).rename(
                    {
                        f"training_{self._auto_pipelines_parameters['scoring']}":
                            f"training_{self._auto_pipelines_parameters['scoring']}_(optimized)"
                    }, axis='columns')
    ...
    def get_pipeline_details(self, pipeline_name: str = None) -> dict:
        ...
        for pipeline in run_params['entity']['status'].get('metrics', []):
            if (pipeline['context']['phase'] == 'global_output' and
                    pipeline['context']['intermediate_model']['name'].split('P')[-1] == pipeline_name[-1]):
