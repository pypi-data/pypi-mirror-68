...
class TestWMLClientWithScikitLearn(unittest.TestCase):
    ...
    def test_06_create_revision(self):
        ...
        self.client.repository.create_model_revision(TestWMLClientWithScikitLearn.model_uid)
        self.client.repository.list_models_revisions(TestWMLClientWithScikitLearn.model_uid)
        model_meta_props = { self.client.repository.ModelMetaNames.NAME: "updated scikit model",
                            }
        published_model_updated = self.client.repository.update(TestWMLClientWithScikitLearn.model_uid, model_meta_props,self.model_data['model'])
        self.assertIsNotNone(TestWMLClientWithScikitLearn.model_uid)
        self.assertTrue("updated scikit model" in str(published_model_updated))
        self.client.repository.create_model_revision(TestWMLClientWithScikitLearn.model_uid)
        rev_details = self.client.repository.get_model_revision(TestWMLClientWithScikitLearn.model_uid,2)
