...
import time
from watson_machine_learning_client.metanames import TrainingConfigurationMetaNames, TrainingConfigurationMetaNamesCp4d30
...
class Training(WMLResource):
    ...
    def run(self, meta_props, asynchronous=True):
        """
                ...
                **Examples**
                    **Example meta_props for Training run creation in Cloud Pack for Data version 3.1.0 or above:
                    >>> metadata = {
    ...
    def list_intermediate_models(self, training_uid):
        ...
        ##For CP4D, check if either spce or project ID is set
        if self._client.ICP_30:
            raise WMLClientError("This method is not supported for IBM Cloud Pack for Data. " )
        ...
        if(training_state=='completed'):
            ...
            if 'metrics' in details[u'entity'][u'status'] and details[u'entity'][u'status'].get('metrics') is not None:
                ...
                print(" There is no metrics available for this training uid. ")
        else:
