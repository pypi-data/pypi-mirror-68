...
class Customization(object):
    ...
    def parse_userdict(self, query, lac_tags):
        """使用人工干预词典修正lac模型的输出"""
        ...
        if not self.ac:
            logging.warning("userdict is not load")
            return
...
if __name__ == '__main__':
    ...
    custom.load_userdict('./icwb2_gold/pku_training_words.utf8')
    query = u"共同创造美好的新世纪——二○○一年新年贺词"
    tags = ['O'] * len(query)
    custom.parse_userdict(query, tags)
