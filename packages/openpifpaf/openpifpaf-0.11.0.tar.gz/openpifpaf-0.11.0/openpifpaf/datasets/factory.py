...
HUMAN36M_IMAGE_DIR_TRAIN = 'data-human36m-eccv2018/Train/IMG'
HUMAN36M_IMAGE_DIR_VAL = 'data-human36m-eccv2018/Val/IMG'
HUMAN36M_ANNOTATIONS_TRAIN = 'data-human36m-eccv2018/Train/POSE'
HUMAN36M_ANNOTATIONS_VAL = 'data-human36m-eccv2018/Val/POSE'
...
def train_cli(parser):
    ...
    group.add_argument('--human36m-train-image-dir', default=HUMAN36M_IMAGE_DIR_TRAIN)
    group.add_argument('--human36m-val-image-dir', default=HUMAN36M_IMAGE_DIR_VAL)
    group.add_argument('--human36m-train-annotations', default=HUMAN36M_ANNOTATIONS_TRAIN)
    group.add_argument('--human36m-val-annotations', default=HUMAN36M_ANNOTATIONS_VAL)
...
def train_human36m_factory(args, target_transforms):
    rescale_t = None
    if args.extended_scale:
        rescale_t = transforms.RescaleRelative(
            scale_range=(0.5 * args.rescale_images, 1.0 * args.rescale_images),
            power_law=True)
    orientation_t = None
    if args.orientation_invariant:
        orientation_t = transforms.RandomApply(transforms.RotateBy90(), args.orientation_invariant)
    preprocess = transforms.Compose([
        transforms.NormalizeAnnotations(),
        transforms.AnnotationJitter(),
        # transforms.RandomApply(transforms.HFlip(COCO_KEYPOINTS, HFLIP), 0.5),
        rescale_t,
        transforms.Crop(args.square_edge, use_area_of_interest=False),
    ...
    train_data = Human36m(
        image_dir=args.human36m_train_image_dir,
        ann_dir=args.human36m_train_annotations,
        preprocess=preprocess,
        target_transforms=target_transforms,
        n_images=args.n_images,
    )
    ...
    val_data = Human36m(
        image_dir=args.human36m_val_image_dir,
        ann_dir=args.human36m_val_annotations,
        preprocess=preprocess,
        target_transforms=target_transforms,
        n_images=args.n_images,
    )
...
def train_factory(args, target_transforms):
    ...
    if args.dataset in ('human36m',):
        return train_human36m_factory(args, target_transforms)
