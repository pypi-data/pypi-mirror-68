...
#--- CHANGES ------------------------------------------------------------------
# 2017-10-02 v0.0.43 PL: - Migrar a PYTHON 3.6
...
def ipv6to4(ip_address6=None):
    """Convertir direccion IPv6 a IPv4
    cuando se trate de una IPv4 con notacion IPv6
    p.ejemplo ::ffff:c000:280 o 0000:0000:0000:0000:0000:ffff:c000:280 que es la representacion de ::ffff:192.0.2.128
    los :: indican que todo el contenido que representan son grupos de 4 ceros, siendo la IPv6 8 grupos de 4 digitos hex
    IPv6 se compone de 8 grupos de numeros hexadecimales completados a 4 digitos con ceros por la izquierda, por tanto
    ::ffff:c000:0280"""
    list_ip4 = list()
    try:
        if ip_address6:
            list_ip6 = ip_address6.split(':')
            # completar grupos a 4 ceros
            for n in range(len(list_ip6)):
                list_ip6[n] = list_ip6[n].zfill(4)
            list_ip4.append(str(int(list_ip6[len(list_ip6)-2][0:2],16)))
            list_ip4.append(str(int(list_ip6[len(list_ip6)-2][2:4],16)))
            list_ip4.append(str(int(list_ip6[len(list_ip6)-1][0:2],16)))
            list_ip4.append(str(int(list_ip6[len(list_ip6)-1][2:4],16)))
            return '.'.join(list_ip4)
        else:
            return None
    except (AddressValueError, NetmaskValueError):
        pass
...
class _BaseV4(object):
    ...
    def _string_from_ip_int(self, ip_int):
        ...
        for _ in list(range(4)):
            octets.insert(0, repr(ip_int & 0xFF))
            ip_int >>= 8
        return '.'.join(octets)
...
class _BaseV6(object):
    ...
    def _ip_int_from_string(self, ip_str):
        ...
        try:
            skip_index, = ([i for i in list(range(1, len(parts) - 1)) if not parts[i]] or [None])
        ...
        try:
            ...
            ip_int = int('0')
            for i in list(range(parts_hi)):
                ip_int <<= 16
                ip_int |= self._parse_hextet(parts[i])
            ip_int <<= 16 * parts_skipped
            for i in list(range(-parts_lo, 0)):
                ip_int <<= 16
                ip_int |= self._parse_hextet(parts[i])
    ...
    def _explode_shorthand_ip_string(self):
        ...
        parts = []
        for i in list(range(self._HEXTET_COUNT)):
