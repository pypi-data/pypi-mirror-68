...
def compare_magnitude_along(self, *args, unit="SI", data_list=[], is_norm=False):
    ...
    *args: list of strings
    ...
    """
    if data_list == []:
        (axis, values) = self.get_magnitude_along(args, unit=unit, is_norm=is_norm)
        return (axis, [values])
    else:
        # Extract requested axes + field values
        result_list = self.get_magnitude_along(args, unit=unit, is_norm=is_norm)
        values = result_list[-1]
        axes = result_list[:-1]
        data_axis_values = []
        data_values = []
        for data in data_list:
            result_list = data.get_magnitude_along(args, unit=unit, is_norm=is_norm)
