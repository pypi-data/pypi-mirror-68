...
class BST(BinaryTree):
    ...
    def _is_bst_satisfied(node):
        ...
        if node.left:
            left_true = node.value > node.left.value and BST._is_bst_satisfied(node.left)
        if node.right:
    ...
    def _remove(node,value):
        ...
        if node.value > value:
            ...
            place_holder = node.right
            
            while place_holder.left:
                place_holder = place_holder.left

            node.value = place_holder.value
