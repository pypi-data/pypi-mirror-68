...
class MovieLensWideDeepKerasModel(DefaultKerasModel):
    """
    ...
    def adjust_parse_args_value(self):
        ...
        self.parser_args.model_version = '22'
    ...
    def load_data(self, *args, **kwargs):
        ...
        batched_train_dataset, batched_validation_dataset, batched_evaluate_dataset = MovieLensDatasetBuilder.split_full_datasets(full_datasets,
                                                                                                      ...
                                                                                                      validation_size=0,
                                                                                                      test_size=0.2)
        np_user_ids = np.array([], dtype=np.int32)
        np_item_ids = np.array([], dtype=np.int32)
        np_labels = np.array([])
        if self.is_inference_phase() or self.is_serving_phase():
    ...
    @print_exec_time
    def fit_xxx(self, *args, **kwargs):
        if not self.is_training_phase():
            return self
        loaded_dataset = kwargs.get("loaded_dataset")
        if loaded_dataset is not None:
            batched_train_dataset = loaded_dataset[0]
            batched_evaluate_dataset = loaded_dataset[1]
            self.model.get_proxy().fit(batched_train_dataset, epochs=self.parser_args.epochs,
    ...
    def fit(self, *args, **kwargs):
        ...
        # change to serving phase
        self.parser_args.train_phase = TrainPhaseEnum.Serving.value
        return self

    @print_exec_time
    def evaluate(self, evaluate_dataset=None, **kwargs):
    ...
    def save_model(self, *args, **kwargs):
        ...
        if not self.model.get_proxy().inputs and batched_evaluate_dataset is not None:
            self.log.info("change self.model.get_proxy().inputs in save_model()...........")
    ...
    def serving(self, *args, **kwargs):
        ...
        if not self.model.get_proxy().inputs:
            self.log.info("change self.model.get_proxy().inputs in serving()...........")
    ...
    @print_exec_time
    def predict_xxx(self, *args, **kwargs):
        """python版的预测接口，返回预测结果"""
        ...
        users = np.array([[1,3,6],[2,3,7]])
        predictions = self.model.get_proxy().predict([[1, 2], [3, 4]])
        predictions = np.array([a[0] for a in predictions])
        recommended_book_ids = (-predictions).argsort()
        # print("用户Id：", int(user_id))
        # print("推荐的新闻Id列表,Top(" + str(len(np_item_ids)) + ")：", recommended_book_ids)
