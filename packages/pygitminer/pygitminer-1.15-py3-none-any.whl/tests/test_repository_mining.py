...
from pygitminer import GitMining, GitRepository
...
@pytest.fixture
def repo(path):
    return list(GitMining(path_to_repo=path).traverse_commits())


@pytest.fixture
def repo_to(path, to):
    return list(GitMining(path_to_repo=path, to=to).traverse_commits())
...
def test_no_url():
    with pytest.raises(Exception):
        list(GitMining().traverse_commits())


# It should fail when URL is not a string or a List
def test_badly_formatted_repo_url():
    with pytest.raises(Exception):
        list(GitMining(path_to_repo=set('repo')).traverse_commits())
...
@pytest.mark.parametrize('path,to,expected', [
    ("https://github.com/ishepard/pygitminer.git",
     datetime(2018, 10, 20),
     159)
])
...
@pytest.mark.parametrize('path,to,expected', [
    (["https://github.com/mauricioaniche/repodriller.git",
      "https://github.com/ishepard/pygitminer"],
     datetime(2018, 10, 20),
     518)
])
...
@pytest.mark.parametrize('path,to,expected', [
    (["test-repos/small_repo", "https://github.com/ishepard/pygitminer.git"],
     datetime(2018, 10, 20),
     164)
])
...
@pytest.mark.parametrize('path,to,expected', [
    (["test-repos/small_repo", "https://github.com/mauricioaniche/repodriller.git",
      "test-repos/branches_merged", "https://github.com/ishepard/pygitminer.git"],
     datetime(2018, 10, 20),
     527)
])
...
def test_badly_formatted_url():
    ...
    with pytest.raises(Exception):
        list(GitMining(path_to_repo='test').traverse_commits())


@pytest.mark.parametrize('path', ["test-repos/histogram"])
def test_diff_without_histogram(git_repo):
    # without histogram
    commit = list(GitMining('test-repos/histogram',
...
def test_ignore_add_whitespaces():
    commit = list(GitMining('test-repos/whitespace',
...
def test_clone_repo_to(tmp_path):
    ...
    url = "https://github.com/ishepard/pygitminer.git"
...
def test_clone_repo_to_not_existing():
    with pytest.raises(Exception):
        list(GitMining("https://github.com/ishepard/pygitminer",
