...
def test_mod_with_file_types():
    lc = list(GitMining('test-repos/different_files',
    ...
    lc = list(GitMining('test-repos/different_files1',
...
def test_only_in_main_branch():
    lc = list(GitMining('test-repos/branches_not_merged').traverse_commits())
...
def test_only_no_merge():
    lc = list(GitMining('test-repos/branches_merged',
...
def test_no_filters():
    lc = list(GitMining('test-repos/different_files').traverse_commits())
...
def test_only_in_branch():
    lc = list(GitMining('test-repos/branches_not_merged',
...
def test_only_in_branches():
    ...
    assert len(list(GitMining('test-repos/branches_not_merged')
                    .traverse_commits())) == 3
    # only analyze b2
    assert len(list(GitMining('test-repos/branches_not_merged',
                                     only_in_branch='b2')
...
def test_only_in_branch_not_exist():
    with pytest.raises(Exception):
        list(GitMining('test-repos/branches_not_merged', only_in_branch='branch2')
             .traverse_commits())


def test_only_authors():
    lc = list(GitMining('test-repos/multiple_authors',
...
def test_only_commits():
    lc = list(GitMining('test-repos/complex_repo',
    ...
    total_commits = len(list(GitMining('test-repos/complex_repo').traverse_commits()))
...
def test_single_commit_head():
    ...
    lc_head = list(GitMining('test-repos/complex_repo', single="HEAD").traverse_commits())
...
def test_single_commit_not_existing_single_commit():
    with pytest.raises(Exception):
        list(GitMining('test-repos/complex_repo',
...
def test_only_releases():
    lc = list(GitMining('test-repos/tags',
...
def test_only_releases_wo_releases():
    lc = list(GitMining('test-repos/complex_repo', only_releases=True).traverse_commits())

    assert len(lc) == 0


def test_date_order():
    date_order = list(GitMining('test-repos/order', order='date-order').traverse_commits())
    author_date_order = list(GitMining('test-repos/order', order='author-date-order').traverse_commits())
...
def test_topo_order():
    topo_order = list(GitMining('test-repos/order', order='topo-order').traverse_commits())
...
def test_should_visit_ascendent_order():
    lc = list(GitMining('test-repos/small_repo').traverse_commits())
    assert len(lc) == 5
...
def test_should_visit_descendent_order():
    lc = list(GitMining('test-repos/small_repo',
...
def test_include_refs():
    commits_no_refs = list(GitMining('test-repos/branches_not_merged/',
    ...
    commits_with_refs = list(GitMining('test-repos/branches_not_merged/',
...
def test_include_remotes():
    repo = Repo('test-repos/pygitminer/')

    # Prove that a recent commit is not in 'test-repos/pygitminer'
    commits_before_fetch = [commit.hash for commit in GitMining('test-repos/pygitminer/').traverse_commits()]
    ...
    repo.git.fetch('--all')
    commits_after_fetch = [commit.hash for commit in GitMining('test-repos/pygitminer/').traverse_commits()]
    ...
    commits_with_remotes = [commit.hash for commit in GitMining('test-repos/pygitminer/', include_remotes=True).traverse_commits()]
