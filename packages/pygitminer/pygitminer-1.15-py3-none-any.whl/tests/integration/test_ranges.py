...
@pytest.fixture
def git_mining_st(path, since, to):
    return list(GitMining(path, since=since, to=to).traverse_commits())


@pytest.fixture
def git_mining_cc(path, from_commit, to_commit):
    return list(GitMining(path, from_commit=from_commit, to_commit=to_commit).traverse_commits())


@pytest.fixture
def git_mining_tt(path, from_tag, to_tag):
    return list(GitMining(path, from_tag=from_tag, to_tag=to_tag).traverse_commits())


@pytest.fixture
def git_mining_complex_tags(path, from_tag, to_tag):
    return list(GitMining('test-repos/tags',
                                 from_tag=from_tag,
...
def test_to_filter(git_mining_st, expected_commits):
    assert len(git_mining_st) == expected_commits
...
def test_since_filter(git_mining_st, expected_commits):
    assert len(git_mining_st) == expected_commits
...
def test_since_and_to_filters(git_mining_st, expected_commits):
    assert len(git_mining_st) == expected_commits
...
def test_to_commit_filter(git_mining_cc, expected_commits):
    assert len(git_mining_cc) == expected_commits
...
def test_from_commit_filter(git_mining_cc, expected_commits):
    assert len(git_mining_cc) == expected_commits
...
def test_from_and_to_commit(git_mining_cc, expected_commits):
    assert len(git_mining_cc) == expected_commits


def test_from_and_to_commit_with_merge_commit():
    commits = GitMining('test-repos/pygitminer',
...
def test_to_tag_filter_new(git_mining_tt, expected_commits):
    assert len(git_mining_tt) == expected_commits


@pytest.mark.parametrize('from_tag,expected_commits', [
    ('v1.4', 3),
])
def test_from_tag_filter(git_mining_tt, expected_commits):
    assert len(git_mining_tt) == expected_commits
...
def test_from_and_to_tag(git_mining_complex_tags, expected_commits):
    assert len(git_mining_complex_tags) == expected_commits
...
def test_multiple_filters_exceptions():
    ...
    with pytest.raises(Exception):
        for commit in GitMining('test-repos/small_repo/',
                                       since=dt1,
