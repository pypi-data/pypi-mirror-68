...
from pygitminer.domain.commit import ModificationType
from pygitminer.git_repository import GitRepository
...
def test_list_commits(repo):
    change_sets = list(repo.list_of_commits())
...
def test_get_last_modified_lines_simple(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('e6d3b38a9ef683e8184eac10a0471075c2808bbd'))
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_multiple(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('9942ee9dcdd1103e5808d544a84e6bc8cade0e54'))
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_rename_simple(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('45ba0a61ccc448625bce0fea0301cf0c1ab32696'))
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_multiple_rename(repo):
    # in this case the algorithm doesn't work because the file has been renamed 2 times!

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('9e858753b3d69f560cf72aaaa297f2608145ebcf'))
    assert len(buggy_commits) == 0


@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_rename_simple_more_commits(repo):

    buggy_commits = repo.get_last_modified_lines(
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_line_uselesss(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('3bc7295c16b7dfc15d5f82eb6962a2774e1b8420'))
    assert len(buggy_commits) == 1
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_line_uselesss2(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('4155c421ee5cbb3c34feee7b68aa78a2ee1bbeae'))
    assert len(buggy_commits) == 0


@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_for_single_file(repo):

    commit = repo.get_commit('0f726924f96621e4965039123098ba83e39ffba6')
    buggy_commits = None
    for mod in commit.modifications:
        if mod.filename == 'A.java':
            buggy_commits = repo.get_last_modified_lines(commit, mod)
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_with_more_modification(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit('c7002fb321a8ba32a28fac200538f7c2ba76f175'))
    assert len(buggy_commits) == 1
...
@pytest.mark.parametrize('path', ['test-repos/small_repo/'])
def test_get_modified_file(repo):

    commits = repo.get_modified_file('file2.java')
...
@pytest.mark.parametrize('path', ['test-repos/small_repo/'])
def test_get_modified_file_missing_file(repo):

    commits = repo.get_modified_file('non-existing-file.java')
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_hyper_blame(repo):

    buggy_commits = repo.get_last_modified_lines(repo.get_commit(
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_hyper_blame_unblamable(tmp_path,
                                                                repo):
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_hyper_blame_ignore_hash(tmp_path, repo):
    p = tmp_path / "ignore.txt"
...
@pytest.mark.parametrize('path', ['test-repos/szz/'])
def test_get_last_modified_lines_hyper_blame_with_renaming(repo):
