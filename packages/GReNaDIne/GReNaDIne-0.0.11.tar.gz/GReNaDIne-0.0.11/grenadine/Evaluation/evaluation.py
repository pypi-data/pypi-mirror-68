...
def get_y_targets(scores, gold_std_grn, n_links=100000):
    """
    ...
    # replacing NaN scores with minimum score -1
    nan_scores = np.isnan(scores)
    min_score =  scores[np.logical_not(nan_scores)].min().min()
    scores[nan_scores] = min_score-1
    # replacing infinite scores with maximum score + 1
    finite_scores = np.isfinite(scores)
    max_score =  scores[finite_scores].max().max()
    scores[np.logical_not(finite_scores)] = max_score+1

    ranks = rank_GRN(coexpression_scores_matrix=scores)
