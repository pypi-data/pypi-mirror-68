...
class VideoEditorServiceClass():
    ...
    def video_add_log(self, videoPath, save_path, logoPngPath = None, logoTextStr = None, Opacity = 1):
        tempPath, save_path = self._video_path_handle(videoPath, save_path)
        ...
        if logoPngPath != None:
            logoPngClip = (ImageClip(logoPngPath)
                    ...
                    .margin(right=8, top=8, opacity= Opacity)  # 水印边距和透明度
                    .set_pos(("right","top")))  # 水印的位置
            clipList.append(logoPngClip)

        if logoTextStr != None:
            FONT_URL = r'C:/Windows/Fonts/simsun.ttc'  #r'C:\Windows\Fonts\simhei.ttf'    #C:\Windows\Fonts\STXINGKA.TTF'
            logoTextClip = (TextClip(txt=logoTextStr, color='white', size=(640, 480), method='caption',
                    align='center', fontsize=25, font=FONT_URL)
                    .set_duration(video.duration)
                    .margin(right=8, top=8, opacity = Opacity)
                    .set_pos(("right","top")))  # 水印的位置
    ...
    def video_add_audio_by_video(self, video_path_source, video_path_target, out_video_path):
        ...
        videoclip.write_videofile(out_video_path)
