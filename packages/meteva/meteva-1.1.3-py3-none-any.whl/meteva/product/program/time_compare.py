...
def time_list_line_error(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,show = False,dpi = 200,title = "不同起报时间预报误差图"):
...
def time_list_line(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,show = False,dpi = 200,title = "观测和不同起报时间预报对比图"):
    sta_ob_and_fos1 = meteva.base.sele_by_dict(sta_ob_and_fos0, s)
    ids = list(set(sta_ob_and_fos1.loc[:,"id"]))
    for id in ids:
        print(id)
...
def time_list_mesh1(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,
    ...
    ids = list(set(sta_ob_and_fos1.loc[:,"id"]))
    for id in ids:
        sta_ob_and_fos2 = meteva.base.in_id_list(sta_ob_and_fos1,[id])
        times_fo = sta_ob_and_fos2.loc[:, "time"].values
        times_fo = list(set(times_fo))
        if (len(times_fo) == 1):
        ...
        ob_time_s = sta_ob_and_fos2["time"] + sta_ob_and_fos2["dtime"] * np.timedelta64(1, 'h')
        ...
        print(dh_x)
        row = int(np.sum(dhs_fo_not0)/dh_y)
        col = int(np.sum(dhs_ob_not0)/dh_x)
        ...
        print(row)
        for j in range(row):
            time_fo = t_fo[-1] - datetime.timedelta(hours=j * dh_y)
            hour = time_fo.hour
            day = time_fo.day
            if ((j * int(dh_y)) % 12 == 0):
        ...
        if step1 > 0:
            ...
            step = step0
        x_plot = x_all[::step]/dh_x
        time_plot = times_ob[::step]
        time_strs = meteva.product.program.get_time_str_list(time_plot, row=2)
        ...
        for d in range(len(data_names)-1):
            ...
            #以最近的预报作为窗口中间的时刻
            if(len(data_names) == 2):
                sta_ob_and_fos3 = sta_ob_and_fos2
            else:
                dns = [data_names[0], data_name]
                sta_ob_and_fos3 = meteva.base.in_member_list(sta_ob_and_fos2,dns)

            print(sta_ob_and_fos3)
            for i in range(col):
                tx = t_ob[0] + datetime.timedelta(hours= 1) * dh_x * i
                #tx = t_ob[i]
                sta1 = meteva.base.in_ob_time_list(sta_ob_and_fos3,[tx])
                for j in range(row):
                    jr = row - j
                    if i * dh_x - jr * dh_y >=0:
                        ty = t_fo[jr]
                        sta =  sta1.loc[sta1["time"] == ty]
                        if( len(sta.index) >0):
                            dat[i,j] = sta.values[0,-1]
                    elif i * dh_x - jr * dh_y > - dh_y:
                        if jr - 1>=0:
                            ty = t_fo[jr-1]
                            sta =  sta1.loc[sta1["time"] == ty]
                            if(len(sta.index) >0):
                                dat[i,j] = sta.values[0,-2]
                    else :
                        pass
            ...
            if plot_error:
                ...
                ax1.set_xlabel('实况时间',fontsize = 14)
                ax1.set_ylabel('起报时间',fontsize = 14)
                ax1.set_xticks(x_plot+0.5)
                ax1.set_xticklabels(time_strs)
                ax1.set_yticklabels(yticks, rotation=360)
                title = '不同时效预报误差和稳定性对比（误差）'+"("+data_name+")"+ "{\'id\':"+str(id)+meteva.base.station_id_name_dict[id] +"}"
            ...
            ax2.set_xlabel('实况时间',fontsize = 14)
            ax2.set_ylabel('起报时间',fontsize = 14)
            ax2.set_xticks(x_plot+0.5)
            ax2.set_xticklabels(time_strs,rotation=360)
            ...
            title1 = '不同时效预报误差和稳定性对比（要素值）' + "(" + data_name + ")" + "{\'id\':" + str(id) +meteva.base.station_id_name_dict[id] +"}"
...
def time_list_mesh(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,
    ...
    sta_ob_and_fos1 = meteva.base.sele_by_dict(sta_ob_and_fos0, s)
    data_names = meteva.base.get_stadata_names(sta_ob_and_fos1)
    sta_ob_all0 = meteva.base.sele_by_para(sta_ob_and_fos1,member=[data_names[0]])

    dtimes = sta_ob_all0["dtime"] * np.timedelta64(1, 'h')
    sta_ob_all0["time"] = sta_ob_all0["time"] + dtimes
    sta_ob_all0["dtime"] = 0
    sta_ob_all0.drop_duplicates()
    if(len(data_names)>2):
        fo_name = data_names[1:]
    else:
        fo_name = [data_names[1]]
    sta_fo_all1 = meteva.base.sele_by_para(sta_ob_and_fos1,member=fo_name)
...
def time_list_mesh_temp(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
    clev, cmap= meteva.base.tool.color_tools.get_clev_and_cmap_by_element_name("temp")
    time_list_mesh(sta_ob_and_fos0,s,save_dir,save_path,clev,cmap,plot_error,cmap_error= "bwr",show = show)

def time_list_mesh_rain01h(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
    ...
    time_list_mesh(sta_ob_and_fos0,s,save_dir,save_path,clev,cmap,plot_error,cmap_error= cmap_error,show = show)

def time_list_mesh_rain03h(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
...
def time_list_mesh_rh(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
...
def time_list_mesh_vis(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
    ...
    time_list_mesh(sta_ob_and_fos0,s,save_dir,save_path,clev,cmap,plot_error,cmap_error= cmap_error)

def time_list_mesh_tcdc(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False):
...
def time_list_mesh_wind(sta_ob_and_fos0,s = None,save_dir = None,save_path = None,plot_error = True,show = False,dpi = 200,title = "预报准确性和稳定性对比图"):
