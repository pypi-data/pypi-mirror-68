...
import sys
import inspect
from terminal.command import Option
...
class Option(object):
    ...
    def __init__(self, name, description=None, action=None, resolve=None, visible=True):
        self.name = name
...
class Command(object):
    ...
    :param multi_args: allow multi arguments, last argument should be a list
    :param alias: alias of the program
    :param spliter: spliter string, like '----------------'
    :param detail: more description
    ...
    def __init__(self, name,  description=None,  version=None, usage=None,
                 ...
                 alias=[],spliter=None, detail=None, visible=True,multi_args=False):
        self._name = name

        self._alias = alias
        self._name_list = '|'.join([name] + alias)

        self._detail  = detail or description
    ...
    def option(self, name, description=None, action=None, resolve=None, visible=True):
        """
        Add or get option.
    ...
    def parse(self, argv=None):
        ...
        while self._argv:
            ...
            if not self.parse_options(arg):
                ...
                if _positional_index < len(self._positional_list):
                    # positional arguments
                    key = self._positional_list[_positional_index]
                    _positional_index += 1
                    self._results[key] = arg
                elif self._multi_args:
                    key = self._positional_list[_positional_index-1]
                    if not isinstance(self._results[key],(tuple, list)):
                        self._results[key] = [self._results[key]]
                    self._results[key].append(arg)
    ...
    def print_help(self):
        ...
        arglen = max([len(o.name) for o in self._option_list if o.visible] + [len(n._name_list) for n in self._command_list if n._visible])
        arglen += 2
