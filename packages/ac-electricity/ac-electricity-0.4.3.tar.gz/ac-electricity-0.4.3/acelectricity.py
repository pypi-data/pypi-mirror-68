...
"""Release History
0.4.3 : add plot data cursors, bode_real(), bode_imag()
...
"""
        ...
        if isinstance(self, (Impedance, Admittance)):
            if 'mode' in kwarg:
                if kwarg['mode'] == 'real':
                    # bode_real()
                    magnitudes = [self.fw_real(x, unit=unit) for x in xvalues]
                elif kwarg['mode'] == 'imag':
                    # bode_imag()
                    magnitudes = [self.fw_imag(x, unit=unit) for x in xvalues]
                else:
...
>>> show()
"""
        self.bode(xmin=xmin, xmax=xmax, xunit=xunit, n=n,
        ...
        if magnitude_unit == 'db':
            ...
            if isinstance(self, (Voltage, Current, Ratio)):
                if 'mode' in kwarg:
                    if kwarg['mode'] == 'real':
                        # bode_real()
                        magnitudes = [self.fw_real(x, unit=unit)
                                      for x in xvalues]
                    elif kwarg['mode'] == 'imag':
                        # bode_imag()
                        magnitudes = [self.fw_imag(x, unit=unit)
                                      for x in xvalues]
                    else:
                        raise ValueError('mode argument: real or imag expected')
                else:
                ...
                if 'mode' in kwarg:
                    if kwarg['mode'] == 'va':
                        # bode_va()
                        unite = 'VA'
                        magnitudes = [self.fw_va(x, unit=unit) for x in xvalues]
                    elif kwarg['mode'] == 'var':
                        # bode_var()
                        unite = 'var'
                        magnitudes = [self.fw_var(x, unit=unit)
                                      for x in xvalues]
                    elif kwarg['mode'] == 'pf':
                        # bode_pf()
                        unite = ''
                        magnitudes = [self.fw_pf(x, unit=unit) for x in xvalues]
                    else:
...
"""
        self.bode(xmin=xmin, xmax=xmax, xunit=xunit, n=n,
                  xscale=xscale, yscale=yscale,
                  magnitude_label=magnitude_label, magnitude_unit='default',
                  title=title, filename=filename,
                  draw_phase=False, mode='real')
