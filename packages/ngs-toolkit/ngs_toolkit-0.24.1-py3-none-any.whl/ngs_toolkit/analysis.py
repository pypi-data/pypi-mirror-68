...
class Analysis(object):
    ...
    def set_samples_input_files(self, overwrite=True):
        ...
        def _format_string_with_sample_attributes(sample, string):
            ...
            to_format = pd.Series(string).str.extractall(r"{(.*?)}")[0].values
            attrs = [x for x in list(sample_dict.keys())]
        ...
        def _set(attr, value, obj, sample):
            ...
            if isinstance(value, dict):
                ...
                for k, v in list(getattr(obj, attr).items()):
        ...
        for data_type in _CONFIG["sample_input_files"]:
            for sample in [s for s in self.samples if s.protocol == data_type]:
                ...
                for attr, value in list(_CONFIG["sample_input_files"][data_type].items()):
    ...
    def load_data(
        ...
        output_map = {k: v for k, v in list(output_map.items()) if k in only_these_keys}

        for name, (file, kwargs) in list(output_map.items()):
    ...
    def get_level_colors(
        ...
        for l, level in enumerate(index.levels):
            ...
            if dtype == "categorical":
                ...
                color_dict = dict(list(zip(list(set(index.get_level_values(level.name))), p)))
                # color for nan cases
    ...
    def unsupervised_analysis(
        ...
        **kwargs: optional
        ...
        cd_kwargs = {k: v for k, v in list(kwargs.items())
        ...
        if "pca" in steps:
            ...
            pcs_order = list(range(pca.n_components_))
    ...
    def differential_overlap(
        ...
        perms = list(
            itertools.permutations(
                list(piv.T.groupby(level=["comparison_name", "direction"]).groups.items()), 2
            )
        )
        ...
        for metric, label, description, fill_value in [
            ...
            if metric == "log_pvalue":
                ...
                r = r.iloc[list(range(0, r.shape[0], 2))]
