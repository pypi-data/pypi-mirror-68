...
class CNVAnalysis(Analysis):
    ...
    def load_data(
        ...
        for name, f in list(output_map.items()):
            for resolution, (file, kwargs) in list(f.items()):
    ...
    def plot_stats_per_chromosome(
        ...
        **kwargs : :obj:`dict`
        ...
        for resolution in tqdm(resolutions, desc="Resolution"):
            ...
            to_plot["chr"] = list([x[0] for x in to_plot.index.str.split(":")])
    ...
    def segment_genome(
        ...
        for resolution in tqdm(resolutions, desc="Resolution"):
            chrom = np.array(
                list([x[0] for x in matrix[resolution].index.str.split(":")])
            )
            start = np.array(
                list(map(
...
def to_igv(matrix, output_file=None, save=True, view_limits=(-2, 2)):
    ...
    igv.loc[:, "Chromosome"] = list([x[0] for x in matrix.index.str.split(":")])
    igv.loc[:, "Start"] = list([int(x[1].split("-")[0]) for x in matrix.index.str.split(":")])
    igv.loc[:, "End"] = list([int(x[1].split("-")[1]) for x in matrix.index.str.split(":")])
