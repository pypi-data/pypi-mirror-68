...
class ATACSeqAnalysis(Analysis):
    ...
    def load_data(
        ...
        Analysis.load_data(
            self,
            output_map={k: v for k, v in list(output_map.items()) if k != "sites"},
    ...
    def get_sex_chrom_ratio(
        ...
        if plot:
            ...
            axis.set_yticks(list(range(1, ratio.shape[0] + 1)))
            if isinstance(ratio.index, pd.MultiIndex):
    ...
    def plot_peak_characteristics(
        ...
        sample_peak_lengths = list(map(get_region_lengths, [s.peaks for s in samples]))
        lengths = pd.melt(
        ...
        chroms = (
            pd.DataFrame(
                list(map(get_regions_per_chromosomes, [s.peaks for s in samples])),
