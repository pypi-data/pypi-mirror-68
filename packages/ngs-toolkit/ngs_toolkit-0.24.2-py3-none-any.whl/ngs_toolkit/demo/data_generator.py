...
def generate_bam_file(count_vector, output_bam, genome_assembly="hg38", chrom_sizes_file=None, index=True):
    ...
    i = [i for i, c in count_vector.items() for _ in range(c)]
    s = s.reindex(i)
    ...
    for sample in analysis.samples:
        ...
        if hasattr(sample, "log2_read_counts"):
            if sample.log2_read_counts is not None:
                for res, file in list(sample.log2_read_counts.items()):
...
def get_random_genomic_locations(
    ...
    csizes = {k: v[-1] for k, v in list(dict(pybedtools.chromsizes(genome_assembly)).items()) if "_" not in k}
    gsize = sum(csizes.values())
    csizes = {k: v / gsize for k, v in list(csizes.items())}
...
def get_random_grnas(n_genes, genome_assembly="hg38", n_grnas_per_gene=4):
    ...
    grna_ns = itertools.chain(*[list(range(n_grnas_per_gene)) for _ in range(gs)])
