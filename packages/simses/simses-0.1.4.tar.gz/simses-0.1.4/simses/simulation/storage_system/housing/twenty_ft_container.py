...
class TwentyFtContainer(Housing):
    ...
    def __init__(self, ambient_thermal_model: AmbientThermalModel):
        ...
        # Container dimensions (from mtcontainer)
        self.outer_length = 6.058  # m
        self.outer_breadth = 2.438  # m
        self.outer_height = 2.591  # m
        self.external_area = self.outer_length * self.outer_height * 2 \
                                       + self.outer_height * self.outer_breadth * 2 \
                                       + self.outer_length * self.outer_breadth * 2  # in m2
        # TODO complete proper integration of above members
        self.thickness_L1 = 0.001  # m, L1 stands for layer 1 - generally stainless steel (taken Aluminium)
        self.thickness_L2 = 0.015  # m, L2 = Layer 2 - generally polyurethane (up to 18mm)
        self.thickness_L3 = 0.0016  # m, L3 = Layer 3 - generally steel (1.6 mm)
        self.depth_corrugation = 0.03  # m (0.254)
        self.inner_length = self.outer_length - 2 * (self.thickness_L1 + self.thickness_L2 + self.thickness_L3 + self.depth_corrugation)  # in m
        self.inner_breadth = self.outer_breadth - 2 * (self.thickness_L1 + self.thickness_L2 + self.thickness_L3 + self.depth_corrugation)  # m
        self.inner_height = self.outer_height - 2 * (self.thickness_L1 + self.thickness_L2 + self.thickness_L3 + self.depth_corrugation)  # m
        ...
        # TODO estimate volume of air within this volume
        ...
        self.convection_coefficient_air_L1 = 5  # W/m2K, Convection coefficient for convection from air to L1 material (exemplary value)
        self.convection_coefficient_L3_air = 5  # W/m2K, Convection coefficient for convection from L3 material to air (exemplary value)

        # Initialize temperatures
        self.__surface_temperature_L1 = self._initial_ambient_temperature  # K
        self.__interface_temperature_L1_L2 = self._initial_ambient_temperature  # K
        self.__interface_temperature_L2_L3 = self._initial_ambient_temperature  # K
        self.__surface_temperature_L3 = self._initial_ambient_temperature  # K,
