...
class PowerElectronics:
    ...
    def update_ac_power_from(self, state: SystemState) -> None:
        ...
        #     fulfillment = 1
        # dc_power_delivered = state.dc_power * fulfillment - state.dc_power_additional
        dc_power_delivered = state.dc_power * state.fulfillment - state.dc_power_additional
        if self.__is_charge(dc_power_delivered):
            ac_power_delivered = self.__acdc_converter.to_dc_reverse(dc_power_delivered)
        else:
            ac_power_delivered = self.__acdc_converter.to_ac_reverse(dc_power_delivered)
        state.ac_power_delivered = ac_power_delivered + state.aux_losses
        state.dc_power_delivered = dc_power_delivered
        state.pe_losses = abs(ac_power_delivered - dc_power_delivered)
    ...
    def __check_power_limits(self, state: SystemState) -> float:
        ...
        ac_power = state.ac_power - state.aux_losses
