...
class SystemState(State):
    ...
    DC_POWER_ADDITIONAL = 'DC power load/generation in W'
    ...
    DC_POWER_DELIVERED = 'DC_P_delivered in W'
    ...
    def dc_power(self) -> float:
        return self.get(self.DC_POWER)

    @dc_power.setter
    def dc_power(self, value: float) -> None:
        self.set(self.DC_POWER, value)
    ...
    @property
    def dc_power_delivered(self) -> float:
        return self.get(self.DC_POWER_DELIVERED)

    @dc_power_delivered.setter
    def dc_power_delivered(self, value: float) -> None:
        self.set(self.DC_POWER_DELIVERED, value)
    ...
    def sum_parallel(cls, system_states: []):
        ...
        # calculate capacity weighted soc
        system_state.soc = 0
        for state in system_states:
