...
from simses.simulation.storage_system.thermal_model.ambient_thermal_model.constant_ambient_temperature_30C import \
    ConstantAmbientTemperature30C
...
class StorageSystemFactory:
    ...
    def create_acdc_converter(self, converter: str, max_power: float,
        ...
        converter_type = converter_configuration[converter][0]
    ...
    def create_stacked_acdc_converter(self, converter: str, acdc_converter: AcDcConverter) -> AcDcConverter:
        try:
            converter_configuration: dict = self.__system_config.acdc_converter
            number_converters: int = int(converter_configuration[converter][1])
    ...
    def create_dcdc_converter(self, converter_type: str, intermediate_circuit_voltage: float) -> DcDcConverter:
    ...
    def create_ambient_temperature_model(self) -> AmbientThermalModel:
        ambient_temperature_model: str = self.__system_config.ambient_temperature_model
        ...
        elif ambient_temperature_model == ConstantAmbientTemperature30C.__name__:
            self.__log.debug('Creating ambient temperature model as ' + ambient_temperature_model)
            return ConstantAmbientTemperature30C()
        else:
            options: [str] = list()
            options.append(ConstantAmbientTemperature.__name__)
            options.append(LocationAmbientTemperature.__name__)
            options.append(ConstantAmbientTemperature30C.__name__)
    ...
    def create_thermal_model_from(self, thermal_model: str, hvac: HeatingVentilationAirConditioning,
                                  ambient_thermal_model: AmbientThermalModel, housing: Housing) -> SystemThermalModel:
        if thermal_model == NoSystemThermalModel.__name__:
            ...
            options.append(ZeroDSystemThermalModel.__name__)
            raise Exception('System temperature model ' + thermal_model + ' is unknown. '
                            'Following options are available: ' + str(options))

    # def create_auxiliary_model(self) -> Auxiliary:
    #     # TODO operation losses depend on data not __analysis_config -> delete __analysis_config dependency (MM, PD)
    #     operation_losses_model = self.__system_config.operation_losses_model
    #     if operation_losses_model == NoOperationLosses.__name__:
    #         self.__log.debug('Creating operation losses model as ' + operation_losses_model)
    #         return NoOperationLosses()
    #     elif operation_losses_model == RfbOperationLosses.__name__:
    #         self.__log.debug('Creating operation losses model as ' + operation_losses_model)
    #         return RfbOperationLosses()
    #     else:
    #         self.__log.warn('Specified operation losses model ' + operation_losses_model + ' not found, '
    #                        'creating ' + NoOperationLosses.__name__)
    #         return NoOperationLosses()

    def create_housing_from(self, housing: str, ambient_thermal_model: AmbientThermalModel) -> Housing:
        if housing == NoHousing.__name__:
            return NoHousing(ambient_thermal_model)
        elif housing == TwentyFtContainer.__name__:
            number_containers = 1
            dc_systems: [[str]] = self.__system_config.storage_systems_dc
            capacity = 0
            for dc_system in dc_systems:
                storage_technologies: dict = self.__system_config.storage_technologies
                storage_technology = storage_technologies[dc_system[2]]
                if storage_technology[1] == self.__lithium_ion_name:
                    capacity += float(storage_technology[0])
            capacity = capacity / 1000  # capacity in kWh
            # maximum_number_racks_per_container = 10
            # volume_per_rack = 1.192  # m3 (from Samsung SDI datasheet)
            # volumetric_energy_density_rack = 140  # kWh/m3 (from Samsung SDI datasheet)
            # # TODO consider getting these values from the battery data
            # # TODO AP please consider that redox flow, PtG and other system could be used
            # energy_per_rack = volume_per_rack * volumetric_energy_density_rack
            # actual_number_racks = math.ceil(capacity / energy_per_rack)
            # if actual_number_racks > maximum_number_racks_per_container:
            #     number_containers = math.ceil(actual_number_racks / maximum_number_racks_per_container)
            # housing_count = 0
            # while housing_count<number_containers:
            #    housing_count += 1
    ...
    def create_hvac_from(self, hvac: str) -> HeatingVentilationAirConditioning:
        ...
        if hvac_type == NoHeatingVentilationAirConditioning.__name__:
            ...
            power: float = float(hvac_configuration[hvac][1])
            temperature_set_point: float = float(hvac_configuration[hvac][2])
            return FixCOPHeatingVentilationAirConditioning(power, temperature_set_point)
        else:
            options: [str] = list()
    ...
    def create_storage_systems_ac(self, data_export: DataHandler) -> [StorageSystemAC]:
        ...
        for system in ac_systems:
            ...
            names.append(name)
            power: float = float(system[1])
            intermediate_circuit_voltage: float = float(system[2])
            converter: str = system[3]
            thermal_model: str = system[4]
            housing: str = system[5]
            hvac: str = system[6]
            ...
            dc_systems: [StorageSystemDC] = self.create_storage_systems_dc(name, data_export,
                                                                           system_thermal_model, ambient_thermal_model,
    ...
    def create_storage_systems_dc(self, name: str, data_export: DataHandler,
                                  system_thermal_model: SystemThermalModel,
        ...
        for dc_system in dc_systems:
            system = dc_system[0]
            dc_type: str = dc_system[1]
            if system == name and DcDcConverter.__name__ in dc_type:
                storage_id += 1
                technology: str = dc_system[2]
    ...
    def create_storage_technology(self, technology: str, data_export: DataHandler,
                                  system_thermal_model: SystemThermalModel, ambient_thermal_model: AmbientThermalModel,
        ...
        capacity: float = float(technology_set[0])
        technology: str = technology_set[1]
        if technology == self.__lithium_ion_name:
            self.__log.debug('Creating ' + technology + ' as storage system id ' + str(storage_id))
            cell = technology_set[2]
            thermal_model = technology_set[3]
            return LithiumIonBattery(cell, voltage, capacity, thermal_model, data_export,
                                     system_thermal_model, ambient_thermal_model, storage_id, system_id,
                                     self.__simulation_config)
        elif technology == self.__redox_flow_name:
            self.__log.debug('Creating ' + technology + ' as storage system id ' + str(storage_id))
            stack_type = technology_set[2]
            nominal_power = float(technology_set[3])
            pump_type: Pump = technology_set[4]
            eta_pump = float(technology_set[5])
            degradation_model = technology_set[6]
            return RedoxFlow(stack_type, pump_type, eta_pump, nominal_power, voltage, capacity, ambient_thermal_model,
                             degradation_model, data_export, storage_id, system_id, self.__simulation_config)
            ...
            return Hydrogen(data_export, fuel_cell, fuel_cell_nominal_power, fuel_cell_thermal_model, fuel_cell_pressure_model, electrolyzer, electrolyzer_nominal_power,
                            electrolyzer_thermal_model, electrolyzer_pressure_model, storage, capacity, max_pressure, ambient_thermal_model, system_thermal_model,
                            system_id, storage_id, self.__simulation_config)
        else:
            options: [str] = list()
