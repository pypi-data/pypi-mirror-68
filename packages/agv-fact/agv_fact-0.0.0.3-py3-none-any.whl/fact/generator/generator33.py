...
class AbstractGeneratorXml(object):
    ...
    def set_sello(self, certificado_file_pem64): #, certificado_file_pem):
        try:
            ...
            self.__cadena_original = self.get_cadena_original(self.get_xml())
            logging.info('LA CADENA ORIGINAL DEL XML')
            digest = self.algoritmoSha.new()
            digest.update(self.__cadena_original)
    ...
    def get_cadena_original(self, xml):
        try:
            ...
            res = None
            try:
                res = requests.get(url, timeout=240, stream=True)
            except (requests.exceptions.Timeout, requests.exceptions.ConnectionError) as e:
                logging.error(u"Hubo un error en el servidor del SAT, por favor intente mas tarde: " + str(e))
                raise Exception(u"Hubo un error en el servidor del SAT, por favor intente mas tarde: " + str(e))
            except Exception as e:
                logging.error(u"Hubo un error en el servidor del SAT, por favor intente mas tarde: " + str(e))
                raise Exception(u"Hubo un error en el servidor del SAT, por favor intente mas tarde: " + str(e))
            if res is None:
                logging.error(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
                raise Exception(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
            elif res.content is None:
                logging.error(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
                raise Exception(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
            elif res.content is b'' or len(res.content) == 0:
                logging.error(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
                raise Exception(u"Hubo un error en el servidor del SAT, por favor intente mas tarde.")
            xslt = etree.fromstring(res.content)
            logging.info('XSLT CADENA ORIGINAL DEL XML')
            parser = etree.XSLT(xslt)
            logging.info('XSLT CADENA ORIGINAL DEL XML 2')
            self.__cadena_original = parser(xml)
            logging.info('XSLT CADENA ORIGINAL DEL XML 3')
            return self.__cadena_original
            # else:
            #     raise ('Error with cadenaoriginal_3_3.xslt')
