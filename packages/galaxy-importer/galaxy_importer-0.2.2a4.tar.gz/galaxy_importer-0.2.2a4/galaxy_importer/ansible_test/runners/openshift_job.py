...
class Job(object):
    ...
    def __init__(self, ocp_domain, namespace, session_token, ca_path, image, job_template, logger):
        ...
        self.job_yaml = job_template.format(
            job_name=self.name,
            image=image,
            requests_cpu=os.environ.get('REQUESTS_CPU', '300m'),
            requests_memory=os.environ.get('REQUESTS_MEMORY', '256Mi'),
            limits_cpu=os.environ.get('LIMITS_CPU', '500m'),
            limits_memory=os.environ.get('LIMITS_MEMORY', '1Gi'),
        )
        self.log = logger or default_logger
