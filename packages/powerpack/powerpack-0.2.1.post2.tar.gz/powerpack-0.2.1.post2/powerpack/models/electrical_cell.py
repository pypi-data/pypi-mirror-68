...
cell_management_system = elec.CellManagementSystem(cell=Cell1)
...
for i in current:
    if i < 0:
        ...
        usecase = 'charge'
    (i_min, i_max), (u_min, u_max) = cell_management_system.Ineq(soc_m, 298.15, usecase, False,
                                               delta_t, internal_parameter_cell)
    current_new = min(i_max, i)
    current_new = max(i_min, current_new)
    p_tuple = cell_management_system.update(current_new, soc_m, 298.15,
