...
class InputChannel(object):
    ...
    async def startTracking(self, usg_msg, csi, channel, sid):
        ...
        host = environ["IDENTITY_PROVIDER_HOST"]
        port = environ["IDENTITY_PROVIDER_PORT"]

        headers = {"Conversation-Id": sid}
        params = {
            "CSI": csi,
            "channel": channel
        }
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                        url='http://' + host + ':' + port + '/startTracking',
                        params=params,
                        headers=headers,
                        timeout=3,
                ) as res:
            ...
            logger.error(f"[{sid}] - Failed to get guid: {traceback_msg}")
        return msg
...
class CollectingOutputChannel(OutputChannel):
    ...
    @staticmethod
    def _message(
        recipient_id, text=None, image=None, video=None, buttons=None, attachment=None, custom=None
    ):
