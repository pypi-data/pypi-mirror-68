...
class DKCloudCommandRunner(object):
    ...
    def print_node(user, kitchen_list, name, prefix, box_char, output):
        ...
        output.append(
            f"{prefix}{box_char}{(connector if box_char != '' else '')}{styled_name}{closed_tag}"
        )
        ...
        def display_vault_info(item, payload):
            ...
            if item in payload['config']:
                ...
                if not item == VAULT_GLOBAL:
                    message += f" inheritable\t\t{payload['config'][item]['inheritable']}{os.linesep}"  # noqa: E501
    ...
    def check_secret_path(path):
        ...
        if wrong_vault_sequence_path in path:
            raise click.ClickException(
                f'Secret key should not contain "{wrong_vault_sequence_path}"'
            )
    ...
    def secret_list(dk_api, path, global_secrets, kitchen_name):
        ...
        for item in items:
            ...
            if item in payload:
                ...
                if item_name != 'global' and source_kitchen:
                    output_message += f'(Inherited from Parent Kitchen {source_kitchen}){os.linesep}'  # noqa: E501
                if 'error' in payload[item] and payload[item]['error']:
                    ...
                    if 'error-message' in payload[item]:
                        ...
                        error_message += f'There was an error getting the secrets for {item_name} vault.{os.linesep}'  # noqa: E501
    ...
    def secret_exists(dk_api, path, kitchen_name):
        ...
        if 'list' in payload[item]:
            complete_path = u'vault://{}'.format(path)
...
"""
    ...
    def create_kitchen(dk_api, parent_kitchen, new_kitchen, description=None):
        ...
        if kl.ok():
            ...
            kl.set_message(
                f'ERROR:  DKCloudCommand.create_kitchen failed\nmessage: {kl.get_message()}'
            )
        return kl
        ...
        try:
            ...
            if not DKKitchenDisk.is_kitchen_root_dir(rp):
                ...
                if recipe_name_found != recipe_name_param:
                    rc.set(
                        rc.DK_FAIL,
                        f"ERROR: DKCloudCommandRunner.get_recipe: Requested recipe '{recipe_name_param}', but you are in folder '{rp}'"  # noqa: E501
                    )
                    return rc
            ...
            if os.path.exists(os.path.join(rp, recipe_name_param)):
                ...
                if remote_only_recipe_tree is not None:
                    ...
                    if not r.save_recipe_to_disk(update_meta=False):
                        rc.set(
                            rc.DK_FAIL,
                            f'Problems saving differences and remote only files to disk. {remote_only_recipe_tree}'  # noqa: E501
                        )
                        return rc
                ...
                if merged_different_files is not None:
                    ...
                    if not r.save_recipe_to_disk(update_meta=False):
                        rc.set(
                            rc.DK_FAIL,
                            f'Problems saving differences and remote only files to disk. {merged_different_files}'  # noqa: E501
                        )
                        return rc
    ...
    def update_local_recipes_with_remote(dk_api, kitchens_root, kitchen_name):
        try:
            ...
            click.secho(
                f'Updating Recipes in the local version of Target Kitchen {kitchen_name} to receive merged changes applied to remote...'  # noqa: E501
            )
            ...
            if DKKitchenDisk.is_kitchen_root_dir(kitchen_path):
                for subdir in os.listdir(kitchen_path):
                    if not dk_api.get_ignore().ignore(subdir):
                        ...
                        if not rc.ok():
                            click.secho(
                                f'Could not properly update recipe {subdir}.\n Error is: {rc.get_message()}'
                            )  # noqa: E501
    ...
    def check_local_recipes(dk_api, kitchens_root, kitchen_name):
        kitchen_path = os.path.join(kitchens_root, kitchen_name)
        click.secho(
            f'Checking local version of \033[1mKitchen {kitchen_name}\033[0m to make sure that it is in sync with remote...'  # noqa: E501
        )
        if DKKitchenDisk.is_kitchen_root_dir(kitchen_path):
            for subdir in os.listdir(kitchen_path):
                if subdir != DK_DIR:
            ...
            click.secho(
                f'...No local version of {kitchen_name} Kitchen found. Skipping check to confirm local is in sync with remote.'  # noqa: E501
            )
...
where 'file_path' is the path as stated in the Merge Preview Results"""
                    ...
                    for filename in filenames:
                        if filename.endswith('.base'):
                            ...
                            if not os.path.isfile(resolved_path):
                                message = f'There are unresolved conflicts, please resolve ' \
    ...
    def orderrun_detail(dk_api, kitchen, pd, full_log=False, log_threshold='INFO'):
        ...
        if order_run is None:
            rc.set(
                rc.DK_FAIL,
                f"No OrderRun information.  Try using 'dk order-list -k {kitchen}' to see what is available."  # noqa: E501
            )
            return rc
...
"""
    ...
    def _get_order_run_top_line(order_run):
        ...
        return f'Recipe ({recipe_name}) in Kitchen({kitchen_name}) with Status({status}) and OrderRun Id({order_run_id})'  # noqa: E501
