...
class DKCLIRecipeChecker:
    ...
    NODE_TYPE_LIST = [
        'DKNode_NoOp', 'DKNode_DataMapper', 'DKNode_Action', 'DKNode_Container', 'DKNode_Ingredient'
    ]
    NODE_ACTION_TYPE_LIST = [
        'DKDataSource_DB2SQL', 'DKDataSource_Dictionary', 'DKDataSource_Container',
        'DKDataSource_NoOp', 'DKDataSource_FTP', 'DKDataSource_MSSQL', 'DKDataSource_MYSQL',
        'DKDataSource_OrderRunInfo', 'DKDataSource_PostgreSQL', 'DKDataSource_S3',
        'DKDataSource_Salesforce', 'DKDataSource_SFTP', 'DKDataSource_SFTP_DSS',
        'DKDataSource_SHELL'
    ]
    DATASINK_TYPE_LIST = [
        'DKDataSink_DB2SQL', 'DKDataSink_Dictionary', 'DKDataSink_Container', 'DKDataSink_NoOp',
        'DKDataSink_FTP', 'DKDataSink_MSSQL', 'DKDataSink_MYSQL', 'DKDataSink_PostgreSQL',
        'DKDataSink_S3', 'DKDataSink_Salesforce', 'DKDataSink_SFTP', 'DKDataSink_SFTP_DSS',
        'DKDataSink_SHELL'
    ]

    CONTAINER_NOTEBOOK_KEY_LIST = [
        'image-repo', 'dockerhub-namespace', 'dockerhub-username', 'dockerhub-password'
    ]
    ...
    def _check_main_files(self, full_path):
        ...
        for file_name in main_files:
            if not os.path.isfile(os.path.join(full_path, file_name)):
                ...
                self._display_error(
                    error_message, DKCLIRecipeChecker.ERROR, os.path.join(full_path, file_name)
                )
    ...
    def _check_nodes(self, full_recipe_path):
        ...
        nodes = [
            name for name in os.listdir(full_recipe_path)
    ...
    def _check_node(self, node_path):
        ...
        if not os.path.isfile(description_json_path):
            self._display_error(
                'No description.json in node.', DKCLIRecipeChecker.ERROR, description_json_path
            )
        ...
        if description_json_contents is None:
            self._display_error(
                'description.json is empty.', DKCLIRecipeChecker.ERROR, description_json_path
            )

        # Check node type
        is_json, description_json_dict = self._check_json_format(
            description_json_path, description_json_contents
        )

        if 'type' not in description_json_dict:
            self._display_error(
                'description.json has no "type" key.', DKCLIRecipeChecker.ERROR,
                description_json_path
            )

        if description_json_dict['type'] not in DKCLIRecipeChecker.NODE_TYPE_LIST:
            error_message = f'Bad node type: "{description_json_dict["type"]}". Needs to be one of this: {DKCLIRecipeChecker.NODE_TYPE_LIST}'  # noqa: E501
        ...
        if 'description' not in description_json_dict:
            self._display_error(
                'description.json has no "description" key.', DKCLIRecipeChecker.WARNING,
                description_json_path
            )

        if 'description' in description_json_dict and not description_json_dict['description']:
            self._display_error(
                'description.json has an empty "description" key.', DKCLIRecipeChecker.ERROR,
                description_json_path
            )
            self._display_error(error_message, DKCLIRecipeChecker.ERROR, description_json_path)
    ...
    def _check_node_DKNode_Action(self, node_path):
        ...
        if not os.path.exists(actions_path):
            self._display_error(
                'Node has no "actions" folder.', DKCLIRecipeChecker.ERROR, actions_path
            )
    ...
    def _check_action(self, actions_path, check='datasources'):
        ...
        json_files = [
            name for name in os.listdir(actions_path)
        ...
        if file_contents is None:
            self._display_error(
                'json file is empty', DKCLIRecipeChecker.ERROR, action_json_file_path
            )
        ...
        self._check_dict_key(
            'type', json_dict, action_json_file_path, expected_values=expected_values
        )
        if 'wildcard' not in json_dict:
            self._check_dict_key('keys', json_dict, action_json_file_path)
        self._check_dict_key(
            'tests', json_dict, action_json_file_path, error_severity=DKCLIRecipeChecker.WARNING
        )

        # check tests action types
        if 'tests' in json_dict:
            for test_key, test_value in six.iteritems(json_dict['tests']):
                if 'action' in test_value and test_value[
                        'action'] not in DKCLIRecipeChecker.TEST_ACTION_TYPE_LIST:
                    error_message = f'In "tests", "{test_key}", "action". '
                    error_message += f'Bad value "{test_value["action"]}", needs to be one of this list: {DKCLIRecipeChecker.TEST_ACTION_TYPE_LIST}'  # noqa: E501
                    self._display_error(
                        error_message, DKCLIRecipeChecker.ERROR, action_json_file_path
                    )
    ...
    def _check_dict_key(
        ...
        if expected_values is not None and the_dict[the_key] not in expected_values:
            error_message = f'Bad value "{the_dict[the_key]}", needs to be one of this list: {expected_values}'  # noqa: E501
    ...
    def _display_error(self, message, severity, file_path):
        formatted_message = f'{os.linesep}---- {DKCLIRecipeChecker.EXCEPTION_MESSAGE_HEADER}{os.linesep}'  # noqa: E501
