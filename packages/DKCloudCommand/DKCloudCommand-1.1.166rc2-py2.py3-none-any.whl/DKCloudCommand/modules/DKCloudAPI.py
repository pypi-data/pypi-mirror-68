...
class DKCloudAPI(object):
    ...
    def get_compiled_order_run(self, kitchen, recipe_name, variation_name):
        ...
        if variation_name is None or isinstance(variation_name, six.string_types) is False:
            ...
            return rc
        url = f'{self.get_url_for_direct_rest_call()}/v2/servings/compiled/get/{kitchen}/{recipe_name}/{variation_name}'  # noqa: E501
        try:
            response = requests.get(url, headers=self._get_common_headers())
        except (RequestException, ValueError, TypeError) as c:
    ...
    def get_compiled_file(self, kitchen, recipe_name, variation_name, file_data):
        ...
        url = f'{self.get_url_for_direct_rest_call()}/v2/recipe/compile/{kitchen}/{recipe_name}/{variation_name}'  # noqa: E501
    ...
    def get_file_history(self, kitchen, recipe_name, file_path, change_count):
        ...
        url = f'{self.get_url_for_direct_rest_call()}/v2/recipe/history/{kitchen}/{recipe_name}/{normalize(file_path, UNIX)}?change_count={change_count}'  # noqa: E501
    ...
    def recipe_validate(self, kitchen, recipe_name, variation_name, changed_files):
        ...
        if variation_name is None or isinstance(variation_name, six.string_types) is False:
            rc.set(rc.DK_FAIL, 'issue with variation_name')
            return rc
        url = f'{self.get_url_for_direct_rest_call()}/v2/recipe/validate/{kitchen}/{recipe_name}/{variation_name}'  # noqa: E501
        try:
    ...
    def kitchens_merge(self, from_kitchen, to_kitchen, resolved_conflicts=None):
        ...
        return f'{self._config.get_ip()}:{self._config.get_port()}/{rdict["merge-kitchen-result"]["url"]}'  # noqa: E501
    ...
    def merge_file(self, kitchen, recipe, file_path, file_contents, orig_head, last_file_sha):
        ...
        url = f'{self.get_url_for_direct_rest_call()}/v2/file/merge/{kitchen}/{recipe}/{adjusted_file_path}'  # noqa: E501
        try:
            response = requests.post(
    ...
    def get_order_run_full_log(self, kitchen, order_run_id):
        url = f'{self.get_url_for_direct_rest_call()}/v2/order/export/logs/{kitchen}/{quote(order_run_id)}'  # noqa: E501
        try:
            response = requests.get(url, headers=self._get_common_headers())
        except (RequestException, ValueError) as c:
    ...
    def list_order(
        ...
        if recipe:
            url = f'{self.get_url_for_direct_rest_call()}/v2/order/status/{kitchen}?start={start}&count={order_count}&scount={order_run_count}&r={recipe}'  # noqa: E501
        else:
            url = f'{self.get_url_for_direct_rest_call()}/v2/order/status/{kitchen}?start={start}&count={order_count}&scount={order_run_count}'  # noqa: E501
        try:
            response = requests.get(url, headers=self._get_common_headers())
        except (RequestException, ValueError, TypeError) as c:
            rc.set(rc.DK_FAIL, f'get_recipe: exception: {c}')
