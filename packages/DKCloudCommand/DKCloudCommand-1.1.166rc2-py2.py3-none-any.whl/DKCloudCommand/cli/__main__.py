...
class Backend(object):
    ...
    def init_context(self):
        ...
        if self.current_command != 'config' and not os.path.isfile(
                self.cfg.get_config_file_location()):
        ...
        if token is None:
            message = f'\nLogin failed. You are in context {dk_context}, do you want to reconfigure your context? [yes/No]'  # noqa: E501
    ...
    def check_version(self, cfg):
        ...
        try:
            ...
            click.secho(
                "Warning: could not get DKCloudCommand latest version number from pip.", fg='red'
            )
        ...
        if latest_version and self.version_to_int(latest_version) > current_version:
            ...
            print(
                ...
                '\033[31m Type "pip install DKCloudCommand --upgrade" to upgrade.\033[39m'
            )
            print(divider + '\n')
            return False
...
def short_command_from_long(long_command):
...
def kitchen_get(backend, kitchen_name, recipe, all):
    ...
    try:
        ...
        if len(recipe) > 0:
            click.secho(
                f"{get_datetime()} - Getting kitchen '{kitchen_name}' and the recipes {recipe}",
                fg='green'
            )
        else:
...
def kitchen_create(backend, parent, description, kitchen):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Creating kitchen {kitchen} from parent kitchen {parent}',
            fg='green'
        )
...
def kitchen_delete(backend, kitchen, yes):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Deleting remote copy of kitchen {kitchen}. Local files will not change.',
            fg='green'
        )
        if kitchen.lower() != MASTER:
...
def kitchen_merge(backend, source_kitchen, target_kitchen, yes):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Merging Kitchen {use_source_kitchen} into Kitchen {target_kitchen}',
            fg='green'
        )
        check_and_print(
...
def recipe_list(backend, kitchen):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            f"{get_datetime()} - Getting the list of Recipes for Kitchen '{use_kitchen}'",
            fg='green'
        )
...
def recipe_create(backend, kitchen, name, template):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            f"{get_datetime()} - Creating Recipe {name} for Kitchen '{use_kitchen}'", fg='green'
        )
        check_and_print(
...
def recipe_copy(backend, kitchen, source, name):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            f"{get_datetime()} - Creating Recipe {name} from recipe {source} for Kitchen '{use_kitchen}'",
            fg='green'
        )
...
def recipe_delete(backend, kitchen, name, yes):
    ...
    try:
        ...
        if not yes:
            confirm = input(
                f'Are you sure you want to delete the local and remote copy of recipe {name}? [yes/No]'
            )
            if confirm.lower() != 'yes':
                return

        click.secho(
            f"{get_datetime()} - Deleting Recipe {name} for Kitchen '{use_kitchen}'", fg='green'
        )
        ...
        if recipe_root_dir is None:
            ...
            if recipe is not None:
                if recipe_name != recipe:
                    raise click.ClickException(
                        f"\nThe recipe name argument '{recipe}' is inconsistent with the current directory '{recipe_root_dir}'"  # noqa: E501
                    )
...
def recipe_compile(backend, kitchen, recipe, variation):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Get the Compiled Recipe {recipe}.{variation} in Kitchen {use_kitchen}',
            fg='green'
        )
        check_and_print(
...
def recipe_variation_list(backend, kitchen, recipe):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Listing variations for recipe {use_recipe} in Kitchen {use_kitchen}',
            fg='green'
        )
        check_and_print(
...
def recipe_ingredient_list(backend):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Listing ingredients for recipe {recipe_name} in Kitchen {kitchen}',
            fg='green'
        )
        check_and_print(
...
def file_merge(backend, source_kitchen, target_kitchen, filepath):
    ...
    try:
        ...
        if recipe_dir is not None:
            ...
            message_3 = f'i.e. dk file-merge -sk source_kitchen_name -tk target_kitchen_name {os.path.normpath("recipe_name/graph.json")}'  # noqa: E501
...
def file_resolve(backend, source_kitchen, target_kitchen, filepath):
    ...
    try:
        ...
        if recipe_dir is not None:
            message_1 = 'Warning: You are inside a recipe. Please go at least one level above. This command is typically used from the level above kitchen level.'  # noqa: E501
            message_2 = 'Keep in mind that the file path needs to be stated as it is coming from kitchen-merge-preview output.'  # noqa: E501
            message_3 = f'i.e. dk file-resolve -sk source_kitchen_name -tk target_kitchen_name {os.path.normpath("recipe_name/graph.json")}'  # noqa: E501
            click.secho(message_1, fg='yellow')
            click.secho(message_2, fg='yellow')
            click.secho(message_3, fg='yellow')
            return
        ...
        click.secho(
            f'{get_datetime()} - File resolve for file {filepath}, source kitchen ({use_source_kitchen}), target kitchen({target_kitchen})'  # noqa: E501
        )
        check_and_print(
...
def file_get(backend, filepath):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Getting File ({filepath}) to Recipe ({recipe}) in kitchen({kitchen})',
            fg='green'
        )
    ...
    try:
        ...
        for item in filepaths:
            if any(invalid_separator in item for invalid_separator in invalid_separators):
                error_message = 'Files to be updated must be delimited by whitespace (i.e. \'file1.txt file2.txt\' as opposed to \'file1.txt,file2.txt\')'  # noqa: E501
                raise click.ClickException(error_message)
        ...
        click.secho(
            f'{get_datetime()} - Updating File(s) ({filepaths}) in Recipe ({recipe}) in Kitchen({use_kitchen}) with message ({message})',  # noqa: E501
            fg='green'
        )
        check_and_print(
...
def file_update_all(backend, message, delete_remote):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Updating all changed files in Recipe ({recipe}) in Kitchen({kitchen}) with message ({message})',  # noqa: E501
            fg='green'
        )
        DKCLIRecipeChecker(backend.cfg).check_recipe(recipe_dir)
        check_and_print(
...
def file_delete(backend, kitchen, recipe, message, filepath):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Deleting ({filepath}) in Recipe ({recipe}) in kitchen({use_kitchen}) with message ({message})',  # noqa: E501
            fg='green'
        )
        check_and_print(
...
def active_serving_watcher(backend, kitchen, interval):
    ...
    try:
        ...
        if use_kitchen is None:
            raise click.ClickException(err_str)
        click.secho(
            f'{get_datetime()} - Watching Active OrderRun Changes in Kitchen {use_kitchen}',
            fg='green'
        )
...
def order_delete(backend, kitchen, order_id, yes):
    ...
    try:
        ...
        if not yes:
            if order_id is None:
                confirm = input(
                    f'Are you sure you want to delete all Orders in kitchen {use_kitchen}? [yes/No]'
                )
            else:
        ...
        if order_id is not None:
            ...
            click.secho(
                f'{get_datetime()} - Delete all orders in Kitchen {use_kitchen}', fg='green'
            )
...
def orderrun_detail(
    ...
    try:
        ...
        if log_threshold not in log_threshold_options:
            ...
            raise click.ClickException(
                f'log_threshold needs to be one of this options: {log_threshold_options_str}'
            )
        ...
        if not runstatus and not disp_order_id and not disp_orderrun_id:
            ...
            click.secho(
                f'{get_datetime()} - Display Order-Run details from kitchen {use_kitchen}',
                fg='green'
            )
        check_and_print(
...
def order_list(backend, kitchen, order_count, order_run_count, start, recipe):
    ...
    try:
        ...
        click.secho(
            f'{get_datetime()} - Get Order information for Kitchen {use_kitchen}', fg='green'
        )
...
def vault_delete(backend, kitchen, yes):
    ...
    try:
        ...
        click.echo(
            click.style(
                f'{get_datetime()} - Deleting the vault configuration for kitchen {kitchen}',
                fg='green'
            )
        )
...
def secret_write(backend, entry, yes, kitchen):
    ...
    try:
        ...
        if kitchen and not DKCloudCommandRunner.is_vault_config_writable(backend.dki, kitchen):
            raise click.ClickException(
                f'No vault service configured for kitchen {kitchen}, or vault is read only.'
            )
        ...
        if secret_exists:
            if not yes:
                confirm = input(
                    f'Are you sure you want to overwrite the existing Vault Secret {path} ? [yes/No]'
                )
                if confirm.lower() != 'yes':
                    return
...
def exit_if_python2():
    if six.PY2:
        sys.exit(
            "Python2 reached end of life and is not supported by DKCloudCommand. Please use pip3 "
