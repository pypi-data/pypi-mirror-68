...
class MySocketServer:
    ...
    def __init__(self,u,pw,h,db):
        ...
        self.feeder = None
        self.laststarttime = datetime.now()
        self.lastseconds=0
        self.lastresetseconds=-1

    def setNewCarsFeeder(self,feeder):
        self.feeder = feeder

    async def handle_client(self,reader, writer, cars, timeout,nextfeedtime):        

        try:

            timenow = datetime.now()
            nextfeedtime2 = self.laststarttime + timedelta(minutes = 5) 

            if timenow == nextfeedtime2 and self.feeder is not None:
                self.lastresetseconds = self.lastseconds #161
                self.laststarttime = timenow
                newcars,timeout,nextfeedtime = self.feeder.getCars(5)
                cars.append(newcars)
                print(f"Now serving {len(cars)} cars")
                print(f"{newcars}")

            data = await reader.read(100)        
            message = data.decode("utf-8")
            requested_time = int(message)
            self.lastSeconds = requested_time
            ...
            for i in range(0,len(cars)):
                if requested_time<=self.lastresetseconds:
                    reading = cars[i].read(requested_time)
                else:
