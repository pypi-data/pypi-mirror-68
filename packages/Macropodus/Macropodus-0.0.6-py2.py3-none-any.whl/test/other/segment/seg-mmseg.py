...
from macropodus.tookit.trie_tree.trie_tree import TrieTree, get_trie_tree_class
from macropodus.base.seg_basic import SegBasic

sb = SegBasic()
wf_keywords = sb.dict_words_freq.keys()
...
class Trie(object):
    ...
    def load(self):
        ...
        #     data = pickle.load(f)
        # self.root = data
        self.root = get_trie_tree_class(wf_keywords)
...
class MMSeg:
    ...
    def __get_start_words(self, sentence):
        if sentence:
            match_words = self.words_dic.match_keyword(sentence) # .get_matches(sentence)
