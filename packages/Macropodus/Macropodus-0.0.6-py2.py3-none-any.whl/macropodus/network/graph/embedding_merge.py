# !/usr/bin/python
# -*- coding: utf-8 -*-
# @time    : 2020/1/28 15:19
# @author  : Mo
# @function: two embedding
...
class Embedding2Graph(graph):
    ...
    def create_model(self, hyper_parameters):
        ...
        hyper_parameters["level_type"] = "char"
        self.char_embedding = Embeddings(hyper_parameters=hyper_parameters)
        # Concatenate
        x = tf.keras.layers.Concatenate(axis=-1)([self.word_embedding.output, self.char_embedding.output])
        # TimeDistributed
        ...
        if self.crf_mode == "pad":
            ...
            self.inputs = [self.word_embedding.input, self.char_embedding.input, x_mask]
        else:
            self.crf = CRF(self.label, mode="reg", name="layer_crf")
            self.output = self.crf(tensor)
            self.inputs = [self.word_embedding.input, self.char_embedding.input]
