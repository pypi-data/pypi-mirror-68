...
from macropodus.conf.path_config import path_model_dir, path_seg_pku_1998_bi_train, \
                                        ...
                                        path_ccks_2020_ner, path_ccks_2020_ner_train, path_ccks_2020_ner_dev
# 计算时间
import time
    ...
    if not hyper_parameters:
        hyper_parameters = {
            ...
            'model': {'label': 14,  # 类别数
                      ...
                      'epochs': 32,  # 训练最大轮次
                      'patience': 3,  # 早停,2-3就好
                      'lr': 5e-4,  # 学习率, bert取5e-5,其他取1e-3,如果acc较低或者一直不变,优先调这个, 对训练会有比较大的影响, 如果准确率一直上不去,可以考虑调这个参数
                      'l2': 0.5,  # l2正则化
                      ...
                      'path_model_dir': os.path.join(path_model_dir, "crf_ccks_2020_ner"),  # 保存当前训练模型的根目录
                      ...
                      'rnn_units': 128,  # rnn隐藏元, 128, 256, 512, 768, 1024等
                      'crf_mode': 'reg',  # crf类型, 可填'other', 'reg', 'pad'(包括句子实际长度)
                      },
            'embedding': {'layer_indexes': [1, 2, 3, 4], #[-1, -2, -3, -4],  # bert取的层数
                          'corpus_path': path_ccks_2020_ner_train,     # embedding预训练数据地址,不配则会默认取conf里边默认的地址, keras-bert可以加载谷歌版bert,百度版ernie(需转换，https://github.com/ArthurRizar/tensorflow_ernie),哈工大版bert-wwm(tf框架，https://github.com/ymcui/Chinese-BERT-wwm)
                          },
            'data': {'train_data': path_ccks_2020_ner_train, # path_tag_people_2014_train, # path_seg_pku_1998_train,  # path_ner_people_1998_train, # 训练数据
                     'val_data': path_ccks_2020_ner_dev # path_tag_people_2014_valid, # path_seg_pku_1998_train  # path_ner_people_1998_valid    # 验证数据
                     },
        }
    ...
    if not hyper_parameters:
        hyper_parameters = {
            ...
            'model': {'label': 14,  # 类别数
                      ...
                      'optimizer_name': 'ADAM',  # 可填'ADAM', 'RADAM', 'RADAM,LOOKAHEAD'
                      'is_training': True,  # 训练后者是测试模型, 训练时候是True, 测试时候是False
                      'path_model_dir': os.path.join(path_model_dir, "ner_ccks_2020_crf"),  # 保存当前训练模型的根目录
                      'model_path': os.path.join(path_model_dir, "bilstm_crf.model"),
                      # 模型地址, loss降低则保存的依据, save_best_only=True, save_weights_only=True
                      'path_hyper_parameters': os.path.join(path_model_dir, "hyper_parameters.json"),
                      # 模型(包括embedding)，超参数地址,
