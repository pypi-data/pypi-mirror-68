...
# Check packages
def check_packages(packages, import_into_global_namespace=True, language="python", verbose=False):
    """
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            for pkg in packages:
                try:
                    package = import_package(pkg)
                    if import_into_global_namespace:
                        globals()[pkg] = package
                    if verbose:
                        print("Importing {}:".format(pkg), True, file=sys.stderr)
                except importing_error:
                    missing_packages.append(pkg)
                    if verbose:
                        print("Importing {}:".format(pkg), False, file=sys.stderr)
