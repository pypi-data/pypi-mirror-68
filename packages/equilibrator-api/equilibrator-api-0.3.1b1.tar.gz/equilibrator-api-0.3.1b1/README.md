...
```python
rxn = eq_api.parse_reaction_formula("kegg:C00002 + kegg:C00001 = kegg:C00008 + kegg:C00009")
```
...
```python
eq_api.p_h = Q_(6.5)  # set pH
eq_api.ionic_strength = Q_("200 mM")  # set I
...
```python
rxn = eq_api.parse_reaction_formula("kegg:C00002 + CHEBI:15377 = metanetx.chemical:MNXM7 + bigg.metabolite:pi")
```
...
```python
rxn = eq_api.search_reaction("beta-D-glucose = glucose")
print(rxn)
```
