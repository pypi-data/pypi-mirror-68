...
class BertTokenizer(object):
    ...
    def __init__(self, base_vocab_path, final_vocab_path, lang, uncased=True):
        self._lang = lang
        self._uncased = uncased
        self._tokenizer = BertWordPieceTokenizer(base_vocab_path, lowercase=uncased)
        ...
        print("Load vocab of size (original {}) = {} ".format(len(self._tokenizer.get_vocab()), len(vocab)))
    ...
    def _map_word_to_subword(self, wor_tokens, sub_data):
        results = []
        for w in wor_tokens:
            if w[0] not in set(sub_data.tokens):
                results.append(w[0])
    ...
    def _tokenize(self, text, mode=CHAR):
        if mode == self.CHAR:
            results = self._tokenizer.encode(text, add_special_tokens=False).tokens

        elif mode == self.WORD:
            wor_tokens = self._word_tokenize(text)
            results = []
            for w in wor_tokens:
                sub_tokens = self._tokenizer.encode(w[0], add_special_tokens=False).tokens
            ...
            sub_res = self._tokenizer.encode(text, add_special_tokens=False)
            wor_tokens = self._word_tokenize(text)
            new_tokens = self._map_word_to_subword(wor_tokens, sub_res)
            results = sub_res.tokens

            for w in new_tokens:
                sub_tokens = self._tokenizer.encode(w, add_special_tokens=False).tokens
                if len(sub_tokens) > 1 and tuple(sub_tokens) in self._valid_vocab:
                    results.append(tuple(sub_tokens))
        else:
            raise Exception("Not supported yet")
...
if __name__ == "__main__":
    base = '/home/tonyzhao/Documents/Projects/Encoders/soco_encoders/resources/bert-base-chinese/bert-base-chinese.txt'
    final = '/home/tonyzhao/Documents/Projects/Encoders/resources/checkpoint-72000/term_vocab.json'
    t = BertTokenizer(base, final, 'zh')
