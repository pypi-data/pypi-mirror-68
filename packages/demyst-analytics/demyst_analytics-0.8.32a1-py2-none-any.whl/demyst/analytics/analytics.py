...
class Analytics(object):
    ...
    def enrich_download(self, id, providers=[], block_until_complete=True):
        ...
        """
        return track_function(lambda: self.__do_enrich_download(id, providers=providers, block_until_complete=block_until_complete),
                              self.__config, "API : Python : Download",
                              { "job_id": id })

    def __do_enrich_download(self, id, providers=[], block_until_complete=True):
        fd, temp_path = tempfile.mkstemp()
        try:
