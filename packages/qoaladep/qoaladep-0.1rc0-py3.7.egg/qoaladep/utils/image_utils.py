...
def draw_label(image_array, 
    ...
    """    
    rad = int(width/6.)
    img = cv2.rectangle(image_array, bbox[0], (bbox[0][0] + width, bbox[0][1] - height), color, thickness=-1)
    img = cv2.rectangle(image_array, (bbox[0] + rad, bbox[0][1] - height - rad), (bbox[0][0] + width - rad, bbox[0][1] - height), color, thickness=-1)
    img = cv2.circle(img, (bbox[0][0] + rad, bbox[0][1] - height), rad, color, -1)
    img = cv2.circle(img, (bbox[1][0] - rad, bbox[0][1] - height), rad, color, -1)
    return img 