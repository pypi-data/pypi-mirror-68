...
class ProblemSummary:
    ...
    def tests_fraction(self) -> str:
        ...
        if self.problem_short == "coloring":
            return f"{numerator}/{denominator} = {numerator * 4}/{denominator * 4}"
        if self.problem_short == "bst-avl":
            return f"{numerator}/{denominator} = {round(numerator * 55/74, 3)}/{55}"
        if self.problem_short == "grocery":
            return f"{numerator}/{denominator} = {round(numerator * 15/20, 3)}/{15}"
        if self.problem_short == "trie":
            return f"{numerator}/{denominator} = {round(numerator * 15/28, 3)}/{15}"
...
def summarize(grading_schema: dict, report: dict) -> ReportSummary:
    ...
    for problem_short, problem in grading_schema["problems"].items():
        problem_summary = summary.problems[problem_short] = ProblemSummary(problem_short)
