...
class ProxyLibrary(object):
    ...
    def store(self, value, proxy_id=None):
        ...
        logger.debug('proc mem: %s, getsizeof: %s' % (proc_mem, mem2))

        if 'memory_usage' in dir(value):
            mem = value.memory_usage()
            if not isinstance(mem, int):
                mem = mem.sum()
            logger.debug('memory usage via sum(): %s' % mem)
    ...
    def pprint(self):
        ...
        logger.debug('-' * 40)
        logger.debug('\t'.join(['i', 'pxid', 't', 'getsizeof', 'memory_usage().sum', 'nbytes']))
        for idx, i in enumerate(ids_by_time):
            ...
            logger.debug('%s\t%s\t%d\t%.1f MB\t%.1f MB\t%.1f MB' % (idx, proxy_id.pxid, t, mem/1e6, mem2/1e6, mem3/1e6))
        logger.debug('-' * 40)
    ...
    def maybe_prune(self):
        ...
        for i in ids_by_time:
            logger.debug('need to prune: %.1f MB' % (need_to_prune / float(1e6)))
            if need_to_prune < 0:
                break
            proxy_id = self.objects_to_id[i]
            obj = self.objects[proxy_id.pxid]
    ...
    def evict(self, obj):
        ...
        logger.debug('evicting %s', proxy_id.pxid)
...
class CloudArrayServer(object):
    ...
    def save(self, proxy_id_ser, apikey, name):
        ...
        if proxy_id.classname == 'pandas.core.frame.DataFrame':
            ...
            with open(path, 'rb') as f:
                ...
                logger.debug('saving %s/%s to s3' % (self.data_bucket, key))
                resp = s3.put_object(Body=f, Bucket=self.data_bucket, Key=key)
                logger.debug('s3 resp', resp)
            return 'ok'
    ...
    def load(self, apikey, name):
        ...
        # logger.debug('read obj:', obj)
        # proxy_id = self.lib.store(obj)
        # logger.debug('got proxy_id', proxy_id)
