...
from pandas_cloud.compat import PY2, StringIO, lrange, lzip, range

from pandas_cloud.core.dtypes.common import is_float_dtype, is_integer_dtype

import pandas_cloud as pd
from pandas_cloud import DataFrame, Index, MultiIndex, NaT, Series
from pandas_cloud.core.indexing import (
    _maybe_numeric_slice, _non_reducing_slice, validate_indices)
from pandas_cloud.tests.indexing.common import Base, _mklbl
...
class TestFancy(CloudArrayTest, Base):
...
class TestMisc(CloudArrayTest, Base):
...
class TestSeriesNoneCoercion(CloudArrayTest, object):
    EXPECTED_RESULTS = [
...
class TestDataframeNoneCoercion(CloudArrayTest, object):
