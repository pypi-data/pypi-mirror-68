...
from numpy_cloud.lib._iotools import ConverterError, ConversionWarning
from numpy_cloud.compat import asbytes, bytes, unicode, Path
...
class TestSaveLoad(CloudArrayTest):
...
class TestSavezLoad(CloudArrayTest):
    def roundtrip(self, *args, **kwargs):
...
class TestSaveTxt(CloudArrayTest):
    def test_array(self):
        a = np.array([[1, 2], [3, 4]], float)
        fmt = "%.18e"
        c = BytesIO()
...
class TestLoadTxt(CloudArrayTest):
...
class Testfromregex(CloudArrayTest):
    def test_record(self):
        c = TextIO()
        c.write('1.312 foo\n1.534 bar\n4.444 qux')
        c.seek(0)
...
class TestFromTxt(CloudArrayTest):
...
class TestPathUsage(CloudArrayTest):
