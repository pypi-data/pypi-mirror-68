...
import numpy_cloud.polynomial.chebyshev as cheb
from numpy_cloud.polynomial.polynomial import polyval
...
class TestPrivate(CloudArrayTest):
...
class TestArithmetic(CloudArrayTest):
...
class TestEvaluation(CloudArrayTest):
...
class TestIntegral(CloudArrayTest):
...
class TestDerivative(CloudArrayTest):
...
class TestVander(CloudArrayTest):
    # some random values in [-1, 1)
    x = np.random.random((3, 5))*2 - 1
...
class TestFitting(CloudArrayTest):
...
class TestInterpolate(CloudArrayTest):
...
class TestCompanion(CloudArrayTest):
...
class TestGauss(CloudArrayTest):
