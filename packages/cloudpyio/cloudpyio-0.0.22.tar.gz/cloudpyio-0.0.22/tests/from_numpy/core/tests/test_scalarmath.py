...
class TestBaseMath(CloudArrayTest):
...
class TestModulus(CloudArrayTest):
...
class TestComplexDivision(CloudArrayTest):
...
class TestMultiply(CloudArrayTest):
...
class TestNegative(CloudArrayTest):
...
class TestSubtract(CloudArrayTest):
    def test_exceptions(self):
        a = np.ones((), dtype=np.bool_)[()]
