...
class TestNonTupleNDIndexDeprecation(CloudArrayTest):
    def test_basic(self):
        a = np.zeros((5, 5))
        with warnings.catch_warnings():
...
class TestRankDeprecation(CloudArrayTest):
...
class TestComparisonDeprecations(CloudArrayTest):
...
class TestDatetime64Timezone(CloudArrayTest):
...
class TestNonCContiguousViewDeprecation(CloudArrayTest):
...
class TestInvalidOrderParameterInputForFlattenArrayDeprecation(CloudArrayTest):
...
class TestArrayDataAttributeAssignmentDeprecation(CloudArrayTest):
...
class TestLinspaceInvalidNumParameter(CloudArrayTest):
...
class TestBinaryReprInsufficientWidthParameterForRepresentation(CloudArrayTest):
    """
...
class TestNumericStyleTypecodes(CloudArrayTest):
    """
...
class TestTestDeprecated(CloudArrayTest):
...
class TestClassicIntDivision(CloudArrayTest):
    """
...
class TestNonNumericConjugate(CloudArrayTest):
    """
...
class TestNPY_CHAR(CloudArrayTest):
    # 2017-05-03, 1.13.0
    def test_npy_char_deprecation(self):
        from numpy_cloud.core._multiarray_tests import npy_char_deprecation
        self.assert_deprecated(npy_char_deprecation)
        assert_(npy_char_deprecation() == 'S1')


class Test_UPDATEIFCOPY(CloudArrayTest):
    """
    ...
    def test_npy_updateifcopy_deprecation(self):
        from numpy_cloud.core._multiarray_tests import npy_updateifcopy_deprecation
        arr = np.arange(9).reshape(3, 3)
        v = arr.T
        self.assert_deprecated(npy_updateifcopy_deprecation, args=(v,))


class TestDatetimeEvent(CloudArrayTest):
    # 2017-08-11, 1.14.0
...
class TestTruthTestingEmptyArrays(CloudArrayTest):
    # 2017-09-25, 1.14.0
...
class TestGeneratorSum(CloudArrayTest):
    # 2018-02-25, 1.15.0
    def test_generator_sum(self):
        self.assert_deprecated(np.sum, args=((i for i in range(5)),))


class TestSctypeNA(CloudArrayTest):
    # 2018-06-24, 1.16
...
class TestPositiveOnNonNumerical(CloudArrayTest):
    # 2018-06-28, 1.16.0
    def test_positive_on_non_number(self):
        self.assert_deprecated(operator.pos, args=(np.array('foo'),))

class TestFromstring(CloudArrayTest):
    # 2017-10-19, 1.14
