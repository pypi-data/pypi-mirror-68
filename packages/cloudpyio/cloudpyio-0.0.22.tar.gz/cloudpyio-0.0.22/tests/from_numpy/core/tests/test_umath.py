...
import numpy_cloud.core.umath as ncu
from numpy_cloud.core import _umath_tests as ncu_tests
...
class TestConstants(CloudArrayTest):
    def test_pi(self):
...
class TestComparisons(CloudArrayTest):
...
class TestDivision(CloudArrayTest):
...
class TestRemainder(CloudArrayTest):
...
class TestPower(CloudArrayTest):
...
class TestFloat_power(CloudArrayTest):
...
class TestLogAddExp2(CloudArrayTest):
...
class TestLogAddExp(CloudArrayTest):
...
class TestLog1p(CloudArrayTest):
    def test_log1p(self):
...
class TestExpm1(CloudArrayTest):
    def test_expm1(self):
...
class TestHypot(CloudArrayTest):
...
class TestHypotSpecialValues(CloudArrayTest):
...
class TestArctan2SpecialValues(CloudArrayTest):
...
class TestLdexp(CloudArrayTest):
...
class TestMaximum(CloudArrayTest):
    def test_reduce(self):
...
class TestMinimum(CloudArrayTest):
    def test_reduce(self):
        dflt = np.typecodes['AllFloat']
        dint = np.typecodes['AllInteger']
        seq1 = np.arange(11)
...
class TestBitwiseUFuncs(CloudArrayTest):
...
class TestFloatingPoint(CloudArrayTest):
    def test_floating_point(self):
        assert_equal(ncu.FLOATING_POINT_SUPPORT, 1)


class TestDegrees(CloudArrayTest):
    def test_degrees(self):
        assert_almost_equal(ncu.degrees(np.pi), 180.0)
        assert_almost_equal(ncu.degrees(-0.5*np.pi), -90.0)


class TestRadians(CloudArrayTest):
    def test_radians(self):
        assert_almost_equal(ncu.radians(180.0), np.pi)
        assert_almost_equal(ncu.radians(-90.0), -0.5*np.pi)


class TestHeavside(CloudArrayTest):
...
class TestAbsoluteNegative(CloudArrayTest):
...
class TestPositive(CloudArrayTest):
    def test_valid(self):
...
class TestSpecialMethods(CloudArrayTest):
    def test_wrap(self):
...
class TestRationalFunctions(CloudArrayTest):
    def test_lcm(self):
...
class TestComplexFunctions(CloudArrayTest):
...
class TestSubclass(CloudArrayTest):
