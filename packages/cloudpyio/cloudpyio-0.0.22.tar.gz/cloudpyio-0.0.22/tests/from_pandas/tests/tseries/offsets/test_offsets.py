...
import pytest
import pytz
from pandas_cloud.compat import range
from pandas_cloud import compat

import numpy_cloud as np

from pandas_cloud.compat.numpy import np_datetime64_compat

from pandas_cloud.core.series import Series
from pandas_cloud._libs.tslibs import conversion
from pandas_cloud._libs.tslibs.frequencies import (get_freq_code, get_freq_str,
                                             INVALID_FREQ_ERR_MSG)
from pandas_cloud.tseries.frequencies import _offset_map, get_offset
from pandas_cloud.core.indexes.datetimes import _to_m8, DatetimeIndex
from pandas_cloud.core.indexes.timedeltas import TimedeltaIndex
import pandas_cloud._libs.tslibs.offsets as liboffsets
from pandas_cloud.tseries.offsets import (BDay, CDay, BQuarterEnd, BMonthEnd,
...
import pandas_cloud.tseries.offsets as offsets
from pandas_cloud.io.pickle import read_pickle
from pandas_cloud._libs.tslibs import timezones
from pandas_cloud._libs.tslib import NaT, Timestamp
from pandas_cloud._libs.tslibs.timedeltas import Timedelta
import pandas_cloud._libs.tslib as tslib
import pandas_cloud.util.testing as tm
from pandas_cloud.tseries.holiday import USFederalHolidayCalendar
...
class TestCommon(CloudArrayTest, Base):
...
class TestDateOffset(CloudArrayTest, Base):
...
class TestBusinessDay(CloudArrayTest, Base):
    _offset = BDay
...
class TestBusinessHour(CloudArrayTest, Base):
...
class TestCustomBusinessHour(CloudArrayTest, Base):
...
class TestCustomBusinessDay(CloudArrayTest, Base):
    _offset = CDay
...
class TestCustomBusinessMonthEnd(CloudArrayTest, CustomBusinessMonthBase, Base):
    _offset = CBMonthEnd
...
class TestCustomBusinessMonthBegin(CloudArrayTest, CustomBusinessMonthBase, Base):
...
class TestWeek(CloudArrayTest, Base):
    _offset = Week
    d = Timestamp(datetime(2008, 1, 2))
    offset1 = _offset()
    offset2 = _offset(2)
...
class TestWeekOfMonth(CloudArrayTest, Base):
    _offset = WeekOfMonth
    offset1 = _offset()
    offset2 = _offset(2)
...
class TestLastWeekOfMonth(CloudArrayTest, Base):
...
class TestSemiMonthEnd(CloudArrayTest, Base):
...
class TestSemiMonthBegin(CloudArrayTest, Base):
...
class TestOffsetNames(CloudArrayTest, object):
...
class TestOffsetAliases(CloudArrayTest, object):
...
class TestReprNames(CloudArrayTest, object):
...
class TestDST(CloudArrayTest, object):
    """
...
class TestCalendarDay(CloudArrayTest, object):
