...
from pandas_cloud.compat import PY3, intern
from pandas_cloud.util._decorators import deprecate_kwarg, make_signature
from pandas_cloud.util._move import BadMove, move_into_mutable_buffer, stolenbuf
# rewrite: omit test decoratores (td) import
from pandas_cloud.util._validators import (
...
class TestDecorators(CloudArrayTest, object):
...
class TestValidateArgs(CloudArrayTest, object):
    fname = 'func'
...
class TestValidateKwargs(CloudArrayTest, object):
    fname = 'func'
...
class TestValidateKwargsAndArgs(CloudArrayTest, object):
    fname = 'func'
...
class TestMove(CloudArrayTest, object):
...
def test_numpy_errstate_is_default():
    ...
    from pandas_cloud.compat import numpy_cloud  # noqa
    # The errstate should be unchanged after that import.
    assert np.geterr() == expected


# rewrite: omit @td...
class TestLocaleUtils(CloudArrayTest, object):
