...
class TestCreateZerosPlain(CloudArrayTest):
    """Check the creation of heterogeneous arrays zero-valued (plain)"""
    _descr = Pdescr

class TestCreateZerosNested(CloudArrayTest):
...
class TestCreateValuesPlainSingle(CloudArrayTest):
    """Check the creation of heterogeneous arrays (plain, single row)"""
    _descr = Pdescr
    multiple_rows = 0
    _buffer = PbufferT[0]

class TestCreateValuesPlainMultiple(CloudArrayTest):
    """Check the creation of heterogeneous arrays (plain, multiple rows)"""
    _descr = Pdescr
    multiple_rows = 1
    _buffer = PbufferT

class TestCreateValuesNestedSingle(CloudArrayTest):
    """Check the creation of heterogeneous arrays (nested, single row)"""
    _descr = Ndescr
    multiple_rows = 0
    _buffer = NbufferT[0]

class TestCreateValuesNestedMultiple(CloudArrayTest):
...
class TestReadValuesPlainSingle(CloudArrayTest):
    """Check the creation of heterogeneous arrays (plain, single row)"""
    _descr = Pdescr
    multiple_rows = 0
    _buffer = PbufferT[0]

class TestReadValuesPlainMultiple(CloudArrayTest):
...
class TestReadValuesNestedSingle(CloudArrayTest):
    """Check the values of heterogeneous arrays (nested, single row)"""
    _descr = Ndescr
    multiple_rows = False
    _buffer = NbufferT[0]

class TestReadValuesNestedMultiple(CloudArrayTest):
    """Check the values of heterogeneous arrays (nested, multiple rows)"""
    _descr = Ndescr
    multiple_rows = True
    _buffer = NbufferT

class TestEmptyField(CloudArrayTest):
...
class TestCommonType(CloudArrayTest):
...
class TestMultipleFields(CloudArrayTest):
    def setup(self):
...
class TestIsSubDType(CloudArrayTest):
...
class TestSctypeDict(CloudArrayTest):
    def test_longdouble(self):
        assert_(np.sctypeDict['f8'] is not np.longdouble)
        assert_(np.sctypeDict['c16'] is not np.clongdouble)


class TestBitName(CloudArrayTest):
    def test_abstract(self):
        assert_raises(ValueError, np.core.numerictypes.bitname, np.floating)


class TestMaximumSctype(CloudArrayTest):
...
class Test_sctype2char(CloudArrayTest):
    ...
    def test_third_party_scalar_type(self):
        from numpy_cloud.core._rational_tests import rational
...
class TestDocStrings(CloudArrayTest):
