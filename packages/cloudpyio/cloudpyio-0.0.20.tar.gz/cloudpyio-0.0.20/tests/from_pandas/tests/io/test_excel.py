...
from pandas_cloud.compat import PY36, BytesIO, iteritems, map, range, u
# rewrite: omit test decoratores (td) import

import pandas_cloud as pd
from pandas_cloud import DataFrame, Index, MultiIndex, Series
from pandas_cloud.core.config import get_option, set_option
import pandas_cloud.util.testing as tm
from pandas_cloud.util.testing import ensure_clean, makeCustomDataframe as mkdf

from pandas_cloud.io.common import URLError
from pandas_cloud.io.excel import (
    ExcelFile, ExcelWriter, _OpenpyxlWriter, _XlsxWriter, _XlwtWriter,
    read_excel, register_writer)
from pandas_cloud.io.formats.excel import ExcelFormatter
from pandas_cloud.io.parsers import read_csv
...
class SharedItems(object):
    ...
    def setup_method(self, datapath):
        if hasattr(super(), "setup_method"): super().datapath_method(setup)
...
class TestXlrdReader(CloudArrayTest, ReadingTestsBase):
    """
...
class TestExcelWriter(CloudArrayTest, _WriterBase):
...
@pytest.mark.parametrize("merge_cells,ext,engine", [
    (None, '.xlsx', 'openpyxl')])
class TestOpenpyxlTests(CloudArrayTest, _WriterBase):
    ...
    def test_write_cells_merge_styled(self, merge_cells, ext, engine):
        from pandas_cloud.io.formats.excel import ExcelCell
...
@pytest.mark.parametrize("merge_cells,ext,engine", [
    (None, '.xls', 'xlwt')])
class TestXlwtTests(CloudArrayTest, _WriterBase):
...
class TestXlsxWriterTests(CloudArrayTest, _WriterBase):
...
class TestExcelWriterEngineTests(CloudArrayTest, object):
...
class TestFSPath(CloudArrayTest, object):
