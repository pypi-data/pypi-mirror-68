...
from pandas_cloud.compat import (zip, range, lrange, StringIO)
from pandas_cloud import DataFrame, Series, Index, MultiIndex, date_range
import pandas_cloud as pd
import numpy_cloud as np

from numpy_cloud.random import randn

from pandas_cloud.util.testing import (assert_series_equal,
                                 assert_frame_equal,
...
from pandas_cloud.tests.frame.common import TestData
...
class TestCompat(CloudArrayTest, object):
...
class TestDataFrameEval(CloudArrayTest, TestData):
...
class TestDataFrameQueryWithMultiIndex(CloudArrayTest, object):
...
# rewrite: omit @td...
class TestDataFrameQueryNumExprPandas(CloudArrayTest, object):
    ...
    def test_query_scope(self):
        from pandas_cloud.core.computation.ops import UndefinedVariableError
        engine, parser = self.engine, self.parser
        skip_if_no_pandas_parser(parser)
    ...
    def test_query_builtin(self):
        from pandas_cloud.core.computation.engines import NumExprClobberingError
        engine, parser = self.engine, self.parser
...
# rewrite: omit @td...
class TestDataFrameQueryNumExprPython(CloudArrayTest, TestDataFrameQueryNumExprPandas):
...
class TestDataFrameQueryPythonPandas(CloudArrayTest, TestDataFrameQueryNumExprPandas):
...
class TestDataFrameQueryPythonPython(CloudArrayTest, TestDataFrameQueryNumExprPython):
...
class TestDataFrameQueryStrings(CloudArrayTest, object):
...
class TestDataFrameEvalWithFrame(CloudArrayTest, object):
