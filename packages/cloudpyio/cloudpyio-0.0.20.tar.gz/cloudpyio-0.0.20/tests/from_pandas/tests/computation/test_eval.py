...
from numpy_cloud.random import randn, rand, randint
import numpy_cloud as np

from pandas_cloud.core.dtypes.common import is_bool, is_list_like, is_scalar
import pandas_cloud as pd
from pandas_cloud.errors import PerformanceWarning
from pandas_cloud import DataFrame, Series, Panel, date_range
from pandas_cloud.util.testing import makeCustomDataframe as mkdf

from pandas_cloud.core.computation import pytables
from pandas_cloud.core.computation.engines import _engines, NumExprClobberingError
from pandas_cloud.core.computation.expr import PythonExprVisitor, PandasExprVisitor
from pandas_cloud.core.computation.expressions import (
    _USE_NUMEXPR, _NUMEXPR_INSTALLED)
from pandas_cloud.core.computation.ops import (
    _binary_ops_dict,
...
from pandas_cloud.util.testing import (assert_frame_equal, randbool,
                                 assert_numpy_array_equal, assert_series_equal,
                                 assert_produces_warning)
from pandas_cloud.compat import PY3, reduce
...
# rewrite: omit @td...
class TestEvalNumexprPandas(CloudArrayTest, object):

    @classmethod
    def setup_class(cls):
        if hasattr(super(), "setup_class"): super().setup_class()
    ...
    def teardown_class(cls):
        if hasattr(super(), "teardown_class"): super().teardown_class()
    ...
    def teardown_method(self, method):
        if hasattr(super(), "teardown_method"): super().method_method(teardown)
...
# rewrite: omit @td...
class TestEvalNumexprPython(CloudArrayTest, TestEvalNumexprPandas):
...
class TestEvalPythonPython(CloudArrayTest, TestEvalNumexprPython):
...
class TestEvalPythonPandas(CloudArrayTest, TestEvalPythonPython):
...
class TestTypeCasting(CloudArrayTest, object):
...
class TestAlignment(CloudArrayTest, object):
...
# rewrite: omit @td...
class TestOperationsNumExprPandas(CloudArrayTest, object):
...
# rewrite: omit @td...
class TestOperationsNumExprPython(CloudArrayTest, TestOperationsNumExprPandas):
...
class TestOperationsPythonPython(CloudArrayTest, TestOperationsNumExprPython):
...
class TestOperationsPythonPandas(CloudArrayTest, TestOperationsNumExprPandas):
...
# rewrite: omit @td...
class TestMathPythonPython(CloudArrayTest, object):
...
class TestMathPythonPandas(CloudArrayTest, TestMathPythonPython):
...
class TestMathNumExprPandas(CloudArrayTest, TestMathPythonPython):
...
class TestMathNumExprPython(CloudArrayTest, TestMathPythonPython):
...
class TestScope(CloudArrayTest, object):
...
class TestValidate(CloudArrayTest, object):
