...
from numpy_cloud.core import umath
from numpy_cloud.random import rand, randint, randn
...
class TestNonarrayArgs(CloudArrayTest):
...
class TestIsscalar(CloudArrayTest):
...
class TestBoolScalar(CloudArrayTest):
...
class TestBoolArray(CloudArrayTest):
    def setup(self):
...
class TestBoolCmp(CloudArrayTest):
    def setup(self):
...
class TestSeterr(CloudArrayTest):
...
class TestFloatExceptions(CloudArrayTest):
...
class TestTypes(CloudArrayTest):
...
class TestFromiter(CloudArrayTest):
    def makegen(self):
        for x in range(24):
            yield x**2
...
class TestNonzero(CloudArrayTest):
...
class TestIndex(CloudArrayTest):
...
class TestBinaryRepr(CloudArrayTest):
    def test_zero(self):
...
class TestBaseRepr(CloudArrayTest):
    def test_base3(self):
...
class TestArrayComparisons(CloudArrayTest):
...
class TestAllclose(CloudArrayTest):
    rtol = 1e-5
    atol = 1e-8
...
class TestIsclose(CloudArrayTest):
    rtol = 1e-5
    atol = 1e-8
...
class TestStdVar(CloudArrayTest):
    def setup(self):
...
class TestStdVarComplex(CloudArrayTest):
    def test_basic(self):
...
class TestCreationFuncs(CloudArrayTest):
...
class TestLikeFuncs(CloudArrayTest):
...
class TestCorrelate(CloudArrayTest):
    def _setup(self, dt):
...
class TestConvolve(CloudArrayTest):
    def test_object(self):
        d = [1.] * 100
        k = [1.] * 3
...
class TestArgwhere(CloudArrayTest):
    def test_2D(self):
...
class TestStringFunction(CloudArrayTest):
...
class TestRollaxis(CloudArrayTest):
...
class TestMoveaxis(CloudArrayTest):
...
class TestCross(CloudArrayTest):
    def test_2x2(self):
        u = [1, 2]
        v = [3, 4]
        z = -2
...
class TestRequire(CloudArrayTest):
...
class TestBroadcast(CloudArrayTest):
...
class TestKeepdims(CloudArrayTest):
...
class TestTensordot(CloudArrayTest):
