...
PyObject *particle_new(PyTypeObject *type, PyObject *args, PyObject *kw)
{
	...
	if (!PyArg_ParseTuple(args, "OOif|f", &arg1, &arg2, &count, &radius, &scale))
	{
	...
	self->figure = pyxieParticle::new_particle(posBuff, colBuff, count, radius, scale);
	return (PyObject *)self;
}
...
PyTypeObject ParticleType = {
	PyVarObject_HEAD_INIT(NULL, 0) "igeCore.particle", /* tp_name */
