...
namespace pyxie
{
	...
	void  camera_dealloc(camera_obj *self)
	{
		...
		if (!IsTerminate()) self->camera->DecReference();
	...
	static PyObject *camera_Render(camera_obj *self, PyObject *args, PyObject* kwargs)
	{
        ...
        pyxieRenderContext* renderContext = pyxieRenderContext::InstancePtr();
		if (!renderContext) return NULL;
		{
			PyxieZoneScopedN("BeginScene");
			auto offscreen = tex ? tex->renderTarget : nullptr;
			renderContext->BeginScene(offscreen, Vec4(v[0], v[1], v[2], v[3]), clearColor, clearDepth);
		}
		{
			PyxieZoneScopedN("showcas Update");
			sco->showcase->Update(0.0f);
		}
		{
	...
	PyObject* camera_getOrthoHeight(camera_obj* self) {
		return PyFloat_FromDouble(self->camera->GetOrthoHeight());
	}
	int camera_setOrthoHeight(camera_obj* self, PyObject* value) {
		if (!(PyFloat_Check(value) || PyLong_Check(value))) {
			PyErr_SetString(PyExc_TypeError, "Only float value can be set to orthoWidth.");
			return -1;
		}
		self->camera->SetOrthoHeight((float)PyFloat_AsDouble(value));
		return 0;
	}
	...
	PyObject* camera_getwidthBase(camera_obj* self) {
		return PyBool_FromLong(self->camera->IsWidthBase());
	}

	int camera_setwidthBase(camera_obj* self, PyObject* value) {

		if (!PyLong_Check(value)) {
			PyErr_SetString(PyExc_TypeError, "Only bool value can be set to widthBase.");
			return -1;
		}
		uint32_t base = (uint32_t)PyLong_AsLong(value);
		if (base > 1) {
			PyErr_SetString(PyExc_TypeError, "Only bool value can be set to widthBase.");
			return -1;
		}
		self->camera->SetWidthBase(base);
	...
	PyGetSetDef camera_getsets[] = {
		...
		{ const_cast<char*>("orthoHeight"),    (getter)camera_getOrthoHeight,    (setter)camera_setOrthoHeight,orthoHeight_doc, NULL },
		...
		{ const_cast<char*>("widthBase"),    (getter)camera_getwidthBase,    (setter)camera_setwidthBase, widthBase_doc, NULL },
		{ NULL, NULL }
	};

	PyTypeObject CameraType = {
		PyVarObject_HEAD_INIT(NULL, 0)
		"igeCore.camera",						/* tp_name */
