...
namespace pyxie
{
	static int GetMaterialIndex(pyxieEditableFigure* obj, PyObject* arg) {
		int index = -1;
		if (PyLong_Check(arg)) {
	...
	{
		if (!IsTerminate()) self->editablefigure->DecReference();
	...
	static PyObject* editablefigure_SetMaterialParam(editablefigure_obj* self, PyObject* args)
	{
		...
		int index = GetMaterialIndex(self->editablefigure, materialNameOrIndex);
		if (index == -1) return NULL;
		...
		if (!self->editablefigure->SetMaterialParam(index, paramName, buff, type)) {
			PyErr_SetString(PyExc_TypeError, "parameter error.");
			return NULL;
		}
		Py_INCREF(Py_None);
	...
	static PyObject* editablefigure_SetMaterialParamTexture(editablefigure_obj* self, PyObject* args, PyObject* kwargs)
	{
		...
		if(!self->editablefigure->SetMaterialParam(index, samplerName, &sampler)) {
			PyErr_SetString(PyExc_TypeError, "parameter error.");
	...
	static PyObject* editablefigure_GetMaterialParam(editablefigure_obj* self, PyObject* args)
	{
		...
		int index = GetMaterialIndex(self->editablefigure, materialNameOrIndex);
		if (index == -1) return NULL;
		auto param = self->editablefigure->GetMaterialParam(index, paramName);
	...
	static PyObject* editablefigure_SetMaterialRenderState(editablefigure_obj* self, PyObject* args, PyObject* kwargs)
	{
		...
		int index = GetMaterialIndex(self->editablefigure, mateNameOrIndex);
		if (index == -1) return NULL;
		...
		self->editablefigure->SetMaterialState(index, (ShaderParameterKey)paramInfo->key, val);
	...
	static PyObject* editablefigure_EnableAlphaModeByTexture(editablefigure_obj* self, PyObject* args)
	{
		char* texturePath;
		if (!PyArg_ParseTuple(args, "s", &texturePath)) return NULL;
		self->editablefigure->EnableAlphaModeByTexture(texturePath);
		Py_INCREF(Py_None);
		return Py_None;
	}
	...
	static PyObject* editablefigure_SetParentJoint(editablefigure_obj* self, PyObject* args) 
	{
		...
		if (index != -1) {
			self->editablefigure->SetParentJoint(drawable, drawable->GetJointMatrix(index));
		}
		Py_INCREF(Py_None);
		return Py_None;
	}
	...
	static PyObject* editablefigure_getJoint(editablefigure_obj* self, PyObject* args) {
		...
		int index = GetJointIndex(self->editablefigure, arg);
		...
		{
			joint = self->editablefigure->GetJoint(index, space);
		}
	...
	static PyObject* editablefigure_setJoint(editablefigure_obj* self, PyObject* args, PyObject* kwargs) {
		...
		int index = GetJointIndex(self->editablefigure, arg);
		if (index != -1) {
			Joint joint = self->editablefigure->GetJoint(index, space);
			float* v;
			int d;
			float buff[4];
			if (arg1) {
			...
			self->editablefigure->SetJoint(index, joint, space);
		}
		else {
	...
	static PyObject* editablefigure_jointNameToIndex(editablefigure_obj* self, PyObject* args)
	{
		char* name;
		if (!PyArg_ParseTuple(args, "s", &name))return NULL;
		int index = self->editablefigure->GetJointIndex(GenerateNameHash(name));
		return PyLong_FromLong(index);
	}

	static PyObject* editablefigure_getJointParentIndex(editablefigure_obj* self, PyObject* args)
	{
		PyObject* arg;
		if (!PyArg_ParseTuple(args, "O", &arg))return NULL;
		int index = GetJointIndex(self->editablefigure, arg);
		if (index == -1) return NULL;
		int parentIndex = self->editablefigure->GetJointParentIndex(index);
		return PyLong_FromLong(index);
	}
	...
	static PyObject* editablefigure_SetMeshAlpha(editablefigure_obj* self, PyObject* args)
	{
		PyObject* meshNameOrIndex;
		float alpha;
		if (!PyArg_ParseTuple(args, "Of", &meshNameOrIndex, &alpha)) return NULL;
		int index = GetMeshIndex(self, meshNameOrIndex);
		if (index == -1) return NULL;
		self->editablefigure->SetMeshAlpha(index, alpha);
		Py_INCREF(Py_None);
		return Py_None;
	}

	static PyObject* editablefigure_GetMeshAlpha(editablefigure_obj* self, PyObject* args)
	{
		PyObject* meshNameOrIndex;
		if (!PyArg_ParseTuple(args, "O", &meshNameOrIndex)) return NULL;
		int index = GetMeshIndex(self, meshNameOrIndex);
		if (index == -1) return NULL;
		return PyFloat_FromDouble(self->editablefigure->GetMeshAlpha(index));
	}

	static PyObject* editablefigure_SetMeshWireframe(editablefigure_obj* self, PyObject* args)
	{
		PyObject* meshNameOrIndex;
		int wire = 1;
		if (!PyArg_ParseTuple(args, "O|i", &meshNameOrIndex, &wire)) return NULL;
		int index = GetMeshIndex(self, meshNameOrIndex);
		if (index == -1) return NULL;
		self->editablefigure->SetMeshWireFrame(index, wire);
		Py_INCREF(Py_None);
		return Py_None;
	}

	static PyObject* editablefigure_GetMeshWireframe(editablefigure_obj* self, PyObject* args)
	{
		PyObject* meshNameOrIndex;
		if (!PyArg_ParseTuple(args, "O", &meshNameOrIndex)) return NULL;
		int index = GetMeshIndex(self, meshNameOrIndex);
		if (index == -1) return NULL;
		return PyLong_FromLong(self->editablefigure->GetMeshWireFrame(index));
	}
	...
	{ "enableAlphaModeByTexture", (PyCFunction)editablefigure_EnableAlphaModeByTexture, METH_VARARGS, NULL},
	...
	{ "jointNameToIndex", (PyCFunction)editablefigure_jointNameToIndex, METH_VARARGS, jointNameToIndex_doc},
	{ "getJointParentIndex", (PyCFunction)editablefigure_getJointParentIndex, METH_VARARGS, getJointParentIndex_doc},
	...
	{ "setMeshAlpha",(PyCFunction)editablefigure_SetMeshAlpha,METH_VARARGS,setMeshAlpha_doc},
	{ "getMeshAlpha",(PyCFunction)editablefigure_GetMeshAlpha,METH_VARARGS,getMeshAlpha_doc},
	{ "setMeshWireframe",(PyCFunction)editablefigure_SetMeshWireframe,METH_VARARGS,setMeshWireframe_doc},
	{ "getMeshWireframe",(PyCFunction)editablefigure_GetMeshWireframe,METH_VARARGS,getMeshWireframe_doc},
