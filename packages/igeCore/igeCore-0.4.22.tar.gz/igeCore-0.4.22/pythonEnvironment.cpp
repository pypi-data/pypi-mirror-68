...
namespace pyxie
{
	...
	static PyObject* environment_getDirectionalLampIntensity(environment_obj* self, PyObject* args) {
		...
		if (i>2) {
			PyErr_SetString(PyExc_TypeError, "directional lamp index is in the range 0-2.");
			return NULL;
		}
	...
	static PyObject* environment_getPointLampRange(environment_obj* self, PyObject* args) {
		...
		if (!PyArg_ParseTuple(args, "i", &i))return NULL;
		if (i > 6) {
			PyErr_SetString(PyExc_TypeError, "point lamp index is in the range 0-6.");
			return NULL;
		}
