...
from ..persistence import PersistenceRecord, PersistenceRecordCollection
...
class SpintopTestRecordData(BaseDataClass):
    summary: TestRecordSummary = None
    features: List[FeatureRecord] = list


class SpintopTestRecord(PersistenceRecord):

     # PersistenceRecord interface
    index: TestIDRecord = None
    data: SpintopTestRecordData = SpintopTestRecordData

    ## Alias properties ##

    @property
    def summary(self) -> TestRecordSummary:
        return self.data.summary

    @summary.setter
    def summary(self, value):
        self.test_id = value.test_id
    ...
    @property
    def features(self):
        return self.data.features if self.data.features else tuple()
...
spintop_test_record_features = FieldsOf(SpintopTestRecord).data.features
spintop_test_record_summary = FieldsOf(SpintopTestRecord).data.summary
...
class SpintopTestRecordView(object):
    feature_prefix = ('features', lambda feat: type_of(feat), lambda feat: feat.complete_name)
    data_prefix = () 
    ...
    def apply(self, record, flatten_dict=True):
        ...
        if self.include_test_id:
            test_id_data = self.apply_default_test_id(record, flatten_dict=flatten_dict, attribute_error_policy='raise')
    ...
    def apply_default_test_id(self, record, key_prefix=None, **apply_kwargs):
        ...
        data = self.default_view.apply(record.summary, key_prefix=key_prefix, **apply_kwargs)
        return data
...
class SpintopTestRecordBuilder(NamedTuple):
    ...
    def build(self):
        record = SpintopTestRecord(summary=self.summary, features=sorted(self.features, key=lambda feature: feature.index))
...
TestUUID = FieldsOf(FeatureRecord).test_id.uuid
