...
class SpintopAPIClientModule(object):
    def __init__(self, api_url, selected_org_id=None, auth=None, messages_publisher=None):
        self._session = None

        self.auth = auth
        
        self._selected_org_id = selected_org_id
        self._messages_publisher = messages_publisher
        self._persistence_facade = SpintopAPIPersistenceFacade(self, messages=messages_publisher)
    ...
    def create_org(self, org_id):
        return self.session.post(self.get_link('tenancy.create_org', org_id=org_id)).json()
