...
class PersistenceFacade(object):
    ...
    def serialize_barrier(self, raw_records):
        raw_records = list(raw_records)
        schema = self.serializer.schema(many=True)

        if raw_records and isinstance(raw_records[0], PersistenceRecord):
    ...
    def create(self, records):
        serialized_records = self.serialize_barrier(records)
    ...
    def retrieve(self, query=None, deserialize=True, limit_range=None, with_data=True):
        ...
        if deserialize:
            yield from self.serializer.schema(many=True, partial=not with_data).load(r.as_dict() for r in records)
        else:
            yield from records
    ...
    def update(self, records, upsert=False):
        serialized_records = self.serialize_barrier(records)
        ret_val = self._update(serialized_records, upsert=upsert)
        self._publish_records_update(serialized_records)
        return ret_val
