...
class _SCMCubeIntegrationTester(object):
    ...
    @pytest.mark.parametrize("kw_args", [{}, {"a": "b"}])
    @pytest.mark.parametrize("memory_error", [True, False])
    def test_get_scm_timeseries_cubes(
        self, test_cube, memory_error, kw_args, regions_to_get
    ):
        tsftlf_cube = "mocked 124"
        tareacell_scmcube = "mocked 4389"

        land_mask_2d = np.array([[1, 0, 0, 1], [1, 0, 1, 0], [1, 1, 0, 1]])
        ...
        mocked_weights = {
            ...
            "World|Northern Hemisphere|Ocean": nh_mask * (1 - land_mask),
            "World|Southern Hemisphere|Ocean": (1 - nh_mask) * (1 - land_mask),
        }
        ...
        test_cube._loaded_paths = ["mocked"]
        ...
        for label, weights in mocked_weights.items():
            ...
            region_areas = {
                ...
                "World|Northern Hemisphere|Land": np.sum(
                    areas_2d * nh_mask_2d * land_mask_2d
                ),
                "World|Southern Hemisphere|Land": np.sum(
                    areas_2d * (1 - nh_mask_2d) * land_mask_2d
                ),
                "World|Northern Hemisphere|Ocean": np.sum(
                    areas_2d * nh_mask_2d * (1 - land_mask_2d)
                ),
                "World|Southern Hemisphere|Ocean": np.sum(
                    areas_2d * (1 - nh_mask_2d) * (1 - land_mask_2d)
                ),
            }
            ...
            exp_cube.cube.attributes["crunch_source_files"] = "Files: ['mocked']"
            ...
            exp_cube.cube.attributes["crunch_netcdf_scm_weight_kwargs"] = json.dumps(
        ...
        if memory_error:
            test_cube._crunch_in_memory = MagicMock(side_effect=MemoryError)
            test_cube.__init__ = MagicMock()
            test_cube.load_data_in_directory = MagicMock()
        ...
        if memory_error:
            assert test_cube.get_scm_timeseries_weights.call_count == 2
...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_get_thetao_data_scm_cubes(self, test_cube, test_cmip6_output_thetao_file):
        ...
        np.testing.assert_allclose(res["World"].cube.data[0, 0], 18.219496)
    ...
    def test_load_data_conflicting_phenomenon_metadata(
        self,
        test_cube,
        test_cmip6_output_conflicting_phenomenon_metadata_directory,
        caplog,
    ):
        test_cube.load_data_in_directory(
            test_cmip6_output_conflicting_phenomenon_metadata_directory
        )

        attempting_workaround_re = re.compile(
            "Concatenating cubes in "
            ".*CMIP6/CMIP/NCAR/CESM2/abrupt-4xCO2/r1i1p1f1/Omon/fgco2/gr/v20190927 "
            "failed, attempting workarounds"
        )
        attempting_workaround_idx = [
            i
            for i, m in enumerate(caplog.messages)
            if attempting_workaround_re.match(m)
        ][0]
        assert caplog.records[attempting_workaround_idx].levelno == logging.WARNING

        removing_time_origin_idx = caplog.messages.index(
            "Removing `time_origin` attribute from all cubes"
        )
        assert caplog.records[removing_time_origin_idx].levelno == logging.WARNING

        forcing_long_name_idx = caplog.messages.index(
            "Forcing `long_name` of all cubes to be `Surface Downward Mass Flux of Carbon as CO2 [kgC m-2 s-1]`"
        )
        assert caplog.records[forcing_long_name_idx].levelno == logging.WARNING
        ...
        assert obs_time_points[0] == cftime.DatetimeNoLeap(
            1, 1, 15, 12, 45, 0, 0, 3, 15
        )
        assert obs_time_points[-1] == cftime.DatetimeNoLeap(
            999, 12, 15, 12, 0, 0, 0, 5, 349
        )

        assert isinstance(test_cube.cube.metadata, iris.cube.CubeMetadata)

    def test_ocean_data_handling(
        self, test_cube, test_cmip6_output_ocean_data_handling_directory,
    ):
        test_cube.load_data_in_directory(
            test_cmip6_output_ocean_data_handling_directory
        )
        test_cube.get_scm_timeseries(
            regions=[
                "World",
