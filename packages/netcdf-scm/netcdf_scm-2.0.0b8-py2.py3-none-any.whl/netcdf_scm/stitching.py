...
def _get_stitched_openscmdf_metadata_header(  # pylint:disable=too-many-arguments
    ...
    (
        ...
        picontrol_file,
    ) = _get_continuous_timeseries_with_meta(fullpath, drs, normalise)
...
def _get_continuous_timeseries_with_meta(infile, drs, normalise):
    ...
    if pexp_id.endswith("piControl"):
        ...
        if normalise is None:
            # branching times not relevant
            return loaded, None, None

        picontrol_file = _get_parent_file_path(infile, parent_replacements, drs)
    ...
    parent_file_path = _get_parent_file_path(infile, parent_replacements, drs)
    (
        parent,
        picontrol_branching_time,
        picontrol_file,
    ) = _get_continuous_timeseries_with_meta(parent_file_path, drs, normalise)
...
def _get_parent_replacements(scmdf):
    # TODO: put this in public API in #16
...
def _get_parent_file_path(infile, parent_replacements, drs):
    # TODO: put in public API in #16
