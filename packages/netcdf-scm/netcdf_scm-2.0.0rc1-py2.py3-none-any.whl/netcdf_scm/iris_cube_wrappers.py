...
class SCMCube:  # pylint:disable=too-many-public-methods
    ...
    def __init__(self):
        ...
        self._loaded_paths = []
        """list(str): List of paths loaded whilst using this cube"""

        self._metadata_cubes = {}
        """dict of str: :obj:`SCMCube`: Cubes associated with this cube's metadata"""

        self._weight_calculators = {}
        """dict of tuple: :obj:`AreaSurfaceFractionWeightCalculator`: Weight calculators used by this cube"""

        self._have_guessed_realm = False
    ...
    def _load_and_concatenate_files_in_directory(
        ...
        try:
            ...
            long_names = []
            logger.warning(
                "Concatenating cubes in %s failed, attempting workarounds", directory
            )
            logger.warning("Removing `time_origin` attribute from all cubes")
            for ec in loaded_cubes_iris:
                long_names.append(ec.long_name)
                # try popping off the time_origin attribute
                ec.coord("time").attributes.pop("time_origin", None)

            long_name_majority_rules = max(set(long_names), key=long_names.count)
            # also try making sure that all long_name are the same
            logger.warning(
                "Forcing `long_name` of all cubes to be `%s`", long_name_majority_rules
            )
            for ec in loaded_cubes_iris:
                ec.long_name = long_name_majority_rules
    ...
    def get_scm_timeseries(self, **kwargs):
        """
        ...
        ----------
        **kwargs
            Passed to :meth:`get_scm_timeseries_cubes`
    ...
    def get_scm_timeseries_cubes(
        ...
        **kwargs
            Passed to :meth:`get_scm_timeseries_weights`

        Returns
        -------
        dict of str: :obj:`SCMCube`
            Dictionary of cubes (region: cube key: value pairs), with
            latitude-longitude mean data as appropriate for each of the requested
            regions.
        """
        scm_timeseries_weights = self.get_scm_timeseries_weights(**kwargs)
        ...
        def crunch_timeseries(region, weights):
            ...
            scm_cube = self._add_metadata_to_region_timeseries_cube(
                scm_cube, region, weight_kwargs=kwargs
            )
        ...
        if lazy or memory_error:
            ...
            # TODO: remove this, it probably has nasty side effects...
            self.__init__()
    ...
    def _add_metadata_to_region_timeseries_cube(self, scmcube, region, weight_kwargs):
        has_root_dir = (
        ...
        weight_kwargs_to_save = {
            k: v for k, v in weight_kwargs.items() if not isinstance(v, SCMCube)
        }

        scmcube.cube.attributes["crunch_netcdf_scm_weight_kwargs"] = json.dumps(
            weight_kwargs_to_save
        )
    ...
    def get_scm_timeseries_weights(
        ...
        weight_calculator = self._get_weight_calculator(cell_weights=cell_weights)

        regions = regions if regions is not None else DEFAULT_REGIONS
        scm_weights = weight_calculator.get_weights(regions)

        return scm_weights

    def _get_weight_calculator(self, cell_weights):
        key = (("cell_weights", cell_weights),)

        if key not in self._weight_calculators:
            self._weight_calculators[key] = self._create_weight_calculator(
                cell_weights=cell_weights
            )

        return self._weight_calculators[key]

    def _create_weight_calculator(self, cell_weights):
        logger.debug("cell_weights: %s", cell_weights)
        if cell_weights == "area-surface-fraction":
            out = AreaSurfaceFractionWeightCalculator(self)

        elif cell_weights == "area-only":
            out = AreaWeightCalculator(self)

        elif cell_weights is not None:  # pragma: no cover # emergency valve
            raise NotImplementedError(
                "Unrecognised cell_weights: {}".format(cell_weights)
            )

        else:
            logger.debug("self.netcdf_scm_realm: %s", self.netcdf_scm_realm)
            if self.netcdf_scm_realm == "ocean":
                out = AreaWeightCalculator(self)
...
class CMIP6Input4MIPsCube(_CMIPCube):
    ...
    def __init__(self):
        ...
        self.source_id = None
        """
        str: The source_id for which we want to load data e.g. ``UoM-REMIND-MAGPIE-ssp585-1-2-0``
        ...
        """str: The dataset_category for which we want to load data e.g. ``GHGConcentrations``"""
