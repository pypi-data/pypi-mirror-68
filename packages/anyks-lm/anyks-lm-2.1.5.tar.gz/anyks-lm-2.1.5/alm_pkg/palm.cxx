...
namespace anyks {
	...
	// Создаём объект языковой модели
	...
	namespace Methods {
		...
		const wstring getEmail() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_EMAIL);
		}
		/**
		...
		const wstring getPhone() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_PHONE);
		}
		/**
		...
		const wstring getAuthor() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_AUTHOR);
		}
		/**
		...
		/**
		 * initScripts Метод инициализации скриптов питона
		 */
		void initScripts() noexcept {
		 ...
		 */
		void setWordScript(const wstring & script) noexcept {
			// Выполняем установку скрипта обработки слов
			alm.setWordScript(alphabet.convert(script));
		}
		/**
		...
		const wstring getContact() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_CONTACT);
		}
		/**
		...
		const wstring getSite() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_SITE);
		}
		/**
		 ...
		 */
		const wstring getWordScript() noexcept {
			// Выводим результат
			return alphabet.convert(alm.getWordScript());
		}
		/**
		...
		const wstring getVersion() noexcept {
			// Выводим результат
			return alphabet.convert(ANYKS_VERSION);
		}
		/**
		...
		const bool isAllowApostrophe() noexcept {
			...
			return tokenizer.isAllowApostrophe();
		}
		/**
		 ...
		 */
		void setUserTokenScript(const wstring & script) noexcept {
			// Выполняем установку скрипта обработки пользовательских токенов
			alm.setUserTokenScript(alphabet.convert(script));
		}
		/**
		 ...
		 */
		const wstring getUserTokenScript() noexcept {
			// Выводим результат
			return alphabet.convert(alm.getUserTokenScript());
		}
		/**
		...
		void switchAllowApostrophe() noexcept {
			...
			tokenizer.switchAllowApostrophe();
		}
		/**
		...
		void readLM(const wstring & filename, const wstring & meta = L"") noexcept {
			...
			if(!filename.empty()){
				...
				if(filename.rfind(L".alm") != wstring::npos){
					...
					ablm_t ablm(alphabet.convert(filename), &alm, &alphabet, &tokenizer, (!logfile.empty() ? logfile.c_str() : nullptr));
					// Если метеданные получены
					if(!meta.empty()) ablm.setMeta(json::parse(alphabet.convert(meta)));
		...
		/**
		 * delInWord Метод очистки текста
		 * @param text текст для очистки
		 * @param flag флаг критерия очистки
		 * @return     текст без запрещенных символов
		 */
		const wstring delInWord(const wstring & text, const wdel_t flag = wdel_t::broken) noexcept {
			// Выполняем проверку флага
			switch(u_short(flag)){
...
PYBIND11_MODULE(alm, m) {
	 ...
	 */
	{
		...
		// initScripts Метод инициализации скриптов питона
		.def("initScripts", &anyks::Methods::initScripts, "Python script initialization method")
		// setLogfile Метод установки файла для вывода логов
		...
		// setWordScript Метод установки скрипта обработки слов
		.def("setWordScript", &anyks::Methods::setWordScript, "Method set script of word processing")
		// getWordScript Метод извлечения скрипта обработки слов
		.def("getWordScript", &anyks::Methods::getWordScript, "Method set script of word processing")
		// getContact Метод извлечения контактных данных автора
		...
		// setUserTokenScript Метод установки скрипта обработки пользовательских токенов
		.def("setUserTokenScript", &anyks::Methods::setUserTokenScript, "Method set script user token processing")
		// setTokenDisable Метод установки списка не идентифицируемых токенов
		...
		// getUserTokenScript Метод извлечения скрипта обработки пользовательских токенов
		.def("getUserTokenScript", &anyks::Methods::getUserTokenScript, "Method for extracting a user token processing script")
		// getTokensUnknown Метод извлечения списка токенов приводимых к <unk>
		...
		// delInWord Метод очистки текста
		.def("delInWord", overload_cast_ <const wstring &, const anyks::wdel_t> ()(&anyks::Methods::delInWord), "Method for delete letter in word", py::arg("text") = L"", py::arg("flag") = anyks::wdel_t::broken)
		// checkSequence Метод проверки существования последовательности
