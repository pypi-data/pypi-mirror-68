...
IMPORT_HttpOperationError_GROUP = [
    "Operation returned an invalid status code 'Http request failed.'",
    "Operation returned an invalid status code 'Could not access the specified path due to permission or read-only "
    "properties. Failed to authorize w",
    "Operation returned an invalid status code 'Failed to create or update the dataset. The dataset has been created "
    "or updated concurrently. ",
    "Operation returned an invalid status code 'DataPath object has no file paths, URLs or SQL queries that enable "
    "fetching data from the datastore. ",
    "Operation returned an invalid status code 'Authentication failed. Error when attempting to list contents of",
]
