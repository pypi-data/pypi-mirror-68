...
from ..common.commonerrorutil import IMPORT_USER_ERROR_GROUP, get_error_group, IMPORT_HttpOperationError_GROUP
...
class ImportTabularDataModule:
    ...
    def process_input_dataset_request_dto(cls, input_dataset_request_dto):
        ...
        3. set sourceProperties as None
        ...
        try:
            ...
            if not is_offline_run:
                ...
                node_id = str(uuid.uuid4())[:8]
            # logger.debug(f'Set name using node_id: {node_id}')
            ...
            if cls.override_data_path is None and cls.override_datastore_name is None:
                ...
                # logger.info(
                #     f"Return original request dto: {json.dumps(request)}")
                return request
            ...
            is_old_http_source = False
            if cls._HTTP_PATH_KEY in request['dataPath']:
                is_old_http_source = True
    ...
    @classmethod
    @retry(ConnectTimeoutError, tries=4, delay=90, backoff=1, logger=logger)
    def register_from_request(cls, request):
    ...
    def generate_dataflow(cls) -> 'Dataflow':
        ...
        try:
            try:
                cls.dataset = cls.register_from_request(request)
            ...
            if any(m in error_message for m in IMPORT_HttpOperationError_GROUP):
                hint_message = 'Register request failure. Please retry. ' + error_message
    ...
    def run(cls, input_dict: dict = None) -> DataFrameDirectory:
        ...
        try:
            ...
            if any(m in error_message for m in IMPORT_USER_ERROR_GROUP):
                ErrorMapping.rethrow(import_exceptions, UserError(error_message))
            if 'Unable to open stream from Azure Blob' in error_message:
                # Reference: AzureFileAccessor.cs
                hint_message = error_message + ' Please retry or revalidate your blob.'
...
def main(args):
    ...
    # logger.info(
    #     f'Received argument --input_dataset_request_dto: {input_dataset_request_dto}.')
    # logger.info(
    #     f'Received argument --override_data_path: {override_data_path}.')
    # logger.info(f'Received argument --output_dir: {output_dir}.')
