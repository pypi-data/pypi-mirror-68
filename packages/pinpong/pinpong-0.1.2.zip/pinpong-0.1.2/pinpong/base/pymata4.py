...
class Pymata4(threading.Thread):
    ...
    def __init__(self, com_port=None, baud_rate=115200,
        ...
        self.the_serial_receive_thread = threading.Thread(target=self._serial_receiver)
        ...
        if self.serial_port:
            print("Arduino compatible device found and connected to %s"%(self.serial_port.port))
        ...
        self.the_serial_receive_thread.start()
        ...
        try:
            ...
            if not firmware_version:
                ...
                print('Arduino Firmware ID: %s'%(firmware_version))
        except TypeError:
        ...
        try:
            ...
            print('Auto-discovery complete. Found %d Digital Pins'
                  ' and %d Analog Pins\n\n'%(len(self.digital_pins),len(self.analog_pins)))
    ...
    def _exit_handler(self, signum, frame):
        self.shutdown()
        if self.serial_port:
          print("_exit_handler close port")
