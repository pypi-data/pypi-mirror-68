...
def test_query_executioner_workflow(query):
    ...
    assert (
        meta_query1["units"]["BEVMK3"] == "Anzahl"
    ), "correct unit was not obtained"
    assert (
...
def test_get_query_specific_unit_meta(): 
    ...
    expected_stat_meta = {"WAHL09": "StatisticsGraphQlMetaDataProvider does not provide unit information."}
    assert query_stat_meta == expected_stat_meta