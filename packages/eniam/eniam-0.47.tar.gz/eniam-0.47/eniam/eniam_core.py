...
class MultiprocessDomparserResult():
    ...
    def _call_dom(self, opts):
        return self.inst._exec_dom_opt(opts).html()

    def html(self):
        pool = Pool()
        return pool.map(self._call_dom, list(map(lambda sentence: [sentence, self.opts], self.input)))
...
class Eniam():
    ...
    def dom(self, input, silent=True):
        if isinstance(input, list):
            ...
            return MultiprocessDomparserResult(input, inst, opts)
        else:
