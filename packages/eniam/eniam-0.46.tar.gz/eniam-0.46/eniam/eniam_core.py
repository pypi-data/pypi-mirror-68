...
class MultiprocessDomparserResult():
    def __init__(self, run_pool):
        self.run_pool = run_pool

    def html(self):
        html_outputs = self.run_pool(lambda out: out.html())
        return html_outputs
    ...
    def dom(self, input, silent=True):
        if isinstance(input, list):
            def run_pool(mapper):
                ...
                return pool.map(lambda opts: mapper(self._exec_dom_opt(opts)), map(lambda sentence: [sentence, opts], input))
            return MultiprocessDomparserResult(run_pool)
        else:
