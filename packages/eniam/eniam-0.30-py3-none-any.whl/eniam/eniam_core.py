...
def run_domparser(args=[], input=None, output=None, lexicon=None, valence=None):
    lexicon_input = "empty" if lexicon is None else b64encode(lexicon.encode()).decode()
    valence_input = "empty" if valence is None else b64encode(valence.encode()).decode()
    return subprocess.Popen(
        create_cmd(["docker", "run", "-i", DOCKER_IMAGE_NAME, "/root/domparser.sh", lexicon_input, valence_input] + args),
        stdin=input,
        stdout=output,
        stderr=output,
        shell=True,
...
class DomparserResult():
    ...
    def _replace_all_links(self, html_code):
        for original_file_name in self.file_map:
            ...
            if entry[0] == 'png':
                html_code = html_code.replace(f'<A HREF="{path}">', self.output_chart_html(entry[1]) + "<a style='display:none;'>")
            else:
    ...
    def show(self):
        ...
        display.display(display.HTML(self._replace_all_links("\n".join(self.html_conent))))
...
class Eniam():
    def __init__(self, lexicon_keywords=None, lexicon=None, valence_names=None, valence=None):
    ...
    def dom(self, input_text, silent=True):
        p = run_domparser(["--no-sem", "--no-def-cat"], PIPE, PIPE, self.lexicon_content, self.valence_content)
        q_out = Queue()
        q_err = Queue()
        ...
        while True:
            try:
                ...
                line_content = ""
                if (not collect_pdf) and (not collect_png):
