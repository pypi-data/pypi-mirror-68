...
class Type:
    ...
    def __call__(self, obj):
        for type_ in self._t:
            if isinstance(obj, type_):
                return obj
        raise ValidationError(f"Expected type '{self._t}', got '{type(obj)}'")

    def __repr__(self):
        typenames = [t.__name__ if isinstance(t, type) else repr(t)
                     for t in self._t]
        return f"Type([{', '.join(typenames)}])"
