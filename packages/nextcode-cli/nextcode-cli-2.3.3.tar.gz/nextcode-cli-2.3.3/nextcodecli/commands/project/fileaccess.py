...
@click.argument('filename', required=True)
@click.option(
    '--bytes', '-n', 'num_bytes', default=1024, help='Number of bytes to return'
)
@click.pass_context
@check_project
def head(ctx, filename, num_bytes):
    ...
    start_byte = 0
    stop_byte = num_bytes
    resp = client.get_object(Bucket=bucket.name, Key=filename, Range='bytes={}-{}'.format(start_byte, stop_byte))
