...
def ensure_num(
    ...
    def wrapper(inst):
        ...
        if isinstance(res, Real):
            ...
            params = inst.get_parameter_values()
            res = evaluate_expression(expr=res, params=params)
        return res
...
class BBox(ParametrizedChildEntity):
    ...
    def from_points(
        ...
        """
        params = parent.get_parameter_values() if parent else {}
        x_coords = [evaluate_expression(p[0], params=params) for p in pts]
        y_coords = [evaluate_expression(p[1], params=params) for p in pts]
    ...
    def _make_cell(self, add_foundry_params=False):
        ...
        try:
            ...
            with nd.Cell(self.full_name, hashme=False) as C:
                ...
                # BB outline
                layer = _get_or_add_layer("bb_outline_layer")
