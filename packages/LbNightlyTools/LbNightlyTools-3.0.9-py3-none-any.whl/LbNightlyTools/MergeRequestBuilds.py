...
def create_mr_slots(sources, platforms, merge, model_slots):
    ...
    target_branches = set(
        mr.attributes['target_branch'] for mr, _ in list(mrs.values()))
    ...
    for mr, project in list(mrs.values()):
    ...
    if not merge:
        ...
        ref_date = mr_getrefdate(list(mrs.values()))
    ...
    description = ('slot for testing {}, based on {}'.format(
        ', '.join(mr_link(mr, p) for mr, p in list(mrs.values())), model_slot.name))
    ...
    if not merge:
        for p, (mr, project) in list(mrs.items()):
    ...
    requested_projects = list(set([
        project.attributes['path'] for _, project in list(mrs.values())
    ]))
