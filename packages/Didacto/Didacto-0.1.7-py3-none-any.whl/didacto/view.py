...
class View(tkinter.Frame):
    ...
    def create_widgets(self):
        ...
        self.l0 = tkinter.Label(self, text="Emplacement:")
        self.l0.grid(row=0, sticky=tkinter.W)
        self.corpora_path = tkinter.StringVar('')
        self.l1 = tkinter.Label(self, textvariable=self.corpora_path)
        self.l1.grid(row=1,column=0, sticky=tkinter.W)
        ...
        self.keyword.set('Double clique sur un mots-clef pour obtenir une explication')
        self.keyword_explained = tkinter.StringVar()
        self.l2 = tkinter.Label(self, textvariable=self.keyword).grid(row=5, sticky=tkinter.W)
        #self.l3 = tkinter.Label(self, textvariable=self.keyword_explained).grid(row=5, column=1,sticky=tkinter.W+tkinter.S)
            ...
            for key in self.model.wordsDict:
                ...
                for list_index in range(len(self.model.wordsDict[key])):
                    if self.checkbox_value.get() is True:
                        path=self.relative_path(self.corpora_path.get(), self.model.wordsDict[key][list_index]['path'])
...
# Gestion de la souris        
    def on_double_click(self, event):
        ...
        if item.startswith('I'):
            ...
            if self.checkbox_value.get() is True:
                ...
                path = self.absolute_path(self.corpora_path.get(), tv_path)
    ...
    def on_right_click(self, event):
        ...
        item = self.tv.identify("item", event.x, event.y)
        name = self.tv.item(item)["values"][1][:-4]+'.ly'
        path = self.absolute_path(self.corpora_path.get(), name)
        self.controller.run_file(path)
...
class PreferencesWindow():
    def __init__(self, root, controller, view):
        ...
        self.controller = controller
        self.view = view
        self.preferences_window = tkinter.Toplevel(self.root)
...
"""\
Didacto 0.1.5\n
Un logiciel libre développé pour vous par Marco de Freitas.
\n
Site web du projet: https://gitlab.inubo.ch/glag/didacto
...
class HelpWindow():
    def __init__(self, arg):
        self.root = arg
        self.filewin = tkinter.Toplevel(self.root)
        text = tkinter.Text(self.filewin)
        text.insert(tkinter.INSERT,
"""\
Didacto est un logiciel d'aide à l'enseignement instrumental, qui facilite l'organisation didactique d'un répertoire d'exercices, d'études, ou authentique. \n
Didacot crée un index des mots clés utilisés dans fichiers pdf de votre répertoire d'enseignement. Pour chaque mots-clés, Didacto affiche la liste des noms de fichiers ainsi que leur emplacement dans votre arborescence de fichier.

Un clique sur un mot-clé ouvre la liste des morceaux contenant ce mots clef\n
...
Documentation complète: https://didacto.readthedocs.io/en/latest/ 
""")
        text.config(wrap=tkinter.WORD, state=tkinter.DISABLED)
        text.pack()
