...
from azureml.contrib.automl.dnn.vision.classification.common.utils import _get_default_device_name
...
from azureml.contrib.automl.dnn.vision.classification.io.write.write_model import _write_model
...
class TestInferenceModelWrapper:
    def test_inference_cpu(self, image_dir):
        ...
        _write_model(inference_model_wrapper.model_wrapper, labels=classes, output_dir='.')
        model_wrapper_pkl = ArtifactsLiterals.MODEL_WRAPPER_PKL
    ...
    def test_featurization_invalid_image_file(self, root_dir, image_dir, image_list_file_name):
        temp_image_class_list_file_path = os.path.join(tempfile.gettempdir(), image_list_file_name)
        try:
            ...
            try:
                os.remove(temp_image_class_list_file_path)
            except:  # ignore
                pass
    ...
    def _featurization_test(root_dir, image_dir, image_class_list_file_path,
        ...
        features_output_file = os.path.join(tempfile.gettempdir(), featurization_file)
        ...
        try:
            ...
            # noinspection PyBroadException
            try:
                os.remove(features_output_file)
            except:  # ignore
                pass
    ...
    def _featurization_batch_test(features_output_file, image_dir,
                                  ...
                                  expected_feature_file_length):
        _featurize_with_model(inference_model_wrapper, root_dir=image_dir,
                              output_file=features_output_file,
                              image_list_file=image_class_list_file_path, batch_size=batch_size)
    ...
    def _score_test(root_dir, image_dir, image_class_list_file_path, expected_score_file_length):
        ...
        predictions_output_file = os.path.join(tempfile.gettempdir(), predictions_file)
        ...
        try:
            ...
            # noinspection PyBroadException
            try:
                os.remove(predictions_output_file)
            except:  # ignore
                pass
