...
class TestDataLoader:
    ...
    def test_get_data_loader(self):
        ...
        dataloader = _get_data_loader(dataset, transform_fn=None, batch_size=2)
    ...
    def test_robust_get_data_loader(self):
        ...
        with pytest.raises(AutoMLVisionDataException):
            dataloader = _get_data_loader(dataset_not_ignore, transform_fn=transform_fn, batch_size=10)
            self._test_data_loader(dataloader)

        dataloader = _get_data_loader(dataset_ignore, transform_fn=transform_fn, batch_size=10)
        self._test_data_loader(dataloader)
