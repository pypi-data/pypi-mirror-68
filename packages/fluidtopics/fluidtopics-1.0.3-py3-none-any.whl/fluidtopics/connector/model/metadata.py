...
from typing import Set, Optional, List, Iterable, Union, Tuple
...
class Metadata:
    ...
    def __init__(self, key: str,
        ...
        try:
            self.value = self._compute_values(value)
        except InvalidMetadata as e:
            raise InvalidMetadata('Invalid "{}" metadata: {}'.format(key, e))
        if key in SemanticMetadata.ALL and not is_simple_value(self.value):
    ...
    def _compute_values(self, value: ValueType) -> Union[str, Set[str], Set[Tuple[str]]]:
        self._check_value(value)
        if is_simple_value(value):
            return value
        elif is_string_value(value) and len(value) == 1:
            return next(iter(value))
        try:
            return set(self._compute_value(v) for v in value)
        except InvalidMetadata as e:
            raise InvalidMetadata('{} in {}'.format(e, value))

    def _compute_value(self, value: Union[str, Iterable[str]]) -> Union[str, Tuple[str]]:
        self._check_value(value)
        if is_simple_value(value):
            return value
        try:
            return tuple(self._compute_value_part(v) for v in value)
        except InvalidMetadata as e:
            raise InvalidMetadata('{} in {}'.format(e, value))

    def _compute_value_part(self, value: str) -> str:
        self._check_value(value)
        return value

    def _check_value(self, value: ValueType):
        if not value:
