...
class Documento(DTE):
    ...
    def __init__(self, vals, resumen=False, tipo_dte=33):
        self.TipoDTE = tipo_dte
        self.resumen = resumen
        util.set_from_keys(self, vals, priorizar=['moneda_decimales', 'TasaIVA', 'MntIVA', 'IVA'])
...
llevar estrictamente las palabras Dice: y debe decir:')
    ...
    def Totales(self):
        ...
        if not self.MntTotal or self.no_product:
            Totales['MntTotal'] = 0
            return Totales
        if not self.es_exportacion and not self.es_boleta() and not self.MntBruto:
