...
class Profile:
    ...
    def __init__(
        self, file_paths, parser, q_axis_name="qdcd", theta_axis_name="dcdtheta", pixel_max=1000000, hot_pixel_threshold=200000
    ):
        self.scans = []
        try:
            kk = len(pixel_max)
        ...
        for i, f in enumerate(file_paths):
            self.scans.append(Scan(f, parser, q_axis_name, theta_axis_name, pixel_max=pixel_max[i], hot_pixel_threshold=hot_pixel_threshold))
        self.q_vectors = None
        ...
        for i in iterator:
            try:
                if self.data['roi1_maxval'][i] > self.pixel_max:
                    to_remove.append(i)
                    continue
                im = image.Image(self.data["file"][i], self.data, self.metadata, hot_pixel_threshold=self.hot_pixel_threshold)
