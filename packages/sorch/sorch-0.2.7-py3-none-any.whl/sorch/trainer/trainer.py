...
class MetricHistory(object):
    ...
    def get_aggregated_value(self,
        ...
        return aggregator(list(filter(metric_point_filter, self.history)))
...
class ScalarMetricHistory(MetricHistory):
    ...
    def add_metric_point(self, metric_point):
        ...
        self.total += metric_point.value

    def get_avg(self, metric_point_filter=lambda x: True):
        return self.total / float(len(self.history)) if self.history else float("nan")
...
class TrainerState(object):
    ...
    def clear_namespace(self, namespace):
        assert namespace in self.metrics
        self.metrics[namespace] = {}

    def get_info_dict(self, avg=False):
        info = {
        ...
        for k, v in six.iteritems(self.__dict__):
            if isinstance(v, (float, six.integer_types, six.string_types)):
                ...
                info[k] = v.get_avg() if avg else v.get_running_value()
            else:
                pass
    ...
    def _main_loop(self,
        ...
        for batch in dts_iterator:
            ...
            formatted_info_string = info_string.format(
                **self.state.get_info_dict(avg=not self.state.is_training)) + "  "
...
if __name__ == '__main__':
    ...
    st = STrainer(update_fn, modules=[module], events=[say_hello], tensorboard_log_dir='a')
    st.restore('a')
    st.train(range(3), steps=None)

    st.evaluate(range(4), steps=None)
    st.evaluate(range(4), steps=None)
