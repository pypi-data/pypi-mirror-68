...
class Calc2tex:
    ...
    def table(self, columns: int=1) -> str:
        ...
        start = ("\\begin{table}[htbp]", "    \\centering", "    \caption{"+language[self.lang]['table']['header']+"}", 
                 "    \\label{tab:input_val}", "    \\begin{tabular}{lcc}", " "*8 + "\\toprule", 
                 " "*8 + language[self.lang]['table']['var'] + " & " + language[self.lang]['table']['val'] + " & " + language[self.lang]['table']['unit'] + "\\\\", 
                 " "*8 + "\\midrule", "")
        end = (" "*8 + "\\bottomrule", "    \\end{tabular}", "\\end{table}")
