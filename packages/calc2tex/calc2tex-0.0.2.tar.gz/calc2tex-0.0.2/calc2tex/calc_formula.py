...
@author: stefa
"""
from .helper import is_float, search_char, search_bracket
from .settings import mult_sign
#Imports for evaluating formula:
import numpy as np
from calc2tex import trigo

#TODO bei basis für val_in Klammer einfügen, da mit Einheit sonst nicht klar erkennbar
#TODO x**x**x wird in Python als x**(x**x) behandelt -> Darstellung in Python
...
def main(formula: str, data: dict) -> (float, str, str):
    ...
    while True:
        ...
        if index == -1:
            break
        elif tex_val[index-1] == "}":        #TODO was ist mit sqrt??
            index_last = index + 14
            index_start = tex_val.rfind("\\SI", 0, index)
