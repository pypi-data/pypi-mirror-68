...
```
mmf_predict config=<REPLACE_WITH_BASELINE_CONFIG> model=<REPLACE_WITH_MODEL_KEY> dataset=hateful_memes run_type=val evaluation.format=csv
```

For running inference (will generate a csv) on test set run the following command:

```
mmf_predict config=<REPLACE_WITH_BASELINE_CONFIG> model=<REPLACE_WITH_MODEL_KEY> dataset=hateful_memes run_type=test evaluation.format=csv
```
...
```
mmf_predict config=<REPLACE_WITH_BASELINE_CONFIG> model=<REPLACE_WITH_MODEL_KEY> dataset=hateful_memes run_type=test evaluation.format=csv checkpoint.resume_zoo=<REPLACE_WITH_PRETRAINED_ZOO_KEY>
```
