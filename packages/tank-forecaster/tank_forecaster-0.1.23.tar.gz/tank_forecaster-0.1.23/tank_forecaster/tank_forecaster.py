...
def forecast_near(df, length = 48, output = 'dict'):
    ...
    if len(df.y) >= 144:
        ...
        status = 2 # kick warning - needs more data to train
        index = len(df)-24
        train = df[:index] # make a model excluding last 24 hours of obs
        test_yhat = ar8(train, 24) 
        test = df[index:] # test against last 24 hours
        mae = mean_absolute_error(test_yhat['mean'], test.y) # calculate MAE
    ...
    if mae < 80 and status == 4: # high accuracy and good amount of training data
        status = 4 # good model
    elif mae < 80 and status == 2: # high accuracy, bad amount of training data
        status = 2 
    elif mae < 110: # mediocre / bad accuracy
        status = 3 
    else:
        return({'status': status_dict[1]}) # horrible model - do not forecast

    # combine predictions with future dataframe
