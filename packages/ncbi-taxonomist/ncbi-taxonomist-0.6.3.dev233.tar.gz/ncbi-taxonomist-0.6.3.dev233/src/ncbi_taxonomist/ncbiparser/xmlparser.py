...
import ncbi_taxonomist.ncbiparser.ncbitaxon

class NcbiTaxoXmlParser:
  ...
  def __init__(self, taxa=None, alias=None):
    self.taxa = NcbiTaxoXmlParser.taxa if taxa is None else taxa
    self.alia = NcbiTaxoXmlParser.alias if alias else alias
  ...
  def parse(self, xml):
    ...
    """
    result = NcbiTaxoXmlParser.Result()
    ...
    while True:
      if status == 0:
        if elem.tag == 'Taxon' and event == 'start':
          taxon = ncbi_taxonomist.ncbiparser.ncbitaxon.NcbiTaxon(isQuery=isQuery)
          event, elem = next(context)
