...
from ncbi_taxonomist.ncbiparser import xmlparser
...
class RemoteMapper(entrezpy.base.analyzer.EutilsAnalyzer):
  ...
  def __init__(self, query:Type[resolvequery.ResolveQuery], converter:Type[ncbitaxon.NcbiTaxonConverter]):
    ...
    self.taxonomy_parser = xmlparser.NcbiTaxoXmlParser(RemoteMapper.taxa)
  ...
  def analyze_result(self, response, request:object):
    ...
    for i in results.taxa:
      self.query.map_query(self.converter.convert_to_model(results.taxa[i].attribute_dict()))
