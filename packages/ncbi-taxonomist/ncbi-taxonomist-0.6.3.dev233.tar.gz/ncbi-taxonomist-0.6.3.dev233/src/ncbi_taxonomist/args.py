...
def parse(basename):
  ...
  collector = subparsers.add_parser('collect',
                                     help='Collect taxa information from Entrez',
                                     parents=[taxa, email]).set_defaults(database=None)


  subtree = subparsers.add_parser('subtree',
                                  help='Extract subtree from given taxa or lineages',
                                  parents=[taxa, db, email, remote])
  subtree.add_argument('--rank',
                       default=None,
                       help='extract this rank')
  subtree.add_argument('--lrank',
                       default=None,
                       help='Extract from lower rank, further from root')
  subtree.add_argument('--hrank',
                       default=None,
                       help='Extract to higher rank, closer to root')

  group = subparsers.add_parser('group',
                                help='Organize taxa groups in local database',
                                parents=[taxa, db])
  group.add_argument('--add',
                     type=str,
                     metavar='<groupname>',
                     help='Add to <groupname>')
  group.add_argument('--rm',
                     type=str,
                     metavar='<groupname>',
                     help='Remove from <groupname>. Without given taxid or names remove group.')
