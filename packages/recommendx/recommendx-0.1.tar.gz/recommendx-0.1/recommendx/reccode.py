...
class RWR:
    ...
    def __init__(self, reg = 0.02, lr = 0.005, n_epochs = 50, n_factors = 50,
    ...
    def fit(self, df, Xi = None):
        ...
        newdf = np.column_stack((useru.cat.codes, itemui.cat.codes, df[:,2]))
        ...
        self.alpha_ = self.init_mean + self.init_std_dev*np.random.randn(self.n_users, self.n_factors)
        self.Z = self.init_mean + self.init_std_dev*np.random.randn(self.n_items, self.n_factors)
        
        if Xi is not None:
            self.n_Xs = Xi.shape[1] - 1
            self.B = self.init_mean + self.init_std_dev*np.random.randn(self.n_users, self.n_Xs)
        
        # Fit the model with observed regressors
        if Xi is not None:
            self.X = Xi[:,1:]
            for _ in range(self.n_epochs):
                for u, i, r_ui in newdf:
                    err = r_ui - self.intercept_ - self.bu[u] - np.dot(self.B[u],self.X[i]) - self.alpha_[u] @ np.transpose(self.Z[i])
                    # Update bu
                    self.bu[u] += self.lr*(err - self.reg*self.bu[u])
                    # Update B
                    self.B[u] = self.B[u] + self.lr*(err*np.transpose(self.X[i]) - self.reg*self.B[u])
                    # Update alpha
                    self.alpha_[u] += self.lr*(err*self.Z[i] - self.reg*self.alpha_[u])
                    # Update Z
                    self.Z[i] += self.lr*(err*self.alpha_[u] - self.reg*self.Z[i])
        else:
            for _ in range(self.n_epochs):
                for u, i, r_ui in newdf:
                    err = r_ui - self.intercept_ - self.bu[u] - self.alpha_[u] @ np.transpose(self.Z[i])
                    # Update bu
                    self.bu[u] += self.lr*(err - self.reg*self.bu[u])
                    # Update alpha
                    self.alpha_[u] += self.lr*(err*self.Z[i] - self.reg*self.alpha_[u])
    ...
    def accuracy(self, df, Xi = None):
        ...
        if Xi is not None:
            ...
            for u, i, r_ui in newdf:
                ...
                err = r_ui - self.intercept_ - self.bu[u] - np.dot(self.B[u],X[i]) - self.alpha_[u] @ np.transpose(self.Z[i])
                err2 = err**2
                sume2 = sume2 + err2
        else:
            # Loop over all the rows in newdf
    ...
    def predict(self, u_p, i_p):
        '''This function predicts the rating user u_p assigns to item i_p
    
        u_p and i_p are individual values so if you wish to use this for multiple predictions,
        you should create a loop
    
        u_p and i_p must be of the same type as the original data
        ...
        if self.X is not None:
            rhat = self.intercept_ + self.bu[u] + np.dot(self.B[u],self.X[i]) + self.alpha_[u] @ np.transpose(self.Z[i])
        else:
            rhat = self.intercept_ + self.bu[u] + self.alpha_[u] @ np.transpose(self.Z[i])
