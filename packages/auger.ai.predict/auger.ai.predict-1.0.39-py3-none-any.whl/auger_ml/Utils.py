...
def convert_simple_numpy_type(obj, allow_nan):
    import numpy as np
    ...
    if isinstance(obj, (np.int_, np.intc, np.intp, np.int8,
        ...
        np.float64)):
        return float(obj)
    elif not allow_nan and is_nan(obj):
        return 0
...
class NumpyJSONEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        res = convert_simple_numpy_type(obj, allow_nan=False)
        if res is not None:
            return res

        return json.JSONEncoder.default(self, obj)

class NumpyJSONEncoderAllowNan(json.JSONEncoder):
    """ Special json encoder for numpy types """
    def default(self, obj):
        res = convert_simple_numpy_type(obj, allow_nan=True)
        if res is not None:
            return res
...
def json_dumps_np(data, allow_nan=True):
    if allow_nan:    
        return json.dumps(data, cls=NumpyJSONEncoderAllowNan, allow_nan=allow_nan)
    else:
        return json.dumps(data, cls=NumpyJSONEncoder, allow_nan=allow_nan)    

def convert_numpy_types(params, allow_nan=False):
    if params is None:
        return params
    ...
    if type(params) is dict:
        ...
        res = convert_simple_numpy_type(params, allow_nan)
        if res is not None:
            params = res
