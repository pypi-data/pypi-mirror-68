...
class DB():
    ...
    def init_defaults(self, *args, **kwargs):
        ...
        configs['paths'] = {**DEFAULTS['paths'], **paths}
        configs['files'] = {**DEFAULTS['files'], **files}
        ...
        if self.paths['data'] == '' and self.paths['code'] == '':
            self.autodetect()
        else:
            self.set_files()
    ...
    def parse_args(self, *args, **kwargs):
        ...
        if 'name' in kwargs:
            ...
            files['csv'] = '{}/csvs/{}.csv.gz'.format(prefix, name)
            files['yml'] = '{}/ymls/{}.yml'.format(prefix, name)
    ...
    def set_files(self, files=None, prefix='.'):
        """
        ...
        if type(files) is str:
            files = {
                'csv': '{}/csvs/{}.csv.gz'.format(prefix, files),
                'yml': '{}/ymls/{}.yml'.format(prefix, files)}
    ...
    def apply_row(self, sid, fdefs, load=None, save=None, fnames=None, header=None, replace=False, clear_arrays=True, **kwargs):
        """
            ...
            if load is not None:
                ...
                for key, fname in to_load.items():
                    if os.path.exists(fname):
                        ...
                        if len(fdefs) > 0: 
                            df_ = self.apply_row(sid, fdefs, load=load, fnames=fnames, header=header, replace=replace, clear_arrays=False)
                            if df_.shape[0] > 0:
    ...
    def new_db(self, name, prefix='.', **kwargs):
        """
        ...
        return DB(name=name, prefix=prefix, **kwargs)
