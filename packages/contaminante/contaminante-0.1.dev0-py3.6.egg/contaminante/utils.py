    ...
    with warnings.catch_warnings():
        ...
        for idx in (range(tpf.shape[1])):
            for jdx in range(tpf.shape[2]):
                ...
                if t_model is not None:
                    samples = np.asarray([np.random.multivariate_normal(w, sigma_w)[-1] for count in np.arange(100)])
...
#            window_length = [(window_length + 1) if ((window_length % 2) == 0) else window_length][0]
            ...
            if (mission.lower() == 'kepler') | (mission.lower() == 'k2'):
                ...
                cbv_dm = lk.DesignMatrix(cbvs, name='cbvs', sparse=False)
                breaks = np.where((np.diff(tpf.time) > (0.0202 * 10)))[0] - 1
                breaks = breaks[breaks > 0]
            ...
            n_knots = np.max([4, int(tpf.time[-1] - tpf.time[0]/(duration * 2))])
            print(n_knots)
            spline_dm = lk.designmatrix.create_spline_matrix(lc.time, n_knots=n_knots)
            if cbvs is not None:
                dm = lk.DesignMatrixCollection([spline_dm, cbv_dm])
            else:
                dm = spline_dm
...
# #        import pdb; pdb.set_trace()
        ...
        #        import pdb;pdb.set_trace()
            ...
            ax.scatter(np.hstack(ra_target), np.hstack(dec_target), c='g', marker='.', s=0.1, label='Target', zorder=11, alpha=0.1)
