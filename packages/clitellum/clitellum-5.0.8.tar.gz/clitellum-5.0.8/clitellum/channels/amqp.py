...
class AmqpMessageProperties:
    ...
    def from_basic_properties(cls, properties):
        ...
        amp.content_encoding = properties[
            'content_encoding'] if 'content_encoding' in properties else None
        amp.message_id = properties[
            'message_id'] if 'message_id' in properties else None
        amp.reply_to = properties[
            'reply_to'] if 'reply_to' in properties else None
        amp.app_id = properties['app_id'] if 'app_id' in properties else None
        amp.expiration = properties[
            'expiration'] if 'expiration' in properties else None
        amp.correlation_id = properties[
            'correlation_id'] if 'correlation_id' in properties else None
        amp.priority = properties[
            'priority'] if 'priority' in properties else None
        amp.headers = properties['headers'] if 'headers' in properties else None
        amp.content_type = properties[
            'content_type'] if 'content_type' in properties else None
        return amp
...
class AmqpMesageFrame:
    ...
    def from_message_frame(cls, frame):
        ...
        amf.consumer_tag = frame[
            'consumer_tag'] if 'consumer_tag' in frame else None
        amf.delivery_tag = frame[
            'delivery_tag'] if 'delivery_tag' in frame else None
        amf.routing_key = frame[
            'routing_key'] if 'routing_key' in frame else None
        amf.redelivered = frame[
            'redelivered'] if 'redelivered' in frame else None
        return amf
...
class AmqpMessage:
    ...
    def __init__(self, frame: AmqpMesageFrame,
                 properties: AmqpMessageProperties, body: bytes):
        self.frame = frame
...
class AmqpConnection:
    ...
    def connect(self):
        ...
        while not self._closing:
            try:
                logger_manager.get_logger().info('Conectando a %s',
                                                 self._configuration.uri)
                self._connect_point()
                self._isConnected = True
                return
            except ConnectionError as ex:
                logger_manager.get_logger().error("Error al conectar: %s", ex)
                logger_manager.get_logger().error(
                    "Intentado reconectar... Host: %s",
                    self._configuration.uri)
    ...
    def _connect_point(self):
        try:
            ...
            self._connection = librabbitmq.Connection(
                ...
                heartbeat=self._configuration.heartbeat)
            ...
            if self._configuration.queue.create and self._configuration.queue.name is not None:
                ...
                self._channel.queue_declare(
                    queue=self._configuration.queue.name,
                    durable=True,
                    auto_delete=False)
    ...
    def publish(self, message: bytes, routing_key: str,
                amqp_message_properties: AmqpMessageProperties):
        """
    ...
    def consume(self):
        self._consumer_tag = self._channel.basic_consume(
            queue=self._configuration.queue.name, callback=self.__read_message)
        logger_manager.get_logger().info("Start Consuming %s",
                                         self._consumer_tag)
    ...
    def __read_message(self, message):
        logger_manager.get_logger().debug("Message Received %s",
    ...
    def ack(self, message: AmqpMessage):
        ...
        try:
            ...
            self._channel.basic_ack(delivery_tag=message.frame.delivery_tag,
                                    multiple=False)
        except Exception as ex:
