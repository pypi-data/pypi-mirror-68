...
class AmqpMessage:
  ...
  def create_from_message(cls, message):
    ...
    return AmqpMessage(amf, amp, message.body.tobytes(), amf.routing_key)
...
class AmqpConnection:
  ...
  def consume(self):
    self._consumer_tag = self._channel.basic_consume(queue=self._configuration.queue.name, callback=self.__read_message)
