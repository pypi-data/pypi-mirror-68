...
A recent paper cited improved results if intermediate representations are vector quantized. I have decided to add this as a feature. To use, you have to specify which layer(s) you would like to vector quantize. Default dictionary size is `256` and is also tunable.
...
I have tried contrastive learning on the discriminator (in step with the usual GAN training) and possibility observed improved stability and final generation results. I have exposed this as a feature, so that others may experiment and potentially corroborate this claim.
