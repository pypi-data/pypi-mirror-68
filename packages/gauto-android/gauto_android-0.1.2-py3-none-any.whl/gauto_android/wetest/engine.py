...
from gauto_android.common.adb_process import *
from gauto_android.wetest.element import Element
from gauto_android.common.protocol import Commands, TouchEvent
from gauto_android.common.socket_client import SocketClient
from gauto_android.common.rpc_thread import RPCReceiveThread
from gauto_android.common.wetest_exceptions import *
...
class GameEngine(object):
    ...
    def find_element(self, name):
        ...
        :param name:
            GameObject.Find的参数
        :Usage:
            >>>import gauto_android.manager as manager
    ...
    def swipe_position(self, start_x, start_y, end_x, end_y, duration=1000):
        ...
        if duration <= 0:
            reason = "duration = {0} is invaild, steps must more than 0".format(duration)
            raise WeTestInvaildArg(reason)
        start_x = int(start_x)
        start_y = int(start_y)
        end_x = int(end_x)
    ...
    def swipe(self, start_element, end_element, duration=1000):
        ...
        :Usage:
            >>>import gauto_android.manager as manager
            >>>engine=manager.get_engine()
            >>>start_element=engine.find_element('Button')
            >>>end_element=engine.find_element('Button2')
        ...
        end_x = 0
        end_y = 0
        if start_element and isinstance(start_element, Element) and end_element and isinstance(end_element,
                                                                                               Element):
        ...
        return self.swipe_position(start_x, start_y, end_x, end_y, duration=duration)
