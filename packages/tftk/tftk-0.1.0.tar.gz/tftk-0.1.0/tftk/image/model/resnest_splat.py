...
class SplitAttentionConv():
    ...
    @classmethod
    def add_split_attention_conv(cls,x:tf.Tensor,channels:int, kernel_size:int, strides=(1, 1), padding=(0, 0),
                 dilation=(1, 1), groups=1, radix=2, *args, in_channels=None, r=2, norm_layer=BatchNor(m, norm_kwargs=None, drop_ratio=0, **kwargs):
        for r in range():
            pass
        # x = tf.keras.layers.Conv2D(filters=channels*radix,kernel_size=kernel_size,strides=strides,padding="same",dilation_rate=dilation)(x)
...
class rSoftMax():
    ...
    def hybrid_forward(self, F, x):
        if self.radix > 1:
            x = x.reshape((0, self.cardinality, self.radix, -1)).swapaxes(1, 2) ## tf.transpose
...
class rSoftmax():
    @classmethod
    def add_rsoftmax(cls,radix,cardinality,x):
        if radix > 1:
            x = tf.reshape(x, (0,cardinality,radix,-1))
            # x = tf.transpose(??)                      ## TODO
            x = tf.keras.backend.softmax(x,axis=1)
            x = tf.reshape((0,-1))
        else:
            x = tf.sigmoid(x)
