...
class DumperHelper:
    def __init__(self, **kwargs):
        ...
        if kwargs.get('file_folder'):
            ...
            stack = inspect.stack()
            self.file_folder = os.path.join(os.path.expanduser('~'), 'Dump',
                                            os.path.basename(os.path.dirname(get_stack_frame(stack)[1])),
                                            os.path.basename(get_stack_frame(stack)[1]).split('.')[0],
