...
class AutoMLBaseSettings:
    ...
    def __init__(self,
        ...
        :param lag_length: How many rows to lag data when featurizing time series data
        ...
        if lag_length != 0:
            msg = "Parameter 'lag_length' will be deprecated. Reference "\
        ...
        for key, value in kwargs.items():
            if key not in self.__dict__.keys():
                msg = "Received unrecognized parameters"
                logging.warning(msg)  # print warning to console
            setattr(self, key, value)

    def __del__(self):
        """Clean up the logger."""
        logging_utilities.cleanup_log_map(
            self.debug_log,
            self.verbosity
        )

    def _get_logger(self) -> ActivityLogger:
        """
        Get a logger.
        ...
        # telemetry handler will be installed into the logger if azureml.telemetry is installed
        logger = logging_utilities.get_logger(
            namespace='azureml.telemetry',
            filename=self.debug_log,
            verbosity=self.telemetry_verbosity,
            component_name=constants.TelemetryConstants.COMPONENT_NAME
        )
        logger_adapter = DummyActivityLogger(logger)
        logger_adapter.update_default_properties(custom_dimensions)
        return logger_adapter

    def _do_blacklist_prophet(self,
                              warning_message: Optional[str],
                              logger: Optional[logging.Logger]) -> None:
        """
        Blacklist prophet and print warning about it.

        :param warning_message: The warning to be printed.
        :param logger: The logger to be used to print the warning.
        """
        ...
        if warning_message is not None and logger is not None:
            logger.warning(warning_message)

    def _verify_settings(self, logger):
        """
        ...
        if self.is_timeseries:
            if isinstance(self.featurization, dict):
                self._validate_timeseries_featurization_settings(self.featurization, logger)
        ...
        if self.whitelist_models is not None:
            if len(self.whitelist_models) == 0:
                ...
                raise ConfigException.create_without_pii(msg, target="whitelist_models")
        ...
        if self.is_timeseries:
            ...
            try:
                ...
                prophet_module = import_utilities.import_fbprophet()
            except ImportError:
                self._do_blacklist_prophet(
                    ('The fbprophet cannot be imported. '
                     'Please make sure it can be imported from the python command line.'), logger)
            # If we were able to import prophet, try to create the instance of the class.
            if prophet_module is not None:
                try:
                    prophet_module.Prophet()
                except BaseException:
                    self._do_blacklist_prophet(
                        ('The fbprophet cannot be instantiated and will be blacklisted.'), logger)

        if self.blacklist_algos is not None:
            enabled_algos = set(self._get_supported_model_names()) - set(self.blacklist_algos)
        ...
        if self.model_explainability:
            try:
                ...
                msg = 'Please install azureml-explain-model package for model explanations'
                raise ConfigException.create_without_pii(msg)
    ...
    def _validate_timeseries_featurization_settings(
            self, featurization: Dict[str, Any], logger: logging.Logger) -> None:
        """
        ...
        self._validate_ts_column_purposes(featurization, logger)

        self._validate_ts_featurization_drop_columns(featurization, logger)

        self._validate_ts_featurization_transform_params(featurization, logger)
    ...
    def _validate_ts_featurization_transform_params(
        ...
        if transformer_params is not None and len(transformer_params) > 0:
            if transformer_params.get(SupportedTransformers.Imputer) is not None:
                self._validate_ts_featurization_strategies(transformer_params, logger)
    ...
    def _validate_ts_column_purposes(self, featurization: Dict[str, Any], logger: logging.Logger) -> None:
        """
        ...
        ts_enabled_column_purposes = set([FeatureType.DateTime, FeatureType.Numeric, FeatureType.Categorical])
