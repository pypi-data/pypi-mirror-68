...
class ConsoleInterface:
    ...
    def _print_guardrail_parameters(self,
        ...
        self.console_writer.println("+" + "+".join(["-" * l for l in max_space_list]) + "+")
        # print header
    ...
    def print_guardrails(self,
        ...
        for f in faults:
            ...
            if include_parameters and len(f[Guardrails.PARAM_TD]) > 0 and number_parameters_output > 0:
                self.console_writer.println(Guardrails.PARAMETERS + " " *
                                            (Guardrails.TITLE_SPACE - len(Guardrails.PARAMETERS)) +
                                            f[Guardrails.PARAM_PREFACE_TD])       # Print DETAILS: ________
