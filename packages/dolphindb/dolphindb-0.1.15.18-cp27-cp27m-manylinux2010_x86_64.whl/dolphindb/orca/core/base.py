...
class _Frame(_InternalAccessor, ArithOpsMixin, LogicalOpsMixin, StatOpsMixin, metaclass=abc.ABCMeta):
    ...
    elif pd.__version__.startswith("0.25") or pd.__version__.startswith("1"):
    ...
    def _pandas_where(self, cond, other=np.nan, inplace=False, axis=None, level=None, errors="raise", try_cast=False):
        ...
        if inplace:
            obj = self.__class__(self.to_pandas().where(
                cond, other, inplace, axis, level, errors=errors, try_cast=try_cast), session=self._session)
            return obj
        else:
