...
def calc_iou_batch(boxes,box):
    '''calculate iou between batch bboxes with one bbox'''
    box1=boxes
    box2=box
    import numpy as np
    box1 = np.array(box1)
...
def rotate_boxes(boxes,angle,cx,cy,h,w):
    ...
    corners=M.dot((corners-np.array([cx,cy])).T).T+np.array([nw//2,nh//2])
    corners=corners.reshape((-1,8))
    boxes=[four_coners_to_ltrb(list(box)) for box in corners]
    return boxes
def rotate_box(box, angle, cx, cy, h, w):
    return rotate_boxes([box],angle,cx,cy,h,w)
######################Convert##########################
def ltrb_to_four_corners(box):
    '''four corners: 8-d vector'''
    l,t,r,b=box
