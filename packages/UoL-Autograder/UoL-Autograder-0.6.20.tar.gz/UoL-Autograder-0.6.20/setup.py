...
def get_current_dir():
    origin = sys._getframe(0) if hasattr(sys, "_getframe") else None
    return os.path.dirname(os.path.abspath(inspect.getfile(origin)))

version = os.environ.get('CI_COMMIT_TAG', None)
if not version:
  with open(os.path.join(get_current_dir(), "VERSION")) as f:
    version = f.read()
else:
  with open(os.path.join(get_current_dir(), "VERSION"), "w") as f:
    f.write(version)
print(version)
