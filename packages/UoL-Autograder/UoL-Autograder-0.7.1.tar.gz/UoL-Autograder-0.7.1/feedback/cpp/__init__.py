...
class CppRunner(Runner):
    ...
    def get_result_from_executable(self, output_file, executable):
        safety_counter = 0
        skip = 0
        partial_result = []
        ...
        while safety_counter < 32:
            ...
            # print(f"Executing with skip: {skip}")

            _, stdout, stderr = util.execute([executable, output_file.absolute().as_posix(), str(skip)], self.tmp_dir, timeout=EXECUTABLE_TIMEOUT)
            ...
            if not result:
                if skip < len(partial_result):
                    skip += 1
                    # print("No result, skipping to next")
                    continue
                else:
                    # print("No result, finishing")
                    break
            ...
            if skip >= len(partial_result):
                # print("Ran out of skips, finishing")
                break
        
        for r in partial_result:
            if r["mark"] == "":
                r["mark"] = "0"
                r["feedback"] = "Execution timed out during this question : FAIL"
    ...
    def test_functionality(self, config):
        ...
        if not executable:
            self.feedbacks.append(CheckResult(config, "functionality", 0, "Test execution failed: Executable not found"))
            return
        ...
        result = None
        try:
            result = self.get_result_from_executable(output_file, executable)
        except FileNotFoundError:
