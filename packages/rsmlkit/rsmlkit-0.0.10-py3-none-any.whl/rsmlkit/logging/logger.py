...
__all__ = ['get_logger', 'set_output_file', 'set_default_level',
           'log_dash', 'log_star', 'log_eq']
_default_level = logging.INFO
_all_loggers = []

log_dash = lambda x: logging.debug(f"\n" + "-" * x + "\n")
log_star = lambda x: logging.debug(f"\n" + "*" * x + "\n")
log_eq = lambda x: logging.debug(f"\n" + "=" * x   + "\n")

class RSLogFormatter(logging.Formatter):
...
def set_output_file(fout, mode='a'):
    if isinstance(fout, str):
        fout = open(fout, mode)
    RSLogFormatter.log_fout = fout

def get_logger(name=None, formatter=RSLogFormatter):
