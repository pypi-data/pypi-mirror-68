...
def get_proxies_by_data5u(order, random=1):
    """
    通过无忧代理（http://www.data5u.com/）API接口获取代理
    :param order: IP提取码
    :param random: 结果定制，默认优先返回速度快的IP
    :return:
    """
    url = "http://api.ip.data5u.com/dynamic/get.html?order={}&ttl=1&json=1&random={}&sep=3".format(order, random)
    proxies = dict()
    for i in range(3):
        try:
...
class JWT:
    ...
    def cracker(self, message):
        message = message.split(".")
        header = json.loads(message[0])
        payload = json.loads(message[1])
        signature = json.loads(message[2])
        cracker = Cracker("")
        cracker.hs256(header + "." + payload)
...
class Cracker:
    ...
    def hs256(self, message):
        for key in self.dictionary:
            print(base64urlencode(hmac.new(key, message.encode(), digestmod="sha256").digest()))


def base64urlencode(plaintext):
    if type(plaintext) is not bytes:
        plaintext = plaintext.encode("utf-8")
    return base64.b64encode(plaintext).rstrip(b"=").replace(b'+', b'-').replace(b'/', b'_')
...
def is_domain(string):
    ...
    :return:
    """
    domain = re.compile(r"^[0-9a-zA-Z]+[0-9a-zA-Z\.-]*\.[a-zA-Z]{2,12}$")
...
def is_url(string):
    ...
    :return:
    """
    url = re.compile(r"^(http|https):\/\/[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&\/\/=]*)$")
    return True if re.match(url, string) else False
...
def is_ip(string):
    ...
    :return:
    """
    ip = re.compile(r"^(25[0-5]|2[0-4]\d|[0-1]?\d?\d)(\.(25[0-5]|2[0-4]\d|[0-1]?\d?\d)){3}$")
    return True if re.match(ip, string) else False


def multiprocess(traget, queue, number=10):
    """
    创建多进程任务
    :param traget:
    :param queue: multiprocessing.Queue 对象
    :param number:
    :return:
    """
...
def get_alexa_rank(domain):
    ...
    if not os.path.exists(file) or time.time() - os.path.getctime(file) > 60*60*24:
...
if __name__ == '__main__':
    print(get_alexa_rank("www.baidu.com"))
