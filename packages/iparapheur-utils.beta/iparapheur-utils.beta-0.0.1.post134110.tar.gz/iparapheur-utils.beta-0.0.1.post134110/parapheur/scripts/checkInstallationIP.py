...
def check_https_service_config(varconfig):
    ...
    if isexistsdirectory("/etc/nginx/ssl"):
        if isexistsfile("/etc/nginx/ssl", "recup_crl_nginx.sh"):
            clean_list = ["/validca.tgz", "/validca.md5sum", "http://", "https://"]
            parse_conf_for_url("/etc/nginx/ssl/recup_crl_nginx.sh", "/usr/bin/wget -q", "validca.libriciel.fr", clean_list)
...
def check_side_service(srvname):
    pprint.header("#", False, ' ')
    pprint.log("---- Check configuration service {0} ----".format(srvname), True)
    isexistsfile("/etc/systemd/system/", "{0}.service".format(srvname))
    if not isexistsdirectory("/opt/{0}".format(srvname)):
        pprint.header("#   ", False, ' ')
        pprint.warning("Le service {0} ne semble pas être en place.".format(srvname))
    else:
        import pwd
        try:
            pwd.getpwnam('{0}'.format(srvname))
            real_owner = pwd.getpwuid(os.stat("/opt/{0}".format(srvname)).st_uid).pw_name
            pprint.header("#", False, ' ')
            pprint.info("Check du compte linux '{0}'".format(srvname).ljust(46), False, ' ')
            pprint.success('{:>10s}'.format("OK"), True)
            pprint.header("#", False, ' ')
            pprint.info("Check des droits sur /opt/{0} ".format(srvname).ljust(46), False, ' ')
            if real_owner == srvname:
                pprint.success('{:>10s}'.format("OK"), True)
            else:
                pprint.error('{:>10s}'.format("KO"), True)
        except KeyError:
            pprint.header("#   ", False, ' ')
            pprint.warning('Le service {0} est mal installé, '
                           'le compte Linux "{1}" est absent.'.format(srvname, srvname))
...
def check_static_ressources(nginxservice):
    ...
    make_script = "make.sh"
    xmlmime = "xmlmime"
    wsdl_clean_list = ["location=\"https://", ":443/ws-iparapheur\"/>", ":443/ws-iparapheur-no-mtom\"/>"]
    make_clean_list = ["SERVER_PATH=\"https://libersign-test.", "SERVER_PATH=\"https://libersign.", "\""]
    ...
    if isexistsdirectory(var_www_root):
        if isexistssubdir(var_www_root, alfresco_root):
            ...
            if isexistsfile(var_www_root + alfresco_root, wsdl):
                parse_conf_for_url(var_www_root + alfresco_root + wsdl, wsdl_regex, nginxservice, wsdl_clean_list)
        ...
        if isexistssubdir(var_www_root, libersign_root):
            if isexistsfile(var_www_root + libersign_root, make_script):
                parse_conf_for_url(var_www_root + libersign_root + make_script, make_regex, make_expected_url, make_clean_list)
...
def check_crontab_jobs(version_ip_int):
    ...
    expected_crontab = os.path.join(os.path.abspath(os.path.dirname(__file__)), '..', "files/crontab/" + str(version_ip_int) + ".txt")
...
# check CRONTAB
if parse_version("4.6.8") <= parse_version(version_ip):
