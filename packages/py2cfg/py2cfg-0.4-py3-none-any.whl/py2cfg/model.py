...
class CFG(object):
    ...
    def _visit_blocks(
        ...
        original_nodelabel = block.get_source()  # label = nodelabel
        ...
        if "_ast.ClassDef" in node_type:
            original_nodelabel = original_nodelabel.splitlines()[0] + " ...\n"
        if "_ast.If" in node_type:
            ...
            for line in original_nodelabel.splitlines():
                line += "\l"
                tmp += line
            original_nodelabel = tmp
        nodelabel = ""
        if (
            self.isShort
            and "_ast.ClassDef" not in node_type
            and "_ast.FunctionDef" not in node_type
            and "_ast.If" not in node_type
            and "_ast.While" not in node_type
        ):
    ...
    def __iter__(self) -> Iterator[Block]:
        ...
        for classcfg in self.functioncfgs.values():
            yield from classcfg
