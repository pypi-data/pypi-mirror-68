...
# Cell
def solution(InitCond,ClockStruct,weather_step,Outputs,Farmer):
    """
    ...
    *Returns:*
    ...
    # Unpack structures
    Soil = InitCond.ParamStruct.Soil
    CO2 = InitCond.ParamStruct.CO2
    if InitCond.ParamStruct.WaterTable == 1:
        Groundwater = InitCond.ParamStruct.zGW.loc[ClockStruct.StepStartTime]
    else:
        Groundwater = 0
    ...
    if ClockStruct.SeasonCounter >= 0:
        ...
        NewCond.CurrentDate = CurrentDate
        ...
        Crop = NewCond.ParamStruct.Seasonal_Crop_List[ClockStruct.SeasonCounter]

        IrrMngt = Crop.IrrMngt



        if GrowingSeason == True:
            FieldMngt = Crop.FieldMngt
        else:
            FieldMngt = NewCond.ParamStruct.FallowFieldMngt
        ...
        Crop = NewCond.ParamStruct.Fallow_Crop
    ...
    if GrowingSeason == True:
        ...
        GDD,NewCond = growing_degree_day(Crop,NewCond,Tmax,Tmin)
        NewCond.GDD = GDD
    ...
    NewCond,Soil = check_groundwater_table(ClockStruct,Soil,NewCond)

    # 2. Root development
    NewCond = root_development(Crop,Soil,NewCond,GDD,GrowingSeason)

    # 3. Pre-irrigation
    NewCond, PreIrr = pre_irrigation(Soil,Crop,NewCond,GrowingSeason,IrrMngt)

    # 4. Drainage
    NewCond,DeepPerc,FluxOut = drainage(Soil,NewCond)

    # 5. Surface runoff
    Runoff,Infl,NewCond = rainfall_partition(P,Soil,FieldMngt,NewCond)

    # 6. Irrigation
    NewCond, Irr = irrigation(NewCond,IrrMngt,Crop,Soil,ClockStruct,GrowingSeason,P,Runoff,Farmer)


    # 7. Infiltration
    NewCond,DeepPerc,RunoffTot,Infl,FluxOut = infiltration(Soil,NewCond,Infl,Irr,IrrMngt,FieldMngt,FluxOut,DeepPerc,Runoff,GrowingSeason)

    # 8. Capillary Rise
    NewCond,CR = capillary_rise(Soil,NewCond,FluxOut)
    ...
    NewCond = canopy_cover(Crop,Soil,NewCond,GDD,Et0,GrowingSeason)

    # 12. Soil evaporation
    NewCond,Es,EsPot = soil_evaporation(ClockStruct,Soil,Crop,IrrMngt,FieldMngt,NewCond, \
                                              Et0,Infl,P,Irr,GrowingSeason)

    # 13. Crop transpiration
    Tr,TrPot_NS,TrPot,NewCond,IrrNet = transpiration(Soil,Crop,IrrMngt,NewCond,Et0,CO2, \
                                                         GrowingSeason,GDD)

    # 14. Groundwater inflow
    NewCond,GwIn = groundwater_inflow(Soil,NewCond)
    ...
    # 17. Harvest index
    NewCond = harvest_index(Soil,Crop,NewCond,Et0,Tmax,Tmin,GrowingSeason)
    ...
    # 19. Root zone water
    Wr,_Dr,_TAW,_thRZ = root_zone_water(Soil,Crop,NewCond)
    ...
    Outputs.Water[row_day,3:] = NewCond.th*NewCond.TAW
    ...
    if ClockStruct.SeasonCounter > -1:
        ...
        and (NewCond.HarvestFlag == False):
            ...
            # Store final outputs
            Outputs.Final.loc[ClockStruct.SeasonCounter] = [ClockStruct.SeasonCounter,Crop.Name,\
...
# Cell
def reset_initial_conditions(ClockStruct,InitCond):
    ...
    *Returns:*
    ...
    ## Extract crop type ##
    CropType = InitCond.ParamStruct.CropChoices[ClockStruct.SeasonCounter]

    ## Extract structures for updating ##
    Soil = InitCond.ParamStruct.Soil
    Crop = InitCond.ParamStruct.Seasonal_Crop_List[ClockStruct.SeasonCounter]
    FieldMngt = Crop.FieldMngt
    CO2 = InitCond.ParamStruct.CO2
    CO2_data = InitCond.ParamStruct.CO2data
    ...
    InitCond.ParamStruct.Seasonal_Crop_List[ClockStruct.SeasonCounter] = Crop
    InitCond.ParamStruct.CO2 = CO2

    return InitCond

# Cell
def update_time(ClockStruct,InitCond,Outputs):
    """
    ...
    *Returns:*
    ...
    if ClockStruct.ModelTermination == False:
        if  (InitCond.HarvestFlag == True) \
        and ((ClockStruct.SimOffSeason=='N')):
            ...
            if ClockStruct.SeasonCounter < ClockStruct.nSeasons-1:
                ...
                InitCond =  reset_initial_conditions(ClockStruct,InitCond)
