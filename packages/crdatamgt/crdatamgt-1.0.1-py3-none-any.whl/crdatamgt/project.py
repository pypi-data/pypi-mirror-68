...
from crdatamgt.helpers import topic_directories, workbook_load_file, workbook_save, rename_dictionary
import datetime
import re
from crdatamgt.formulations import form_return, load_and_update_excel_formulations
...
def project_load_from_workbooks(workbooks, **kwargs):
    ...
    for result in result_wb:
        ...
        formulation_data = result_data.pop('Formulation', None)
        if not formulation_data.empty:
            formulation_data.loc[:, 'Topic ID'] = result_data.get('Summary').get('Topic ID').values
    ...
    formulation_excel_sheet = load_and_update_excel_formulations(
        cleaned_formulations.drop(columns='topic id', errors='ignore'), kwargs.get('FORMULATION DIRECTORY'))
    updated_formulations = form_return(cleaned_formulations, formulation_excel_sheet)
...
def formulation_work(data_frame_list):
    ...
    cleaned = list(
        map(lambda sheet: sheet.rename(str.lower, axis='columns').rename(columns=rename_dictionary()), cleaned))
    clean_frame = pd.concat(cleaned)
    clean_frame.drop_duplicates(inplace=True)
    clean_frame.reset_index(inplace=True)
    clean_frame.drop(columns=['index', 'units'], errors='ignore', inplace=True)
