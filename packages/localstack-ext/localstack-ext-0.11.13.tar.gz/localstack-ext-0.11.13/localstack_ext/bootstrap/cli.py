...
def import_auth():
 try:
  ...
  raise BcyYU('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=BcyYF)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=BcyYo)
def get_params(args,names=[],mandatory=BcyYF):
 result=()
 for name in names:
  value=BcyYL
  if name in args:
  ...
  if not value and mandatory:
   raise BcyYU('Please provide %s=... parameter'%name)
  result+=(value)
 if BcyYO(result)==1:
  return result[0]
 return result
