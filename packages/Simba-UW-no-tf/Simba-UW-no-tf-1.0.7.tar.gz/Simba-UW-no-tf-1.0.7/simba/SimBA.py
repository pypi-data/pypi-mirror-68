...
from simba.dlc_change_yamlfile import select_numfram2pick,updateiteration,update_init_weight,generatetempyaml,generatetempyaml_multi
from simba.extract_features_wo_targets import extract_features_wotarget
from simba.sklearn_DLC_RF_train_model import RF_trainmodel
from simba.run_RF_model import rfmodel
from simba.merge_frames import merge_frames_config
...
from simba.merge_movie_ffmpeg import generatevideo_config_ffmpeg
...
from simba.process_movement import analyze_process_movement
import webbrowser
        ...
        if val == 'csv':
            ...
            button_importsinglejson = Button(label_singlejsonimport, text='Import single .json to project folder',
                                             command=lambda: json2csv_file(self.projectconfigini,
                                                                           self.file_json.file_path), fg='navy')
            # import json into projectfolder
            label_multijsonimport.grid(row=1, sticky=W)
            self.folder_json.grid(row=0, sticky=W)
            button_import_json.grid(row=1, sticky=W)
