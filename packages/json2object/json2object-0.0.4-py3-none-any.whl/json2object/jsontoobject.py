...
def deserialize(data, model):
    ...
    if isinstance(data, list): # list of dictionaries that' represent the model
        ...
        for d in data:
            obj = copy.copy(model)            
            results = deserialize(d, obj) # This will pass in a dictionary 
            models.append(results)
        return models
    
    elif isinstance(data, dict): # single model in dict format
        ...
        for k,v in data.items():
            if not isinstance(v, dict) and not isinstance(v, list):
                ...
                attr = getattr(obj, k) # the list need to make sure to have at least 1 item in the collection.  
