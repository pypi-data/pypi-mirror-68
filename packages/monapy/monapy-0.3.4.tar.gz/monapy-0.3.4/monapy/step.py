...
def to_step(step):
    if isinstance(step, Step):
        ...
        raise TypeError('to_step(%s)' % type(step))
...
class Step:
    ...
    def loop(self, loop_step):
        return LoopStepChain([self], loop_step)

    def union(self, step):
        return UnionStep(step)
...
class StepChain(Step):
    ...
    def loop(self, loop_step):
        return LoopStepChain(self._chain, loop_step)
    ...
    def _raw_tree(self, **kwargs):
        ...
        center_pos = round((len(self.__class__.__name__) + 1) / 2)
        spaces = ' ' * (center_pos - 1)
        unders = '__'
        sep = '|'
...
class LoopStepChain(Step):
    def __init__(self, steps, loop):
        self._chain = list(map(to_step, steps))
    ...
    def make(self, value = object(), **kwargs):
        ...
        last_chain_step = self._chain[-1]
        while True:
            iterable = reduce(
                lambda iterable, step: step.make_all(iterable, **kwargs),
                chain([iterable], [self._loop_step, last_chain_step])
            )
