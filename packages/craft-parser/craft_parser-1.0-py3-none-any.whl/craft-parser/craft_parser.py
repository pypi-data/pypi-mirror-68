...
class CraftParser:
  ...
  @staticmethod
  def graph_mass_distribution_3d(craft_filepath: str, masses: dict):
    parts = []
    craft_lines = []
    with open(craft_filepath) as craft:
      craft_lines = [line.strip() for line in craft.readlines()]
    ...
    ax.scatter(xs=[item[1][0] for item in parts], ys=[item[1][1] for item in parts],
      zs=[item[1][2] for item in parts], s=[100* 2**item[0] for item in parts], depthshade=False)
