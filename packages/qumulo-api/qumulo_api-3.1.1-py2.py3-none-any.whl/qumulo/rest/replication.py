...
def replicate(conninfo, credentials, relationship):
    ...
    return request.rest_request(
        conninfo, credentials, method, str(uri))
...
def delete_source_relationship(
        conninfo, credentials, relationship_id, if_match=None):
    method = "DELETE"
    uri = "/v2/replication/source-relationships/{}".format(relationship_id)
    return request.rest_request(
        conninfo, credentials, method, uri, if_match=if_match)
...
def get_object_relationship_status(conninfo, credentials, relationship_id):
    method = 'GET'
    uri = '/v2/replication/object-relationships/{}/status'.format(
        relationship_id)
