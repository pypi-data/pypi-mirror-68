...
def create_snapshot(
    ...
    if path != None:
        id_ = qumulo.rest.fs.get_file_attr(
            conninfo, credentials, path=path).lookup('file_number')
    if id_ != None:
...
def modify_policy(conninfo, credentials, policy_id,
                  name=None, schedule_info=None, enabled=None, if_match=None):
...
def get_all_snapshot_tree_diff(
    ...
    return request.PagingIterator(
        str(uri), get_a_snapshot_tree_diff, page_size=limit)
...
def get_all_snapshot_file_diff(
    ...
    return request.PagingIterator(
        str(uri), get_a_snapshot_file_diff, page_size=limit)
