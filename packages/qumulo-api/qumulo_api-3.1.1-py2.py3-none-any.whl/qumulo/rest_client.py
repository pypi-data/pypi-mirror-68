...
class RestClient(object):
    ...
    def __init__(
        ...
        # for request, make this class act like a RestModule
        # use a weakref proxy to avoid creating a reference cycle, which would
        # defeat refcount GC, which might defer connection close until a the GC
        # goes looking for cyclic isolates, which might not happen for any
        # arbitrary amount of time including forever.
        self.client = weakref.proxy(self)
    ...
    def connected(self):
        ...
        '''
        return self.conninfo.connected()
    ...
    def close(self):
        ...
        This client may be re-used after being closed (with the effect of
        implicitly re-opening the connection).
        '''
        self.conninfo.close()
