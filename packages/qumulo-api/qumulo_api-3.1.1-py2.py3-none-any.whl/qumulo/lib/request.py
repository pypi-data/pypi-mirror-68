...
from future.utils import bytes_to_native_str, raise_, tobytes
...
class HTTPSConnectionWithSocketOptions(httplib.HTTPSConnection):
    ...
    def connect(self):
        ...
        if hasattr(socket, 'TCP_KEEPIDLE') and hasattr(socket, 'TCP_KEEPCNT') \
            ...
            # Windows uses this ioctl to set keepalive values.
            self.sock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1)
            self.sock.ioctl(socket.SIO_KEEPALIVE_VALS, (
                1, # enable
...
class RequestError(Exception):
    ...
    def __init__(self, status_code, status_message, json_error):
        ...
        formatted = '\n'.join(
            ' ' * 4 + frame for frame in self.stack)
...
class APIRequest(object):
    ...
    def __init__(
        ...
        if request_content_type != CONTENT_TYPE_JSON:
            ...
            assert not isinstance(body_file, (TextIOBase)), \
    ...
    def send_request(self):
        ...
        try:
            ...
            raise_(wrap, None, sys.exc_info()[2])
    ...
    def get_response(self):
        ...
        if self._response_data and length != "0":
            try:
                self.response_obj = json.loads(
                    bytes_to_native_str(self._response_data))
