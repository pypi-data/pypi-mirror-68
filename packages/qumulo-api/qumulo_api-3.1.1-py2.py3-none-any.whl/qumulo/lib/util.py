...
def tabulate(table, headers=None):
    ...
    for row in table:
        ...
        col_widths = [max(m, len(str(v)))
            for m, v in zip(col_widths, row)]

    sep = u'  '
    lines = []
    if headers:
        lines.append(sep.join(str(u'{:<{}}').format(str(h), w)
            for h, w in zip(headers, col_widths)))
        lines.append(sep.join(str(u'=') * w for w in col_widths))
    for row in table:
        lines.append(sep.join(str(u'{:<{}}').format(str(v), w)
