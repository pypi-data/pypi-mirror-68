...
ADMIN_USER_ID   = str('500')
ADMINS_GROUP_ID = str('512')
USERS_GROUP_ID  = str('513')
GUEST_USER_ID   = str('501')
GUESTS_GROUP_ID = str('514')
...
class Identity(qumulo.lib.obj.Object):
    ...
    def __str__(self):
        ...
        '''
        name = self._unicode_helper()
        if isinstance(name, str):
            # python3's `str()` errors when given a string and an encoding.
            return name
        else:
            return str(name, getattr(sys.stdout, 'encoding', None) or 'utf-8')
    ...
    def _unicode_helper(self):
        attrs = self.dictionary()
        if attrs.get('auth_id') == str(EVERYONE_ID):
            return EVERYONE_NAME
        if attrs.get('auth_id') == str(GUEST_USER_ID):
            return GUEST_NAME
