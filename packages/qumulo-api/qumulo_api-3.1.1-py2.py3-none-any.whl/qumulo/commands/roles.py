...
class CreateRoleCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument("-p", "--privileges-file", type=str_decode,
...
class ModifyRoleCommand(qumulo.lib.opts.Subcommand):
    ...
    def main(conninfo, credentials, args):
        ...
        if args.privileges_file:
            ...
            with open(args.privileges_file, 'rb') as f:
                final_privileges = \
                    ListPrivilegesCommand.parse(all_privileges, f)
        elif args.grant or args.revoke:
            initial_role = roles.list_role(
                conninfo, credentials, args.role)
...
class ListPrivilegesCommand(qumulo.lib.opts.Subcommand):
    ...
    DESCRIPTION = 'List all privileges or privileges associated with a role. '\
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("-r", "--role", type=str_decode,
    ...
    def main(conninfo, credentials, args):
        ...
        assert not (args.verbose and args.json),\
            '--verbose is meaningless with --json'
        shown_privileges = privileges if args.verbose else granted
        ListPrivilegesCommand.print_privileges(
            shown_privileges, args.json, denied)
