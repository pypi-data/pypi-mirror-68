...
class CreateSnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("-n", "--name", type=str_decode, default=None,
            help="Name of the snapshot.")

        group = parser.add_mutually_exclusive_group(required=False)
        group.add_argument("-e", "--expiration", type=str_decode, default=None,
            help=expiration_msg(True))
        group.add_argument("-t", "--time-to-live", type=str_decode,
            default=None, help=SNAP_TTL_HELP)

        group = parser.add_mutually_exclusive_group(required=False)
        group.add_argument("--path", type=str_decode, default=None,
            help="Path to directory to snapshot.")
        group.add_argument("--id", type=str_decode, default=None,
...
class ModifySnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("-i", "--id", type=str_decode, required=True,
...
class GetSnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Gets a single snapshot."
...
class GetSnapshotStatusCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = 'Gets a single snapshot status.'
...
class DeleteSnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Deletes a single snapshot."
...
hourly_daily_common_parser.add_argument(
    '-d', '--days-of-week', type=str_decode, default='ALL',
...
daily_monthly_common_parser.add_argument(
    '-a', '--at', type=str_decode, required=True,
...
class CreatePolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        common_parser.add_argument('-n', '--name', type=str_decode,
            required=True, help='Name of the policy.')
        common_parser.add_argument(
            '-t', '--time-to-live', type=str_decode, default=None,
        ...
        group.add_argument("--file-id", type=str_decode, default=None,
    ...
    def main(conninfo, credentials, args):
        if args.path:
            attr = fs.get_file_attr(conninfo, credentials, path=args.path)
...
class ModifyPolicyCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        common_parser.add_argument( '-n', '--name', type=str_decode,
            default=None, help='Name of the policy.')
        common_parser.add_argument(
            '-t', '--time-to-live', type=str_decode, default=None,
            help=POLICY_TTL_HELP)
...
class CalculateUsedCapacity(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument('-i', '--ids', type=str_decode,
...
class GetUsedCapacityPerSnapshotCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Get the approximate amount of space for each snapshot that "\
...
class SnapshotTreeDiffCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "List the changed files and directories between two "\
               "snapshots."
