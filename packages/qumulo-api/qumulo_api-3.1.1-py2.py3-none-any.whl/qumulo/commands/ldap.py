...
class LdapPostCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--use-ldap", type=str_decode, required=True,
            choices={"true", "false"},
            help="Whether or not to enable the use of the LDAP server on "
            "the cluster.")
        parser.add_argument("--bind-uri", type=str_decode, required=True,
            help="LDAP URI used to bind. Example: "
                "ldap://ldap-server.example.com")
        parser.add_argument("--base-dn", type=str_decode, required=True,
            help="Base DNs (Distinguished Names). Separate multiple DNs using "
            "semicolons. Example: dc=account,dc=example,dc=com")

        # Optional arguments.
        parser.add_argument("--bind-username", type=str_decode, required=False,
            default='', help="Binding users's DN. Default is empty.")
        parser.add_argument("--bind-password", type=str_decode, required=False,
        ...
        parser.add_argument("--encrypt-connection", type=str_decode,
...
class LdapPatchCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--use-ldap", type=str_decode, default=None,
            choices={"true", "false"},
            help="Enable or disable the use of standalone LDAP.")
        parser.add_argument("--bind-uri", type=str_decode, default=None,
            help="LDAP URI used to bind. Example: "
                "ldap://ldap-server.example.com")
        parser.add_argument("--base-dn", type=str_decode, default=None,
            help="Base DNs (Distinguished Names). Example: "
            "dc=account,dc=example,dc=com")
        parser.add_argument("--bind-username", type=str_decode, default=None,
            help="Binding users's DN.")
        parser.add_argument("--bind-password", type=str_decode, default=None,
...
class UidNumberToLoginNameGetCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--uid-number", type=str_decode, required=True)
...
class LoginNameToGidNumbersGetCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Query the LDAP server for the gid numbers for all the " \
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--login-name", type=str_decode, required=True)
