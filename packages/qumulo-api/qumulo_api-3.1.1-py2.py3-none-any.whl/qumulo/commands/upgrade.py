...
class UpgradeConfigCommand(qumulo.lib.opts.Subcommand):
    NAME = "upgrade_config"
    SYNOPSIS = "List current upgrade prepare config"
...
class UpgradePrepareCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Prepare an upgrade using a file on fs"

    @staticmethod
    def options(parser):
        parser.add_argument('--path', type=str_decode, default=None,
                            required=True, help="FS path to upgrade image")
        parser.add_argument('--target', type=str_decode, default='idle',
...
class UpgradeStatusCommand(qumulo.lib.opts.Subcommand):
    NAME = "upgrade_status"
    SYNOPSIS = "List current upgrade status"
