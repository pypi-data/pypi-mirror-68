...
class SetSSLCertificateCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Set the SSL certificate chain and private key for the " \
...
class SetSSLCACertificateCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Set SSL CA certificate. This certificate is used to " \
...
class GetSSLCACertificateCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Get SSL CA certificate. This certificate is used to " \
        "authenticate connections to external LDAP servers."
...
class DeleteSSLCACertificateCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Delete SSL CA certificate. This certificate is used to " \
        "authenticate connections to external LDAP servers."
...
class SetClusterSlotConfigCommand(qumulo.lib.opts.Subcommand):
    ...
    SYNOPSIS = "Set the attributes for the given cluster slot.  " \
        "Currently only led_pattern may be set."
    ...
    @staticmethod
    def main(conninfo, credentials, args):
        print(cluster.set_cluster_slot_config(
            conninfo, credentials, args.slot, args.locate))
...
class CreateCluster(qumulo.lib.opts.Subcommand):
    ...
    max_drive_failures_choices = dict(
        ...
        **{str(v): k for k, v in cluster.PROTECTION_LEVEL_MAP.items() if v > 0}
    )

    @classmethod
    def protection_level_value(cls, value):
