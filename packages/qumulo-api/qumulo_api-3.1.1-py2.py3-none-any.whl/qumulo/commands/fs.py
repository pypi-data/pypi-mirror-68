...
class GetFileAttrCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        stream_group.add_argument(
            "--stream-id", help="Stream ID", type=str_decode)
        stream_group.add_argument(
            "--stream-name", help="Stream name", type=str_decode)
...
class SetFileAttrCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        group_group.add_argument("--group",
            help="File group as auth_id", type=str_decode)
...
class SetAclCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument("--file",
            ...
            required=False, type=str_decode)
    ...
    def options(parser):
        ...
        add_entry.add_argument("-r", "--rights", type=str_decode, nargs="+",
            ...
            help="The rights that should be allowed or denied.  Choices: "
                 + (", ".join(RIGHT_CHOICES)))
        add_entry.add_argument("-f", "--flags", type=str_decode, nargs="*",
        ...
        remove_entry.add_argument("-f", "--flags", type=str_decode, nargs="*",
            metavar='FLAG', choices=FLAG_CHOICES,
        ...
        modify_entry.add_argument("--old-rights", type=str_decode, nargs="+",
            ...
            help="Modify an entry with these rights.  Choices: "
                 + (", ".join(RIGHT_CHOICES)))
        modify_entry.add_argument("--old-flags", type=str_decode, nargs="*",
            metavar='FLAG', choices=FLAG_CHOICES,
        ...
        modify_entry.add_argument("--new-rights", type=str_decode, nargs="+",
            ...
            help="Set the rights of the entry.  Choices: "
                 + (", ".join(RIGHT_CHOICES)))
        modify_entry.add_argument("--new-flags", type=str_decode, nargs="*",
            metavar='FLAG', choices=FLAG_CHOICES,
...
class CopyCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("source", type=str_decode,
            help="Source file absolute path")
        parser.add_argument("target", type=str_decode,
            help="Target file absolute path")

        parser.add_argument("--source-snapshot", type=int,
        ...
        parser.add_argument('--no-named-streams', action='store_true',
    ...
    def main(conninfo, credentials, args):
        ...
        try:
            ...
            if args.overwrite:
                ...
                raise ValueError("Target file {} exists. Use --overwrite "
        ...
        with tqdm(
                ...
                total=total_bytes,
                unit_scale=True,
                disable=args.quiet) as copy_progress:

            copy_progress.set_description(
                'Copying stream 1/{}'.format(total_streams))
...
class ReadFileCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            "--file", help="File to receive data", type=str_decode)
        parser.add_argument(
...
class ReadDirectoryCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument("--snapshot", help="Snapshot ID to read from",
                            type=int)
        parser.add_argument("--smb-pattern", type=str_decode,
...
class ReadDirectoryCapacityCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--path", help="Directory path", type=str_decode,
            required=True)
...
class TreeWalkCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--path", help="Path to tree root", type=str_decode,
                            required=False, default='/')

        file_type_filter = parser.add_mutually_exclusive_group()
        file_type_filter.add_argument('--file-only',
            help="Only display files",
            action="store_true",
            required=False,
            default=False)
        file_type_filter.add_argument('--directory-only',
            help="Only display directories",
            action="store_true",
            required=False,
            default=False)
        file_type_filter.add_argument('--symlink-only',
            help="Only display symlinks",
            action="store_true",
            required=False,
            default=False)

        parser.add_argument("--display-ownership",
    ...
    def write_file_tree_to_file(conninfo, credentials, args, fh):
        ...
        def file_should_be_filtered(f):
            return (args.file_only and f['type'] != 'FS_FILE_TYPE_FILE') or\
                (args.symlink_only and f['type'] != 'FS_FILE_TYPE_SYMLINK') or\
                (args.directory_only and f['type'] != 'FS_FILE_TYPE_DIRECTORY')
...
class LockIter(object):
    ...
    def __next__(self):
        try:
            ...
            if len(self.cur_page.data[self.res_type]) != self.page_limit:
                raise
            self._advance_to_page(self.request_fn(
                self.conninfo,
                self.credentials,
                'GET',
    ...
    def _advance_to_page(self, new_page):
        ...
        if self.want_paths:
            ...
            new_ids = list({l['file_id'] for l in locks
        ...
        if self.want_hostnames:
            ...
            new_ips = list({l['owner_address'] for l in locks
            ...
            if new_ips:
                ...
                for h in new_hostnames:
                    ...
                    if h['result'] != 'OK':
                        ...
                        self.resolved_hostnames[h['ip_address']] = \
                            h['hostname'] or None
                for l in locks:
                    l['owner_hostname'] = self.resolved_hostnames[
                        l['owner_address']]
    ...
    def by_file(
        ...
        return LockIter(
            conninfo,
            credentials,
            fs.list_locks_by_file(conninfo, credentials, protocol, lock_type,
    ...
    def all(
        ...
        return LockIter(
            ...
            fs.list_locks_by_client(conninfo, credentials, protocol, lock_type,
...
class ListLocksCommand(qumulo.lib.opts.Subcommand):
    ...
    @staticmethod
    def options(parser):
        parser.add_argument("--protocol", type=str_decode, required=True,
            choices = { p for p, t in fs.VALID_LOCK_PROTO_TYPE_COMBINATIONS },
            help="The protocol whose locks should be listed")
        parser.add_argument("--lock-type", type=str_decode, required=True,
        ...
        parser.add_argument(
            "--sort", choices=["file", "client"], default=None,
...
class ListWaitersByClientCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument(
            "--address", help="Client IP address", type=str_decode)
...
class ReleaseNLMLockCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        ...
        parser.add_argument("--snapshot",
            help="Snapshot ID of the specified file", type=str_decode)
...
class AclExplainPosixModeCommand(qumulo.lib.opts.Subcommand):
    ...
    def options(parser):
        group = parser.add_mutually_exclusive_group(required=True)
        group.add_argument(
            "--path", help="Path to file/directory", type=str_decode)
        group.add_argument("--id", help="File ID", type=str_decode)
