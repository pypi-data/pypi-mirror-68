...
# # error装饰器 //k200518: 弃用, 使用log_error装饰器
# def decorate_error_log(parse):
...
#                 os.mkdir("log")
#             tb_txt = traceback.format_exc(limit=10)
#             print(tb_txt)
#             with open("./log/{0}.txt".format("error"), "a", encoding="utf-8") as f:
#                 f.write("{0}\n{1}\n\n\n".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), tb_txt))
#     return record_error
...
class myRequest():
    ...
    def req(self, url, *, proxies=None, selector=False, auth=None, allow_redirects=True,
        ...
        # 下面的 data参数 必须要先用json.dumps()转成str后，才能发送请求！！！
