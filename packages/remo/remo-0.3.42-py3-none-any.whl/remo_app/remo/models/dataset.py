...
def get_image_dimensions(image):
    ...
    if settings.STORAGE == 'local':
        ...
        if response.status_code == 200:
            img = pyvips.Image.new_from_buffer(response.content, "access=sequential")
    if img:
...
class DatasetImage(models.Model):
    ...
    def _open_image_from_buffer(self, buffer):
        buffer.seek(0)
        return pyvips.Image.new_from_buffer(buffer.read(), "access=sequential")
...
def purge_orphan_image(sender, instance, **kwargs):
    # decrease dataset size and quantity when delete image
    if instance.image_object:
        size = instance.image_object.size
        dataset = instance.dataset
        if dataset:
            dataset.size_in_bytes -= size
            dataset.quantity -= 1
            dataset.updated_at = timezone.now()
            dataset.save()

    """Delete Image record if it doesn't belong to any dataset"""
    exists = DatasetImage.objects.filter(image_object=instance.image_object).first()
    if not exists:
        instance.image_object.delete()
