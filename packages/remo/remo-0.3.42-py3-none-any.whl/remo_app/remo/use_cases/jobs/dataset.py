...
from remo_app.remo.models import AnnotationSet, Dataset, DatasetImage, ImageFolder, Annotation # Job,
from remo_app.remo.use_cases.jobs.update_annotation_set_statistics import update_annotation_set_statistics
from remo_app.remo.api.constants import TaskType # JobStatus,
...
# def process_files(dir_path, user, dataset: Dataset, annotation_task, folder: ImageFolder = None,
#                   annotation_set_id=None, class_encoding=None, skip_new_classes=False):
#     wait_for_job_record()
#     try:
#         images_count, annotations_count, errors = LocalDatasetUploader(user, dataset, dir_path,
#                                                                        annotation_task=annotation_task, folder=folder,
...
# def job_upload_dataset_from_archive(dir_path, file_path, user, dataset: Dataset, file_name, annotation_task):
#     wait_for_job_record()
#     try:
#         extract_dir = f'{dir_path}/extract'
#         unpacker_factory(file_path).unpack(file_path, extract_dir)
#         images_count, annotations_count, errors = LocalDatasetUploader(user, dataset, extract_dir, file_name,
#                                                                        annotation_task).upload()
#
#     finally:
#         utils.remove_dir(dir_path)


# def job_upload_dataset(url, user, dataset: Dataset, annotation_task=None, folder: ImageFolder = None,
#                        annotation_set_id=None, class_encoding=None, skip_new_classes=False):
#     wait_for_job_record()
#     job = get_current_job()
#     Job.update_status(job.id, JobStatus.in_progress.name)
#     logger.info(f'Current job: {job.id}')
#
#     extract_dir = f'{settings.RQ_TMP_DIR}/{job.id}'
#     images_count, annotations_count, errors = RemoteDatasetUploader(url, user, dataset, annotation_task, folder,
#                                                                     annotation_set_id=annotation_set_id,
#                                                                     class_encoding=class_encoding,
#                                                                     skip_new_classes=skip_new_classes).upload(extract_dir)
#
#     logger.info(f'[OK] job: {job.id}')
#     Job.update_status(job.id, JobStatus.done.name)
#     return True


# def wait_for_job_record():
#     job = get_current_job()
#     retry = 10
#     while retry > 0 and not Job.objects.filter(id=job.id).exists():
#         time.sleep(1)
#         retry -= 1
#
#     if retry == 0:
#         raise ConnectionError(f'Please check database connection, job record failed to save. Job id: {job.id}')


# def job_exception_handler(job, *exc_info):
#     Job.update_status(job.id, JobStatus.failed.name)


class RemoteDatasetUploader():
    def __init__(self, url, user, dataset: Dataset, annotation_task=None, folder: ImageFolder = None,
                 annotation_set_id=None, class_encoding=None, skip_new_classes=False):
        self.url = url
        self.user = user
    ...
    def upload(self, extract_dir):
        try:
            ...
            images_count, annotations_count, errors = LocalDatasetUploader(self.user, self.dataset, extract_dir,
                                                                           annotation_task=self.annotation_task,
                                                                           folder=self.folder,
                                                                           annotation_set_id=self.annotation_set_id,
                                                                           class_encoding=self.class_encoding,
                                                                           skip_new_classes=self.skip_new_classes).upload()
        finally:
...
class LocalDatasetUploader:
    def __init__(self, user, dataset: Dataset, dataset_dir, archive_name='', annotation_task=None,
                 folder: ImageFolder = None, annotation_set_id=None, class_encoding=None, skip_new_classes=False):
        self.user = user
        self.dataset = dataset
    ...
    def upload(self) -> (int, int, []):
        ...
        return image_count + arr_image_count, annotations_count + arr_annotations_count, img_errors + arr_errs + errs

    def upload_images(self):
        start = datetime.now()
        logger.debug(f'Uploading imgs to dataset {self.dataset}, dir {self.dataset_dir}, task {self.annotation_task}')
    ...
    def upload_archives(self, files) -> (int, int, []):
        ...
        for file_path in files:
            ...
            try:
                unpacker_factory(file_path).unpack(file_path, extract_dir)
                images, annotations, errs = LocalDatasetUploader(self.user, self.dataset, extract_dir,
                                                                 os.path.basename(file_path),
                                                                 self.annotation_task, self.folder).upload()
                total_images += images
    ...
    def _upload_annotations(self) -> (int, []):
        ...
        if len(formats) == 0:
            if self.annotation_task:
                err_msg = f'Annotation format for {self.annotation_task} not recognised. ' \
                          'Please check the documentation to see supported formats.'
                logger.warning(err_msg)
                errs.append(err_msg)
            return 0, errs

        annotation_parser = AnnotationParser(self.user, self.dataset, self.annotation_task, self.folder,
                                             annotation_set_id=self.annotation_set_id,
                                             class_encoding=self.class_encoding,
...
class AnnotationParser:
    def __init__(self, user, dataset: Dataset, annotation_task, folder: ImageFolder = None, annotation_set_id=None,
                 class_encoding=None, skip_new_classes=False):
        self.user = user
        self.dataset = dataset
        self.annotation_task = annotation_task
        self.folder = folder
    ...
    def parse(self, file_path, annotation_fp):
        msg = ''
        annotation_file_name = os.path.basename(file_path)
        err_msg = f"Annotation format for {self.annotation_task} not recognised for file '{annotation_file_name}'. " \
                  "Please check the documentation to see supported formats."
        formats = get_supported_formats(self.annotation_task)
        if len(formats) == 0:
            if self.annotation_task:
        ...
        for annotation_format in formats:
            if annotation_format.is_applicable(file_path, annotation_fp):
                ...
                try:
                    annotations_count += parser.save_annotations(self.dataset.id, self.user.id, images_mapping,
                                                                 annotation_set_id=self.annotation_set_id, skip_new_classes=self.skip_new_classes)
                except Exception as err:
                    logger.error(err)
...
def is_annotation_file(path):
    try:
        ...
        return extension in settings.ANNOTATION_FILE_EXTENSIONS
    except Exception as err:
        logger.error(err)
        return False


def resolve_archive_files(pathes):
    try:
        result = resolve_files(pathes, is_archive)
    except Exception as err:
        logger.error(err)
        result = []
    return result
...
def resolve_annotation_files(pathes):
    try:
        result = resolve_files(pathes, is_annotation_file)
    except Exception as err:
        logger.error(err)
        result = []
    return result
...
def resolve_image_files(pathes):
    try:
        result = resolve_files(pathes, is_image)
    except Exception as err:
        logger.error(err)
        result = []
    return result
...
def resolve_files(pathes, filter_func):
    """
...
def upload_local_files(user, local_files: List[str], dataset: Dataset, folder: ImageFolder,
                       annotation_task: TaskType = None, annotation_set_id=None, class_encoding=None, skip_new_classes=False) -> (int, int, []):
    if not local_files:
        return 0, 0, []
    arch_images, arch_annotations, arch_errs = _upload_local_archives(user, local_files, dataset, folder,
                                                                      annotation_task,
                                                                      annotation_set_id=annotation_set_id,
                                                                      class_encoding=class_encoding)
    img_count, img_errs = _upload_local_images(local_files, dataset, folder)
    annotation_count, errs = _parse_local_annotations(user, local_files, dataset, folder, annotation_task,
                                                      annotation_set_id=annotation_set_id,
                                                      class_encoding=class_encoding, skip_new_classes=skip_new_classes)
    return img_count + arch_images, annotation_count + arch_annotations, img_errs + errs + arch_errs


def _upload_local_archives(user, local_files: List[str], dataset: Dataset, folder: ImageFolder,
                           annotation_task: TaskType, annotation_set_id=None, class_encoding=None):
    ...
    try:
        images, annotations, errs = LocalDatasetUploader(user, dataset, extract_dir,
                                                         annotation_task=annotation_task, folder=folder,
                                                         annotation_set_id=annotation_set_id,
                                                         class_encoding=class_encoding) \
...
def _parse_local_annotations(user, local_files: List[str], dataset: Dataset, folder: ImageFolder,
                             annotation_task: TaskType, annotation_set_id=None, class_encoding=None, skip_new_classes=False):
    files = resolve_annotation_files(local_files)
    total_count = 0
    errs = []
    parser = AnnotationParser(user, dataset, annotation_task, folder, annotation_set_id=annotation_set_id,
                              class_encoding=class_encoding, skip_new_classes=skip_new_classes)
    for file_path in files:
        try:
