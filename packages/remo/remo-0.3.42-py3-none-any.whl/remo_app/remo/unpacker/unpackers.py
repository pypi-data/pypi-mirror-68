...
class Downloader:
    ...
    def __init__(self, url, destination, filename=''):
        ...
        self.filepath = os.path.join(destination, filename)
    ...
    def download(self):
        ...
        if utils.is_tool_exists('aria2c'):
            cmd = f"""aria2c -x4 -d "{self.destination}" -o "{self.filename}" "{self.url}" """
            logger.info(f'Running shell cmd: {cmd}')
            subprocess.run(cmd, shell=True, check=True)
        else:
            with requests.get(self.url, stream=True) as resp:
                self._download_from_requests_stream(resp)

        logger.debug(f'Elapsed time {datetime.now() - start}, finished download from url: {self.url}')
    ...
    def _raise_for_disk_space(self, file_size):
        ...
        logger.info(f'Free disk space before download: {utils.human_size(free)}')
        logger.info(f'File size: {utils.human_size(file_size)}')
...
class Unpacker:
    ...
    def _unpack(archive_path, extract_dir, archive=None, remove_archive=True, members=None):
        ...
        try:
            ...
            logger.debug(f'Elapsed time {elapsed}, finished extracting archive {archive_path}')
            if remove_archive:
    ...
    def _raise_for_disk_space(archive_path, extract_dir):
        ...
        logger.info(f'Free disk space before unpack: {utils.human_size(free)}')
        logger.info(f'Minimum required size: {utils.human_size(size)}')
    ...
    def unpack(archive_path, extract_dir, remove_archive=True):
        if utils.is_tool_exists('unzip'):
            ...
            try:
                ...
                logger.debug(f'Elapsed time {datetime.now() - start}, finished extracting archive {archive_path}')
                if remove_archive:
                    os.remove(archive_path)
            except:
