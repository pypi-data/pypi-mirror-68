...
from remo_app.remo.use_cases.annotation.class_encoding.class_encoding import ClassEncodingType, CustomClassEncoding
...
from remo_app.remo.use_cases.jobs.dataset import LocalDatasetUploader, \
    upload_local_files, RemoteDatasetUploader, is_annotation_file # job_upload_dataset, process_files
...
class DatasetViewSet(UpdateDatasetModelMixin,
    ...
    def upload(self, request, pk):
        ...
        if request.content_type.startswith('application/json') or request.content_type.startswith(
            ...
            return self._upload_from_url(request, dataset, annotation_set_id=annotation_set_id)
        elif request.content_type.startswith('multipart/form-data'):
            return self._upload_from_request(request, dataset, annotation_set_id=annotation_set_id)

        return Response({
            'error': True,
            'detail': f"Not supported payload format: {request.content_type}",
        }, status=400)
    ...
    def _upload_from_url(request, dataset: Dataset, annotation_set_id=None):
        ...
        with ProcessPoolExecutor(max_workers=1) as executor:
            results = executor.submit(process_upload_from_url, request.user, data.get('local_files'), dataset, folder, annotation_task, annotation_set_id, class_encoding, skip_new_classes, urls)
    ...
    def _store_file(file, dir_path):
        ...
        if is_annotation_file(file_path):
            time.sleep(1)
    ...
    def _upload_from_request(request, dataset: Dataset, annotation_set_id=None):
        folder_id = DatasetViewSet._parse_folder_id(request)
        if folder_id and not DatasetViewSet._is_valid_folder_id(folder_id, dataset):
            return Response({
                'error': True,
                'detail': "wrong 'folder_id' query parameter",
                'next': None,
                'previous': None,
                'results': [],
                'count': 0
        ...
        start = datetime.now()
        logger.debug(f'Start upload images and annotations from request for dataset {dataset}')
        ...
        annotation_set, resp = DatasetViewSet._create_new_annotation_set(dataset, annotation_set_name, annotation_task,
                                                               annotation_set_id, request.user.id)
        if resp:
            return resp
        ...
        with ProcessPoolExecutor(max_workers=1) as executor:
            results = executor.submit(process_upload_from_request, request.user, request.data.getlist('local_files'), dataset, folder, annotation_task, annotation_set_id, class_encoding, skip_new_classes, dir_path)
    ...
    def _upload_local_files(user, local_files: List[str], dataset: Dataset, folder: ImageFolder,
                            annotation_task: TaskType = None, annotation_set_id=None, class_encoding=None, skip_new_classes=False) -> (
        int, int, []):
        if local_files:
            return upload_local_files(user, local_files, dataset, folder, annotation_task,
                                      annotation_set_id=annotation_set_id, class_encoding=class_encoding, skip_new_classes=skip_new_classes)
        return 0, 0, []
...
def process_upload_from_url(user, local_files, dataset, folder, annotation_task, annotation_set_id, class_encoding, skip_new_classes, urls, send_end=None):
