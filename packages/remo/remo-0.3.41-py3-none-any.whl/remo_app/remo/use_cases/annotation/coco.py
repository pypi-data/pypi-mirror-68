...
class CocoJsonInstanceSegmentation(CocoBase):
    ...
    def retrieve_annotations(self, images_mapping: dict = None):
        ...
        for item in self._annotation_items():
            ...
            if not annotation:
                annotation = self.annotation_class(image_id, image_name=self.coco_images.get(image_id, image_id))
    ...
    def _find_dataset_image(self, dataset_id, file_name):
    ...
    def _download_image(self, file_name, urls):
        ...
        logger.debug('Downloading image %s - Done', file_name)
    ...
    def _download_to_file(self, urls, fp):
        ...
        downloaded_bytes = 0
        urls = list(
            filter(lambda url: len(url),
                   map(lambda url: str(url).strip(), urls))
        )
        ...
        for url in urls:
            ...
            try:
                ...
                logger.warning(
                    'URL %s: file size exceeds MAX_EXTERNAL_DOWNLOAD_IMAGE_SIZE, abort',
                    url
                )
    ...
    def export_annotations(self, annotation_set, export_coordinates='pixel', full_path=False, export_classes=False):
        ...
        return exporter.export_annotations(annotation_set, export_coordinates=export_coordinates, full_path=full_path,
...
class COCOInstanceSegmentationExporter(BaseExporter):
    ...
    def get_coordinates(self, annotation, obj, export_coordinates='pixel'):
        ...
        for p in obj.coordinates:
            if export_coordinates == 'pixel':
                ...
                coordinates.append({'x': float(p['x']) / width,
                                    'y': float(p['y']) / height})
        return coordinates
    ...
    def parse_annotation_object(self, annotation, obj, export_coordinates='pixel'):
        bbox, segments, area = self.get_bbox_segments_area(annotation, obj, export_coordinates=export_coordinates)
    ...
    def export_annotations(self, annotation_set, export_coordinates='pixel', full_path=False, export_classes=False):
        ...
        for annotation in Annotation.objects.filter(annotation_set=annotation_set):
            ...
            for obj in AnnotationObject.objects.filter(annotation=annotation):
                annotations.append(self.parse_annotation_object(annotation, obj, export_coordinates=export_coordinates))
