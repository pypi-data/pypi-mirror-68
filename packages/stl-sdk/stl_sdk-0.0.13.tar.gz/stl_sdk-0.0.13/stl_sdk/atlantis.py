...
class AtlantisClient:
    ...
    def client_id(self):
        if self._client_id is None:
            raise InvalidInitParameters('invalid initialization of the required parameters.')
    ...
    def get_public_key(self):
        ...
        :rtype: str
        """
        response = requests.get('{}/api/public-key'.format(self.atlantis_url))
        response.raise_for_status()
        return response.json()
...
class AtlantisClientFlask(AtlantisClient):
    ...
    def init_app(self, app):
        ...
        self._client_secret = app.config['ATLANTIS_CLIENT_SECRET']
