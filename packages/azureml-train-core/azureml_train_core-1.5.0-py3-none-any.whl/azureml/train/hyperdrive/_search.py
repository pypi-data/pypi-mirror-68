...
from azureml.exceptions import TrainingException, UserErrorException

import azureml.train.restclients.hyperdrive as HyperDriveClient
from azureml.train.restclients.hyperdrive.models import CreateExperimentDtoGeneratorConfig, \
    CreateExperimentDtoPolicyConfig, CreateExperimentDtoPrimaryMetricConfig, ErrorResponseException, \
    RunKey
...
def search(hyperdrive_config, workspace, experiment_name, **kwargs):
    ...
    if compute_target is not None \
        ...
        raise UserErrorException("Automated hyperparameter tuning is only supported for types AmlCompute, "
                                 "DsvmCompute, RemoteCompute and str.")

    _current_run_host = hyperdrive_config._get_service_address_exp(workspace, experiment_name)
    if hyperdrive_config._resume_from is not None and len(hyperdrive_config._resume_from) > 0:
        ...
        if _current_run_host != _resume_from_sample_run["run_scope"]["host"]:
            raise UserErrorException("The new run should have the same host as the resume_from runs.")
    ...
    if hyperdrive_config._resume_child_runs is not None and len(hyperdrive_config._resume_child_runs) > 0:
        ...
        if _current_run_host != _resume_child_runs_sample["run_scope"]["host"]:
            raise UserErrorException("The new run should have the same host as resume_child_runs.")
        ...
        try:
            with tempfile.TemporaryDirectory() as temporary_config:
                ...
                with open(config_path, 'rb') as config_f:
                    api_response = hyperdrive_client.create_experiment(config_f)
...
def _is_supported_compute_target(compute_target):
    ...
    return isinstance(compute_target, (AmlCompute, DsvmCompute, RemoteCompute, str))
