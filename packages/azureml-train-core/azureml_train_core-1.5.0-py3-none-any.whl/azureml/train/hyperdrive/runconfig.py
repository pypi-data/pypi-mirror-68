...
class PrimaryMetricGoal(enum.Enum):
    ...
    def from_str(goal):
        ...
        raise TrainingException("Unknown goal")
...
class HyperDriveConfig(object):
    ...
    def __init__(self,
        ...
        if self._run_config is not None and not isinstance(self._run_config, ScriptRunConfig):
            raise UserErrorException("Got invalid 'run_config' type, expected 'ScriptRunConfig'.")
        ...
        if resume_from is not None:
            ...
            if len(resume_from) > MAX_PARENT_RUNS:
                raise UserErrorException(("The number of parent runs should "
                                          "not be greater than {}. ").format(MAX_PARENT_RUNS))
    ...
    def _validate_resume_child_runs(self, resume_child_runs, is_bayesian):
        ...
        if len(resume_child_runs) > self._max_total_runs:
            raise UserErrorException("The number of resume_child_runs cannot "
                                     "exceed max_total_runs of current run. ")
        ...
        for run in resume_child_runs:
            if run.id in child_set:
                raise UserErrorException("resume_child_runs cannot contain duplicate runs.")
            child_set.add(run.id)

            if not isinstance(run, Run):
                raise TypeError("resume_child_runs must be azureml.core.run.Run objects")
            if run.get_status() not in TERMINAL_STATES:
                raise UserErrorException(("All resume_child_runs should be in a terminal state. Found "
                                          "a run with state '{}'.").format(run.get_status))
            if run.parent is None:
                raise UserErrorException("Every run in resume_child_runs should have a parent run.")

            parent_dict[run.parent.id] = run.parent

        if len(parent_dict) > MAX_PARENT_RUNS:
            raise UserErrorException(("resume_child_runs should not come from more than {} unique parents. "
                                      "{} unique parents were found.").format(
                                          MAX_PARENT_RUNS,
                                          len(parent_dict)))
    ...
    def _validate_warm_start(self, resume_list, is_bayesian, is_resume_from=False):
        ...
        for hdrun in resume_list:
            ...
            if hdrun._runtype != "hyperdrive":
                raise TypeError("All parent runs should be HyperDriveRun objects.")

            if hdrun.hyperdrive_config._primary_metric_config["name"] != self._primary_metric_config["name"]:
                raise UserErrorException(("The primary_metric_name should be the same for all runs. The current run's "
                                          "primary_metric_name is '{}', while a run with primary_metric_name "
                                          "'{}' was found.").format(
                                              self._primary_metric_config["name"],
            ...
            if hdrun.hyperdrive_config._primary_metric_config["goal"] != self._primary_metric_config["goal"]:
                raise UserErrorException(("The primary_metric_goal should be the same for all runs. The current run's "
                                          "primary_metric_goal is '{}', while a run with primary_metric_goal "
                                          "'{}' was found.").format(
                                              self._primary_metric_config["goal"],
            ...
            if hdrun.hyperdrive_config._platform_config["ServiceAddress"] \
                ...
                raise UserErrorException("The host should be the same for all runs.")

            if is_bayesian:
                if hdrun.hyperdrive_config._generator_config["parameter_space"] \
                        != self._generator_config["parameter_space"]:
                    raise UserErrorException("The parameter_space should be the same "
                                             "for all runs when using Bayesian sampling.")
            if is_resume_from:
                if hdrun.id in hdrun_set:
                    raise UserErrorException(("resume_from cannot contain duplicate runs. More than one "
                ...
                if hdrun.get_status() != RunStatus.COMPLETED \
                    ...
                    raise UserErrorException(("All resume_from runs should be completed or canceled. A resume_from "
                                              "run with status '{}' was found.").format(hdrun.get_status()))
                _total_child_runs += hdrun.hyperdrive_config._max_total_runs

        if is_resume_from and _total_child_runs > RESUME_FROM_TOTAL_RUNS:
            raise UserErrorException(("The number of total runs should not "
    ...
    def _register_datastore_for_resume_child_runs(self, resume_child_runs):
        ...
        for index, (workspace_id, runs) in enumerate(workspace_id_to_runs_dict.items()):
            ...
            if len(runs) > 0:
                # _get_blob_azureml_datastore function registers the datastore and returns it.
                runs[0]._get_blob_azureml_datastore()
    ...
    @staticmethod
    def _get_service_address_exp(workspace, run_name):
        return HyperDriveConfig._get_service_address("experimentation", workspace, run_name)
    ...
    def _get_platform_config(self, workspace, run_name, **kwargs):
        ...
        service_address = self._get_service_address_exp(workspace, run_name)
    ...
    def _validate_and_remove_duplicate_parameters(pipeline, generator_config, pipeline_parameters=None):
        ...
        for param in parameter_space:
            if param not in graph_params:
                raise TrainingException("Found a parameter in HyperDrive parameter space "
