...
_MONGO_CONNECTIONS = []
_UNIQUE_ERROR_FORMAT_1 = r'.* \(E11000 duplicate key error collection: ' \
                         r'\w+ index: (\w+)_1_(\w+)_id_1\)'
_UNIQUE_ERROR_FORMAT_2 = r'.* \(E11000 Duplicate Key Error\)'
_REFERENCE_ERROR_FORMAT = r'Could not delete document \((\w+)\.\w+ refers to it\)'
_LOGGER = logging.getLogger(__name__)


def _raise_unique_error(message, unique_fields):
    m = re.findall(_UNIQUE_ERROR_FORMAT_1, message)
    if len(m) > 0:
        raise ERROR_NOT_UNIQUE_KEYS(keys=list(m[0]))
    else:
        m = re.findall(_UNIQUE_ERROR_FORMAT_2, message)
        if len(m) > 0:
