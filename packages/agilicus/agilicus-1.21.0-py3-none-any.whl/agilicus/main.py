...
def user_completion(ctx, args, incomplete):
    context.setup(ctx)
    _users = json.loads(users.query(ctx))["users"]
    results = []
    for _user in _users:
...
def get_user_id_from_email(ctx, email, org_id=None):
    _users = json.loads(users.query(ctx, email=email, org_id=org_id))["users"]
    if len(_users) == 1:
        return _users[0]
    print(f"Cannot find user email:{email}")


# For readibility please keep this list in alphabetical order per subsection
...
@click.option("--scope", default=DEFAULT_SCOPES, multiple=True)
@click.pass_context
def cli(
    ctx,
    token,
...
def _delete_credentials(ctx, **kwargs):
    credentials.delete_credentials(ctx)
...
def list_users(ctx, organisation, org_id, **kwargs):
    ...
    output_list_users(org_by_id, json.loads(users.query(ctx, org_id, **kwargs))["users"])
...
def list_groups(ctx, organisation, org_id, type, **kwargs):
    ...
    output_list_groups(
        org_by_id, json.loads(users.query(ctx, org_id, type, **kwargs))["groups"]
    )
...
def list_sysgroups(ctx, organisation, org_id, **kwargs):
    ...
    output_list_groups(
        org_by_id,
        json.loads(users.query(ctx, org_id, type="sysgroup", **kwargs))["groups"],
    )
...
@click.argument("app_name", autocompletion=app_completion)
@click.argument("env_name", autocompletion=env_completion)
@click.option("--org_id", default=None)
@click.pass_context
def delete_environment(ctx, app_name, **kwargs):
    _app = _get_app(ctx, app_name)
    if _app:
        apps.delete_environment(ctx, app_id=_app["id"], **kwargs)
...
@click.option("--org_id", default=None)
@click.pass_context
def delete_application(ctx, app_name, **kwargs):
    _app = _get_app(ctx, app_name)
    if _app:
        apps.delete(ctx, _app["id"], **kwargs)
...
def _get_app(ctx, app_name, org_id=None):
    _app = apps.get_app(ctx, org_id, app_name)
    if _app:
        return _app
    else:
...
@click.argument("app_name", autocompletion=app_completion)
@click.pass_context
def show_application(ctx, app_name, **kwargs):
    _app = _get_app(ctx, app_name)
    if _app:
        output_entry(_app)
...
@click.option("--org_id", default=None)
@click.pass_context
def update_application(ctx, app_name, org_id, **kwargs):
    _app = _get_app(ctx, app_name, org_id=org_id)
    if _app:
...
@click.pass_context
def add_role(ctx, app_name, role_name):
    _app = _get_app(ctx, app_name)
    if _app:
...
@click.option("--hostname", default=None)
@click.pass_context
def rules_from_csv(ctx, app_name, role_name, file_name, org_id, hostname):
    _app = _get_app(ctx, app_name, org_id=org_id)
    if _app:
...
@click.pass_context
def add_definition(ctx, app_name, key, json_path):
    _app = _get_app(ctx, app_name)
    if _app:
...
@click.pass_context
def add_rule(
    ctx, app_name, role_name, method, path_regex, query_param, json_pointer, **kwargs,
):
    apps.add_rule(
        ctx,
        app_name,
...
@click.option("--token", default=None)
@click.pass_context
def show_token_introspection(ctx, refresh=None, token=None, **kwargs):
    my_token = token
    if not my_token:
        my_token = whoami.whoami(ctx, refresh, **kwargs)
    result = tokens.get_introspect(ctx, my_token, **kwargs)
    print(result)
...
def _format_redirects(redirects):
    return [redirect for redirect in redirects]
            ...
            for client in issuer.clients:
                table.add_row(
                    [
                        ...
                        client.secret,
                        client.application,
                        _format_redirects(client.redirects),
                    ]
                )
        else:
            table.add_row(
                [issuer.id, issuer.issuer, "-", "-", issuer.org_id, "-", "-", "-"]
            )
    print(table)
...
def list_clients(ctx, **kwargs):
    ...
    table = PrettyTable(
        ["id", "issuer_id", "org_id", "name", "secret", "application", "redirects"]
    )
    table.align = "l"
...
@click.argument("issuer_id", default=None)
@click.argument("name", default=None)
@click.argument("secret", default=None)
@click.option("--application", default=None)
@click.option("--org_id", default=None)
@click.option("--redirects", default=None, multiple=True)
@click.pass_context
def add_client(ctx, **kwargs):
    output_entry(issuers.add_client(ctx, **kwargs))
...
def delete_redirect(ctx, **kwargs):
    issuers.delete_redirect(ctx, **kwargs)


@cli.command(name="list-managed-upsteam-providers")
@click.argument("issuer_id", default=None)
@click.pass_context
def list_managed_upsteam_providers(ctx, issuer_id=None, **kwargs):
    issuer = issuers.show(ctx, issuer_id, **kwargs)
    upsteams = issuer.get("managed_upstreams", [])
...
@cli.command(name="update-managed-upsteam-provider")
@click.argument("issuer_id", default=None)
@click.argument("name", default=None)
@click.option("--enabled/--disabled", required=True, default=None)
@click.pass_context
def update_managed_upsteam_provider(
...
@cli.command(name="list-oidc-upsteam-providers")
@click.argument("issuer_id", default=None)
@click.pass_context
def list_oidc_upsteam_providers(ctx, issuer_id=None, **kwargs):
    issuer = issuers.show(ctx, issuer_id, **kwargs)
    upsteams = issuer.get("oidc_upstreams", [])
    table = PrettyTable(
        [
            "name",
            "client_id",
...
@cli.command(name="update-oidc-upsteam-provider")
@click.argument("issuer_id", default=None)
@click.argument("name", default=None)
...
@click.pass_context
def update_oidc_upsteam_provider(
    ctx,
    issuer_id=None,
    name=None,
    client_id=None,
...
@cli.command(name="add-oidc-upsteam-provider")
@click.argument("issuer_id", default=None)
@click.argument("name", default=None)
@click.option("--client_id", default=None)
@click.option("--client_secret", default=None)
...
@cli.command(name="delete-oidc-upsteam-provider")
@click.argument("issuer_id", default=None)
@click.argument("name", default=None)
@click.pass_context
def delete_oidc_upsteam_provider(ctx, issuer_id=None, name=None, **kwargs):
