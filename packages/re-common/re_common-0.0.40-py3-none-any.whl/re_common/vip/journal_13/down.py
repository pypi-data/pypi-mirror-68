...
def work():
    ...
    while True:
        ...
        print("注意事项：期刊编号为6时（不用输入years,下载时,输入卷,期）")
        while True:
            journalChouse = input("请输入期刊对应编号：")
            if int(journalChouse) not in [i+1 for i in range(13)]:
                print([i+1 for i in range(13)])
                print("-----------请输入正确期刊对应编号-------------")
                continue
            if journalChouse == "1":
            ...
            # if journalChouse == "5" or journalChouse == "6":
            #
            #     vol = input("请输入需要采集的卷次:")
            # if (journalChouse) == "5":
            #     print("如期次有：Issue 1 Supplement情况，输入期次时：期次+ '_'+suppl(1_suppl)!!!!!!!!")
            #
            #
            # if journalChouse == "7":
