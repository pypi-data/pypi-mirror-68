...
@options.pass_state
def annotate_calls(
    state, fields, run_async, delete_if_exists, keep_ref_unknown_calls
):
...
def to_vcf(historical):
    """Render calls to VCF file format."""
    if historical:
        return [sinks.HistoricalVcfFileSink()]
    return [sinks.VcfFileSink()]
...
@options.build_option
@options.pass_state
def load_annotations(state, data_set, data_set_version, workers):
