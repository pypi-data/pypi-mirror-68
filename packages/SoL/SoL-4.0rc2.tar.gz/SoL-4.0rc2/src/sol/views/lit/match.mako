...
<form class="ui center aligned form${' error' if error else ''}" method="POST">
  ...
  <table class="ui large celled table">
    <thead>
      <tr>
        <th class="center aligned" colspan="${3 if championship.trainingboards else 4}">
          % if not championship.trainingboards and not match.breaker:
            <div class="checker">
      ...
      <tr>
        ...
        <th class="center aligned">${_('Misses') if championship.trainingboards else _('Coins')}</th>
        % if not championship.trainingboards:
    ...
    <tbody>
        ...
        <tr id="board_${board.number}">
          ...
          <td class="center aligned">
            <input type="number" name="coins_${board.number}_1"
                   min="0" max="${ 99 if championship.trainingboards else 12 }"
      ...
      <tr>
        ...
        <td colspan="${4 if championship.trainingboards else 6}" class="center aligned">
...
<script>
 ...
 function compute_scores_and_totals(board) {
     ...
     % if championship.trainingboards:
     if(boards > 1) {
         total_1 = total_1 / boards
         total_2 = total_2 / boards
     }
     % endif
     ...
     var q1 = $('input[name="queen_' + (boards - 1) + '"][value="1"]').prop('checked'),
         q2 = $('input[name="queen_' + (boards - 1) + '"][value="2"]').prop('checked');

     if((q1 || q2) && boards < 10 && total_1 < 25 && total_2 < 25) {
         $('button.ui.center.aligned.primary.button').removeClass('disabled');
     } else {
         $('button.ui.center.aligned.primary.button').addClass('disabled');
     }
 ...
 $(document).ready(function() {
     ...
     $('button.ui.center.aligned.primary.button').click(function() {
         ...
         if(board == 1 || (board < 10 && (c1 || c2) && parseInt(c1) < 10 && parseInt(c2) < 10 && (q1 || q2) && parseInt(t1) < 25 && parseInt(t2) < 25)) {
