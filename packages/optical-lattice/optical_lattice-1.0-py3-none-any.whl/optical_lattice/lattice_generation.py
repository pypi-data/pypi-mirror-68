...
class GeneratedLatticeImage():
    ...
    def __init__(
        ...
        # x-pixel locations
        ccd_x = np.arange(0, CCD_resolution, 1)

        # y-pixel locations
        ccd_y = np.arange(0, CCD_resolution, 1)
        ...
        # combine the sampled x-locations from atoms and dark counts
        x_loc = np.concatenate((x_loc, dark_count_location_x)).astype(int)

        # combine the sampled y-locations from atoms and dark counts
        y_loc = np.concatenate((y_loc, dark_count_location_y)).astype(int)

        # convert counts to intensity values for each pixel
        for x, y in zip(x_loc, y_loc):
            if(x < CCD_resolution
               and y < CCD_resolution and x >= 0 and y >= 0):
    ...
    def grid_plot(self, num_sites=1, invert_y=False):
        ...
        lattice_coords = coords[
            (coords[:, 0] > xlims[0])*(coords[:, 0] < xlims[1])
            * (coords[:, 1] > ylims[0])*(coords[:, 1] < ylims[1]),
            :
        ]

        reduced_lattice_coords = lattice_coords[  # noqa: F841
            (lattice_coords[:, 0] < (num_sites * self.M + xlims[0]))
    ...
    def plot(self, invert_y=False, alpha=0.25, markersize=0.1):
        ...
        ax.set_yticks(
            np.arange(
                -0.5-1*self.M,
                (self.N+1)*self.M+0.5, self.M
            ) + self.lattice_origin[1])  # horizontal lines as visual aid
