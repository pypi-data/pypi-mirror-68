...
# Purpose:     Style editor for the wx.StyledTextCtrl
#
# Author:      Riaan Booysen
#
# Created:     2001/08/20
# RCS-ID:      $Id: STCStyleEditor.py,v 1.13 2004/04/25 15:23:34 kasplat Exp $
...
# KEA 2004-01-22
# quick workaround for wxPython 2.5
# I'm defining commonPropDefs in OnAddsharebtnButton below where it is used
# to avoid a seg fault on Mac OS X with wxPython 2.5
...
class STCStyleEditDlg(wx.Dialog):
    ...
    def _init_ctrls(self, prnt):
        ...
        wx.Dialog.__init__(self, id=wxID_STCSTYLEEDITDLG, name='STCStyleEditDlg',
              parent=prnt, pos=wx.Point(567, 279), size=wx.Size(493, 482),
        ...
        wx.EVT_CHOICE(self.speedsettingCh, wxID_STCSTYLEEDITDLGSPEEDSETTINGCH,
              self.OnSpeedsettingchChoice)

        self.elementLb = wx.ListBox(choices=[], id=wxID_STCSTYLEEDITDLGELEMENTLB,
              name='elementLb', parent=self, pos=wx.Point(8, 70),
              size=wx.Size(175, 128), style=0, validator=wx.DefaultValidator)
        ...
        wx.EVT_LISTBOX(self.elementLb, wxID_STCSTYLEEDITDLGELEMENTLB,
        ...
        wx.EVT_BUTTON(self.addCommonItemBtn, wxID_STCSTYLEEDITDLGADDCOMMONITEMBTN,
        ...
        wx.EVT_BUTTON(self.removeCommonItemBtn,
        ...
        self.stc = stc.StyledTextCtrl(id=wxID_STCSTYLEEDITDLGSTC, name='stc',
        ...
        self.okBtn = wx.Button(id=wxID_STCSTYLEEDITDLGOKBTN, label='OK',
              name='okBtn', parent=self, pos=wx.Point(316, 423), size=wx.Size(75,
              23), style=0)
        ...
        wx.EVT_BUTTON(self.okBtn, wxID_STCSTYLEEDITDLGOKBTN, self.OnOkbtnButton)
        ...
        wx.EVT_BUTTON(self.cancelBtn, wxID_STCSTYLEEDITDLGCANCELBTN,
        ...
        self.fixedWidthChk = wx.CheckBox(id=wxID_STCSTYLEEDITDLGFIXEDWIDTHCHK,
              ...
              23), size=wx.Size(16, 19), style=0)
        self.fixedWidthChk.SetToolTipString('Check this for Fixed Width fonts')
        wx.EVT_CHECKBOX(self.fixedWidthChk, wxID_STCSTYLEEDITDLGFIXEDWIDTHCHK,
        ...
        self.faceCb = wx.ComboBox(choices=[], id=wxID_STCSTYLEEDITDLGFACECB,
              ...
              size=wx.Size(101, 21), style=0, validator=wx.DefaultValidator,
              value='')
        ...
        self.sizeCb = wx.ComboBox(choices=[], id=wxID_STCSTYLEEDITDLGSIZECB,
              ...
              size=wx.Size(80, 21), style=0, validator=wx.DefaultValidator,
              value='')
        ...
        wx.EVT_BUTTON(self.fgColBtn, wxID_STCSTYLEEDITDLGFGCOLBTN,
        ...
        wx.EVT_BUTTON(self.bgColBtn, wxID_STCSTYLEEDITDLGBGCOLBTN,
        ...
        wx.EVT_CHECKBOX(self.taEOLfilledCb, wxID_STCSTYLEEDITDLGTAEOLFILLEDCB,
        ...
        wx.EVT_CHECKBOX(self.taUnderlinedCb, wxID_STCSTYLEEDITDLGTAUNDERLINEDCB,
        ...
        wx.EVT_CHECKBOX(self.taItalicCb, wxID_STCSTYLEEDITDLGTAITALICCB,
        ...
        wx.EVT_CHECKBOX(self.taBoldCb, wxID_STCSTYLEEDITDLGTABOLDCB,
    ...
    def __init__(self, parent, langTitle, lang, configFile, STCsToUpdate=()):
        ...
        stc.EVT_STC_MARGINCLICK(self.stc, wxID_STCSTYLEEDITDLGSTC, self.OnMarginClick)
        ...
        self.defNames, self.defValues = parseProp(\
              self.styleDict.get(stc.STC_STYLE_DEFAULT, ''))
...
#---Control population methods--------------------------------------------------
    ...
    def populateCtrls(self):
        self.clearCtrls(self.styleNum == stc.STC_STYLE_DEFAULT,
        ...
        self.populateProp(list(self.defValues.items()), True,
            self.styleNum != stc.STC_STYLE_DEFAULT)
...
#---Main GUI events-------------------------------------------------------------
    def OnElementlbListbox(self, event):
        ...
        # normal style element selected
            ...
            if self.styleNum == stc.STC_STYLE_DEFAULT:
...
#---STC events------------------------------------------------------------------
    ...
    def checkBraces(self, style):
        if style == stc.STC_STYLE_BRACELIGHT and 'good' in self.braceInfo:
        ...
        elif style == stc.STC_STYLE_BRACEBAD and 'bad' in self.braceInfo:
...
class CommonDefDlg(wx.Dialog):
    def _init_ctrls(self, prnt):
        ...
        self.propTypeRBx = wx.RadioBox(choices = ['Colour value', 'Font face', 'Size value'], id = wxID_COMMONDEFDLGPROPTYPERBX, label = 'Property type', majorDimension = 1, name = 'propTypeRBx', parent = self, point = wx.Point(8, 8), size = wx.Size(168, 92), style = wx.RA_SPECIFY_COLS, validator = wx.DefaultValidator)
        ...
        wx.EVT_BUTTON(self.okBtn, wxID_COMMONDEFDLGOKBTN, self.OnOkbtnButton)
        ...
        wx.EVT_BUTTON(self.cancelBtn, wxID_COMMONDEFDLGCANCELBTN, self.OnCancelbtnButton)
...
def setSelectionColour(stcwindow, style):
    names, values = parseProp(style)
    if 'fore' in names:
        stcwindow.SetSelForeground(True, strToCol(values['fore']))
    if 'back' in names:
        stcwindow.SetSelBackground(True, strToCol(values['back']))

def setCursorColour(stcwindow, style):
    names, values = parseProp(style)
    if 'fore' in names:
        stcwindow.SetCaretForeground(strToCol(values['fore']))

def setEdgeColour(stcwindow, style):
    names, values = parseProp(style)
    if 'fore' in names:
        stcwindow.SetEdgeColour(strToCol(values['fore']))
...
def setSTCStyles(stcwindow, styles, styleIdNames, commonDefs, lang, lexer, keywords):
    ...
    else: prop = styleDict[stc.STC_STYLE_DEFAULT]
    names, vals = parseProp(prop)
    if 'back' in names:
        bkCol = strToCol(vals['back']%commonDefs)
    if bkCol is None:
        bkCol = wx.WHITE
    stcwindow.SetBackgroundColour(bkCol)

    # Set the styles on the wx.STC
    stcwindow.StyleResetDefault()
    stcwindow.ClearDocumentStyle()
    stcwindow.SetLexer(lexer)
    stcwindow.SetKeyWords(0, keywords)
    stcwindow.StyleSetSpec(stc.STC_STYLE_DEFAULT,
          styleDict[stc.STC_STYLE_DEFAULT] % commonDefs)
    stcwindow.StyleClearAll()

    for num, style in list(styleDict.items()):
        if num >= 0:
            stcwindow.StyleSetSpec(num, style % commonDefs)
        elif num == -1:
            setSelectionColour(stcwindow, style % commonDefs)
        elif num == -2:
            setCursorColour(stcwindow, style % commonDefs)
        elif num == -3:
            setEdgeColour(stcwindow, style % commonDefs)

    stcwindow.Colourise(0, stcwindow.GetTextLength())
...
#-------------------------------------------------------------------------------
def initSTC(stcwindow, config, lang):
    ...
    setSTCStyles(stcwindow, styles, styleIdNames, commonDefs, lang, lexer, keywords)
...
if __name__ == '__main__':
    ...
    config = os.path.abspath(os.path.join(home, STC_CONFIG_FILE))
    if 0:
        f = wx.Frame(None, -1, 'Test frame (double click for editor)')
        stcwindow = stc.StyledTextCtrl(f, -1)
        def OnDblClick(evt, stc=stc):
            dlg = STCStyleEditDlg(None, 'Python', 'python', config, (stcwindow,))
            try: dlg.ShowModal()
            finally: dlg.Destroy()
        stcwindow.SetText(open('STCStyleEditor.py').read())
        wx.EVT_LEFT_DCLICK(stc, OnDblClick)
        initSTC(stcwindow, config, 'python')
        f.Show(True)
        app.MainLoop()
    else:
