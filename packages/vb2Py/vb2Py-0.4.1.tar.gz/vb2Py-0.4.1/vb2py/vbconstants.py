...
vbNullString = chr(
    0)  # String having value 0 Not the same as a zero-length string (""); used for calling external procedures
vbObjectError = -2147221504  # User-defined error numbers should be greater than this value.
# For example: Err.Raise Number = vbObjectError + 1000
...
vbNormalNoFocus = 4  # Window is restored to its most recent size and position. The active window remains active.
...
TemporaryFolder = 2  # The Temp folder is used to store temporary files. Its path is found in the TMP env variable.

# StrConv Codes
vbUpperCase = 1  # Converts the string to uppercase characters.
vbLowerCase = 2  # Converts the string to lowercase characters.
vbProperCase = 3  # Converts the first letter of every word in string to uppercase.
vbWide = 4  # Converts narrow (single-byte) chars in string to wide (double-byte) chars. Applies to Far East locales.
vbNarrow = 8  # Converts wide (double-byte) chars in string to narrow (single-byte) chars. Applies to Far East locales.
