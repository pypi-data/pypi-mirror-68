...
class Patron(object):
    ...
    def gather_data(self, detector_name=None):
        return self._harvester.harvest_data(detector_name=detector_name)
...
class Harvester(ABC):
    ...
    def set_time_interval(self, start, stop, wagasci_database=None):
        ...
        if isinstance(start, datetime) and isinstance(stop, datetime):
            ...
            assert start is not None and isinstance(start, int), \
                "Type of start argument {} is not supported".format(type(start).__name__)
            assert stop is not None and isinstance(stop, int), \
...
class WagasciHarvester(Harvester, ABC):
    ...
    def harvest_data(self, detector_name=None):
        ...
        for dif in self._detectors.get_detector(detector_name=detector_name):
            if dif.has_tree():
...
class WagasciPotHarvester(WagasciHarvester):
    ...
    def harvest_data(self, detector_name=None):
        super(WagasciPotHarvester, self).harvest_data(detector_name)
...
class WagasciSpillHarvester(WagasciHarvester):

    def __init__(self, *args, **kwargs):
        self._active_branches = ["spill_mode", "spill_number", "timestamp"]
        super(WagasciSpillHarvester, self).__init__(*args, **kwargs)
    ...
    def harvest_data(self, detector_name=None):
        super(WagasciSpillHarvester, self).harvest_data(detector_name)
        detector = self._detectors.get_detector(detector_name=detector_name)
        if detector.is_enabled():
            spill_number_list = []
            timestamp_list = []
...
class WagasciFixedSpillHarvester(WagasciHarvester):
    ...
    def harvest_data(self, detector_name=None):
        super(WagasciFixedSpillHarvester, self).harvest_data(detector_name)
        detector = self._detectors.get_detector(detector_name=detector_name)
        if detector.is_enabled():
