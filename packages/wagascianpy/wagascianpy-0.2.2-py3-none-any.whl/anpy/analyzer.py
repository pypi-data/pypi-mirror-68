...
class Analyzer(ABC):
    ...
    def multiple_file_loop(self, input_files, chains):
        for input_file in sorted(input_files):
            ...
            dif_id = int(anpy.utils.extract_dif_id(input_file))
            ...
            if dif_id not in chains:
                ...
                chain.link = anpy.analysis.WagasciAnalysis(ENV["WAGASCI_LIB"])
...
class Decoder(Analyzer, ABC):
    ...
    _default_args = anpy.utils.get_arguments_ordered_dict(anpy.analysis.WagasciAnalysis.decoder)
    ...
    def __init__(self, **kwargs):
        ...
        self.acq_config_xml = anpy.utils.acqconfigxml_file_finder(self.run_root_dir, self.run_name)
        ...
        anpy.utils.renametree(run_root_dir=self.run_root_dir, run_name=self.run_name, dif_topology=self._topology)
    ...
    def spawn(self, chains):
        if os.path.isfile(self.run_root_dir) and self.run_root_dir.endswith('.raw'):
            ...
            input_files = anpy.utils.find_files_with_ext(self.run_root_dir, 'raw')
        if chains:
...
class SpillNumberFixer(Analyzer, ABC):
    ...
    _default_args = anpy.utils.get_arguments_ordered_dict(anpy.analysis.WagasciAnalysis.spill_number_fixer)
...
class BeamSummaryData(Analyzer, ABC):
    ...
    _default_args = anpy.utils.get_arguments_ordered_dict(anpy.beam_summary_data.beam_summary_data)
