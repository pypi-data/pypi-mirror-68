...
def renametree(run_root_dir, run_name, dif_topology):
    ...
    file_dif_list = []
    for file_name in sorted(anpy.utils.find_files_with_ext(path=run_root_dir, extension='raw')):
    ...
    for tmp_file_name in sorted(anpy.utils.find_files_with_ext(path=run_root_dir, extension='tmp')):
