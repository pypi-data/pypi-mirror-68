...
def change_config(dif_topology, bitstream_root_dir, input_dac, trigger_threshold):
    ...
    anpy.utils.mkdir_p(bitstream_root_dir)
    ...
    process = anpy.analysis.WagasciAnalysis(environ['WAGASCI_MAINDIR'] + "/lib")
    overwrite_flag = True
    edit_flag = True
    ...
    for dif, chip_map in sorted(iter(dif_topology.items()),
        ...
        for chip, n_channels in sorted(iter(chip_map.items()),
            ...
            # Input DAC
            for channel in range(anpy.analysis.SPIROC2D_NCHANNELS):
