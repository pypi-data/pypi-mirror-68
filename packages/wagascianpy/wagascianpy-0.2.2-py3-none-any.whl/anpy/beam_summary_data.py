...
import anpy.utils
import anpy.spill
import wagascidb.bsddb
import wagascidb.db_record
...
def _open_wagasci_tree(input_file, mode="READ"):
    ...
    raw_tree = getattr(tfile, anpy.utils.extract_raw_tree_name(tfile))
...
def _set_wagasci_spills(input_file, matched_spills):
    ...
    empty_bsd_spill = anpy.spill.SpillFactory.get_spill("bsd")
    ...
    for event in raw_tree:
        ...
        if current_spill_mode == _NO_BEAM_MODE:
            matched_spill = None
        else:
        ...
        if matched_spill is None:
            bsd_spill = anpy.spill.SpillFactory.get_spill("bsd")
        else:
...
def _get_wagasci_spills(input_file):
    ...
    print("Run start time %s" % wagascidb.db_record.DBRecord.timestamp2str(start_time))
    print("Run stop time %s" % wagascidb.db_record.DBRecord.timestamp2str(stop_time))
    print("DIF %s" % dif_id)

    wagasci_spills = []
    for event in raw_tree:
        # print("Spill number = %s" % event.spill_number)
        # print("Spill mode = %s" % event.spill_mode)
        # print("Fixed spill number = %s" % event.fixed_spill_number)
        # print("Is good spill = %s" % (not bool(event.good_spill_flag)))

        if event.spill_mode == _NO_BEAM_MODE:
            continue
        wagasci_spill = anpy.spill.SpillFactory.get_spill("wagasci")
        ...
        wagasci_spill.good_spill = bool(event.good_spill_flag)
...
def _get_bsd_spills(bsd_database, bsd_repository, t2krun, start_time, stop_time):
    bsd_tree = ROOT.TChain("bsd")
    with wagascidb.bsddb.BsdDataBase(bsd_database, bsd_repository, None, t2krun) as database:
        bsd_records = database.get_time_interval(start_time, stop_time)
        if not bsd_records:
        ...
        for bsd_record in bsd_records:
            ...
            if not os.path.exists(file_path):
                ...
                file_path = anpy.utils.find_first_match(bsd_record["name"], bsd_repository)
                if file_path is None:
                    raise RuntimeError("Please specify a valid BSD local repository")
            bsd_tree.Add(bsd_record["file_path"])
    ...
    for event in bsd_tree:
        ...
        bsd_spill.pot = float(event.ct_pot[wagascidb.bsddb.BunchNumber.TotalCurrent])
        bsd_spill.timestamp = float(bsd_tree.trg_sec[wagascidb.bsddb.TriggerTime.RubidiumClock])
        bsd_spill.timestamp += float(bsd_tree.trg_nano[wagascidb.bsddb.TriggerTime.RubidiumClock]) / 1e9
        bsd_spill.bsd_good_spill_flag = bool(event.good_spill_flag)
...
def _match_spills(wagasci_spills, bsd_spills, start_time, stop_time):
    ...
    good_spills = (wagasci_spill for wagasci_spill in wagasci_spills
                   if wagasci_spill.good_spill is True and wagasci_spill.spill_mode == _BEAM_MODE)
    starting_index = 0
...
def beam_summary_data(input_path, bsd_database, bsd_repository, t2krun, recursive):
    ...
    for input_file in input_files:
        ...
        bsd_spills = _get_bsd_spills(bsd_database, bsd_repository, t2krun, start_time, stop_time)
