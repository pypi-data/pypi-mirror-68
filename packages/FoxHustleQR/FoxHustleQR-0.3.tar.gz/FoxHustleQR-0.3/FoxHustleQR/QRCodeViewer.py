...
class QRGenerator:
    ...
    def __call__(self, url, version=5, qr="colored light"):
        if qr == "colored light":
            color1 = "black"
            color2 = "black"
            color3 = "black"
            ...
            print("not found this type of qr")
            sys.exit(-1)
        pyqr = pyqrcode.create(url, error='H', version=version, mode='binary')
        self.code =  pyqr.code
        count = len(self.code)
        img = Image.new('RGBA', (20+10*count, 20+10*count), wallpaper)
        idraw = ImageDraw.Draw(img)

        width = 100 + 10*(count % 2) - 4
