...
def get_arguments():
    ...
    parser.add_argument("--cache_path", metavar="CACHE PATH", type=str, default="./FlickrMirroring/.flickr/", help="specify the absolute path where the photos downloaded from Flickr need to be cached")
...
class FlickrUserPhotostreamMirroringAgent:
    ...
    def __generate_path(self, photo):
        ...
        path = "./FlickrMirroring/.flickr/" + self.__user.username + "/"
    ...
    def __lifo_strategy(self):
        ...
        path = pathlib.PosixPath("./FlickrMirroring/.flickr/lifo.cache")
        if path.is_file():
            file_ = path.open("r+")
            data = file_.read()
            if data != '':
        ...
        while True:
            ...
            file_ = pathlib.PosixPath("./FlickrMirroring/.flickr/lifo.cache").open("w+")
    ...
    def __fifo_strategy(self):
        ...
        #reading downloaded cache
        path = pathlib.PosixPath("./FlickrMirroring/.flickr/fifo.cache")
        if path.is_file():
            file_ = path.open("r+")
            data = file_.read()
            if data != '':
        ...
        while True:
            ...
            #store downloaded cache
            file_ = pathlib.PosixPath("./FlickrMirroring/.flickr/fifo.cache").open("w+")
...
def main():
    ...
    mirroring_agent = FlickrUserPhotostreamMirroringAgent(args.username, args.consumer_key, args.consumer_secret,\
         image_only=args.image_only, info_only=args.info_only, info_level=args.info_level, caching_strategy=_caching_strategy)
