...
class RawDataApi(object):
    ...
    def get_em_stock_price(self, start_date: str, end_date: str, stock_list: Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    EmStockPrice
    ...
    def get_em_daily_info(self, stock_list: Tuple[str] = (), date_list: Tuple[str] = ()):
        with RawDatabaseConnector().managed_session() as db_session:
            try:
                query = db_session.query(
                    EmStockDailyInfo
                ...
                if date_list:
                    query = query.filter(
                        EmStockDailyInfo.DATES.in_(date_list)
                    )
                return pd.read_sql(query.statement, query.session.bind)
            except Exception as e:
