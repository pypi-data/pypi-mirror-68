...
class RqIndexWeight(Base):
    ...
    index_id = Column(CHAR(20), primary_key=True) # 股票
    datetime = Column(DATE, primary_key=True) # 日期
...
class EmStockPrice(Base):
    ...
    UPDATE_TIME = Column("update_time", DATETIME, nullable=False, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP')) # 更新时间
