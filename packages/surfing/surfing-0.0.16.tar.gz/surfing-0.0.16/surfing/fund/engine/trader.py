    ...
    def finalize_trade(self, dt, trades: list, 
        ...
        for trd in trades:
            ...
            if self.asset_param.EnableCommission:
                trd.commission = trd.amount * (self.asset_param.PurchaseDiscount * self.asset_param.AssetPurchaseRate.__dict__[trd.index_id] if trd.is_buy 
                                                else self.asset_param.RedeemDiscount * self.asset_param.AssetRedeemRate.__dict__[trd.index_id])
    ...
    def calc_fund_trade(self, dt, fund_weight: FundWeight, cur_fund_position: FundPosition, 
        ...
        for fund_id, index_id in all_funds.items():
            ...
            if abs(target_fund_amt - cur_fund_amt) > fund_tot_mv * self.fund_param.MinCountAmtDiff or (target_fund_amt == 0 and cur_fund_amt > 0):
                ...
                # print(f'(fund){fund_id} (p){p} (amt0){cur_fund_amt} (amt1){target_fund_amt} (idx){index_id} (amt){abs(target_fund_amt - cur_fund_amt)}')
