...
class TiVendor:
    ...
    def __init__(self, config):
        ...
        self.logger_name = config.get('LOGGER_NAME') or None
    ...
    def get_api_info(self, api_num):
        ...
        try:
            ...
            self.logger.error('get api info fault,status: %s,resp:%s' % (r.status_code, r.text) ,exc_info=True)
            raise
        ...
        v_redis.set(key, json.dumps(info),300)
...
def desensitization(data, sm4_secret):
    ...
    if len(sm4_secret) != 16:
        return data

    if isinstance(data, (dict, list)):
        data = json.dumps(data)

    data = encrypt_ecb(data, sm4_secret)
