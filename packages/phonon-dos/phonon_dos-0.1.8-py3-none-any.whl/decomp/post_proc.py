...
Ruc, masses_for_animation = 0,0 #atm you dont plot eigenvectors
...
if(mode==4):
    ...
    plot.plot4(distances, frequencies, frequencies_disp, ks_path, x_labels)
...
if(mode==5):
    ...
    for i in range(Nqpoints):
        ...
        for j in range(len(qpoints_scaled)):
            if(np.allclose(qpoint, qpoints_scaled[j]) and (qpoint.tolist() not in accepted_qpoints)):
                accepted_qpoints.append(qpoint.tolist())
                for m in range(Nqpoints):
                    if(np.allclose(qpoint,qpoints_scaled[m])):
                        freqs_from_disp[count, :] = freqs_disp[m,:]
    ...
    plot.plot_k(freq,ZQS,ZQS_proj, x_labels,[freqs_from_disp],max_Z,modes,title=['freq from 0K dispersion'])
