...
class Segmentation(object):
    ...
    def predict(self, image, score_threshold=0.1, top_k=100, class_name='person'):
        ...
        masks = t[3][:top_k]
        classes, scores, boxes = [x[:top_k].detach().numpy() for x in t[:3]]
        ...
        for j in reversed(range(num_dets_to_consider)):
            ...
            if _class == class_name:
                if mask is None:
                    mask = np.asarray(masks[j].detach().numpy().squeeze() * 255, np.uint8)
                else:
                    mask1 = np.asarray(masks[j].detach().numpy().squeeze() * 255, np.uint8)
                    mask = mask | mask1
