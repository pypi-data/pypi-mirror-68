...
class DSMRReader():
    ...
    def extract_data_from_telegram(self, telegram):
      ...
      data = dict(
        unix_timestamp = int(time.time()),
        current_electricity_usage = str(telegram.CURRENT_ELECTRICITY_USAGE * 1000),
        current_electricity_delivery = str(telegram.CURRENT_ELECTRICITY_DELIVERY * 1000),
        electricity_active_tariff = str(telegram.ELECTRICITY_ACTIVE_TARIFF),
        usage_total_low = str(telegram.ELECTRICITY_USED_TARIFF_1 * 1000),
        usage_total_high = str(telegram.ELECTRICITY_USED_TARIFF_2 * 1000),
        redelivery_total_low = str(telegram.ELECTRICITY_DELIVERED_TARIFF_1 * 1000),
        redelivery_total_high = str(telegram.ELECTRICITY_DELIVERED_TARIFF_2 * 1000),
        usage_gas_total = str(telegram.HOURLY_GAS_METER_READING * 1000)
      )
