...
class DSMRReader():
    ...
    def format_telegram(self, telegram):
      ...
      '''
      body = dict(
        current_electricity_usage = float(telegram[obis_ref.CURRENT_ELECTRICITY_USAGE].value),
        current_electricity_delivery = float(telegram[obis_ref.CURRENT_ELECTRICITY_DELIVERY].value),
        electricity_active_tariff = int(telegram[obis_ref.ELECTRICITY_ACTIVE_TARIFF].value),
        electricity_used_tariff_1 = float(telegram[obis_ref.ELECTRICITY_USED_TARIFF_1].value),
        electricity_used_tariff_2 = float(telegram[obis_ref.ELECTRICITY_USED_TARIFF_2].value),
        electricity_delivered_tariff_1 = float(telegram[obis_ref.ELECTRICITY_DELIVERED_TARIFF_1].value),
        electricity_delivered_tariff_2 = float(telegram[obis_ref.ELECTRICITY_DELIVERED_TARIFF_2].value),
        gas_meter_reading = float(telegram[obis_ref.GAS_METER_READING].value)
      )
    ...
    def read(self):
        ...
        try:
          ...
          for telegram in self.serial_reader.read_as_object():
            ...
            obj = self.format_telegram(telegram)
