...
def _properties_are_equal(properties_a, properties_b):
...
def _set_value(result, primitive,
               primitive_type):
...
class _NodeAndType(object):  # pytype: disable=ignored-metaclass
  ...
  def get_custom_property(self, attr):
...
def _text_to_property_type(text):
...
def _is_serialized_artifact_type(from_json):
...
def _is_serialized_artifact(from_json):
...
def _create_artifact_struct_from_json_helper(from_json):
...
def create_json(artifact_struct):
...
def is_instance(struct, coercable_to_type):
...
class ExecutionType(object):
  ...
  def _is_equal(self, execution_type):
...
class Execution(_NodeAndType):
  ...
  @classmethod
  def create(cls, execution_type, input_struct,
             output_struct, **kwargs):
  ...
  @classmethod
  def _get_from_protos(cls, store,
                       executions
                      ):
  ...
  def is_input_consistent(self):
    return self.type.input_type.is_type_of(self.input_struct)

  def is_output_consistent(self):
...
def _register_type_as_used(registered_types, node_type):
...
def _create_execution_type_proto(name, properties
                                ):
...
def _get_suffixes(paths_and_artifacts
                 ):
  return [
...
def _create_local_event(execution_id,
                        event_type, path,
                        artifact):
