...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account events users domains default images linodes kernels stackscripts lke longview managed networking nodebalancers object-storage profile sshkeys regions tickets tags volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
            COMPREPLY=( $(compgen -W "view update cancel update-card invoices-list invoice-view invoice-items notifications-list clients-list client-create client-view client-update client-delete client-reset-secret payments-list payment-create payment-view paypal-start paypal-execute settings settings-update enable-managed transfer --help" -- ${cur}) )
            return 0
            ;;
        events)
        ...
        images)
            ...
            ;;
        linodes)
            COMPREPLY=( $(compgen -W "list create view update delete backups-list snapshot backups-cancel backups-enable backup-view backup-restore boot clone configs-list config-create config-view config-update config-delete disks-list disk-create disk-view disk-update disk-delete disk-clone disk-reset-password disk-resize ips-list ip-add ip-view ip-update ip-delete migrate upgrade reboot rebuild rescue resize shutdown transfer-view volumes types type-view --help" -- ${cur}) )
            return 0
            ;;
        kernels)
        ...
        stackscripts)
            ...
            ;;
        lke)
            COMPREPLY=( $(compgen -W "clusters-list cluster-create cluster-view cluster-update cluster-delete pools-list pool-create pool-view pool-update pool-delete api-endpoints-list kubeconfig-view versions-list version-view --help" -- ${cur}) )
            return 0
            ;;
        longview)
        ...
        networking)
            ...
            ;;
        nodebalancers)
            COMPREPLY=( $(compgen -W "list create view update delete configs-list config-create config-view config-update config-delete rebuild nodes-list node-create node-view node-update node-delete --help" -- ${cur}) )
            return 0
            ;;
        object-storage)
            COMPREPLY=( $(compgen -W "clusters-list clusters-view keys-list keys-create keys-view keys-update keys-delete cancel --help" -- ${cur}) )
            return 0
            ;;
        profile)
