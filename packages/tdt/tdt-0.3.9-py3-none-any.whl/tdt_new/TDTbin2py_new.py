...
class NexWriter(object):
    def __init__(self, timestampFrequency, useNumpy=False, useNEX5=True):
        """
        Constructor
        :param timestampFrequency: timestamp frequency in Hertz.
                Timestamps are stored as integers representing
                number of ticks, where tick = 1.0/timestampFrequency
        :param useNumpy: option to use numpy
        """
        self.theFile = None
        self.useNEX5 = useNEX5
        self.maxVariables = 1024 # TODO: this is our temp variable, change this to use the real nvars?
        self.tsFreq = timestampFrequency
        self.useNumpy = useNumpy
        self.fromTicksToSeconds = 1.0/timestampFrequency

        self.varHeaderKeys = ['Type', 'Version', 'Name', 'DataOffset', 'Count', 'Wire',
            'Unit', 'Gain', 'Filter', 'XPos', 'YPos','SamplingRate', 'ADtoMV', 'NPointsWave', 'NMarkers', 'MarkerLength', 'MVOffset', 'PreThrTime', 'Padding']

        self.nex5VarHeaderKeys = ['Type', 'Version', 'Name', 'DataOffset', 'Count', 'TsDataType', 'ContDataType',
            'SamplingRate', 'Units', 'ADtoMV', 'MVOffset', 'NPointsWave', 'PreThrTime', 'MarkerDataType', 'NMarkers', 'MarkerLength',
    ...
    def _CalculateScaling(self, var, values):
        # values = var['ContinuousValues']
        varType = var['Header']['Type']
        if varType in [NexFileVarType.CONTINUOUS, NexFileVarType.WAVEFORM]:
            var['Header']['ADtoMV'] = 1
            return
    ...
    def _VarNumDataBytes(self, var, data=None):
        """
        :param var: file data variable
        :return: number of bytes in variable data
        """
        # data = var['WaveformValues'] for NexFileVarType.WAVEFORM
        # data = var['ContinuousValues'] for NexFileVarType.CONTINUOUS
        ...
        if varType == NexFileVarType.NEURON or varType == NexFileVarType.EVENT:
            ...
            #Tracer()()
            return (self._BytesInTimestamp(var) + 4) * len(var['Timestamps']) + self._BytesInContValue(var) * len(data)
    ...
    def _VarWriteWaveformsNumpy(self, var, data):
        # data = var['WaveformValues']
        #Tracer()()
        if self._BytesInContValue(var) == 2:
            np.round(data / var['Header']['ADtoMV']).astype(np.int16).tofile(self.theFile)
        else:
            data.astype(np.float32).tofile(self.theFile)
        self._CalculateScaling(var, data)

    def _VarWriteContinuousValuesNumpy(self, var, data):
        #Tracer()()
        # data = var['ContinuousValues']
        if self._BytesInContValue(var) == 2:
            np.round(data / var['Header']['ADtoMV']).astype(np.int16).tofile(self.theFile)
        else:
            data.astype(np.float32).tofile(self.theFile)
    ...
    def AddMarker(self, name, timestamps, fieldNames, markerFields):
        ...
        if var['AllNumbers']:
            ...
            var['Header']['MarkerDataType'] = 0
        self._VarWriteData(var) # replaces var['ContinuousValues'] = values
    ...
    def addVariable(self, var, data=None):
        self.fileData['Variables'].append(var)
        self.temp_counts.append(self._VarCount(var))
        
        this_numbytes = self._VarNumDataBytes(var, data)  #TODO: keep track of this when it is written
        if len(self.temp_offsets) == 0:
            nvars = self.maxVariables
            if self.useNEX5:
                dataOffset = 356 + nvars * 244
            else:
                dataOffset = 544 + nvars * 208
            self.temp_offsets.append(dataOffset)
            self.temp_offsets.append(dataOffset + this_numbytes)
        else:
            self.temp_offsets.append(self.temp_offsets[-1] + this_numbytes)
    
    def writeHeader(self, dummy=False):
    
        self.theFile.seek(0, 0) # start at beginning of file
        if dummy:
            if self.useNEX5:
                # fileHeaderFormat = '<i <i 256s <d <q <i <Q <q 56s'
                headerSize = 356 # 4 + 4 + 256 + 8 + 8 + 4 + 8 + 8 + 56
                self.theFile.write(b'\0' * headerSize) # write 0s for header section, for now
            
                temp_nvars = self.maxVariables
                self.theFile.write(b'\0' * (temp_nvars*244))
            else:
                # fileHeaderFormat = '<i <i 256s <d <i <i <i 260s'
                headerSize = 544 # 4 + 4 + 256 + 8 + 4 + 4 + 4 + 260
                self.theFile.write(b'\0' * headerSize) # write 0s for header section, for now
            
                temp_nvars = self.maxVariables
                self.theFile.write(b'\0' * (temp_nvars*208))
            return
        ...
        for v in self.fileData['Variables']:
            maxTs = max(maxTs, self._VarMaxTimestampNumpy(v))
        tsAs64 = 0
        ...
        for i, v in enumerate(self.fileData['Variables']):
            v['Header']['Count'] = self.temp_counts[i]
            v['Header']['DataOffset'] = self.temp_offsets[i]

        if self.useNEX5:
            varHeaderFormat = '<i <i 64s <Q <Q <i <i <d 32s <d <d <Q <d <i <i <i <i 60s'.split()
            #varHeaderSize = 4 + 4 + 64 + 8 + 8 + 4 + 4 + 8 + 32 + 8 + 8 + 8 + 8 + 4 + 4 + 4 + 4 + 60 # 244
            for v in self.fileData['Variables']:
    ...
    def _VarWriteData(self, var, data=None):
        varType = var['Header']['Type']
        if varType == NexFileVarType.NEURON or varType == NexFileVarType.EVENT:
            ...
            self._VarWriteTimestamps(var, var['Timestamps'])
            if data is not None:
                self._VarWriteWaveformsNumpy(var, data)
            return
        elif varType == NexFileVarType.POPULATION_VECTOR:
            return
        elif varType == NexFileVarType.CONTINUOUS:
            ...
            if data is not None:
                self._VarWriteContinuousValuesNumpy(var, data)
            return
        elif varType == NexFileVarType.MARKER:
            self._VarWriteTimestamps(var, var['Timestamps'])
    ...
    def AddContVarWithSingleFragment(self, name, timestampOfFirstDataPoint, SamplingRate, values):
        ...
        var = {'Header': dict(zip(self.varHeaderKeys, vhValues))}
        self._AddNex5VarHeaderFields(var)
        var['Timestamps'] = np.array([timestampOfFirstDataPoint])
        ...
        if values.dtype == np.int16:
            ...
            saveContValuesAsFloats = 1
        var['Header']['ContDataType'] = saveContValuesAsFloats
        self._VarWriteData(var, values) # replaces var['ContinuousValues'] = values
        self.addVariable(var, values)
        
    def writeMetaDataAndClose(self):
        metaData = {"file": {}, 'variables': []}
        metaData["file"]["writerSoftware"] = {}
        metaData["file"]["writerSoftware"]["name"] = 'TDTbin2py.py'
        metaData["file"]["writerSoftware"]["version"] = 'Sep-19-2019'
        for v in self.fileData['Variables']:
...
'''
    m = data.streams[key]
    for j in range(m.data.shape[0]):
        name = '{0}_{1}'.format(m.name, j+1)
        print('adding', name)
        
        # convert floating point units to microvolt for compatibility
        if m.data.dtype in [np.int8, np.int16, np.int32]:
            sf = 1
        else:
            sf = 1e6
        
        timestampOfFirstDataPoint = 1/m.fs
        SamplingRate = m.fs
        values = m.data[j,:]*sf
        ...
        vhValues = [NexFileVarType.CONTINUOUS, 100, name, 0, 1, 0, 0, 0, 0, 0, 0, SamplingRate, 1.0,
                    len(values), 0, 0, 0, 0, '']
        var = {'Header': dict(zip(nexWriter.varHeaderKeys, vhValues))}
        nexWriter._AddNex5VarHeaderFields(var)
        
        var['Timestamps'] = np.array([timestampOfFirstDataPoint])
        var['FragmentIndexes'] = [0]
        var['FragmentCounts'] = [len(values)]
        #var['ContinuousValues'] = values
        nex_writer.addVariable(var)
'''
        ...
        if nchan == 1:
            ...
            for chan in range(nchan):
                ...
                if len(chan_index) != nchunks:
                    warnings.warn('channel {0} was shortened to {1} chunks'.format(chan, nchunks), Warning)
...
def parse_tbk(tbk_path):
    ...
    try:
        ...
        warnings.warn('Bad tbk file, try running the TankRestore tool to correct. See https://www.tdt.com/technotes/#0935.htm', Warning)
        return None
...
def read_sev(sev_dir, *, channel=0, event_name='', t1=0, t2=0, fs=0, ranges=None,
    ...
    optional keyword arguments:
        ...
        t2          scalar, retrieve data ending at t2 (default = 0 for end
                        of recording)
        channel     integer or list, choose a single channel or list of
    ...
    t1 = np.round((t1/2.56e-6)) * 2.56e-6
    t2 = np.round((t2/2.56e-6)) * 2.56e-6
    if isinstance(ranges, np.ndarray):
        ranges = np.round((ranges/2.56e-6)) * 2.56e-6
    ...
    if nfiles < 1:
        if just_names:
            return []
        #warnings.warn('no sev files found in {0}'.format(sev_dir), Warning)
        return None
    ...
    for this_event in event_names:
        ...
        for ch in channels:
            try:
                ...
                warnings.warn('channel {0} not found in {1} event'.format(ch, temp_event_name), Warning)
                continue
        search_ch = min_chan
        ...
        for jj in range(num_ranges):
            ...
            absolute_start_sample[jj] = np.max(np.ceil(valid_time_range[0,jj] * fs), 0) + 1
            if np.isinf(valid_time_range[1,jj]):
                absolute_end_sample[jj] = total_samples
            else:
                absolute_end_sample[jj] = np.minimum(np.max(np.floor(valid_time_range[1,jj] * fs), 0) + 1, total_samples)            
            curr_samples = 0
            for jjj in hour_values:
        ...
        if export is None:
            for jj in range(num_ranges):
                data[varname]['data'][jj] = np.zeros((len(channels), absolute_end_sample[jj] - absolute_start_sample[jj] + np.uint64(1)), dtype=data_format)
            ...
            if export in ['csv', 'interlaced','binary']:
                ...
                if export in ['binary']:
                    ...
                    for x in set(channels):
                        if prefix:
                            outname = os.path.join(outdir, '{0}{1}_{2}_{3}.{4}'.format(outfile, prefix, temp_event_name, x, ftype))
                        else:
                            outname = os.path.join(outdir, '{0}{1}_{2}.{3}'.format(outfile, temp_event_name, x, ftype))
                        file_handles[x] = open(outname, 'wb+')
                        outfile_paths[x] = outname
                elif export in ['csv', 'interlaced']:
                    if prefix:
                        outname = os.path.join(outdir, '{0}{1}_{2}.{3}'.format(outfile, prefix, temp_event_name, ftype))
                    else:
                        outname = os.path.join(outdir, '{0}{1}.{2}'.format(outfile, temp_event_name, ftype))
                    ...
                    for chan in channels:
                        ...
                        with open(file_list_temp[file_num]['fullname'], 'rb') as f:
                            ...
                            if temp_data is None:
                                temp_data = np.zeros((len(channels), read_size), dtype=data_format)
                            temp_data[arr_index, :] = ddd
                            chan_index[chan] += read_size
                            arr_index += 1
                    ...
                    for kk in range(start_hour_file[ii], end_hour_file[ii] + np.uint64(1)):
                        ...
                        with open(file_list_temp[file_num]['fullname'], 'rb') as f:
                            ...
                            if export in ['binary']:
                                ...
                                data[varname].data[ii][arr_index, int(chan_index):int(chan_index + read_size)] = ddd
                                arr_index += 1
...
def read_block(block_path, *, bitwise='', channel=0, combine=None, headers=0,
    ...
    optional keyword arguments:
        ...
        channel     integer or list, choose a single channel or list of channels
        ...
        export      string, choose a data exporting format.
                        csv:        data export to comma-separated value files
                                    streams: one file per store, one channel per column
                                    epocs: one column onsets, one column offsets
                        nex:        all data exported to NeuroExplorer format
                        nex5:       all data exported to NeuroExplorer5 format
                        binary:     streaming data is exported as raw binary files
                                    one file per channel per store
                        interlaced: streaming data exported as raw binary files
                                    one file per store, data is interlaced
                        edf:        streaming data exported to European Data Format
        scale       float, scale factor for exported streaming data. Default = 1.
        dtype       string, data type for exported binary data files
                        None: Uses the format the data was stored in (default)
                        'i16': Converts all data to 16-bit integer format
    ...
    if not use_outside_headers:
        ...
        if len(tsq_list) < 1:
            ...
            if 'streams' in evtype:
                warnings.warn('no tsq file found, attempting to read sev files', Warning)
    ...
    try:
        ...
        #warnings.warn('Synapse Notes file could not be processed', Warning)
        pass
        ...
        for note_line in notes_txt_lines:
            ...
            if len(note_line) >= eee + 2:
                if note_line.startswith(test_str):
                    ...
                    #note_index = int(note_line.split(':')[0].split('-')[-1])
        ...
        while True:
            ...
            for store_code in store_codes:
                ...
                if store_code['code'] == 0:
                    warnings.warn('Skipping unknown header code 0', Warning)
                    continue
                    ...
                    for temp in block_notes:
                        if temp.StoreName == store_code['name']:
                            if 'Enabled' in temp.keys():
                                if temp.Enabled == '2':
                                    warnings.warn('{0} store DISABLED'.format(temp.StoreName), Warning)
                                    skip_disabled = 1
                                    break
                ...
                if len(note_str) > 0 and store_code['type_str'] in ['scalars', 'epocs']:
                    ...
                    if np.any(note_index):
                        ...
                        # round timestamps to the nearest sample
                        note_ts = np.round((note_ts/2.56e-6)) * 2.56e-6
                ...
                if store_code['type_str'] != 'epocs':
                    ...
                    vvv = np.round((vvv/2.56e-6)) * 2.56e-6
    ...
    # round time ranges to the nearest sample
    valid_time_range = np.round((valid_time_range/2.56e-6)) * 2.56e-6
    ...
    for var_name in header.stores.keys():
        ...
        if 'ts' in header.stores[var_name].keys():
            if current_type_str == 'streams':
                ...
                data[current_type_str][var_name].filtered_ts = [[] for jj in range(num_ranges)]
            ...
            if hasattr(data[current_type_str][var_name], 'sortcode'):
                data[current_type_str][var_name].filtered_sort_code = [[] for jj in range(num_ranges)]
            if hasattr(data[current_type_str][var_name], 'data'):
                data[current_type_str][var_name].filtered_data = [[] for jj in range(num_ranges)]
                ...
                if hasattr(data[current_type_str][var_name], 'chan'):
                    if hasattr(data[current_type_str][var_name], 'filtered_chan'):
                        try:
                            ...
                            data[current_type_str][var_name].chan = data[current_type_str][var_name].filtered_chan[0]
    ...
    if export in ['nex5','nex']:
        useNEX5 = export == 'nex5'
        nex_writer = NexWriter(24414.0625, useNEX5=useNEX5)
        nex_writer.theFile = open(outfile, 'wb') # TODO: handle outdir, outfile, etc
        nex_writer.writeHeader(dummy=True)
        ...
        if current_type_str == 'epocs':
            if export in ['csv']:
                ...
                with open(outname, 'w') as f:
                    ...
                    f.write('\n')
            elif export in ['nex5', 'nex']:
                m = data[current_type_str][current_name]
                name = m.name

                # add Notes as markers
                if current_name == 'Note':
                    print('adding', name, 'as Marker')
                    nex_writer.AddMarker(name, m.onset, [name], [m.notes.tolist()])
                else:
                    # add other epocs as Markers and Intervals
                    print('adding', name, 'as Marker')
                    
                    # Markers only work with strings and integers? Convert values
                    nex_writer.AddMarker(name, m.onset, [name],
                                     [[int(x) for x in m.data.tolist()]])
                    
                    name = 'int_' + name
                    print('adding', name, 'as Interval')
                    
                    # if last offset is missing, make it the end of the block
                    if np.isinf(m.offset[-1]):
                        m.offset[-1] = data.info.duration.total_seconds()
                    
                    nex_writer.AddIntervalVariable(name, m.onset, m.offset)
        elif current_type_str == 'scalars':
            if len(data[current_type_str][current_name].chan) > 0:
                nchan = np.max(data[current_type_str][current_name].chan)
            else:
                nchan = 0
            if nchan > 1:
            ...
            if not nodata:
                ...
                if export in ['nex5', 'nex']:
                    ...
                    m = data[current_type_str][current_name]
                    for target_chan in np.unique(m.chan):
                        chan_ind = m.chan == target_chan
                        
                        # loop through all sort codes
                        for target_sort in np.unique(m.sortcode[chan_ind]):
                            sortcode_ind = m.sortcode == target_sort
                            
                            # get timestamps for this channel + sortcode only
                            bbb = chan_ind & sortcode_ind
                            timestamps = m.ts[bbb]
                            ...
                            if target_sort == 31:
                                ...
                                sort_letter = string.ascii_lowercase[target_sort-1]
                            
                            name = '{0}_{1}{2}_{3}'.format(key, target_chan,
                                                           sort_letter, m.sortname)
                            
                            print('adding', name)
                            wire = int(target_chan)
                            unit = int(target_sort)
                            
                            # add as neuron
                            writer.AddNeuron(name, timestamps, wire=wire, unit=unit)
                            
                            # add as wave
                            name = name + '_Wave'
                            
                            # typically the threshold is 1/4 of the way through the waveform
                            wavelen = m.data.shape[1]
                            prethresh_s = int(wavelen / 4) / m.fs
                            
                            nex_writer.AddWave(name, timestamps, m.fs, m.data[bbb.T[0]]*1e6,
                                           wavelen, prethresh_s, wire=wire, unit=unit)
                    '''
            ...
            if current_name in sev_names:
                ...
                if d is not None: # exporter returns None
                    ...
                    data[current_type_str][current_name].start_time = np.ceil(valid_time_range[0,0] * d[current_name].fs) / d[current_name].fs
            else:
                ...
                for jj in range(num_ranges):
                    ...
                    b_nex_internal = False
                    if export in ['nex5','nex']:
                        if size_in_bytes <= 1024**3: # hold in local memory before writing if store < 1GB
                            b_nex_internal = True
                            ddd = np.zeros([nchan, npts * np.uint64(len(these_offsets)) // nchan], dtype=current_data_format)
                    elif export in ['csv', 'interlaced', 'binary']:
                    ...
                    # try to optimally read data from disk in bigger chunks
                    if export in ['csv', 'interlaced', 'binary', 'nex5', 'nex']:
                    ...
                    if export in ['csv', 'interlaced','binary','nex5','nex']:
                        ...
                        if export in ['binary'] or (export in ['nex5','nex'] and not b_nex_internal): # create bin files as temp files for nex5
                            file_handles = {}
                            outfile_paths = {}
                            for x in set(fc):
                                if outfile and export not in ['nex5','nex']:
                                    # this is temp file for nex5, so keep it easy. delete when done
                                    outname = os.path.join(outdir, '{0}_{1}_{2}.bin'.format(outfile, current_name, x))
                                else:
                                    if prefix:
                                        outname = os.path.join(outdir, '{0}{1}_{2}_{3}.{4}'.format(outfile, prefix, current_name, x, ftype))
                                    else:
                                         outname = os.path.join(outdir, '{0}{1}_{2}.{3}'.format(outfile, current_name, x, ftype))
                                file_handles[x] = open(outname, 'wb+')
                                outfile_paths[x] = outname
                        else:
                            if prefix:
                                outname = os.path.join(outdir, '{0}{1}_{2}.{3}'.format(outfile, prefix, current_name, ftype))
                            else:
                                outname = os.path.join(outdir, '{0}{1}.{2}'.format(outfile, current_name, ftype))
                            if export not in ['nex5','nex']: # we handle this with NexWriter
                                file_handle = open(outname, 'w')
                            if export == 'csv':
                    ...
                    for f in range(len(arr)):
                        ...
                        for kk in range(len(relative_offsets)):
                            ...
                            if not np.any(ind[kk,:] <= len(tev_data)):
                                if not found_empty:
                                    warnings.warn('Data missing from tev file for store:{0} channel:{1} time:{2}s'.format(current_name, chan, np.round(t1 + chan_index[chan] / current_freq, 2)), Warning)
                                    found_empty = True
                                chan_index[arr_index] += npts
                                continue
                            found_empty = False
                            if export in ['csv', 'interlaced', 'binary'] or (export in ['nex5','nex'] and not b_nex_internal):
                            ...
                            elif export in ['nex5','nex'] and b_nex_internal:
                                ddd[chan, chan_index[chan]:(chan_index[chan] + npts)] = tev_data[ind[kk,:].flatten().astype(np.uint64)]
                            else:
                    ...
                    if export in ['csv', 'interlaced']:
                        ...
                        for j in range(ddd.shape[0]):
                            name = '{0}_{1}'.format(current_name, j+1)
                            print('adding', name)
                            # convert floating point units to microvolt for compatibility
                            if current_data_format in [np.int8, np.int16, np.int32]:
                                sf = 1
                            else:
                                sf = 1e6
                            nex_writer.AddContVarWithSingleFragment(name, 1/current_freq, current_freq, ddd[j,:]*sf)
                            
                    elif export in ['binary'] or (export in ['nex5','nex'] and not b_nex_internal):
                        for x in sorted(file_handles.keys()):
                            file_handles[x].seek(0,0)
                            v = np.frombuffer(file_handles[x].read(), dtype=current_data_format)
                            name = os.path.split(outfile_paths[x])[-1].split('.')[0]
                            
                            print(outfile_paths[x], name)
                            ...
                            nex_writer.AddContVarWithSingleFragment(name, 1/current_freq, current_freq, v*sf)
                            
                            file_handles[x].close()
                            os.remove(outfile_paths[x])
                            
                            #Tracer()()
        
                            #nex_writer.AddContVarWithSingleFragment(v)
                            #print(outfile_paths[x])
                            
                            # read file, then write it to nex_file, then delete file
                            #nex_writer._VarWriteData(v)
                            #if x in ['nex5']:
                            #outfile_paths[x]
                    else:
                        ...
                        # round timestamps to the nearest sample
                        tlo_sample = np.float64(np.uint64(valid_time_range[0,jj]/2.56e-6))
                        if np.isinf(valid_time_range[1,jj]):
                            thi_sample = MAX_UINT64
                        else:
                            thi_sample = np.float64(np.uint64(valid_time_range[1,jj]/2.56e-6))
                        td_sample = np.float64(np.uint64(data[current_type_str][current_name].start_time[jj]/2.56e-6))
                        
                        sf = 1/(2.56e-6 * current_freq)
                        minSample = np.uint64(np.maximum(np.round((tlo_sample-td_sample)/sf),0))
                        maxSample = np.maximum(np.round((thi_sample-td_sample)/sf),0)
                        maxSample = np.uint64(np.minimum(maxSample, data[current_type_str][current_name].data[jj].shape[1]))
                        
                        data[current_type_str][current_name].data[jj] = data[current_type_str][current_name].data[jj][:,minSample:maxSample]
                        data[current_type_str][current_name].start_time[jj] = data[current_type_str][current_name].start_time[jj] + minSample / current_freq
        ...
        for sev_name in sev_names:
            ...
            if sev_name not in [data.streams[fff].name for fff in data.streams.keys()]:
                ...
                if d is not None: # exporting returns None
                    ...
                    data.streams[sev_name].start_time = np.ceil(t1 * d[current_name].fs) / d[current_name].fs
    ...
    if export in ['nex5','nex']:
        nex_writer.writeHeader()
        nex_writer.writeMetaDataAndClose()
...
if __name__ == '__main__':
    ...
    #data = read_block(BLOCK_PATH, evtype=['epocs'], export='csv')
    data = read_block(BLOCK_PATH, evtype=['snips'], export='nex5', outfile='pytest.nex5')
    #data = read_block(BLOCK_PATH, export='interlaced')
    #data = read_block(BLOCK_PATH, export='binary')
    #data = read_block(BLOCK_PATH, export='nex5', outfile='pytest.nex5')
    ...
    """
    Set TDT master clock rate in Hz.  Only change this if using a faster clock (48828.125, 97656.25, or 195312.5)
    """
    timestampFrequency = 24414.0625

    """
    Set the output file name. This will put the output file in our block folder
    """
    outfile = BLOCK_PATH + '\\output.nex5'

    data = read_block(BLOCK_PATH, store=['eNe1','Pu1e','LFP1'])

    """
    ### Initialize the NEX writer
    """
    writer = nexfile.NexWriter(timestampFrequency, useNumpy=True)

    """
    Add snippets as Neurons and Waves
    """
    for key in data.snips.keys():
        ...
        for target_chan in np.unique(m.chan):
            ...
            for target_sort in np.unique(m.sortcode[chan_ind]):
                ...
                writer.AddWave(name, timestamps, m.fs, m.data[bbb.T[0]]*1e6,
                               wavelen, prethresh_s, wire=wire, unit=unit)

    """
    Write the NEX file
    """
    writer.WriteNex5File(outfile, saveContValuesAsFloats=1)

    """
    To read .nex or .nex5 files, use the following code:
    """
    reader = nexfile.Reader(useNumpy=True)
    fileData = reader.ReadNexFile(outfile)
    '''
    '''
        export      string, choose a data exporting format.
                        x csv: comma-separated value files, one per store, one channel per column
                        nex: all selected data is exported to NeuroExplorer format
                        nex5: all selected data is exported to NeuroExplorer5 format
                        x binary: streaming data is exported as raw binary files
                            one file per channel per store
                        x interlaced: streaming data is exported as raw binary files
                            one file per store, data is interlaced
                        edf: streaming data is exported to European Data Format
        outdir      string, output directory for exported files. Defaults to current
                        block folder if not specified
        outfile    string, output file name for exported files. Defaults to 'export' 
                        if not specified
