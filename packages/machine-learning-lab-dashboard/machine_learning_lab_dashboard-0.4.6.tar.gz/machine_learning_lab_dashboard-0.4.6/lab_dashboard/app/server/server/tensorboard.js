...
class Tensorboard {
    ...
    start() {
        return __awaiter(this, void 0, void 0, function* () {
            let path = PATH.join(consts_1.LAB.analytics, consts_1.LAB.tensorboardLogDir);
            yield util_1.rmtree(path);
            yield util_1.mkdir(path, { recursive: true });
            for (let r of this.runs) {
                try {
                    yield util_1.symlink(PATH.join(consts_1.LAB.experiments, r.experimentName, r.info.uuid, 'tensorboard'), PATH.join(path, `${r.experimentName}_${r.info.uuid}`));
                }
                catch (e) {
                    console.log(e);
                }
