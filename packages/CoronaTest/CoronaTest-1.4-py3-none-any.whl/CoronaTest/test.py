...
def get_prob(data):
    ...
    if fever_flag==1 and data[5]==1 and data[6]==0:
        sums+=15
    if fever_flag==1 and data[5]==1 and data[6]==1:
        sums -= 10
    if fever_flag==0 and data[7]==1:
        sums+=body_hot
    ...
    data[6+2]*Dry_Cough+     \
    data[7+2]*Fatigue+  \
    data[8+2]*Sputum_Production+    \
    data[9+2]*Shortness_Breath+ \
    data[10+2]*Shortness_Breath_Disease+    \
    data[11+2]*Pain+     \
    data[12+2]*Smoking+ \
    data[13+2]*Sore_throat+     \
    data[14+2]*Headache+    \
    data[15+2]*Chills+    \
    data[16+2]*Vomiting+    \
    data[17+2]*Nasal_Congestion+    \
    data[18+2]*Diarrhoea+     \
    data[19+2]*Past_Disease+     \
    data[20+2]*TRAVEL+    \
    data[21+2]*day
