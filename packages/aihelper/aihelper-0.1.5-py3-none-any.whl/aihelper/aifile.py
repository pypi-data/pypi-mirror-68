...
def topic_directories(path) -> (list, list):
    ...
    for t, td in zip(topics, directories):
        for r, d, f in os.walk(td):
            # Datafile present
            if [m for m in f if (os.path.splitext(m)[-1] =='.csv' or os.path.splitext(m)[-1] == '.txt')]:
                data = [m for m in f if (os.path.splitext(m)[-1] =='.csv' or os.path.splitext(m)[-1] == '.txt')]
    ...
    return directory_listing, method_listing
