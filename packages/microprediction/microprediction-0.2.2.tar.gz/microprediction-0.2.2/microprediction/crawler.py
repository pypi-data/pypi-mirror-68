...
class MicroCrawler(MicroWriter):
    """To create a crawler just derive from this class and overwrite:
    ...
    def update_seconds_until_next(self,exclude=None):
        ...
        self.seconds_until_next = 10*60 if upcoming is None else min(upcoming)
    ...
    def predict_and_submit(self, name, delay, lagged_times ):
        ...
        if len(lagged_values or []) < self.min_lags:
            ...
            if earliest>latest:
                earliest = latest
            next_predict_time = np.random.rand()*(latest-earliest)+earliest
            message.update({'earliest': earliest, 'latest': latest})
            message.update({'expected_at':expected_at})
            
            # Check for staleness 
            if next_predict_time>time.time():
                message.update({'t_next':next_predict_time})
                self.next_prediction_time[horizon] = next_predict_time
            else:
    ...
    def run(self):
        ...
        while True:
            ...
            if ((self.seconds_until_next>30) and (np.random.rand()<0.01)) or (len(self.active)==0):
            ...
            # Maybe we chill, or do something productive like offline estimation
            self.update_seconds_until_next()
