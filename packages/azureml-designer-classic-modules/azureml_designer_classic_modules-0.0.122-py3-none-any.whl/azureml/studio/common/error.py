...
class ErrorInfo:
    ...
    def to_dict(self):
        ...
        if dump_len < self.MAX_LENGTH:
            return result
        # delta is the length to be truncated, +30 is for placeholder in the truncated string
        delta = dump_len - self.MAX_LENGTH + 30
        result['Traceback'] = truncate_string(result['Traceback'], len(result['Traceback']) - delta)
        return result
...
class InvalidUriError(ModuleError):
    def __init__(self, invalid_url=None):
        super().__init__(**_valid_kwargs(locals()))
...
class InvalidTrainingDatasetError(ModuleError):
    def __init__(self, data_name=None, learner_type=None, reason=None, action_name=None):
        super().__init__(**_valid_kwargs(locals()))
...
class ErrorMapping:
    ...
    def get_exception_message(cls, ex: BaseException):
        if isinstance(ex, SystemExit):
            message = f"{ex.__class__.__name__}: {ex.__str__()}"
        else:
            message = ex.__str__()
