...
def plot_confusion_matrix(confusion_matrix, display_labels, plot_name, cmap='Blues', format='%.2g'):
    ...
    cm = pd.DataFrame(confusion_matrix, index=display_labels, columns=display_labels)
    # thanks for seaborn
    ax = sn.heatmap(cm,
                    annot=True,
    ...
    cbar = fig.colorbar(im_, ax=ax, fraction=0.012, pad=0.02, aspect=80, format=format)
