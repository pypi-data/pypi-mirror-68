...
from azureml.studio.common.error import ColumnNotFoundError, ErrorMapping, TooFewRowsInDatasetError
...
class LDATransform(BaseTransform):
    ...
    def apply(self, dataset: DataTable):
        ...
        if excluded_column_names:
            raise ColumnNotFoundError(column_names=list(excluded_column_names), arg_name_has_column='Transformation',
                                      arg_name_missing_column='Dataset')

        # Drop null value in terms of self._target_column_names for bug 742325.
        df = dataset.data_frame
        with TimeProfile("Remove missing target column instances."):
            df.dropna(subset=self._target_column_names, inplace=True)
            df.reset_index(drop=True, inplace=True)

        module_logger.info(f"Cleaned data has {df.shape[0]} Row(s) and {df.shape[1]} Columns.")
        if df.shape[0] < 1:
            target_column_names = ','.join(self._target_column_names)
        ...
        if len(self._target_column_names) > 1:
            ...
            vectorized_sentences = self.vectorizer.fit_transform(combined_target_columns['all_text'])
        else:
            vectorized_sentences = self.vectorizer.fit_transform(selected_data_df[self._target_column_names[0]])
