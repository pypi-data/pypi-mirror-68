...
from typing import Any, Callable, cast, Dict, List, Optional, Set, Tuple, Type, TypeVar
...
from .constants import DataclassHooks
...
__all__ = ["debug_print", "DPOptions", "debug_print_str", "debug_print0", "get_default_dpoptions"]
...
eop_log = replace(get_default_dpoptions(), max_initial_levels=6)
...
def debug_print_typelike(x: TypeLike, dp_compact, dpa, opt: DPOptions, prefix: str, args) -> str:
    ...
    if hasattr(x, "__name__"):
        ...
        if n == "frozendict":
            ...
            return color_ops("fdict")
        if n == "frozenset":
            # return 'frozen' + color_ops('set')
            return color_ops("fset")
...
def debug_print_set(
    ...
    if len(x) == 0:
        if opt.omit_type_if_empty:
            return color_ops(empty_set) + ps
        else:
            return dps(type(x)) + " " + color_ops(empty_set) + ps
    ...
    if nlines == 0 and tlen < get_max_col(stack):
        sep = color_ops(",")
        res = color_ops(popen) + sep.join(ss) + color_ops(pclose) + ps
        if opt.omit_type_if_short:
            return res
        else:
            return dps(type(x)) + " " + res
...
def debug_print_dataclass_instance(
    ...
    if False:
        r[termcolor.colored("$schema", "blue")] = dp_compact(type(x))
