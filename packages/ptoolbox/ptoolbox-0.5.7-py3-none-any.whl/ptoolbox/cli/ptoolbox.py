...
@click.option('-c', '--code', default='',
              prompt='Problem code, leave empty by default for auto generation from problem name',
              help='Problem code, use as problem folder name')
...
def beestar_get_all(username, password, output):
    """
    sample:
    ...
    beestar = Beestar()
    beestar.get_all_quizzes(username, password, output)
...
def beestar_quiz_to_ucode(credential, course, chapter, filename):
    ...
    beestar = Beestar()
    title, quiz = beestar.read_quiz_from_file(
        filename)
    ...
    if not title or not quiz:
        CLog.error(f"Cannot read quiz from file {filename}")
        return
    ...
    ucode = UCode(api_url, token)
    #
    lesson_id = ucode.create_lesson_item(course_id=course, chapter_id=chapter, lesson_name=title, type="quiz")
    CLog.info(f"Lesson #{lesson_id} created!")

    for question in quiz:
        ucode.create_problem(lesson_id=lesson_id, problem=question, lang='en', statement_format='html')
