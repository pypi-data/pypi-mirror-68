...
"""
from typing import cast, Dict, Optional, Any, Tuple, List, TYPE_CHECKING
import datetime
import json
import logging
import os
...
from ._telemetry_activity_logger import TelemetryActivityLogger
...
class AutoMLRun(Run):
    ...
    def __init__(self, experiment, run_id, **kwargs):
        ...
        try:
            ...
            logging_utilities.log_traceback(e, self._get_logger(None))
    ...
    def cancel(self):
        ...
        settings = AzureAutoMLSettings(
            experiment=self.experiment,
            **json.loads(properties['AMLSettingsJsonString']))
        logger = self._get_logger(settings)
    ...
    def continue_experiment(
        ...
        if (automl_settings.compute_target not in (None, constants.ComputeTargets.LOCAL)) or spark_context:
            try:
                ...
                logging_utilities.log_traceback(e, automl.logger)
                raise
    ...
    def get_output(self,
        ...
        logger = self._get_logger(automl_settings)
        with logging_utilities.log_activity(
            ...
            curr_run, child_runs_and_scores = self._get_run_internal(iteration, metric, logger)
            ...
            if return_onnx_model:
                ...
                fitted_model = self._download_automl_model(curr_run, logger)
    ...
    def _download_automl_model(self, curr_run: Run, logger: TelemetryActivityLogger) -> Optional[Any]:
        with logging_utilities.log_activity(
            logger,
    ...
    def register_model(self, model_name=None, description=None, tags=None, iteration=None, metric=None):
        ...
        with logging_utilities.log_activity(
            ...
            best_run, _ = self._get_run_internal(iteration, metric, logger)
    ...
    def _get_logger(self, settings: Optional[AzureAutoMLSettings]) -> TelemetryActivityLogger:
        """Get the telemetry activity logger."""
        return _logging.get_azureml_logger(
