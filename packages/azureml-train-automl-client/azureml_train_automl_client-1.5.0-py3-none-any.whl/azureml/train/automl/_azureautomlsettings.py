...
from ._logging import get_azureml_logger
...
class AzureAutoMLSettings(AutoMLBaseSettings):
    ...
    def __init__(self,
        ...
        self._local_managed_run_id = kwargs.get("_managed_run_id", None)
    ...
    def as_serializable_dict(self) -> Dict[str, Any]:
        return self._filter(['spark_context', 'logger', '_experiment'])

    def _verify_settings(self, logger: Optional[logging.LoggerAdapter] = None) -> None:
        """
        ...
        if self.compute_target is not None and not isinstance(self.compute_target, str) and \
            ...
            raise ConfigException('Input parameter \"compute_target\" needs to be an AzureML compute target. '
                                  ...
                                  .format(type(self.compute_target), self.compute_target)).with_generic_msg(
                'Input parameter compute_target needs to be an AzureML compute target.'
            )
        ...
        if isinstance(self.compute_target, DatabricksCompute) or \
            ...
            raise ConfigException.create_without_pii(
                'Databricks compute cannot be directly attached for AutoML runs. Please pass in a spark context '
                'instead using the spark_context parameter and set compute_target to "local".')
    ...
    def from_string_or_dict(val: Union[str, Dict[str, Any], AutoMLBaseSettings],
        ...
        if isinstance(val, AzureAutoMLSettings):
            ...
            raise ConfigException.create_without_pii("`input` parameter is not of type string or dict")
