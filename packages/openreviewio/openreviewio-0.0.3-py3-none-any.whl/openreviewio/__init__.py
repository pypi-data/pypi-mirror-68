...
from openreviewio.openreviewio.Content import Content
from openreviewio.openreviewio.Note import Note
from openreviewio.openreviewio.Status import Status
from openreviewio.openreviewio.MediaReview import MediaReview
...
def load_media_review(media_review_path: Union[str, Path]) -> MediaReview:
    ...
    for status in review_xml.find('statuses').iter('status'):
        ...
        statuses.append(Status(date=status_attributes.get('date'), author=status_attributes.get('author'),
                               value=status.text))
    ...
    for n in review_xml.find('notes').iter('note'):
        ...
        note = Note(author=note_attributes.get('author'), date=note_attributes.get('date'),
                    parent=note_attributes.get('parent', ''))

        # Contents
        contents = []
        for a in n.find('contents').iter():
            if a.attrib:
                content = getattr(Content, a.tag)
