...
def noticeForgetExcerciseViewer(begin_date=None, end_date=None):
    from ..exam.models import Paper
    from .models import Video
    from ..dailylog.models import Performance
    from ..message.helper import create_task
    from django.contrib.contenttypes.models import ContentType
    from django_szuprefix.utils.datautils import list_dict
    ...
    for vid, pids in vd.iteritems():
        vqset = Performance.objects.filter(
            owner_type=vct,
            owner_id=vid,
            percent__gte=30,
        ...
        if (len(vuids) > 0):
            ...
            if fuids:
                ...
                unique_id = "video.has_exer:%d@%s" % (vid, begin_date.isoformat().replace('-', '')[2:8])
                print unique_id
