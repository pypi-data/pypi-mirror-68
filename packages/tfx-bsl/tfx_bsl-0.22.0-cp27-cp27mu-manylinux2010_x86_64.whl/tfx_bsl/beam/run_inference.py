...
from tfx_bsl.types_compat import Any, Generator, Iterable, List, Mapping, Sequence, Text,\
    Tuple, Union
...
@beam.typehints.with_input_types(Union[tf.train.Example,
                                       tf.train.SequenceExample])
@beam.typehints.with_output_types(prediction_log_pb2.PredictionLog)
def _Classify(pcoll                         ,  # pylint: disable=invalid-name
              inference_spec_type                                  ):
...
@beam.typehints.with_input_types(Union[tf.train.Example,
                                       tf.train.SequenceExample])
@beam.typehints.with_output_types(prediction_log_pb2.PredictionLog)
def _Regress(pcoll                         ,  # pylint: disable=invalid-name
             inference_spec_type                                  ):
...
@beam.typehints.with_input_types(Union[tf.train.Example,
                                       tf.train.SequenceExample])
@beam.typehints.with_output_types(prediction_log_pb2.PredictionLog)
def _Predict(pcoll                         ,  # pylint: disable=invalid-name
             inference_spec_type                                  ):
...
@beam.typehints.with_input_types(Union[tf.train.Example,
                                       tf.train.SequenceExample])
@beam.typehints.with_output_types(prediction_log_pb2.PredictionLog)
def _MultiInference(pcoll                         ,  # pylint: disable=invalid-name
                    inference_spec_type                                  ):
...
class _BaseDoFn(beam.DoFn):
  ...
  class _MetricsCollector(object):
    ...
    def __init__(self, inference_spec_type                                  ):
      operation_type = _get_operation_type(inference_spec_type)
...
def _retry_on_unavailable_and_resource_error_filter(exception           ):
...
class _RemotePredictDoFn(_BaseDoFn):
  ...
  @staticmethod
  def _sending_as_binary(input_name      )        :
    """Whether data should be sent as binary."""
    return input_name.endswith('_bytes')

  @staticmethod
  def _parse_feature_content(values               , attr_name      ,
                             as_binary      )                 :
...
def _signature_pre_process(signature               )                 :
...
def _get_signatures(model_path      , signatures                ,
                    tags                )                        :
...
def _get_meta_graph_def(saved_model_pb             ,
                        tags                )                 :
...
class _Clock(object):
  ...
  def get_current_time_in_microseconds(self)       :
