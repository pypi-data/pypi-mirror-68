...
from tfx_bsl.types_compat import List, Optional, Text
...
class _RawRecordTFXIO(record_based_tfxio.RecordBasedTFXIO):
  ...
  def __init__(self, raw_record_column_name      ,
  ...
  def SupportAttachingRawRecords(self)        :
    return True
  ...
  def RawRecordToRecordBatchInternal(self,
    ...
    def _PTransformFn(raw_record_pcoll                         ):
...
def _BatchedRecordsToArrow(records             ,
                           raw_record_column_name      ,
                           should_produce_large_types      )                  :
...
class RawTfRecordTFXIO(_RawRecordTFXIO):
  ...
  def __init__(self, file_pattern      , raw_record_column_name      ,
  ...
  def RawRecordBeamSourceInternal(self)                   :
    @beam.typehints.with_input_types(beam.Pipeline)
