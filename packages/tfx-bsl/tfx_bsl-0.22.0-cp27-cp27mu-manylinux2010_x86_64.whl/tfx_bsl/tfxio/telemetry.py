...
from tfx_bsl.types_compat import Iterable, Callable, List, Optional, Text
...
def ProfileRecordBatches(
    ...
    logical_format      ,
    physical_format      ,
    distribution_update_probability        = 0.1)                   :
...
class _ProfileRecordBatchDoFn(beam.DoFn):
  ...
  def __init__(self, telemetry_descriptors                      ,
               logical_format      ,
               physical_format      , dist_update_prob       ):
  ...
  def _UpdateNumColumnsDist(self, record_batch                )        :
  ...
  def _UpdateNumValuesDist(self, record_batch                )        :
  ...
  def _UpdateNumCellsCounters(self, record_batch                )        :
    num_rows = record_batch.num_rows
    for column in record_batch:
