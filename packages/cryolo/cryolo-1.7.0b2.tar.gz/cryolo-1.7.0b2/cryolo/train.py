...
def main(args=None):
    ...
    print("")
    print("crYOLO will train in mode:", cryolo_mode)




    if cryolo_mode == "NON-SQUARE":
        width = sizes[0][0]
        height = sizes[0][1]
        ar = height/width
        if ar < 1:
    ...
    print("See http://xy.com for more details.")
    print("###############################################")

    grid_w, grid_h = config_tools.get_gridcell_dimensions(config)

    # parse annotations of the training set
    train_imgs, train_labels = preprocess.parse_annotation(
        train_annot_pairs,
    ...
    if os.path.exists(config["valid"]["valid_annot_folder"]):
        ...
        valid_imgs, valid_labels = preprocess.parse_annotation(
            valid_annot_pairs,
            grid_dims=(grid_w, grid_h, num_patches),
            anchor_size=int(config["model"]["anchors"][0]),
        )
