...
def add_tracking_event_dates(shipment: pd.Series, events_shipment_df: pd.DataFrame) -> pd.Series:
    ...
    shipment["pickup_date"] = pd.to_datetime(extract_pickup_event_date(events_df_filtered))
    shipment["departure_date"] = pd.to_datetime(extract_departure_event_date(events_df_filtered))
    shipment["arrival_date"] = pd.to_datetime(extract_arrival_event_date(events_df_filtered))
    shipment["delivery_date"] = pd.to_datetime(extract_delivery_event_date(events_df_filtered))
