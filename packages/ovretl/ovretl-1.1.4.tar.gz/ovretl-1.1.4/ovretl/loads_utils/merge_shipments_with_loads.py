...
def merge_with_propositions_loads_then_with_shipments_loads(
    ...
    shipments_df = pd.merge(
        ...
        propositions_loads_df[
            ["total_volume", "total_weight", "taxable_weight", "weight_measurable", "proposition_id"]
        ],
        how="left",
        on="proposition_id",
    )
    ...
    shipments_df_without_total_weight = shipments_df[shipments_df["total_weight"].isnull()].drop(
        ["total_weight", "total_volume", "taxable_weight", "weight_measurable"], axis=1
    )

    shipments_df_without_total_weight = pd.merge(
        shipments_df_without_total_weight,
        shipments_loads_df[["total_volume", "total_weight", "taxable_weight", "weight_measurable", "shipment_id"]],
        how="left",
        on="shipment_id",
    )

    return pd.concat([shipments_df_with_total_weight, shipments_df_without_total_weight], sort=False).reset_index(
        drop=True
    )
...
def merge_shipments_with_loads(shipments_df: pd.DataFrame, loads_df: pd.DataFrame):
    ...
    shipments_df = shipments_df.sort_values(by=["total_weight", "total_volume"], ascending=False).drop_duplicates(
        subset=["shipment_id"]
    )
    return shipments_df
