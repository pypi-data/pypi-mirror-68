...
def cq_missing_categories(computed_quotations_df: pd.DataFrame):
    category_complete = computed_quotations_df.groupby("category").apply(lambda group: group["kronos_selected"].any())
...
def add_missing_categories(computed_quotations_with_ocp_df: pd.DataFrame):
    cq_fullness = computed_quotations_with_ocp_df.groupby(by=["computed_quotation_id"]).apply(
        lambda cq: cq_missing_categories(cq)
    )
    ...
    return computed_quotations_with_ocp_df.astype({"proposition_id": int})
