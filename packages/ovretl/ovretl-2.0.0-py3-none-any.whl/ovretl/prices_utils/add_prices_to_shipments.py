...
def calculate_margins(shipment: pd.Series, df_prices: pd.DataFrame, df_prices_initial: pd.DataFrame) -> pd.Series:
    ...
    shipment["initial_margin_without_insurance"] = df_prices_initial[
        "margin_price_in_eur"
    ].sum() - select_category_price(df_prices_initial, key="margin_price_in_eur", category="insurance")
    shipment["initial_margin_insurance"] = select_category_price(
        df_prices_initial, key="margin_price_in_eur", category="insurance"
    )
    shipment["margin_without_insurance"] = df_prices["margin_price_in_eur"].sum() - select_category_price(
        df_prices, key="margin_price_in_eur", category="insurance"
    )
...
def add_prices_to_shipments(shipment: pd.Series, find_most_relevant_prices) -> pd.Series:
    ...
    df_prices_initial = find_most_relevant_prices(
