...
def check_shipment_active(shipment: pd.Series, shipment_events: pd.DataFrame):
    ...
    if not pd.isna(shipment["arrival_date"]) and (datetime.now() - shipment["arrival_date"]).days > -10:
        return True
    if not pd.isna(shipment["departure_date"]) and (datetime.now() - shipment["departure_date"]).days < 2:
        return True
    return False
