...
def merge_with_propositions_containers_then_with_shipments_containers(
    ...
    propositions_containers_df: pd.DataFrame,
    shipments_containers_df: pd.DataFrame,
) -> pd.DataFrame:
    shipments_df = pd.merge(
        shipments_df,
        propositions_containers_df[["teus", "tc", "proposition_id"]],
        how="left",
        on="proposition_id",
    )
    ...
    shipments_df_without_teus = pd.merge(
        ...
        shipments_containers_df[["teus", "tc", "shipment_id"]],
        how="left",
        on="shipment_id",
    )

    return pd.concat(
        [shipments_df_with_teus, shipments_df_without_teus], sort=False
    ).reset_index(drop=True)


def split_propositions_shipments_containers(containers_df: pd.DataFrame):
    propositions_containers_df = containers_df[
        ~containers_df["proposition_id"].isnull()
    ]
...
def merge_shipments_with_containers(
    shipments_df: pd.DataFrame, containers_df: pd.DataFrame
):
    (
        propositions_containers_df,
        shipments_containers_df,
    ) = split_propositions_shipments_containers(containers_df)
