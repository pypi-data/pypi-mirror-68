...
def test_extract_arrival_event():
    ...
    assert arrival_date == pd.Timestamp("2020-01-02 23:00:00")
...
def test_add_tracking_events():
    ...
    shipment = pd.Series(
        data=[0, False, "in_progress", np.nan], index=["shipment_id", "cancelled", "shipment_status", "finished_date"]
    )
    assert add_tracking_event_dates(shipment, events_shipment_df).equals(shipment)
