...
class DependentParameter(Descriptor):
    ...
    def __init__(self, index, factor, mode):
        """
...
class Category(platypus.Subset):
...
# TODO: Cleanup this mostly duplicated class by figuring out
class ParameterEH(IOBase):
    mode_options = {"Set", "Multiply", "Add"}
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert mode in self.mode_options, f"Invalid mode option"
        self.mode = mode
        self._add_reprs(["parameter_path", "value_descriptor"], True)

    def transformation_function(self, hub, value) -> None:
        """Mutates the building based on the value provided."""
        if self.mode is "Multiply":
            pf.set_by_path(
                hub.__dict__,
                self.parameter_path,
                (value * pf.get_by_path(hub.__dict__, self.parameter_path)),
            )
        if self.mode is "Add":
            pf.set_by_path(
                hub.__dict__,
