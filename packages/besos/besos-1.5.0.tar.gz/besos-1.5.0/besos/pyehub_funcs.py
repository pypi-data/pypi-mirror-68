...
def pyehub_parameter_editor(hub, parameters, values: list):
    ...
    if len(parameters) > 0:  # There are paremeters to edit
        for (parameter, value) in zip(parameters, values):
            ...
            set_by_path(hub.__dict__, parameter.parameter_path, value)
...
def ep_to_eh(input, index_size):
    ...
    # Need to have a better joining on index for the pandas dataframe instead of appending (difficulties to be noted with inserting dictionaries into cells)
    columnnames = []
    for j in input_dict:
        columnnames.append(j)

    # 2x for the number of seasons in the splitting will need to have some value stored from the smarter splitting
    # or make an easy spot to modify a value that propigates through the whole system.
    ...
    for objective in input_dict:
        for series in input_dict[objective]:
            ...
            # instead of appending I can edit the value of the specific cell with the input (ah that gets difficult with dictionaries)
