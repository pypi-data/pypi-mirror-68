...
from pyaware import async_swap, async_wrap, from_coroutine, events
...
class Execute:
    ...
    def do(self, cmd, *args, **kwargs):
        if from_coroutine():
            return self._do_async(cmd, *args, **kwargs)
        else:
            return self._do_sync(cmd, *args, **kwargs)

    def _do(self, cmd):
        kwargs = {k: cmd["parameters_read"][v] for k, v in self.from_read_params.items() if v in cmd["parameters_read"]}
        kwargs.update({k: cmd["data"][v] for k, v in self.from_command_params.items() if v in cmd["data"]})
        kwargs.update(self.kwargs)
        return kwargs

    def _do_sync(self, cmd, *args, **kwargs):
        ret_val = self.handle(**self._do(cmd))
        if self.return_id:
            cmd["parameters_read"][self.return_id] = ret_val

    async def _do_async(self, cmd, *args, **kwargs):
...
class Commands:
    ...
    def cmd_error(cmd_id, msg="", error_code: CmdResponse = CmdResponse.ERROR):
        resp = {"id": cmd_id, "type": error_code}
        if msg:
            resp["message"] = msg
        return resp
    ...
    def progress(cmd_id, msg=""):
        resp = {"id": cmd_id, "type": CmdResponse.PROGRESS}
        if msg:
            resp["message"] = msg
        return resp
    ...
    @staticmethod
    def ack(cmd_id):
        return {"id": cmd_id, "type": CmdResponse.ACK}

    @staticmethod
    def success(cmd_id, cmd):
        if cmd.get("return_values"):
            return {"id": cmd_id, "type": CmdResponse.SUCCESS, "data": cmd["return_values"]}
        else:
            return {"id": cmd_id, "type": CmdResponse.SUCCESS}
    ...
    def on_callback(self, data):
        events.publish("mqtt_command_response", data=data, timestamp=datetime.utcnow())

    @async_swap
    def execute(self, cmd, response_cbf: typing.Callable = print):
        pass

    def _execute_sync(self, cmd, response_cbf: typing.Callable = print):
        kwargs = {}
        cmd["parameters_read"] = {}
        ...
        try:
            ...
            if isinstance(target, list):
                self.execute_actions(cmd, response_cbf, target, **kwargs)
            else:
                target.execute(cmd, response_cbf=response_cbf, **kwargs)
        except InvalidDestination:
            response_cbf(self.cmd_error(cmd["id"], error_code=CmdResponse.INVALID_DESTINATION))
        except InvalidCommandData:
            response_cbf(self.cmd_error(cmd["id"], error_code=CmdResponse.INVALID_COMMAND_DATA))
        except InvalidCommandName:
            response_cbf(self.cmd_error(cmd["id"], error_code=CmdResponse.INVALID_COMMAND_NAME))
        except:
            exc_type, exc_value, exc_tb = sys.exc_info()
            tbe = traceback.TracebackException(
                exc_type, exc_value, exc_tb,
            )
            response_cbf(self.cmd_error(cmd["id"], tbe.format()))

    async def _execute_async(self, cmd, response_cbf: typing.Callable = print):
        kwargs = {}
        cmd["parameters_read"] = {}
        cmd["parameters_written"] = {}
        cmd["return_values"] = {}
        try:
            ...
            if isinstance(target, list):
                ...
                await async_wrap(target.execute(cmd, response_cbf=response_cbf, **kwargs))
            ...
            log.warning(f"Command failed to execute {self.device_id}: {cmd} {''.join(tbe.format())}")
            response_cbf(self.cmd_error(cmd["id"], ''.join(tbe.format())))

    @async_swap
    def execute_actions(self, cmd, response_cbf: typing.Callable, actions: list, **kwargs):
        pass

    def _execute_actions_sync(self, cmd, response_cbf: typing.Callable, actions: list, **kwargs):
        cmd_id = cmd["id"]
        response_cbf(self.ack(cmd_id))
        for action in actions:
            response_cbf(self.progress(cmd_id, repr(action)))
            action.do(cmd, **kwargs)
        response_cbf(self.success(cmd_id, cmd))

    async def _execute_actions_async(self, cmd, response_cbf: typing.Callable, actions: list, **kwargs):
        cmd_id = cmd["id"]
        response_cbf(self.ack(cmd_id))
        for action in actions:
        ...
        if self.key:
            try:
                ...
                if self.optional:
                    ...
                    raise InvalidCommandData(f"Key {self.key} not present in command payload {cmd['data']}")
        else:
            val = list(cmd["data"].values())
            if len(val) > 1:
                raise InvalidCommandData("Need to specify key if multiple data keys are given")
            val = val[0]
        ...
        try:
            if not (self.start <= val <= self.stop):
                if self.allow_zero and val == 0:
                    return
                raise InvalidCommandData(f"Data {val} is not in range {self.start} - {self.stop}")
        except TypeError:
...
@dataclass
class ValidateGroup:
    validators: typing.Collection[typing.Union[ValidateIn, ValidateRange, ValidateHandle, ValidateAnyHandle]]
    exclusive: bool = True
...
class WaitTopic:
    ...
    async def do(self, cmd: dict, *args, **kwargs):
        cmd[f"{self.topic}_response"] = await pyaware.events.wait(self.topic, self.timeout)
...
class TopicTask:
    ...
    async def do(self, cmd: dict, *args, **kwargs):
        new_kwargs = {k: cmd["data"][v] for k, v in self.key_map.items() if v in cmd["data"]}
