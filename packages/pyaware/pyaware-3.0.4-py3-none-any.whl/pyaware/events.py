...
def matches_case_sensitive(topic, a_filter):
    if "#" not in a_filter and "+" not in a_filter:
        ...
        # else use regex
        match_pattern = re.compile(a_filter
                                   .replace('#', '.*')
                                   .replace('$', '\$')
                                   .replace('+', '[/\$\s\w\d]+'))
...
class Topic:
    ...
    def publish(self, **data):
        ...
        if pyaware.from_coroutine():
            ...
            for handle in self.handles_async.values():
                futures.append(_loop.create_task(pyaware.async_call(handle, **data)))
            for handle in self.handles_executor.values():
                futures.append(_loop.run_in_executor(None, partial(handle, **data)))
        else:
            for handle in self.handles_async.values():
                if not _loop.is_running() or evt_stop.is_set():
                    reset()
                    raise RuntimeError("Event loop is stopped, Raising error in all connected threads")
    ...
    def subscribe(self, handle, in_thread=False):
        """
        ...
        """
        if in_thread:
            self.handles_executor[id(handle)] = handle
        else:
            self.handles_async[id(handle)] = handle
...
def publish(__topic: str, **data):
    ...
    for topic in _topics:
        if matches(__topic, topic):
            futures.extend(_topics[topic].publish(**data))
    return TopicTasks(futures)


def _form_subscription_topic(topic):
    topic = topic.casefold()  # Topic shouldn't be case sensitive
...
def subscribe(*handles, topic: str, in_thread=False):
    """
    ...
    for handle in handles:
        ...
        _topics[_form_subscription_topic(topic)].subscribe(handle, in_thread)
    else:
        ...
        def _tmp(handle):
            ...
            for param in inspect.signature(handle).parameters:
                break
            if param == "self":
                # It is a bound method not a pure function
                # We want to create a subscription for each instance instead of once off on class definition
                # By adding some function parameters, the @enable can read these to subscribe at object creation
                handle.topic = topic
                handle.in_thread = in_thread
                return handle
            else:
                # It is a function not a class method
                # Side effect subscription
                _topics[topic].subscribe(handle, in_thread)
    ...
    def __new__(cls, *args, **kwargs):
        ...
        for attr in dir(inst):
            try:
                ...
                _topics[handle.topic.format(id=id(inst))].subscribe(handle, handle.in_thread)
                log.info(f"Subscribed to {handle.topic}:{id(handle)}")
            except AttributeError:
                # This attribute hasn't been marked by @subscribe
                pass
...
# _loop is global so that it uses the module's characteristic as a singleton instead of using a single shot class.
# Use reset to set back to
_loop = None
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        async def producer():
            while True:
                await asyncio.sleep(1)
                publish("test/2/3/4", data="Hello")

        async def consumer():
            await asyncio.sleep(2)
            while True:
                try:
                    resp = await wait("test", 0.8)
                    print(f"{resp['data']}")
                except asyncio.TimeoutError:
                    print("Timed out consumer")

        def blocking_consumer():
            while True:
                try:
                    print(f"blocking{wait('test', 5)}")
                except asyncio.TimeoutError:
                    print("Timeout out blocking consumer")

        @subscribe(topic="test/+/3/4", in_thread=True)
        def blocking_sub(data):
            print(f"Starting long task 'test/+/3/4': {threading.get_ident()}")
            time.sleep(2)
            print(f"Done 'test/+/3/4': {threading.get_ident()}: {data}")

        @subscribe(topic="test/2/+/4", in_thread=False)
        async def blocking_io_sub(data):
            print(f"Starting long io task 'test/2/+/4': {threading.get_ident()}")
            await asyncio.sleep(2)
            print(f"Done 'test/2/+/4': {threading.get_ident()}: {data}")

        @subscribe(topic="#")
        async def all(data):
            print(f"From all {data}")
        ...
        asyncio.create_task(consumer())
        asyncio.get_event_loop().run_in_executor(None, blocking_consumer)
        prod.cancel()
        await publish("test/2/3/4", data="Topic test/2/3/4").all()
        await publish("test/2/5/4", data="Topic test/2/5/4").all()
        await publish("test/5/3/4", data="Topic test/5/3/4").all()
        await publish("test/7/7/7", data="Topic test/7/7/7").all()
