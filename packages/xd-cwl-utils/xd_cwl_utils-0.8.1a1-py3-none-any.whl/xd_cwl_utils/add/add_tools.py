...
from xd_cwl_utils.classes.cwl.make_cwl import _initialize_command_line_tool_file_yaml
...
def add_tool(tool_name, version_name, subtool_names=None, biotools_id=None, has_primary=False, root_repo_path=Path.cwd(), init_cwl=False):
    ...
    if parent_metadata.featureList:
        for subtool in parent_metadata.featureList:
            ...
            git_keep_file.touch()
            if init_cwl:
                _initialize_command_line_tool_file_yaml(tool_name, version_name, subtool, base_dir=root_repo_path)
...
def add_subtool(tool_name, tool_version, subtool_name, root_repo_path=Path.cwd(), update_featureList=False, init_cwl=False):
    ...
    git_keep_file.touch()
    if init_cwl:
        _initialize_command_line_tool_file_yaml(tool_name, tool_version, subtool_name, base_dir=root_repo_path)
    return
