...
class DataPreparer(ABC):
    ...
    def __init__(self, dataprep_json_obj: Dict[str, Any], logger: logging.Logger):
        """
        ...
        self._logger = logger
        with logging_utilities.log_activity(logger=self._logger, activity_name='ParsingDataprepJSON'):
            self._parse(dataprep_json_obj)

        with logging_utilities.log_activity(logger=self._logger, activity_name="BuildingDataCharacteristics"):
    ...
    def prepare(self, automl_settings_obj: AzureAutoMLSettings) -> Dict[str, Any]:
        ...
        try:
            ...
            logging_utilities.log_traceback(e, self._logger)
            ...
            if isinstance(e, DataflowValidationError):
                ...
                self._logger.error(generic_msg)
    ...
    def _buiid_data_characterstics(self):
        ...
        if self._original_training_data is not None:
            try:
                self._logger.info('Starting data characteristics calculation. This might take a while...')
                ...
                self._logger.info('data characteristics calculation failed')
...
class DataPreparerFromDataSet(DataPreparer):
    ...
    def __init__(self, dataprep_json_obj: Dict[str, Any], logger: logging.Logger):
        super().__init__(dataprep_json_obj, logger)

    def _parse(self, dataprep_json_obj: Dict[str, Any]) -> None:
        self._logger.info('Creating dataflow from datasets for training_data and validation_data.')
...
class DataPreparerFromSerializedDataflows(DataPreparer):
    ...
    def _parse(self, dataprep_json_obj: Dict[str, Any]) -> None:
        self._logger.info('Deserializing dataflow.')
...
class DataPreparerFromDatasetOptions(DataPreparer):
    ...
    def _parse(self, dataprep_json_obj: Dict[str, Any]) -> None:
        self._logger.info('Creating dataflow from dataset.')
...
class DataPreparerFromDataprepOptions(DataPreparer):
    ...
    def _parse(self, dataprep_json_obj: Dict[str, Any]) -> None:
        self._logger.info('Creating dataflow from options.')
...
class DataPreparerFactory:
    ...
    @staticmethod
    def get_preparer(dataprep_json: str, logger: logging.Logger) -> DataPreparer:
        """
        ...
        try:
            ...
            if 'activities' in dataprep_json_obj:
                preparer = DataPreparerFromSerializedDataflows(dataprep_json_obj, logger)  # type: DataPreparer
            elif 'datasetId' in dataprep_json_obj:
                preparer = DataPreparerFromDatasetOptions(dataprep_json_obj, logger)
            elif 'datasets' in dataprep_json_obj:
                preparer = DataPreparerFromDataSet(dataprep_json_obj, logger)
            else:
                preparer = DataPreparerFromDataprepOptions(dataprep_json_obj, logger)
...
class DataPreparerUtils:
    ...
    def _get_dict_from_dataflows(training_dflow: Any,
        ...
        training_data = DataPreparerUtils._get_inferred_types_dataflow(training_dflow, logger)
        validation_data = None
        if validation_dflow is not None:
            validation_data = DataPreparerUtils._get_inferred_types_dataflow(validation_dflow, logger)
    ...
    def _get_dict_from_dataflow(dflow: Any,
        ...
        if not automl_settings_obj.enable_streaming:
            ...
            X = DataPreparerUtils._get_inferred_types_dataflow(X, logger)
            ...
            training_data = DataPreparerUtils._get_inferred_types_dataflow(training_data, logger)
            validation_data, training_data = DataPreparerUtils._split_data_train_valid(
                logger, training_data, train_data_row_count, automl_settings_obj.validation_size)
