...
from azureml.automl.core.shared.exceptions import ArgumentException, ErrorTypes
...
class _AdbAutomlExperiment:
    ...
    def __init__(self,
        ...
        self.logger = _logging.get_azureml_logger(automl_settings=self.automl_settings,
                                                  parent_run_id=self.parent_run_id,
                                                  child_run_id=None)
        # the logger is actually a loggeradapter, so we have to get the underlying logger
        self.logger.get_logger().addHandler(logging.StreamHandler(sys.stdout))
        ...
        try:
            ...
            log_utils.log_traceback(e, self.logger, is_critical=False)
            ...
            try:
                ...
                if run_id.endswith("setup"):
                    ...
                    input_data = get_input_datamodel_from_dataprep_json(
                        ...
                        log_message=self.log_message, logger=self.logger, verifier=verifier)
                    ...
                    if (self.automl_settings.enable_cache is True or
                            not skip_featurization(self.automl_settings.featurization,
                ...
                if run_id.endswith("setup"):
                    try:
                        ...
                        log_utils.log_traceback(ae, self.logger, is_critical=False)
    ...
    def _recover_dataset(self, current_run, verifier, onnx_cvt=None):
        ...
        if self.dataset is None:
            ...
            input_data = get_input_datamodel_from_dataprep_json(
                dataprep_json=self.dataprep_json, automl_settings=self.automl_settings,
                log_message=self.log_message, logger=self.logger)
    ...
    def _load_data_from_cache(self, cache_store: CacheStore) -> Optional[ClientDatasets]:
        with self.logger.log_activity(self.logger, activity_name='_load_data_from_cache'):
            try:
                ...
                log_message(logger=self.logger, logging_level=logging.ERROR, parent_run_id=self.parent_run_id,
                            worker_id=self.worker_id,
                            message="Continuing without cache as trying to load from cache failed with exception")
                return None
