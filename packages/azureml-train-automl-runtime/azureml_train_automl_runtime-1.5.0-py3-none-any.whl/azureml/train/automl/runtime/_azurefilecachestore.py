...
from azureml.automl.runtime.shared.cache_store import CacheStore, _create_temp_dir, _SavedAsProtocol, \
    _CacheConstants
from azureml.automl.core.shared.exceptions import CacheException, MemorylimitException
...
class AzureFileCacheStore(CacheStore):
    ...
    def add(self, keys, values):
        ...
        :param values: values
        """
        self.module_logger.info('Adding data to Azure file store cache')
    ...
    def unload(self):
        ...
        try:
            ...
            logging_utilities.log_traceback(e, self.module_logger, is_critical=False)
            self.module_logger.warning('Failed to delete share.')
    ...
    def _get_azure_file_lists(self, path):
        ...
        :param path: path
        """
        self.module_logger.debug('Getting list of files from Azure file store')
    ...
    def _try_remove_temp_file(self, path):
        self.module_logger.debug('Removing temporary file')
        try:
            os.remove(path)
        except OSError as e:
            logging_utilities.log_traceback(e, self.module_logger, is_critical=False)
            self.module_logger.warning("Failed to remove temporary file.")
    ...
    def _persist_saved_as_file(self) -> None:
        ...
        self.module_logger.info("Persisted saved_as file")

    def _serialize_numpy_ndarray(self, file_name: str, obj: np.ndarray) -> Any:
        assert isinstance(obj, np.ndarray)
        self.module_logger.debug('Numpy saving and uploading file to cache')
    ...
    def _serialize_scipy_sparse_matrix(self, file_name: str, obj: Any) -> Any:
        ...
        self.module_logger.debug('Scipy saving and uploading file to cache')
        path = os.path.join(self.temp_location, file_name)
        sparse.save_npz(path, obj)
        return path

    def _serialize_object_as_pickle(self, file_name: str, obj: Any) -> Any:
        self.module_logger.debug('Pickling and uploading file to cache')
        path = os.path.join(self.temp_location, file_name)
    ...
    def _download_and_deserialize(self, file_name: str) -> Dict[str, Any]:
        ...
        if saved_as == _SavedAsProtocol.SCIPY:
            self.module_logger.debug('Downloading and de-serializing file via. Scipy from cache')
            ...
            self.module_logger.debug('Downloading and de-serializing file via. Numpy from cache')
            deserialized_obj = self._deserialize_object(
                '.'.join([file_name, _CacheConstants.NUMPY_FILE_EXTENSION]), np.load)
        else:
            self.module_logger.debug('Downloading and unpickling file from cache')
