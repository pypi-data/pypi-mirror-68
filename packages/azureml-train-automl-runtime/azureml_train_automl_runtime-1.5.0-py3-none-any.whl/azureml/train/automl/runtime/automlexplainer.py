...
def retrieve_model_explanation(child_run):
    ...
    try:
        ...
        raise OptionalDependencyMissingException.from_exception(e).with_generic_msg('Import of dependency failed')
...
def explain_model(fitted_model, X_train, X_test, best_run=None, features=None, y_train=None, **kwargs):
    ...
    :rtype: (Union[list, list[list]], list, list, list, list, list)
    """
    logger = _logging.get_azureml_logger(automl_settings=None, parent_run_id=None, child_run_id=None)
    ...
    try:
        ...
        raise OptionalDependencyMissingException.from_exception(import_error).with_generic_msg(message)
