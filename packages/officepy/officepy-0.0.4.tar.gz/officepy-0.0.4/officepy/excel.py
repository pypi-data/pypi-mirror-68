...
from . import runtime as OfficeExtension
...
class Worksheet(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, address):
        if isinstance(address, tuple):
            (row, col) = address
            r = self.getCell(row, col)
        else:
            r = self.getRange(address)
        ...
        if isinstance(value, list):
            if len(value) > 0:
                if isinstance(value[0], list):
                    r = r.getCell(0,0).getResizedRange(len(value) -1, len(value[0]) -1)
                    r.values = value
                else:
                    r = r.getCell(0,0).getResizedRange(0, len(value) -1)
                    r.values = [value]
        else:
            r.values = value
    
    def getAllValues(self):
        with self.context.batchScope():
            r = self.getUsedRange(True)
            self.context.load(r, "values")
            self.context.sync()
            return r.values

    def addTableFromDataFrame(self, df: pandas.DataFrame, name=None, position=None):
        """
        Static method. Creates a new table on the given worksheet (current active by default),
        at given position (top left corner by default), and with the same dimensions as the
        shape of given DataFrame.
        """
        if position is None:
            position = (0, 0)

        table_range = self.getCell(position[0], position[1]).getResizedRange(
            df.values.shape[0], df.values.shape[1] - 1)

        number_format = table_range.numberFormat
        for r in range(df.values.shape[0]):
            for c in range(df.values.shape[1]):
                if isinstance(df.values[r][c], pandas.Timestamp):
                    number_format[r + 1][c] = 'm/d/yyyy'

        table_range.numberFormat = number_format

        table = self.tables.add(table_range, True)
        table.updateFromDataFrame(df)
        if name is not None:
            table.name = name
    ...
    def shapes(self) -> 'ShapeCollection':
        if self._shapes is None:
            self._shapes = ShapeCollection(self.context, _createPropertyObjectPath(self.context, self, "Shapes", True, False))
        return self._shapes
    ...
    def _handleResult(self, value: dict) -> None:
        ...
        if "PivotTables" in obj:
            self.pivotTables._handleResult(obj.get("PivotTables"))
        if "Shapes" in obj:
            self.shapes._handleResult(obj.get("Shapes"))
...
class Shape(OfficeExtension.ClientObject):
    # Begin_PlaceHolder_Shape_Custom_Members
    # End_PlaceHolder_Shape_Custom_Members
    def __init__(self, context: OfficeExtension.ClientRequestContext, objectPath: OfficeExtension.ObjectPath):
        super(self.__class__, self).__init__(context, objectPath)
        self._id = None
    ...
    def id(self) -> 'str':
        _loadIfInstantSyncExecutionMode(self, "id", self._id)
        _throwIfNotLoaded("id", self._id, "Shape", self._isNull)
        return self._id
    
    def delete(self) -> None:
        # Begin_PlaceHolder_Shape_Delete
        # End_PlaceHolder_Shape_Delete
        _createMethodAction(self.context, self, "Delete", OfficeExtension.OperationType.Default, [])
        _syncIfInstantSyncExecutionMode(self)
    ...
    def load(self, option = None) -> 'Shape':
        _load(self, option)
        return self
    
    def reload(self) -> 'Shape':
        _load(self, None)
        return self
...
class ShapeCollection(OfficeExtension.ClientObject):
    # Begin_PlaceHolder_ShapeCollection_Custom_Members
    ...
    # End_PlaceHolder_ShapeCollection_Custom_Members
    def __init__(self, context: OfficeExtension.ClientRequestContext, objectPath: OfficeExtension.ObjectPath):
        super(self.__class__, self).__init__(context, objectPath)
        self.__items = None
    
    @property
    def items(self) -> 'list of Shape':
        _loadIfInstantSyncExecutionMode(self, "items", self.__items)
        _throwIfNotLoaded("items", self.__items, "ShapeCollection", self._isNull)
        return self.__items
    
    def addImage(self, base64ImageString : 'str') -> 'Shape':
        # Begin_PlaceHolder_ShapeCollection_AddImage
        # End_PlaceHolder_ShapeCollection_AddImage
        ret = Shape(self.context, _createMethodObjectPath(self.context, self, "AddImage", OfficeExtension.OperationType.Default, [base64ImageString], False, True))
        _syncIfInstantSyncExecutionMode(self)
        return ret

    def addSvg(self, xml : 'str') -> 'Shape':
        # Begin_PlaceHolder_ShapeCollection_AddSvg
        # End_PlaceHolder_ShapeCollection_AddSvg
        ret = Shape(self.context, _createMethodObjectPath(self.context, self, "AddSvg", OfficeExtension.OperationType.Default, [xml], False, True))
        _syncIfInstantSyncExecutionMode(self)
        return ret
    ...
    def _handleResult(self, value: dict) -> None:
        ...
        if OfficeExtension.Constants.items in obj:
            ...
            data = obj.get(OfficeExtension.Constants.items)
            for i, itemData in enumerate(data):
                item = Shape(self.context, _createChildItemObjectPathUsingIndexerOrGetItemAt(True, self.context, self, itemData, i))
                item._handleResult(itemData)
                self.__items.append(item)

    def load(self, option = None) -> 'ShapeCollection':
        _load(self, option)
        return self
    
    def reload(self) -> 'ShapeCollection':
        _load(self, None)
        return self

class CustomFunctionManager(OfficeExtension.ClientObject):
    # Begin_PlaceHolder_CustomFunctionManager_Custom_Members
    # End_PlaceHolder_CustomFunctionManager_Custom_Members
    def __init__(self, context: OfficeExtension.ClientRequestContext, objectPath: OfficeExtension.ObjectPath):
        super(self.__class__, self).__init__(context, objectPath)
    
    def load(self, option = None) -> 'CustomFunctionManager':
        _load(self, option)
        return self
    
    def reload(self) -> 'CustomFunctionManager':
        _load(self, None)
        return self

    def register(self, metadata:str, javascript:str) -> None:
        # Begin_PlaceHolder_CustomFunctionManager_Register
        # End_PlaceHolder_CustomFunctionManager_Register
        _createMethodAction(self.context, self, "Register", OfficeExtension.OperationType.Default, [metadata, javascript])
        _syncIfInstantSyncExecutionMode(self)

    @staticmethod
    def newObject(context: OfficeExtension.ClientRequestContext) -> 'CustomFunctionManager':
        ret = CustomFunctionManager(context, _createNewObjectObjectPath(context, "Microsoft.ExcelServices.CustomFunctionManager", False))
        return ret
...
class Range(OfficeExtension.ClientObject):
    ...
    def toDataFrame(self) -> pandas.DataFrame:
        with self.context.batchScope():
            self.context.load(self, "values, numberFormat, rowCount, columnCount")
            self.context.sync()
            df = pandas.DataFrame(self.values)

            # TODO: accelerate this piece using list comprehension / function map
            #       and dots elimination.
            number_formats = self.numberFormat
            time_origin = pandas.Timestamp('1899-12-30')
            for r in range(self.rowCount):
                for c in range(self.columnCount):
                    if any(sym in number_formats[r][c] for sym in 'sShHdDmMyY') and not isinstance(df[c][r], str):
                        df.loc[r, c] = time_origin + pandas.Timedelta(days=df[c][r])
        return df

    def updateFromDataFrame(self, df: pandas.DataFrame, include_header='auto', fit_to_df_size=True):
        """
        Assigns the values in the Range to values in given DataFrame.

        Args:
          df:              a pandas.DataFrame object, from which the values will be assigned
          include_header:  either True, False, or 'auto' - whether the header names of the DataFrame
                           should be included.
          fit_to_df_size:  If False, the DataFrame will be cropped to the Range's size if it is larger,
                           or filled with empty cells if its size is smaller than the Range's size.
                           If True, will call `updateFromDataFrame` on the Range with same size.
        """
        with self.context.batchScope():
            self.context.load(self, 'values, columnCount, rowCount')
            self.context.sync()

            coldiff, rowdiff = self.columnCount - df.shape[1], self.rowCount - df.shape[0]
            if include_header:
                rowdiff -= 1

            if fit_to_df_size and (not coldiff == 0 or not rowdiff == 0):
                if rowdiff < self.rowCount and coldiff < self.columnCount:
                    return self.getResizedRange(-rowdiff, -coldiff) \
                               .updateFromDataFrame(df, include_header, fit_to_df_size)

            if include_header == 'auto':
                include_header = not isinstance(df.columns, pandas.RangeIndex)
            df_list = df.values.tolist()

            time_origin, one_day = pandas.Timestamp('1899-12-30'), pandas.to_timedelta(1, unit='D')
            df_list = [
                [(t - time_origin) / one_day if isinstance(t, pandas.Timestamp) else t
                 for t in list_row] for list_row in df_list]

            if not fit_to_df_size:
                if coldiff > 0:
                    for list_row in df_list:
                        list_row += [''] * coldiff

                if rowdiff > 0:
                    df_list += [[''] * self.columnCount for _ in range(rowdiff)]

            if not include_header:
                self.values = df_list
            else:
                header = df.columns.tolist()
                self.values = [header] + df_list
            self.context.sync()
...
class RangeViewCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'RangeView':
        item = self.getItem(key)
        return item
...
class NamedItemCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'NamedItem':
        item = self.getItem(key)
        return item
...
class Table(OfficeExtension.ClientObject):
    ...
    def toDataFrame(self) -> pandas.DataFrame:
        with self.context.batchScope():
            body_range = self.getDataBodyRange()
            header_range = self.getHeaderRowRange()
            self.context.load(header_range, 'columnCount, values')
            self.context.load(body_range, 'columnCount')

            # The syncing is not neccessary, as we already have batch execution with sync
            # inside Range.toDataFrame. However, this is more fool-proof.
            self.context.sync()

            df = body_range.toDataFrame()
            if header_range is not None and header_range.columnCount == body_range.columnCount:
                df.columns = header_range.values[0]
        return df

    def updateFromDataFrame(self, df: pandas.DataFrame, fit_to_df_size=True):
        table_range = self.getRange()
        table_range.updateFromDataFrame(df, include_header=True, fit_to_df_size=fit_to_df_size)
...
class TableColumnCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'TableColumn':
        item = self.getItem(key)
        return item
...
class TableRowCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'TableRow':
        item = self.getItem(key)
        return item
...
class RangeBorderCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'RangeBorder':
        item = self.getItem(key)
        return item
...
class ChartCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'Chart':
        item = self.getItem(key)
        return item
...
class ChartSeriesCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key: 'int') -> 'ChartSeries':
        item = self.getItemAt(key)
        return item
...
class ChartPointsCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key: 'int') -> 'ChartPoints':
        item = self.getItemAt(key)
        return item
...
class CustomXmlPartScopedCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'CustomXmlPart':
        item = self.getItem(key)
        return item
...
class ConditionalFormatCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key: 'int') -> 'ConditionalFormat':
        item = self.getItemAt(key)
        return item
...
class ConditionalRangeBorderCollection(OfficeExtension.ClientObject):
    ...
    def __getitem__(self, key) -> 'ConditionalRangeBorder':
        item = self.getItem(key)
        return item
...
class RequestContext(OfficeExtension.ClientRequestContext):
    def __init__(self, url: str = None, executionMode: OfficeExtension.RequestExecutionMode = OfficeExtension.RequestExecutionMode.instantSync):
        super(self.__class__, self).__init__(url, executionMode)
        self._workbookSessionId = None
    ...
    def close(self) -> None:
        ...
        if self._workbookSessionId is not None:
            request = httphelper.RequestInfo()
            request.method = "POST"
            request.url = OfficeExtension.Utility.combineUrl(self.url, "closeSession")
            for key, value in self.requestHeaders.items():
...
def getWorkbook(url: str = None) -> Workbook:
    ret = RequestContext(url)
    if (url is not None):
        accessToken = _getAccessToken()
        sessionId = _createSession(url, accessToken)
        ret.requestHeaders['Authorization'] = 'Bearer ' + accessToken
        ret.requestHeaders['workbook-session-id'] = sessionId
        ret._workbookSessionId = sessionId
    return ret.workbook

def _getAccessToken() -> str:
    home = os.path.expanduser("~")
    officepyDirectory = os.path.join(home, ".officepy")
    if not os.path.exists(officepyDirectory):
        os.makedirs(officepyDirectory)
    accessTokenFile = os.path.join(officepyDirectory, "access_token.json")
    accessTokenFileContent = ""
    if (os.path.isfile(accessTokenFile)):
        with open(accessTokenFile, 'r') as file:
            accessTokenFileContent = file.read().replace('\n', '')
    if len(accessTokenFileContent) > 0:
        accessTokenInfoObj = json.loads(accessTokenFileContent)
        if accessTokenInfoObj['expires'] > time.time():
            return accessTokenInfoObj['access_token']

    oauthToken = auth.getDeviceFlowOAuthToken("https://login.microsoftonline.com/common/oauth2/v2.0", "7833c94f-f400-4d80-8de4-fa89a0d21af0", "https://graph.microsoft.com/Files.ReadWrite")
    accessTokenInfoObj = {
        'access_token': oauthToken.access_token,
        'expires': time.time() + oauthToken.expires_in * 0.8
        }
    
    with open(accessTokenFile, 'w') as file:
        accessTokenFileContent = json.dumps(accessTokenInfoObj, default = lambda o: o.__dict__)
        file.write(accessTokenFileContent)
    return oauthToken.access_token

def _createSession(url: str, accessToken: str) -> str:
    request = httphelper.RequestInfo()
    request.method = "POST"
    request.headers['Authorization'] = 'Bearer ' + accessToken
    request.url = OfficeExtension.Utility.combineUrl(url, "createSession")
    response = httphelper.HttpUtility.invoke(request)
    if response.statusCode == 200 or response.statusCode == 201:
        responseObj = json.loads(response.body)
        return responseObj['id']
    raise OfficeExtension.Utility.createRuntimeError(OfficeExtension.ErrorCodes.generalException)
