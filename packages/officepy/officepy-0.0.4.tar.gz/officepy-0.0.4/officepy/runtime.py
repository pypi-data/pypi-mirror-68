...
class Constants:
    ...
    jupyterInputPromptPrefixForApi = "{635FE197-E26B-407E-9C3C-C3C34DC3AFE0}|"
    jupyterInputPromptPrefixForRichApiRequest = jupyterInputPromptPrefixForApi + "RICHAPI|"
    jupyterInputPromptPrefixForOperationMethodRequest = jupyterInputPromptPrefixForApi + "OPERATIONMETHOD|"
...
class HttpRequestExecutor(IRequestExecutor):
    def execute(self, requestInfo: httphelper.RequestInfo) -> httphelper.ResponseInfo:
        if (requestInfo.url.startswith(Constants.localDocument)):
            requestInfoStr = json.dumps(requestInfo, default = lambda o: o.__dict__)
            #print(requestInfoStr)
            responseInfoStr = Utility.invokeInputChannel(Constants.jupyterInputPromptPrefixForRichApiRequest + requestInfoStr)
            #responseInfoDict = {"statusCode": 200, "headers": {"Cach
            responseInfoDict = json.loads(responseInfoStr)
            responseInfo = httphelper.ResponseInfo()
            responseInfo.statusCode = responseInfoDict.get("statusCode")
            responseInfo.body = responseInfoDict.get("body")
            if (responseInfoDict.get("headers")):
                for key, value in responseInfoDict.get("headers").items():
...
class ClientRequestContext:
    ...
    def __init__(self, url: str, executionMode: RequestExecutionMode):
        ...
        if self._executionMode is None:
            self._executionMode = RequestExecutionMode.instantSync
    ...
    @contextmanager
    def batchScope(self):
        prevExecutionMode = self._executionMode
        try:
            self._executionMode = RequestExecutionMode.batch
            yield self
        finally:
            self._executionMode = prevExecutionMode
    ...
    def sync(self):
        ...
        queryStrings = dict()
        queryStrings[Constants.flags] = req.flags
        if self._customData is not None and len(self._customData) > 0:
            queryStrings[Constants.customData] = self._customData
        requestInfo.url = requestInfo.url + "?" + urllib.parse.urlencode(queryStrings)
...
class Utility:
    ...
    @staticmethod
    def setInputChannelMock(prompt: str, result: str) -> None:
        Utility._inputChannelMock[prompt] = result
    @staticmethod
    def clearInputChannelMock() -> None:
        Utility._inputChannelMock.clear()

    @staticmethod
    def invokeInputChannel(prompt: str) -> str:
        if prompt in Utility._inputChannelMock:
            return Utility._inputChannelMock.get(prompt)
        ret = input(prompt)
        if Utility.isNullOrEmptyString(ret):
            raise Utility.createRuntimeError(ErrorCodes.generalException, Utility._getResourceString(ResourceStrings.generalException))

        if ret[0] != "{" :
            # It's not a JSON object
            raise Utility.createRuntimeError(ret)

        return ret

class OperationMethodRequestInfo:
    def __init__(self):
        self.id = None
        self.arguments = None

class OperationMethodResponseInfo:
    def __init__(self):
        self.error = None
        self.result = None

class OperationMethodUtility:

    @staticmethod
    def invoke(methodId: int, args: dict = None) -> any:
        prompt = OperationMethodUtility.buildInputChannelRequestMessage(methodId, args)
        resultString = Utility.invokeInputChannel(prompt)

        resultObj = json.loads(resultString)
        if not isinstance(resultObj, dict):
            raise Utility.createRuntimeError(ErrorCodes.generalException, Utility._getResourceString(ResourceStrings.generalException))

        if "error" in resultObj and not Utility.isNullOrUndefined(resultObj.get('error')):
            raise Utility.createRuntimeError(ErrorCodes.generalException, resultObj.get("error"))

        if not "result" in resultObj:
            raise Utility.createRuntimeError(ErrorCodes.generalException, Utility._getResourceString(ResourceStrings.generalException))
    ...
    @staticmethod
    def buildInputChannelRequestMessage(methodId: int, args: dict = None) -> str:
        requestInfo = OperationMethodRequestInfo()
        requestInfo.id = methodId
        requestInfo.arguments = args
        jsonString = json.dumps(requestInfo, default = lambda o: o.__dict__)
        prompt = Constants.jupyterInputPromptPrefixForOperationMethodRequest + jsonString
        return prompt

    @staticmethod
    def buildInputChannelResponseMessage(result) -> str:
        responseInfo = OperationMethodResponseInfo()
        responseInfo.result = result
        jsonString = json.dumps(responseInfo, default = lambda o: o.__dict__)
        return jsonString

class OperationMethods(enum.IntEnum):
    invalid = 0,
    getMicrosoftGraphOAuthToken = 1,
    getImplicitGrantFlowOAuthToken = 2,
    getAuthorizationCode = 3,

if __name__ == "__main__":
    req = httphelper.RequestInfo()
    req.url = 'https://www.google.com/'
    response = httphelper.HttpUtility.invoke(req)
    print(response.body)
