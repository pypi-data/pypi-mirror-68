...
BASE_IMAGE_NAMES = ["openedx", "forum", "openedx-dev"]
...
def build_image(root, config, image, no_cache, build_arg):
    ...
    for img in BASE_IMAGE_NAMES:
        if image in [img, "all"]:
            ...
            images.build(
                grvlms_env.pathjoin(root, "build", img),
                tag,
                no_cache=no_cache,
    ...
    for plugin, hook in plugins.iter_hooks(config, "build-image"):
        for img, tag in hook.items():
            if image in [img, "all"]:
                tag = grvlms_env.render_str(config, tag)
                images.build(
                    grvlms_env.pathjoin(root, "plugins", plugin, "build", img),
                    tag,
                    no_cache=no_cache,
                    build_args=build_arg,
                )
...
def pull_image(config, image):
    ...
    for _plugin, hook in plugins.iter_hooks(config, "remote-image"):
        for img, tag in hook.items():
            if image in [img, "all"]:
                tag = config["DOCKER_REGISTRY"] + grvlms_env.render_str(config, tag)
                images.pull(tag)
