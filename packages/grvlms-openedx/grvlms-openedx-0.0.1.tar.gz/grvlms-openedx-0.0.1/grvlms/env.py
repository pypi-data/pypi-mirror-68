...
TEMPLATES_ROOT = pkg_resources.resource_filename("grvlms", "templates")
...
class Renderer:
    ...
    def __init__(self, config, template_roots, ignore_folders=None):
        ...
        environment.globals["GRVLMS_VERSION"] = __version__
    ...
    def render_file(self, path):
        ...
        try:
            return self.__render(template)
        except (jinja2.exceptions.TemplateError, exceptions.GrvlmsError):
...
def upgrade_obsolete(root):
    # grvlms.conf was renamed to _grvlms.conf in order to be the first config file loaded
    # by nginx
    nginx_grvlms_conf = pathjoin(root, "apps", "nginx", "grvlms.conf")
    if os.path.exists(nginx_grvlms_conf):
        os.remove(nginx_grvlms_conf)
...
def check_is_up_to_date(root):
    if not is_up_to_date(root):
        message = (
            ...
            "v{} while the 'grvlms' binary is at v{}. You should upgrade "
...
def is_up_to_date(root):
    """
    Check if the currently rendered version is equal to the current grvlms version.
    """
