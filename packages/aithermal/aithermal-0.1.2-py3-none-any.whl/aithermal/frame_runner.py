...
import pandas as pd
import glob
from aithermal.utils import (excel_pathing, csv_list_multi, csv_list)
...
def derivative(df):
    ...
    delta['Unsubtracted Weight'] = delta['Unsubtracted Weight'].ewm(span=50).mean().div(delta['Time'])
    delta = pd.concat([delta, df['Sample Temperature']], axis=1).dropna().drop(columns=['Time', 'Sample Temperature'],
                                                                               errors='ignore')
...
def wl_ext(frame, wavelength):
    ...
    extracted_frame["index"] = extracted_frame['index'].astype(float).map(lambda x: x / 60).round(3)
    extracted_frame.rename(columns={'index': 'Time'}, inplace=True)
    return extracted_frame


def load_dsc(path, method, baseline_path=None, rms=False, wavelengths=None, file_type='csv'):
    ...
    if method == 'tg':
        gradient_args['columns'] = ["Time", "Unsubtracted Weight", 'Sample Temperature', "Unsubtracted Heat Flow"]
        isotherm_args['columns'] = ["Time", "Unsubtracted Weight", 'Sample Temperature', "Unsubtracted Heat Flow"]

        gradient_args['baseline_path'] = baseline_path
        isotherm_args['baseline_path'] = baseline_path

    if method == 'ir':
        gradient_args['wavelength'] = wavelengths
        isotherm_args['wavelength'] = wavelengths
        if rms:
            gradient_args['rms'] = gradient_files.get('rms')
            isotherm_args['rms'] = gradient_files.get('rms')
    if method == "gc":
        gradient_args['data'] = [tic_file for tic_file in gradient_files.get('standard') if 'tic' in tic_file.lower()]
        isotherm_args['data'] = [tic_file for tic_file in gradient_files.get('standard') if 'tic' in tic_file.lower()]
...
def ms_runner(file):
    data_frame = pd.read_table(file, sep="\s+", header=None,
...
def tg_runner(baseline, columns_to_use, file, isotherm_pattern):
    ...
    baseline_corrected = correct(data_frame, corrective_frame, ['Unsubtracted Heat Flow'])
    baseline_corrected.rename(columns={'Unsubtracted Heat Flow': 'Corrected Heat Flow'}, inplace=True)
    data_frame = pd.concat([data_frame, deriv, percent, baseline_corrected], axis=1)
    return data_frame


def ir_runner(file, wavelength):
    data_frame = pd.read_csv(file, engine='python',
