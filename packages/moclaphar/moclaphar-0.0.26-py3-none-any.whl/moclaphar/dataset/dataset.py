...
def make_training_data(data_root, save_root, window_size=300, stride=1, chunk_size=50,
    ...
    for i in tqdm(range(len(data))):
        if np.sum(np.isnan(data[i].flatten())) > 0:
            print("NaN exits prior to normalizing on {} idx".format(i))
    ...
    for i in tqdm(range(len(data))):
        if np.sum(np.isnan(data[i].flatten())) > 0:
            print("NaN exits post to normalizing on {} idx".format(i))
    ...
    for i, dd in enumerate(training_data):
        if np.sum(np.isnan(dd.flatten())) > 0:
            print("NaN Exist after generate training test! (training) on {}".format(i))

    for i, dd in enumerate(test_data):
        if np.sum(np.isnan(dd.flatten())) > 0:
            print("NaN Exist after generate training test! (training) on {}".format(i))
