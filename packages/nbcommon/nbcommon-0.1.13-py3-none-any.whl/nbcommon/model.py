...
class StartTimeEncoder:
    ...
    def weekday_class(self, ts: int):
        d = datetime.fromtimestamp(ts).isoweekday()
        # using piece index as label
...
class FeatureSchema:
    def __init__(self, schema):
        ...
        self.straight_dist_encoder = self._create_straight_dist_encoder(features)
    ...
    def gen_trip_vector(self, raw: RawInput) -> FeatureVector:
        return FeatureVector(
            ...
            start_hour_class=self.start_time_encoder.hour_class(raw.start_time),
            start_weekday_class=self.start_time_encoder.weekday_class(raw.start_time),
            eda_scaled=self.eda_encoder.scale(raw.eda),
            eta_scaled=self.eta_encoder.scale(raw.eta),
            straight_dist_scaled=self.straight_dist_encoder.scale(raw.straight_dist)
        )
    ...
    def _check_version(self, schema):
        ...
        if str(schema["version"]) != "0.1":
            raise Exception(f"schema.version mismatch, expect: 0.1, got: {schema['version']}")

    def _create_start_time_encoder(self, features) -> StartTimeEncoder:
        if "start_time" not in features:
            raise Exception("schema.features: start_time missing\n{}".format(features))
        if "hour" not in features["start_time"]:
            raise Exception("schema.features: start_time.hour missing\n{}".format(features))
        if "weekday" not in features["start_time"]:
            raise Exception("schema.features: start_time.weekday missing\n{}".format(features))
    ...
    def _create_latlng_encoder(self, features) -> LatLngEncoder:
        if "latlng" not in features:
            raise Exception("schema.features: latlng missing\n{}".format(features))
    ...
    def _create_eda_encoder(self, features) -> ScaleEncoder:
        if "EDA" not in features:
            raise Exception("schema.features: EDA missing\n{}".format(features))
    ...
    def _create_straight_dist_encoder(self, features) -> ScaleEncoder:
        if "straight_dist" not in features:
            raise Exception("schema.features: straight_dist missing\n{}".format(features))
