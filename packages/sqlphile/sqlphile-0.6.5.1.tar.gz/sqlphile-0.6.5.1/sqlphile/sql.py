...
class SQL:
    ...
    def insert (self, table, alias = None):
        self._template = "INSERT INTO " + render_table (table, alias) + " ({this.columns}) VALUES ({this.values})"
        return self

    def update (self, table, alias = None):
        self._template = "UPDATE " + render_table (table, alias) + " SET {this.pairs}"
        self._check_filter = True
        return self

    def select (self, table, alias = None):
        self._template = "SELECT {select} FROM " + render_table (table, alias)
        self.feed (select = ["*"])
        return self

    def delete (self, table, alias = None):
        self._template = "DELETE FROM " + render_table (table, alias)
        self._check_filter = True
        return self
    ...
    def _join_select_fields (self):
        if "select" in self._feed and isinstance (self._feed ["select"], list):
