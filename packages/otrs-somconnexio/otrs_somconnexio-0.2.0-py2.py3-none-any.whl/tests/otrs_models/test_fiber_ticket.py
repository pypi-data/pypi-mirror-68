...
class FiberTicketTestCase(unittest.TestCase):
    ...
    def test_build_ticket(self, MockTicket):
        ...
        otrs_configuration = Mock(spec=[
            ...
            'fibre_ticket_queue',
            'fibre_ticket_state',
            'fibre_ticket_priority',
            'fibre_process_id',
            'fibre_activity_id',
        ])
        otrs_configuration.fibre_ticket_type = 'fiber_ticket_type'
        otrs_configuration.fibre_ticket_queue = 'fiber_ticket_queue'
        otrs_configuration.fibre_ticket_state = 'fiber_ticket_state'
        otrs_configuration.fibre_ticket_proprity = 'fiber_ticket_priority'
        otrs_configuration.fibre_process_id = 'fiber_process_id'
        otrs_configuration.fibre_activity_id = 'fiber_activity_id'

        expected_ticket_arguments = {
            "Title": "Sol·licitud fiber {}".format(eticom_contract.id),
            "Type": otrs_configuration.fibre_ticket_type,
            "Queue": otrs_configuration.fibre_ticket_queue,
            "State": otrs_configuration.fibre_ticket_state,
            "Priority": otrs_configuration.fibre_ticket_proprity,
            "CustomerUser": 'contact@mail.com',
            "CustomerID": 'contact@mail.com'
        }

        FiberTicket(eticom_contract, otrs_configuration)._build_ticket()
    ...
    def test_build_article(self, MockInternetArticle):
        ...
        FiberTicket(eticom_contract, otrs_configuration)._build_article()
    ...
    def test_build_dynamic_fields(self, MockFiberDynamicFields):
        ...
        FiberTicket(eticom_contract, otrs_configuration)._build_dynamic_fields()

        MockFiberDynamicFields.assert_called_with(
            eticom_contract,
            otrs_configuration.fibre_process_id,
            otrs_configuration.fibre_activity_id,
        )
    ...
    def test_create(self, MockOTRSClient):
        ...
        ticket = FiberTicket(eticom_contract, otrs_configuration)
        ticket.create()
