...
URI_PATTERN = r"^(?P<schema>tcp|rqdata)://(?P<username>\S+?):(?P<password>\S+)@(?P<hostname>\S+):(?P<port>\d+$)"
...
def _set_sock_factory(proxy_info):
    ...
    if proxy_info is None:
        import socket
        Connection.set_sock_factory(socket.create_connection)
        return
    ...
    if proxy_type.upper() == "HTTP":
        ...
        raise ValueError("proxy type {} not supported".format(proxy_type))

    Connection.set_sock_factory(partial(socks.create_connection, proxy_type=proxy_type, addr=host, port=port,
...
def init(username=None, password=None, addr=("rqdatad-pro.ricequant.com", 16011), *_, **kwargs):
    ...
    Environment Variable: RQDATAC_CONF / RQDATAC2_CONF. When init called with no argument, the value in
    RQDATAC2_CONF is used as uri, then RQDATAC_CONF.

    :param username: string
    :param password: string
    :param addr: ('127.0.0.1', 80) or '127.0.0.1:80'

    :param uri: keyword only. a uri like rqdata://username:password@host:port or tcp://username:password@host:port
    :param connect_timeout: socket connect connect timeout, default is 5 sec.
    :param timeout: socket time out, default is 60 sec.
    :param lazy: True by default, means "do not connect to server immediately".
    ...
    _set_sock_factory(kwargs.pop("proxy_info", None))
