...
def export_as_api(f=None, name=None, namespace=None):
    if f is None:
    ...
    if getattr(namespace, name, None) is not None:
        raise ValueError("api {} is already defined with {}".format(name, getattr(rqdatac, name)))
    setattr(namespace, name, c)
    return f
