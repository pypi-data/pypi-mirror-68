...
def apply_cats(df, trn):
    ...
    for n, c in df.items():
        if (n in trn.columns) and (trn[n].dtype.name == 'category'):
            df[n] = c.astype('category').cat.as_ordered()
            df[n].cat.set_categories(
                trn[n].cat.categories, ordered=True, inplace=True)
...
def scale_vars(df, mapper):
    ...
    if mapper is None:
        map_f = [([n], StandardScaler())
                 for n in df.columns if is_numeric_dtype(df[n])]
...
def get_nn_mappers(df, cat_vars, contin_vars):
    ...
    [(o, LabelEncoder()) for o in cat_vars]
    [([o], StandardScaler()) for o in contin_vars]


def datatype(df):
    """
    Use with Categorify to settle dtypes for random forest 

    Categorify is a fastai.tabular Class that is extended from pandas
    ...
    # may contain other type of data
...
def display_all(df, inverted=True):
    """
    this just lets you display more stuff
    """
