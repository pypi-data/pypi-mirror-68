...
from conductor.subscriptions import ZeroMQSubscription, WebsocketSubscription
...
class ConductorSubject(object):
    ...
    def __init__(self, session, subject_id, instance=conductor.PRODUCTION,
                 _data=None):
    ...
    def client_edge_url(self):
        if self._uat:
            if self.instance == conductor.HOSPITALITY:
                return "https://clientedge-hospitality-uat.airfinder.com/" \
                        "clientEdge"
            else:
                return "https://clientedge-conductor-uat.link-labs.com/" \
                        "clientEdge"
        else:
...
class UplinkSubject(ConductorSubject):
    ...
    def get_messages_time_range(self, start, stop=None):
        ...
        while more_results:
            ...
            messages.extend([self._build_msg(self.session, m.get('uuid'),
                                             self.instance, m)
    ...
    def get_last_messages(self, count=1):
        ...
        messages = [self._build_msg(self.session, m.get('uuid'),
                                    self.instance, m)
