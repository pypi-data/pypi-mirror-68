...
class Executor(object):
    ...
    def __init__(self, rout):
        ...
        self.pass_count = 0
        self.fail_count = 0
        self.last_reg = {"addr": None, "value":None}
    ...
    def get_expression_reg(self, reg_string):
        ...
        if rets:
            ...
            reg_range = rets[0][1]
            if reg_addr != self.last_reg["addr"]:
                self.last_reg["addr"] = reg_addr
                orgi_value = self.rout.read_uint32(reg_addr)
                self.last_reg["value"] = orgi_value
            else:
                orgi_value = self.last_reg["value"]
                print("Indirect reading:{:#x}:{:#x}".format(reg_addr, orgi_value))
