...
from tensorflow_probability.python.internal.backend.jax import ops as variables_module
...
def assert_is_batch_matrix(tensor):
  ...
  sh = _ops.TensorShape(tensor.shape)
...
def _reshape_for_efficiency(a,
  ...
  if _ops.TensorShape(a.shape).ndims is None or _ops.TensorShape(b.shape).ndims is None:
  ...
  b_extra_ndims = _ops.TensorShape(b.shape).ndims - _ops.TensorShape(a.shape).ndims
  ...
  a_domain_sz_ = _ops.TensorShape(a.shape)[-2 if adjoint_a or transpose_a else -1]
  b_eq_sz_ = _ops.TensorShape(b.shape)[-2 if adjoint_b or transpose_b else -1]
  b_extra_sz_ = (
      np.prod(_ops.TensorShape(b.shape)[:b_extra_ndims].as_list())
      if _ops.TensorShape(b.shape)[:b_extra_ndims].is_fully_defined() else None)
  ...
  perm = (
      np.concatenate(
          (np.arange(b_extra_ndims, _ops.TensorShape(b.shape).ndims),
