...
from tensorflow_probability.python.internal.backend.jax import debugging as check_ops
...
class LinearOperatorBlockLowerTriangular(linear_operator.LinearOperator):
  ...
  >>> _ops.TensorShape(operator_99.shape)
  TensorShape([2, 3, 9, 9])
  ...
  >>> _ops.TensorShape(y_blockwise[0].shape)
  TensorShape([2, 3, 4])
  >>> _ops.TensorShape(y_blockwise[1].shape)
  TensorShape([2, 3, 5])
  ...
  * If `is_X == None` (the default), callers should have no expectation either
  ...
  def _shape_tensor(self):
    ...
    if _ops.TensorShape(self.shape).is_fully_defined():
      return ops.convert_to_tensor(
          _ops.TensorShape(self.shape).as_list(), dtype=dtypes.int32, name="shape")
      ...
      if linear_operator_util.arg_is_blockwise(block_dimensions, x, arg_dim):
        for i, block in enumerate(x):
          if not isinstance(block, linear_operator.LinearOperator):
            ...
            block_dimensions[i].assert_is_compatible_with(_ops.TensorShape(block.shape)[arg_dim])
            x[i] = block
      else:
        ...
        op_dimension.assert_is_compatible_with(_ops.TensorShape(x.shape)[arg_dim])
  ...
  def matvec(self, x, adjoint=False, name="matvec"):
    ...
    with self._name_scope(name):
      ...
      if linear_operator_util.arg_is_blockwise(block_dimensions, x, -1):
        for i, block in enumerate(x):
          if not isinstance(block, linear_operator.LinearOperator):
            block = ops.convert_to_tensor(block)
            # self._check_input_dtype(block)
            block_dimensions[i].assert_is_compatible_with(_ops.TensorShape(block.shape)[-1])
            x[i] = block
      ...
      op_dimension.assert_is_compatible_with(_ops.TensorShape(x.shape)[-1])
  ...
  def solve(self, rhs, adjoint=False, adjoint_arg=False, name="solve"):
    ...
    with self._name_scope(name):
      ...
      if blockwise_arg:
        ...
        op_dimension.assert_is_compatible_with(_ops.TensorShape(rhs.shape)[arg_dim])
  ...
  def solvevec(self, rhs, adjoint=False, name="solve"):
    ...
    with self._name_scope(name):
      ...
      op_dimension.assert_is_compatible_with(_ops.TensorShape(rhs.shape)[-1])
...
distribution_util = lazy_loader.LazyLoader(
    ...
    "tensorflow_probability.python.internal._jax.distribution_util")
