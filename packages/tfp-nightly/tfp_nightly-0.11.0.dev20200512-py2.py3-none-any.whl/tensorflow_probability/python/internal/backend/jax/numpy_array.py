...
from tensorflow_probability.python.internal.backend.jax.linalg_impl import einsum
from tensorflow_probability.python.internal.backend.jax.linalg_impl import norm
from tensorflow_probability.python.internal.backend.jax.linalg_impl import tensordot
...
def _gather(  # pylint: disable=unused-argument
  ...
  res = np.array([
      ...
      for i in onp.ndindex(*params.shape[:batch_dims])
  ])
  return np.reshape(
      res,
