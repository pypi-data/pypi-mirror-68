...
class DryRunServiceCaller(AE3PServiceCallerBase):
    ...
    def create_unsubmitted_pipeline_run_async(self, creation_info_with_graph, experiment_name):
        ...
        entity = PipelineRunEntity(id=id, description=creation_info_with_graph.creation_info.description,
                                   ...
                                   data_set_definition_value_assignment=creation_info_with_graph.creation_info.
                                   data_set_definition_value_assignments)
        # Mimic actual service caller output
        dataset_definition_value_assignments = creation_info_with_graph.creation_info.\
            data_set_definition_value_assignments
        additional_props_value_assignments = {}

        for key, value in dataset_definition_value_assignments.items():
            saved_dataset_reference_dict = {'Id': value.saved_data_set_reference.id}
            value_dict = {'LiteralValue': None, 'DataSetReference': None,
                          'SavedDataSetReference': saved_dataset_reference_dict}
            additional_props_value_assignments[key] = value_dict
        entity.additional_properties["DataSetDefinitionValueAssignments"] = additional_props_value_assignments
