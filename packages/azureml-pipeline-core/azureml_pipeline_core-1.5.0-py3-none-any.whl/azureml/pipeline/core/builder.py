...
from azureml.data.constants import _DATASET_ARGUMENT_TEMPLATE
from azureml.data.dataset_consumption_config import DatasetConsumptionConfig
from azureml.pipeline.core.graph import Graph, Node, InputPortBinding, OutputPortBinding, \
    _PipelineIO, PipelineDataset, DataSource, ParamDef
...
class PipelineStep(object):
    ...
    def __init__(self, name, inputs, outputs, arguments=None, fix_port_name_collisions=False, resource_inputs=None):
        ...
        PipelineStep._update_inputs_with_arguments(arguments, inputs, outputs)
    ...
    @staticmethod
    def _update_inputs_with_arguments(arguments, inputs, outputs):
        # This method goes over arguments and first transform them into the corresponding string representation
        # then if the argument is not in inputs, update the inputs list with the original object.
        for i in range(len(arguments)):
            ...
            if not isinstance(argument, PipelineDataset) and not isinstance(argument, PipelineOutputAbstractDataset) \
            ...
            for input_port in node.inputs:
                ...
                if edge is not None and not isinstance(edge, Edge) and edge.source is not None:
                    ...
                    if isinstance(edge, _PipelineDataEdgeBuilder):
                        ...
                        peer = edge.source
                        peer._ensure_saved(self._context._workspace)
                        datasource_def = DataSourceDef(peer.name, pipeline_dataset=peer)
