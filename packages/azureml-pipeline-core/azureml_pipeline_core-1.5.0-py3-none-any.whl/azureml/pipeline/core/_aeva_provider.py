...
class _AevaGraphProvider(_GraphProvider):
    ...
    def create_graph_from_run(self, context, pipeline_run_id):
        ...
        if pipeline_run.additional_properties:
            dataset_def_value_assignments = pipeline_run.additional_properties.get('DataSetDefinitionValueAssignments')
        return self.create_graph_from_graph_id(context, pipeline_run.description, pipeline_run.graph_id,
                                               pipeline_run.parameter_assignments, dataset_def_value_assignments)
    ...
    def create_graph_from_graph_entity(service_caller, context, name, graph_entity, parameter_assignments,
        ...
        for datasource_node in graph_entity.dataset_nodes:
            ...
            if datasource_node.dataset_id is not None:
                ...
                dataset_id = dataset_definition_value_assignments[dataset_name]['SavedDataSetReference']['Id']
                saved_dataset = Dataset.get_by_id(workspace=context._workspace, id=dataset_id)
                datasource_def = DataSourceDef(dataset_name, pipeline_dataset=saved_dataset)
                datasource = DataSource(None, datasource_def)
                node = DataSourceNode(graph=graph, name=dataset_name, node_id=node_id, datasource=datasource)
            else:
                saved_id = datasource_node.data_set_definition.value.saved_data_set_reference.id
                saved_dataset = Dataset.get_by_id(workspace=context._workspace, id=saved_id)
                dataset_name = saved_dataset.name or saved_dataset.id
                if datasource_node.data_set_definition.value.data_set_reference:
                    registered_id = datasource_node.data_set_definition.value.data_set_reference.id
                    if registered_id:
...
class _AevaPublishedPipelineProvider(_PublishedPipelineProvider):
    ...
    def get_graph(self, context, name, graph_id, pipeline_id):
        ...
        dataset_def_values = None
        if pipeline_entity.additional_properties:
            dataset_def_values = pipeline_entity.additional_properties.get('DataSetDefinitionValueAssignments')
...
class _AevaScheduleProvider(_ScheduleProvider):
    ...
    def create_schedule(self, name, published_pipeline_id, experiment_name, recurrence=None,
                        datastore_name=None, polling_interval=None, data_path_parameter_name=None,
...
class _AevaPipelineDraftProvider(_PipelineDraftProvider):
    ...
    def get_graph_draft(self, context, pipeline_draft_id, graph_draft_id):
        ...
        if pipeline_draft_entity.pipeline_submission_info is not None:
            ...
            if pipeline_draft_entity.pipeline_submission_info.additional_properties:
                props = pipeline_draft_entity.pipeline_submission_info.additional_properties
                if 'DataSetDefinitionValueAssignments' in props:
                    dataset_vals = props['DataSetDefinitionValueAssignments']
