...
class Model:
    ...
    def serialize(self):
        ...
        # print('serializing', self._registered_relationships.get(self.__class__, {}).keys())
        for key in self._registered_relationships.get(self.__class__, {}).keys():
            loaded = self._relationships.get(key)
            print('loading', key)
            if loaded and key not in serialized_dictionary:
                print('dic so far:', serialized_dictionary)
