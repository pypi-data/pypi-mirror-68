...
class YieldFunction(metaclass=abc.ABCMeta):
    ...
    def from_fit(cls, stress_states, initial_params=None, force_fit=False, **kwargs):
        ...
        # print(f'fitting_param_names: {fitting_param_names}')
        ...
        if not fitting_param_names:
            ...
            if 'exponent' in fitting_param_names:
                bounds = (
                    [-2] * (len(initial_params_all) - 1) + [1],
                    [+2] * (len(initial_params_all) - 1) + [8],
                )
            else:
                bounds = (
                    [-2] * len(initial_params_all),
                    [+2] * len(initial_params_all),
                )

            # print(f'initial_params_all: {initial_params_all}')
            # print(f'fitting_param_names: {fitting_param_names}')
            # print(f'bounds:\n{bounds}')
    ...
    def compare_3D(cls, yield_functions, normalise=True, resolution=DEF_3D_RES,
        ...
        if backend == 'pyvista':
            ...
            for idx, (yld_func, i) in enumerate(zip(yield_functions, values_all)):
                ...
                # valid_idx = np.isfinite(i)

                # print(f'valid_idx len: {valid_idx.sum()}')

                # i_valid = i[valid_idx]
                # stress_X_valid = stress_X[valid_idx]
                # stress_Y_valid = stress_Y[valid_idx]
                # stress_Z_valid = stress_Z[valid_idx]

                # print(f'i.shape: {i_valid.shape}')
                # print(f'i: {i}')

                # print(f'max(i_valid): {max(i_valid)}, min(i_valid): {min(i_valid)}')

                # print(f'i_valid[:100]:\n{i_valid[:100]!r}')
                # print(f'stress_X_valid[:100]:\n{stress_X_valid[:100]!r}')
                # print(f'stress_Y_valid[:100]:\n{stress_Y_valid[:100]!r}')
                # print(f'stress_Z_valid[:100]:\n{stress_Z_valid[:100]!r}')

                near_zero_idx = np.where(np.abs(i) < 1e-3)
                #print(f'near_zero_idx: {near_zero_idx}')

                near_zero_stress = np.array([
                    stress_X[near_zero_idx],
                    stress_Y[near_zero_idx],
                    stress_Z[near_zero_idx],
                ]).T

                #print(f'near_zero_stress: {near_zero_stress.shape}\n{near_zero_stress!r}')
    ...
    def get_numerical_lankford(self, fit_domain=0.05, resolution=100, tol=2e-2):
        ...
        xy *= equiv_stress * fit_domain
        xy += np.array([[equiv_stress, 0]]).T
        ...
        # [# For fitting, normalise x, y values by equivalent stress (we just want the
        # # direction of the tangent vector in the σ_3 = 0 plane)] ?
        # xy_norm = xy / equiv_stress

        fit_coords = xy[:, good_idx]
        # Get x, y values to fit:
        ...
        tangent_x = [np.min(xy[0]), np.max(xy[0])]
        tangent_y = maths_utils.line(np.array(tangent_x), m, c)

        normal = np.array([-m, 1])
        # print(f'normal: {normal}')

        normal_n = normal / np.linalg.norm(normal)
        # print(f'normal_n: {normal_n}')

        lankford = normal_n[1] / (-normal[0] - normal[1])

        return (lankford, (tangent_x, tangent_y))
