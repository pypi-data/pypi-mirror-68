...
PARALLEL_RUN_DEPRECATION_STATEMENT = (
...
class ParallelRunStep(_PythonScriptStepBase):
    ...
    def _warn_deprecated_fields(self):
        ...
        if self._models is not None:
            warnings.warn(PARALLEL_RUN_DEPRECATION_STATEMENT.format("models"), DeprecationWarning)
        if self._tags is not None:
            warnings.warn(PARALLEL_RUN_DEPRECATION_STATEMENT.format("tags"), DeprecationWarning)
        if self._properties is not None:
            warnings.warn(PARALLEL_RUN_DEPRECATION_STATEMENT.format("properties"), DeprecationWarning)
    ...
    def _generate_runconfig(self):
        ...
        if run_config.environment.python.user_managed_dependencies:
            warnings.warn("""
ParallelRunStep will include it's dependencies if conda_dependencies is being used.
For user_managed_dependencies, please ensure the following dependencies are installed:
  ...
  filelock~=3.0
  azureml-dataprep[fuse,pandas]>=1.4.10a,<1.6.0a
  psutil~=5.0""", UserWarning)
        ...
        run_config.environment.python.conda_dependencies.add_pip_package("azureml-dataprep[fuse,pandas]>=1.4.10a,<1.6.0a")
