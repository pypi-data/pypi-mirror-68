...
class woe_bin(object):
    ...
    def split_onevar(self, tabname, varname):
        ...
        if len(np.unique(n_nulltab[varname])) > 1:
            ...
            if woe_direct == 0:
                n_nullmapiv = self.get_firstnull_mapiv(n_nulltab, t0, t1)
            else:
    ...
    def split_data(self):
        ...
        feature_list = list(self.indata)
        feature_list1 = [i for i in feature_list if 'target' not in i]

        mapiv = pd.DataFrame()
        for var_i in feature_list1:
            print(var_i)
            mapiv1 = self.split_onevar(self.indata[[var_i, 'target']], var_i)
