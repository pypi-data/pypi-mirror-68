...
class Api(object):
    ...
    def get_node_info(self, ip=None):
        ...
        for index,item in enumerate(node_list):
            ...
            state = ret["data"][0] if ret["state"] == 1 else "offline"
            node_list[index]["state"] = state
        return node_list
    ...
    def get_test_detail_information(self, test_key):
        self.db_client.benchmark_resource.get_test_detail_information(test_key)

    def get_step_detail_information(self, step_key):
        self.db_client.benchmark_resource.get_step_detail_information(step_key)

    def get_real_time_results(self, key, mini_index=None):
        self.db_client.benchmark_resource.get_real_time_results(key, mini_index)

    def get_test_state(self, key):
        self.db_client.benchmark_resource.get_test_state(key)


# if __name__ == '__main__':
#     API = Api()
#     API.set_project_name("172.29.131.117", "G+")
