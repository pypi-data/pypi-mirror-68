...
class PGPConfig(object):
    SIGNED = "signed"
    SIGNED_AND_ENCRYPT = "signedencrypt"
    ENCRYPTED = "encrypt"
    NONE = "none"
    ...
    def _find_pgp_sig(self, msg_obj):
        ...
        for idx in range(attachments):
            try:
                return self.pub_key.verify(msg_obj.get_payload(0).as_string(), PGPSignature.from_blob(msg_obj.get_payload(idx).get_payload()))
            except PGPError:
    ...
    def unlock(self, msg_as_string):
        mime_msg = CustomMIMEWrapper(msg_as_string)
        is_encrypted = mime_msg.is_encrypted()
        is_verified = False
        fingerprints = []
        message_type = "Encrypted" if is_encrypted else "Cleartext"
        ...
        if is_signed:
            ...
            message_type += " & Signed"
            sig_verify = self._find_pgp_sig(mime_msg.msg)
            sigs = list(sig_verify.good_signatures)
            is_verified = len(sigs) > 0
            if is_verified:
                message_type += " & Verified"
            fingerprints = [str(sig.by.fingerprint) for sig in sigs]

        email_data = EmailMasterParser(base64_encode_bytes(str(mime_msg.msg).encode())).parse()
    ...
    def _format_signed_payload(self, mime_message, payload, signed_payload, dt):
        bound = "wECpsh8cn1DyqgthMDFfJ9avZxL6XeJ90"  # TODO generate this
