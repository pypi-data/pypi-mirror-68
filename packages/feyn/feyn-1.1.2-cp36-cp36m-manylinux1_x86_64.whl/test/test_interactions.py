...
class TestInteractions(unittest.TestCase):
    ...
    def test_tanh_assign_to_state_is_not_allowed(self):
        with self.assertRaises(AttributeError):
            fail = feyn.Interaction('tanh', latticeloc=(0,0,0))
            fail.state = {}

    def test_tanh_state_update(self):
        interaction = feyn.Interaction('tanh', latticeloc=(0,0,0))
    ...
    def test_sine_interaction_state(self):
        sine_interaction = feyn.Interaction('sine', latticeloc=(0,0,0))
        state = {
            'x0': 0,
            'k': 1.0
        }
    ...
    def test_gaussian_interaction_state(self):
        gaussian_interaction = feyn.Interaction('gaussian', latticeloc=(0,0,0))
        state = {
            'center0': 0,
            'center1': 1.0,
            'w0': 0.2,
    ...
    def test_handle_interaction_without_properties(self):
        multiply_interaction = feyn.Interaction('multiply', latticeloc=(0,0,0))
    ...
    def test_category_register_interaction_state(self):
        register_cat_interaction = feyn.Interaction('cat', latticeloc=(0,0,0))
        state = {
            'categories': [
                ('red', 0.1),
                ('blue', 0.15),
    ...
    def test_category_register_interaction_update(self):
        register_cat = feyn.Interaction('cat', name='myinput', latticeloc=(0,0,-1))
        ...
        out = feyn.Interaction("fixed", name="out", latticeloc=(1,0,-1))
        out._set_source(0,0)
    ...
    def test_interaction_depth(self):
        reg = feyn.Interaction("fixed", latticeloc=(0,0,-1))
        tanh = feyn.Interaction("tanh", latticeloc=(0,0,0))
