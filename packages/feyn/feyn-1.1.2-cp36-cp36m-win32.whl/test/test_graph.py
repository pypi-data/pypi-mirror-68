...
class TestGraph(unittest.TestCase):
    ...
    def test_query_accepts_np(self):
        input = feyn.Interaction("fixed", latticeloc=(0,0,-1), name="input")
        input._set_source(0, -1)

        out = feyn.Interaction("fixed", latticeloc=(1,0,-1), name="out")
        out._set_source(0, 0)
    ...
    def test_invalid_graph(self):
        ...
        with self.assertRaises(RuntimeError):
            graph = feyn.Graph(1)
            node = feyn.Interaction('tanh', latticeloc=(0,0,-1), name="in")
        ...
        with self.assertRaises(RuntimeError):
            ...
            register = feyn.Interaction('fixed', latticeloc=(0,0,-1), name='in')
            register._set_source(0, -1)

            node = feyn.Interaction('tanh', latticeloc=(0,0,0), name="tanh")
