...
class Graph(_feyn.Graph):
    ...
    The constructor is for internal use. You will typically use `QGraph.select` to extract graphs from QGraphs, or load them from a file with Graph.load().
    ...
    def _fit(self, data, epochs, loss_function, verbose=False):
        out_reg = self[-1]
        Y = data[out_reg.name]
        ...
        for epoch in range(epochs):
            predictions = super()._query(data, Y)
            self.loss_epoch = loss_function(Y.astype(float), predictions)

            if self.loss_ema is None:
                self.loss_ema = self.loss_epoch
            else:
                self.loss_ema = .8*self.loss_ema + .2*self.loss_epoch

            if verbose:
                print("Epoch %i: loss %.4f (%.4f)" %(epoch, self.loss_ema, self.loss_epoch))
        ...
        for ix in range(len(self)):
            ...
            for ordinal, src in enumerate(interaction.sources):
                if src != -1:
                    links.append({
                        'source': src,
                        'target': interaction._location,
                        **interaction._linkdata[ordinal]
                    })
    ...
    def _get_interaction(data: dict) -> _feyn.Interaction:
        interaction = feyn.Interaction(data["interaction_type"], tuple(data["location"]), data["gluamine"], data["name"])
        interaction.state._from_dict(data["state"])
        interaction._set_source(0, -1)
        interaction._linkdata = [{}, {}]
    ...
    def _from_dict(gdict):
        ...
        for edge in gdict["links"]:
            ...
            interaction._linkdata[ord] = edge
        return graph
