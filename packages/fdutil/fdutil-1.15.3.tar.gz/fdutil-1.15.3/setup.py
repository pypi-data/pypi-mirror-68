...
MODULE_NAME = u'fdutil'  # The module name must match this!
...
setup(
    ...
    install_requires=[
        u'pip>=18.1',
        u'future>=0.16.0',
        u'ruamel.yaml>=0.15.35',
        u'pyperclip>=1.5.27',
        u'pywin32>=223;platform_system=="Windows"',
        u'logging-helper>=1.7.0',
        u'configurationutil>=3.7.0',
        u'conversionutil>=1.1.0',
        u'stateutil>=1.4.0',
        u'watchdog>=0.8.3',
        u'dominate>=2.3.1'
    ] + ([u'pywin32>=223'] if sys.platform.startswith(u'win') else []),
    ...
    package_data={
        '': ['*.md', '*.rst', '*.db', '*.txt', '*.json', '*.xml', '*.js'],
        'fdutil.resources': ['*.*', '**/*.*', '**/**/*.*'],  # Folders up to 2 levels deep will be included
    },
