...
#         if output_offset == 0:
#             if outarray.shape != expected_shape:
#                 raise ValueError(f"Output array is expected to have shape"
#                                  f" {expected_shape} but got {outarray.shape}")
#         else:
        print("Checking output array shape is disabled, make sure portion of"
              f" output array has shape {expected_shape}")
        if outarray.dtype != dtype:
            raise TypeError(f"Output array should have dtype {dtype}"
                            f" but got {outarray.dtype}")
...
#     assert np.allclose(y, fft(kernel, fft_length))
    ...
    #a = np.zeros((n_blocks, nfft), dtype='<c16')
    #xs = np.zeros((n_blocks, nfft), dtype='<c16')
...
#             print(f"Computing block {ii} of {n_blocks - 1}")
    ...
    assert expected_shape[axis] == tot_samples
    return outarray