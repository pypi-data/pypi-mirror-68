...
try: from . import objectname,shortrepr
...
class ObjectBrowser():
    title="Python对象浏览器"
    obj_image=None
    def __init__(self,master,obj,verbose=False):
        self.master=master
    ...
    def on_doubleclick(self,event):
        ...
        for selection in self.tvw.selection():
            ...
            browse(obj,self.verbose)

def browse(object,verbose=False):
    """以图形方式浏览一个Python对象。
verbose:与describe相同,是否打印出对象的特殊方法(如__init__)"""
    root=tk.Tk()
    ObjectBrowser(root,object,verbose)
    root.mainloop()
