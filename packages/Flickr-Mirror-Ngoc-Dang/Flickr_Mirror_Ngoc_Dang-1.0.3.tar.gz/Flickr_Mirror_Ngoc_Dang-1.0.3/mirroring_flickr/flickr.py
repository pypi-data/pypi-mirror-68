...
class FlickrApi:
    ...
    def get_photos(self, user_id, page=1, per_page=100):
        ...
        return (list_result,
                int(data['photos']['pages']),
                int(data['photos']['total']))
...
class FlickrUserPhotostreamMirroringAgent:
    ...
    def run(self):
        ...
        if self.__caching_strategy is CachingStrategy.LIFO:
            self.__download_lifo(page_count, total)

        # download with fifo method
        elif self.__caching_strategy is CachingStrategy.FIFO:
            self.__download_fifo(page_count, total)


    # first in first out function
    def __download_fifo(self, page_count, total):
        """
        ...
        path = os.path.join(
            self.__cache_root_path_name, f"{self.user.username}_fifo.json")

        path_lifo = os.path.join(
            self.__cache_root_path_name,
            f"{self.user.username}_lifo.json")
        ...
        if os.path.exists(path):
            ...
            with open(path, 'r') as cache_file:
                ...
                download_before = data['index']
                page_before = data['page']
                orginal_page_count = data['page_count']
                total_download_before = data['total_download']

                # check if all has been downloaded
                if total == data['total_download']:
                    logging.info(
                        f"All data have been downloaded before")
                    return

                changes_in_page = page_count - orginal_page_count
                number_file = download_before
                continue_number = download_before

            # if the number pages is still the same
            if changes_in_page == 0:

                # just keep moving with previous data
                last_page = page_before

            # if there are changes
            elif changes_in_page > 0:

                # reset the starting index number
                last_page = page_count - page_before
        ...
        if fifo_jumping is False:
            # record download number in total
            total_download = 0
        else:
            total_download = total_download_before

        # ends after first page
        while last_page >= 1:

            logging.info(f"Scanning page {last_page}/{page_count}")
            ...
            while photo < len(photos):
                ...
                try:
                    with open(path_lifo, 'r') as lifo_file:

                        data_lifo = json.loads(lifo_file.read())

                        if data_lifo['total_download'] + total_download == total:
                            logging.info(
                                f"All data have been downloaded before")
                            return
    ...
    def __download_lifo(self, page_count, total):
        """
        ...
        :total (int): the number of total
        """

        # remainder of the last pages
        left_over = total % 100
        ...
        path_fifo = os.path.join(
            self.__cache_root_path_name,
            f"{self.user.username}_fifo.json")
        ...
        if os.path.exists(path):
            ...
            with open(path, 'r') as cache_file:
                ...
                # get last download data
                download_before = data['index']
                page_before = data['page']
                total_before = data['total']
                total_download_before = data['total_download']

                # keep moving the recorded index in the json file
                number_file = download_before

                # all has been downloaded
                if data['total_download'] == total:
                    logging.info(
                        f"All data have been downloaded before")
                    return

                # calculate difference
                difference = total - total_before
            ...
            if difference == 0:
                ...
                # keep moving the download with previous image index
                continue_number = download_before

            # if there are changes
            elif difference > 0:

                # reset the stats by the new upload photos data
                new_pages = difference

                # new_pages = 200

                page_left_over = new_pages // 100
                new_left_over = new_pages % 100

                if new_left_over == 0:

                    # range_used = range(1, page_left_over + 1)
                    range_used = 1

                elif new_left_over > 0:

                    # range_used = range(1, page_left_over + 2)
                    range_used = page_left_over + 1

                start_left_over = 1

                # download new added images
                while start_left_over <= range_used:

                    photo_list, _, _ = \
                        self.__api_set.get_photos(
                            self.user.user_id,
                            page=start_left_over)

                    logging.info(
                        f"Scanning page {start_left_over}/{page_count}")

                    photo_number = 0
                    while photo_number < len(photo_list):
                        if start_left_over == range_used:
                            if photo_number == new_left_over + 1:
                                break

                        # download based on image only and info only
                        if self.__image_only is True:
                            self.__download_photo_image(
                                photo_list[photo_number])

                        elif self.__info_only is True:
                            self.__download_info(photo_list[photo_number])
                ...
                new_ratio = new_pages + download_before
                start_page = range_used
                continue_number = new_ratio % 100 + 1
                total_download = total_download_before + new_pages
                return
        ...
        while start_page <= page_count:
            ...
            logging.info(f"Scanning page {start_page}/{page_count}")
            ...
            while photo < len(photos):
                ...
                try:
                    with open(path_fifo, 'r') as lifo_file:
                        data_fifo = json.loads(lifo_file.read())
                        if data_fifo['total_download'] + total_download == total:
                            logging.info(
                                f"All data have been downloaded before")
                            return
