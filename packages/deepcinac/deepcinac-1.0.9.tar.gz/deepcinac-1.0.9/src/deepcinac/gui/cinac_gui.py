...
from deepcinac.utils.utils import load_movie, get_source_profile_param, scale_polygon_to_source, \
    get_continous_time_periods, smooth_convolve, build_raster_dur_from_onsets_peaks

from deepcinac.utils.cinac_file_utils import CinacFileReader, CinacFileWriter, read_cell_type_categories_yaml_file
from keras.models import model_from_json

from deepcinac.cinac_predictor import predict_transient_from_model, CinacPredictor
...
class ManualOnsetFrame(tk.Frame):
    ...
    RAW_TRACE = "Cell"
    NEUROPIL_TRACE = "Neuropil"
    RAW_M_NEUROPIL_TRACE = "Cell - Neuropil"
    ...
    def _build_traces(self):
        ...
        for cell in np.arange(self.nb_neurons):
            ...
            neuropil_traces[cell, :] = (neuropil_traces[cell, :] - np.mean(self.data_and_param.raw_traces[cell, :])) \
                                       / np.std(self.data_and_param.raw_traces[cell, :])

        self.traces_dict[self.NEUROPIL_TRACE] = neuropil_traces
        self.traces_dict[self.RAW_M_NEUROPIL_TRACE] = self.raw_traces - neuropil_traces
