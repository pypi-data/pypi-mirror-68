...
class M2M(Integration):
    ...
    def init(self):
        ...
        self.requests.headers.update({
            'X-Secret-Token': State.config.m2m.auth.secret_key,
            'X-Access-Token': State.config.m2m.auth.access_key
        })

    def load(self):
        super().load()

    def search(self, url: str, item_model = None, params: dict = None, page: int = 1, page_size: int = 25) -> Page:
        page_params: dict = {
            'page': page,
            'page_size': page_size
        }
        ...
        page: Page = Page(**self.requests.get(url, params=params).json())
