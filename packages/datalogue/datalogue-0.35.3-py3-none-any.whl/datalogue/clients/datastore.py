...
from datalogue.models.datastore import Datastore, _datastore_from_payload, FileFormat, Cell, _cell_from_payload, \
    NodeClassification, ReclassificationBehavior
...
class _DatastoreClient:
    ...
    def classify_fields(self,
                        ...
                        reclassification_behavior: ReclassificationBehavior = ReclassificationBehavior.ReplaceAutomatic
                        ) -> Union[DtlError, UUID]:
        """
        ...
        res = self.http_client.execute_authed_request(
            self.service_uri + "/datastores/" + str(datastore_id) + f"/classify?reclassification-behavior={str(reclassification_behavior.value)}&classifier={str(classifier_id)}",
            HttpMethod.POST,
            body={})
