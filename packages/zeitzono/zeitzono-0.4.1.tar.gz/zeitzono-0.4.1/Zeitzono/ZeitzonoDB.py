...
class ZeitzonoDB:
    ...
    def _search_parser(self, search):
        ...
        #
        #   (pattern, boolean) - pattern to search for
        #                        if boolean is True, search against city names
        #                        else search against country/admin codes
        #
        patterns = []
        tokens = search.strip().split()
        for tok in tokens:
            if not tok.startswith(":"):
                patterns.append((tok.lower(), True))
            else:
                if len(tok) > 1:
                    patterns.append((tok[1:].lower(), False))
        return patterns

    def _city_search_match(self, pattern, city):
        # return True if pattern matches city
    ...
    def db_search(self, search, limit=None):
        ...
        for pattern, iscity in patterns:
            newmatched = []
