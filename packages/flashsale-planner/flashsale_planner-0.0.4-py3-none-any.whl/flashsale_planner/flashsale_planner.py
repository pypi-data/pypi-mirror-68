...
def initialize(project='datasciprac'):
    global poly,scaler,categories,features,sel_clf,model_hgb_clf,sel_reg,model_hgb_reg
...
def predict(itemid,shopid,flash_sale_stock,discount,price_before_discount,flash_bin=['0-12','12-18','18-21','21-24']):
    ...
    sold_out = model_hgb_clf.predict_proba(data.loc[:,sel_clf.get_support()]).T[1] > .7
    sold_out_prob = model_hgb_clf.predict_proba(data.loc[:,sel_clf.get_support()]).T[1]
    sold_rate = model_hgb_reg.predict(data.loc[:,sel_reg.get_support()])
