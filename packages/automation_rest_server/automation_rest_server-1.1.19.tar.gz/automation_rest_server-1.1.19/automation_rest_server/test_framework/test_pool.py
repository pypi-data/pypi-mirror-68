...
class TestPool(object):
        ...
        while True:
            for index, thread_ in enumerate(self.thread_pool):
                if thread_["state"] == State.NOT_START:
                    ...
                    if thread_["state"] == State.FAIL:
                        log.INFO("Test: {} failed, abort all not start tests".format(thread_["test_name"]))
    ...
    def get_current_node_state(self):
        tests = [test for test in self.test_pool if test["state"] in [State.NOT_START, State.RUNNING]]
        if tests:
    ...
    def _start_test_thread(self, thread_info):
        ...
        if test_type == TestType.TestCase:
            ...
            thread_, test_object = self._start_test_case_thread(test_name)
        elif test_type == TestType.TestSuite:
            self.set_test_parameters(test_parameters)
            self.test_set_parameters(test_parameters)
            thread_, test_object = self._start_test_suite_thread(test_name)
    ...
    def _start_test_suite_thread(self, test_name):
        test_suite = TestSuite()
        thread_ts = MyThread(target=test_suite.run, args=(test_name,))
        thread_ts.setDaemon(True)
        thread_ts.start()
        return thread_ts, test_suite

    def _start_test_case_thread(self, test_name):
        print("_start_test_case_thread", test_name)
        test_case = TestCase(test_name)
        thread_tc = MyThread(target=test_case.run, args=())
