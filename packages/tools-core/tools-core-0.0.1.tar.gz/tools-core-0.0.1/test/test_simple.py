...
def test_db():
    from tools_core import get_db_factory, DBACTION
...
def test_redis():
    from tools_core import get_redis_client
    config_map = {
        'host': '52.82.48.136',
        'port': '6389',
        'pass': 'sensedeal'
...
def test_encrypt():
    from tools_core import Encryption, log_info
    key = 'sdai0000'
...
def test_encrypt_dict():
    from tools_core import Encryption
...
def test_mq_fanout_consumer():
    import json
    from tools_core import RabbitConsumer
    def handler(body):
        print(body)
        st = json.loads(body)
        print(st)
...
def test_send_ali_msg():
    from tools_core.send_ali_msg import send_sms
