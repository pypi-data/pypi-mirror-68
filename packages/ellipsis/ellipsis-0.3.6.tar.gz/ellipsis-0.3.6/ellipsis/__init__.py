...
def geometryAdd(mapId, layer, features, token, callsPerMinute = 120, useThreading = False):
    ...
    if useThreading:
        ...
        for i in np.arange(callsPerMinute):
            ...
            tr = threading.Thread(target = upload)
            tr.start()
            trs = trs + [tr]
...
def rasterRaw(mapId, bands, timestamp, tileId = None, xMin = None, xMax = None, yMin = None, yMax = None, useThreading = True, callsPerMinute = 30 , token = None):
