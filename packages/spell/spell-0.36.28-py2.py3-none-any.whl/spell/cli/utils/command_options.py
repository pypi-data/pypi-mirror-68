...
def dependency_params(include_python_version=True, include_conda=True):
    ...
    def dependency_params_inner(f):
        ...
        @click.option(
            "-e", "--env", "envvars", multiple=True, help="Add an environment variable to the run"
        )
        @click.option(
            "--docker-image",
            "--docker_image",
        ...
        if include_python_version:
            wrapper = (
                click.option("--python2", is_flag=True, help="set python version to python 2")
            )(wrapper)
            wrapper = (
                click.option(
                    "--python3", is_flag=True, help="set python version to python 3 (default)"
                )
            )(wrapper)
        if include_conda:
            wrapper = (
...
def workspace_spec_params(f):
    ...
    @click.option("-c", "--commit-ref", default="HEAD", help="Git commit hash to run")
...
def machine_config_params(f):
    ...
    @click.option(
        ...
        " available frameworks see https://spell.run/docs/run_overview#dependency-management-and-customization",
    )
    @click.option(
        "-m",
        "--mount",
