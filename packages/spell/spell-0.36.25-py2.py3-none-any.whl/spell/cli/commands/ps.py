...
@click.option("--raw", is_flag=True, help="Display output in raw format", cls=HiddenOption)
...
@click.option(
    "--include-jupyter", is_flag=True, help="Include jupyter workspace runs", cls=HiddenOption
)
@click.pass_context
def ps(ctx, raw, trunc, repository, labels, verbose, include_jupyter):
    """
    ...
    with api_client_exception_handler():
        ...
        if repository is None:
            runs = client.list_runs(include_jupyter=include_jupyter, labels=labels)
        else:
            ...
            runs = client.list_runs(
                [str(repo.id) for repo in repos], include_jupyter=include_jupyter, labels=labels
            )
    ...
    for run in runs:
        ...
        if run.labels:
            labelsExist = True
            run.labels = ", ".join((l["name"] for l in run.labels))
        else:
            run.labels = ""
        if run.distributed:
