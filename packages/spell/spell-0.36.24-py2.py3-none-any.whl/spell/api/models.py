...
class ModelVersion(Model):
    def __init__(self, id, version, creator, created_at, resource, files, **kwargs):
        self.id = id
...
class ServingGroup(Model):
    ...
    compare_fields = ["id", "name", "config"]

    def __init__(
        self, id, name, accelerators, is_spot, disk_size_gb, min_nodes, max_nodes, config, **kwargs
    ):
        self.id = id
        self.name = name
...
class ModelFileSpec:
    ...
    def __init__(self, resource_path, destination_path):
        self.resource_path = resource_path
