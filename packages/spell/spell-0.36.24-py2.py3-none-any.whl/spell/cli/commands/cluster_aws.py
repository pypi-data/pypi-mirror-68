...
def eks_init(
    ctx, cluster, auth_api_url, eks_cluster_name, nodes_min, nodes_max, node_volume_size, zones
):
    """
    ...
    try:
        ...
        raise ExitException(
            "boto3 and kubernetes are both required. "
            "Please `pip install boto3 kubernetes` and rerun this command"
        )

    if not is_installed("aws-iam-authenticator"):
        raise ExitException(
            "`aws-iam-authenticator` is required, please install it before proceeding"
        )
    ...
    if cluster["cloud_provider"] != "AWS":
        raise ExitException(
            "Input cluster is using cloud provider {} and therefore cannot use EKS. "
            "Only AWS is suppported".format(cluster["cloud_provider"])
        )

    # Setup clients with the provided profile
    aws_profile = click.prompt(
        "We need your AWS profile to get your Access Key ID and Secret "
    ...
    click.confirm(
        "Profile '{}' has Access Key ID '{}' and region '{}' - continue?".format(
    ...
    try:
        with tempfile.NamedTemporaryFile(mode="r", suffix=".yaml") as f:
            ...
            config_yaml = f.read()
        with api_client_exception_handler():
            status_code = spell_client.set_kube_config(cluster["name"], config_yaml)
            if status_code == 202:
                click.echo(
        ...
        raise ExitException("Uploading config to Spell failed. Error was: {}".format(e))
...
def eks_add_nodegroup(config_contents, aws_profile):
    with tempfile.NamedTemporaryFile(suffix=".yaml", mode="w") as f:
        f.write(config_contents)
        f.flush()
        try:
            subprocess.check_call(
                ["eksctl", "create", "nodegroup", "--profile", aws_profile, "--config-file", f.name]
            )
        except Exception as e:
            raise ExitException(str(e))


def eks_scale_nodegroup(session, name, min_nodes, max_nodes):
    ...
    if asg is None:
        raise ExitException("No autoscaling group found for serving group {}".format(name))
...
def delete_eks_cluster(profile):
    if not is_installed("eksctl"):
        if click.confirm(
            ...
            raise ExitException("`eksctl` is required, please install it at https://eksctl.io/")
    click.echo(
        "Detected an associated Model Server EKS Cluster. Listing out the known EKS clusters:"
    )
    try:
        cmd = ["eksctl", "get", "clusters", "--profile", profile]
        subprocess.check_call(cmd)
        if click.confirm(
            "Would you like to delete the associated Model Server EKS cluster "
            "listed above? If you do NOT see it listed above, respond 'n' to skip "
            "this step."
        ):
            eks_name = click.prompt(
                "What is the name of the EKS cluster? "
                "Please choose one of the names listed above. "
                "NOTE: This is the name used in EKS, NOT the name in Spell)",
