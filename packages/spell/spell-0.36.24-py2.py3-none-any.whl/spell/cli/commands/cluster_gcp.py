...
def gke_init(ctx, cluster, auth_api_url, gke_cluster_name, nodes_min, nodes_max, node_disk_size):
    ...
    try:
        ...
        raise ExitException(
            "kubernetes (for Python) is required. "
            "Please `pip install --upgrade 'spell[cluster-gcp]'` and rerun this command"
        )
    ...
    if cluster["cloud_provider"] != "GCP":
        raise ExitException(
            "Input cluster is using cloud provider {} and therefore cannot use GKE. "
    ...
    response = click.prompt(
        "Create a GKE cluster for model serving? "
    ...
    if response == "create":
        create_gke_cluster(
            gke_cluster_name,
            gcp_project_name,
            role_creds_gcp["service_account_id"],
            zone,
            nodes_min,
            nodes_max,
            node_disk_size,
        ...
        correct_kube_ctx = click.confirm(
            "Is context '{}' the GKE cluster to use for model serving?".format(kube_ctx)
        )
        if not correct_kube_ctx:
            raise ExitException(
                "Set context to correct GKE cluster with `kubectl config use-context`"
            )
    ...
    try:
        ...
        with api_client_exception_handler():
            status_code = spell_client.set_kube_config(cluster["name"], yaml_str)
            if status_code == 202:
                click.echo(
                    "Config successfully uploaded to Spell, "
                    "please wait a few minutes for DNS to be processed."
                )
            else:
...
def gke_scale_nodepool(credentials, serving_group, name, min_nodes, max_nodes):
    from googleapiclient import discovery
    ...
    name_path = "{}/nodePools/{}".format(config["parent"], name)
    autoscaling_cfg = {
        "autoscaling": {"enabled": True, "minNodeCount": min_nodes, "maxNodeCount": max_nodes},
    }
...
def gke_add_nodepool(config_contents):
    import google.auth
    from googleapiclient import discovery
    from googleapiclient.errors import HttpError
...
def check_import_and_authenticate():
    ...
    try:
        ...
        try:
            g_api_version = pkg_resources.get_distribution("google-api-core").version
        except pkg_resources.DistributionNotFound:
            click.echo("Couldn't find python package 'google-api-core', required by spell.")
            click.echo(
                "Try re-running `pip install --upgrade 'spell[cluster-gcp]'`, or contact spell support."
            )
            return False
        if version.parse(g_api_version) > version.parse("1.15"):
            click.echo("Invalid version of package 'google-api-core'.")
            click.echo(
                "Please `pip install --upgrade 'spell[cluster-gcp]'` and rerun this command."
            )
            return False
...
def add_gs_bucket(ctx, bucket_name, cluster):
    ...
    if bucket_name not in bucket_names:
        ...
        if not is_gs_bucket_public(bucket_name):
            click.echo(
                "Bucket {} is not publicly accessible and is not part of project {}".format(
...
def delete_gc_registry(ctx, cluster):
    ...
    if service_account_tag not in policy["roles/storage.objectViewer"]:
        click.echo("Spell had to access to registry, no IAM updates required.")
        return
...
def update_gcp_cluster(ctx, cluster):
    ...
    current_version = cluster[u"version"]
    if current_version < 5:
        click.echo("Ensuring firewall rules are up to date...")

        compute_service = discovery.build("compute", "v1", credentials=credentials)

        region = cluster[u"networking"][u"gcp"][u"region"]
        subnet_name = cluster[u"networking"][u"gcp"][u"subnet"]
        request = compute_service.subnetworks().get(
            project=project_id, region=region, subnetwork=subnet_name
        )
        subnet = request.execute()

        cidr = subnet[u"ipCidrRange"]
        network_url = subnet[u"network"]
        rule_name = "{}-internal".format(cluster[u"name"])
        body = {
            "name": rule_name,
            "description": "Allow traffic between all instances within VPC",
            "network": network_url,
...
def delete_gcp_cluster(ctx, cluster):
    ...
    if cluster["has_kube_config"]:
        delete_gke_cluster(project_id)
...
def delete_gke_cluster(project_id):
    try:
        cluster_utils.echo_delimiter()
        click.echo(
            "Detected an associated Model Server GKE Cluster. Listing out the known GKE clusters:"
        )
        cmd = ["gcloud", "container", "clusters", "list", "--project", project_id]
        subprocess.check_call(cmd)
        if click.confirm(
            "Would you like to delete the associated Model Server GKE cluster "
            "listed above? If you do NOT see it listed above, respond 'n' to skip "
            "this step."
        ):
            gke_name = click.prompt(
                "What is the name of the GKE cluster? "
                "Please choose one of the names listed above. "
                "NOTE: This is the name used in GKE, NOT the name in Spell)",
                default=u"spell-model-serving",
            ).strip()
            click.echo("Deleting GKE cluster {}. This can take a while!".format(gke_name))
            cmd = ["gcloud", "container", "clusters", "delete", gke_name, "--project", project_id]
            subprocess.check_call(cmd)
    except subprocess.CalledProcessError:
        click.echo(
            "Failed to run `gcloud`. Make sure it's installed correctly and "
