...
def txt3image(txt,filename=None,fontfile='msyh.ttf',fontsize=18,color=(25,0,255,255)):
    """多行文本转换成图像的命令
    txt：待转换所多行文本
    filename：输出文件名
    fontfile：ttf字体文件
    ...
    for line in lines:               # 文件中的每一行
      info = line
      if info =='':info="\n"
      im = txt2image(info,fontfile=fontfile,fontsize=fontsize,color=color)
      images.append(im)
...
def txt4image(txtfile=None,filename=None,fontfile="msyh.ttf",fontsize=18,color=(25,0,255,255)):
    """把文本文件转换成图像的函数
    ...
    try:
      ...
      f.close()      
    if filename==None:
       return txt3image(c,fontfile=fontfile,fontsize=fontsize,color=color)
    else:
       size = txt3image(c,filename=filename,fontfile=fontfile,fontsize=fontsize,color=color)
       return size
...
class Sprite(Turtle):
    ...
    def coloroverlap(self,color1,color2,sps=[],threshold=127):
        ...
        """        
        if sps == []:sps = self.find_overlapping_sprites()
        collideflag = False
        for other in sps:              
            bbox1 = self.bbox()                          # 主碰方的绑定盒
            bbox2 = other.bbox()                         # 被碰方的绑定盒
    ...
    def gotorandom(self,left=None,right=None,bottom=None,top=None):
        """到随机坐标,可以包括范围
           left：最左x坐标
           right：最右x坐标
           bottom：最下y坐标
           top：最上y坐标
        """
        if left==None or right==None or bottom==None or top==None:
    ...
    def clone(self):
        ...
        q.set_tag(q._tag)
        q._update()
##        print(q.turtle._item,q.currentLineItem,q.drawingLineItem,q.items)
##        qq = q._draw_bubble_turtle
##        print(qq.turtle._item,qq.currentLineItem,qq.drawingLineItem,qq.items)
##        
        return q
    ...
    def find_overlapping_sprites(self,exclude=None):
         ...
         #spsitems = [sp.turtle._item for sp in sps]                  # 所有角色的项目编号
...
##           支持保存为PGM,PPM,GIF,PNG图像文件格式。这个保存的不支持反锯齿，
##        """
##        key = self.shape()
##        photo = self.screen._shapes[key]
##        photo.save(imgname,x,y) 
    def saveshape(self,imgname,x=1,y=1):
        """imgname：图像文件名，x：横向缩放系数，y：纵向缩放系数
           本方法保存角色当前造型图片,只支持image类型角色,并且只能放大。
...
if __name__ == "__main__":  
  ...
  screen = Screen()
  screen.bgcolor("#003278")
  redsquare = Image.new(size=(100,100),
                        mode="RGBA",
                        color=(255,0,0,255))
  redsquare.save('redsquare.png')
  
  redsquare = Sprite("redsquare.png")
  bug = Sprite()
  bug.left(45)
  bug.saveshape('c:/bug.png',4,4)
  
  while True:
      redsquare.goto(mouse_pos())
      r = redsquare.pixelcollide(bug)
      if r:
          screen.title(r)
      else:
          screen.title('')
