...
class ScalarMetric(InlineMetric):
    ...
    def create_v2_dto(self):
        converted_value = Metric.get_converted_value(self.value)
        value_type = type(converted_value)
        if value_type in self._type_to_metric_type:
            value_metric_type = self._type_to_metric_type[value_type]
            if value_metric_type == AZUREML_FLOAT_METRIC_TYPE:
                value_metric_type = AZUREML_DOUBLE_METRIC_TYPE
        else:
            raise NotImplementedError()
