...
class Dataset(_AbstractRunConfigElement):
    """Represents a reference to a saved dataset which will be used in the run.
    ...
    def _validate(self):
        if not self.id:
            raise UserErrorException("The Dataset must be saved, which means it has an id.")
    ...
    def create(data):
        ...
        if isinstance(data, DatasetConsumptionConfig):
            if not data.dataset.id:
                raise RuntimeError("Dataset can only be used in experiments where an Azure Machine Learning workspace "
                                   "is present")
