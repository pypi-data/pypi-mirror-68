...
class AmlCompute(ComputeTarget):
    ...
    :param name: The name of the of the AmlCompute object to retrieve.
    :type name: str
    """
    ...
    def provisioning_configuration(vm_size='', vm_priority="dedicated", min_nodes=0,
                                   ...
                                   identity_type=None, identity_id=None):
        ...
        :param identity_type: SystemAssigned or UserAssigned.
    ...
    def add_identity(self, identity_type, identity_id=None):
        ...
        if identity_id and identity_type != "UserAssigned":
            raise ComputeTargetException('Invalid configuration, identity_id is mandatory only when specifying '
                                         'user assigned identity.')
        elif identity_id is None and identity_type == "UserAssigned":
            raise ComputeTargetException('Invalid configuration, identity_id is mandatory when specifying '
                                         'user assigned identity.')
        elif identity_id and identity_type == "UserAssigned":
    ...
    @staticmethod
    def list_usages(workspace, show_all=False):
    ...
    @staticmethod
    def update_quotas(workspace, vm_family, limit=None):
...
class AmlComputeProvisioningConfiguration(ComputeTargetProvisioningConfiguration):
    ...
    def validate_configuration(self):
        ...
        if self.identity_type:
            if self.identity_type not in ["UserAssigned", "SystemAssigned"]:
                raise ComputeTargetException("Invalid configuration, identity must be set to "
                                             "'UserAssigned' or 'SystemAssigned'.")
            if self.identity_type == "UserAssigned" and not self.identity_id:
                raise ComputeTargetException('Invalid configuration, identity_id is mandatory when specifying '
                                             'user assigned identity flag.')
            elif self.identity_type != "UserAssigned" and self.identity_id:
                self.identity_id = None
                print('identity_id field is not required and will be ignored '
                      'since using a system assigned identity.')
