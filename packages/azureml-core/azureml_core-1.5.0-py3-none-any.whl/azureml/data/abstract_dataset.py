...
class AbstractDataset(object):
    ...
    def __init__(self):
        ...
        if self.__class__ == AbstractDataset:
            raise RuntimeError('Cannot create instance of absctract class AbstractDataset')
    ...
    def get_by_name(workspace, name, version='latest'):
        ...
        :rtype: azureml.data.TabularDataset | azureml.data.FileDataset
        """
    ...
    def get_all(workspace):
        ...
        :rtype: dict[str, azureml.data.TabularDataset | azureml.data.FileDataset]
        """
    ...
    def _dataflow(self):
        ...
        if not isinstance(self._definition, dataprep().Dataflow):
            try:
            ...
            except dataprep().api.errorhandlers.UnexpectedError as e:
                ...
                raise RuntimeError('{}. Please install the latest version with "pip install -U '
    ...
    def _create(cls, definition, properties=None, registration=None, telemetry_info=None):
        ...
        if telemetry_info is not None and not isinstance(telemetry_info, _DatasetTelemetryInfo):
            raise TypeError('telemetry_info must be instance of `_DatasetTelemetryInfo`')
        dataset = cls()
