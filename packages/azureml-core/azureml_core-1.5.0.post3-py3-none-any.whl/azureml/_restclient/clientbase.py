...
from .constants import (ATTRIBUTE_CONTINUATION_TOKEN_NAME, ATTRIBUTE_VALUE_NAME,
                        ...
                        QUERY_SKIP_TOKEN, ARG_SKIP_TOKEN,
                        RequestHeaders)
...
class ClientBase(ChainedIdentity):
    ...
    def _call_paginated_api(self, func, *args, **kwargs):
        ...
        while (True):
            ...
            token = None
            token_name = None
            # Paginated DTOs may have a continuation or next link
            if hasattr(paginated_dto, ATTRIBUTE_CONTINUATION_TOKEN_NAME):
                module_logger.info("Found {0} field in DTO".format(ATTRIBUTE_CONTINUATION_TOKEN_NAME))
            ...
            elif hasattr(paginated_dto, ATTRIBUTE_NEXT_LINK_NAME):
                # DTO has a 'next_link'
                module_logger.info("Found {0} field in DTO".format(ATTRIBUTE_NEXT_LINK_NAME))
                ...
                if QUERY_SKIP_TOKEN in parsed_query:
                    module_logger.info("Found {0} in query".format(QUERY_SKIP_TOKEN))
