...
class DataStorageType(FireStoreBase):
    ...
    def get_field(self, field_name):
        ...
        '''
        self._ext_storage_fields[field_name]
    ...
    def get_dataStorageType(cls, storageName):
        docRef = DataStorageType.get_firestore_client().document(
            DataStorageType.basePath()+'/'+storageName)
        return DataStorageType.getInstance(docRef)
    ...
    def save_or_update(cls, update_data):
        ...
        for datum in update_data.get('data', []):
            ...
            if fldName is None:
                logger.debug("skipping field due to missing field name")
            else:
                upDatum = {}
                for k in datum.keys():
...
class StorageField(object):
    ...
    def check_boolean(self, inValue):
        ...
        if (inValue is None or inValue.strip() == ""):
        ...
        if (inValue is not None and len(str(inValue)) > 0):
            if inValue.lower() == 'n/a': # saying that this field is not applicable
                return True

            if (inValue == True or inValue == False or inValue.lower() == "true" or
                    inValue.lower() == "false" or inValue == 1 or inValue == 0 or inValue == "1" or inValue == "0"):
                return True
            else:
                return False
        return False
