...
class StorageBlob(FireStoreBase):
    ...
    CPI = 'cust_plant_item.*.{"Customer_Name": customer_name, "Customer_Id": customer_id, "Location_Name": location_name, "Location_Id": location_id, "Type":`Customer`,"Product_Name":Product_Name,"Item_Num":Cust_Item_Num,"plant_image":plant_image[0].image_url}'
    PI = 'plant_item.*.{"Customer_Name": `master`, "Customer_Id": ``, "Location_Name": `master`, "Location_Id":   ``, "Type":`Master`,"Product_Name":Product_Name,"Item_Num":Item_Num,"plant_image":plant_image[0].image_url}'
    ...
    def get_all_plant_items(cls,use_cache=True):
        # TODO implement a call to cache..
        #resp = memcache.get(ALL_PLANT_ITEMS)
        #resp = QuickStorage.getValue(ALL_PLANT_ITEMS)
        #if resp and use_cache:
            #return StorageBlob.render_array_response(resp)
        #    return resp

        logging.info("Cached results not found for plant info... pulling data.. (USE CACHE: "+str(use_cache)+")")
        resp = {'plant_item':{},'cust_plant_item':{}}

        t = 'plant_item'
        #pi = StorageBlob.query(StorageBlob.data_type == t)
        pi = StorageBlob.get_firestore_client().collection_group(t)
        for sbSnap in pi.stream():
            sb = StorageBlob.getInstance(sbSnap)
            resp[t][sb.data_number_lookup] = sb.get_plant_item_data()
        ...
        #memcache.set(ALL_PLANT_ITEMS, resp, DEFAULT_CACHE_TIME * 24)
        #QuickStorage.setValue(ALL_PLANT_ITEMS,resp)
        # TODO implement some kind of caching render_resp = StorageBlob.set_memcache_pi_values(resp)

        return StorageBlob.__render_array_response(resp) 
    ...
    def get_plant_item(cls, item_number):
        ...
        doc = dnl.get()
        if doc.exists:
            sbRef = StorageBlob.get_firestore_client().document(doc.get('path'))
            sb = StorageBlob.getInstance(sbRef)
        if not sb:
            return None
    ...
    def get_plant_item_data(self):
        ...
        image_details = []
        if pi_images:
            for image in pi_images:
                if image:
                    image_detail = Image.getImageInfo(image)
                    image_details.append(image_detail)

        pi_data['plant_image'] = image_details
        par = self.get_parent()
    ...
    def query_with_jmespath(cls,storeName, jmPath, selectField, includeId):
        ...
        '''
        jmKey = storeName+jmPath
        qHits = QuickStorage.getValue(jmKey)
        ...
        QuickStorage.setValue(jmKey,returnOptions,360)
