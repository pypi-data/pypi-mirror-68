...
class Utils(object):
	...
	def count_file_code(cls, scan_info, build_dir):
		...
		try:
			...
			command_line = "/scan --json --out /scan/sken-cloc.json  --exclude-dir node_modules,target,dist,build,node,.git,.hg --skip-archive '(zip|tar|jar|war(.(gz|Z|bz2|xz|7z))?)' --exclude-ext json,xml,svg,md"
