...
class Tags(Plugin):
    ...
    def __init__(self, daemon):
        ...
        self.daemon = daemon
        if self.plugin_conf.keys().isdisjoint(['genre']):
            self.log.error('Found no config for %s plugin!' % self)
            return
    ...
    def _callback_need_track(self):
        trks = list()
        target = self.plugin_conf.getint('tracks_to_add')
        limit = 0
    ...
    def get_trk(self):
        """Get a single track according to filter
        """
        trk = None
        art = None
        # get list of artists
        artists = list(self.player._client.find....)
        if self.mode == 'sensible':
            ...
            while artists:
                ...
                if self.daemon.sdb.get_bl_artist(art, add_not=True):
                    self.log.debug('Tags: Blacklisted "%s"', art)
                    continue
        ...
        if art is None:
            return None
        self.log.debug('Tags art: {}'.format(art))
