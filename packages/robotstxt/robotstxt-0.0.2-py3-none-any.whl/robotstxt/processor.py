...
class RobotsFile:
    def __init__(self, contents):
        ...
        sitemap_list.sort()
        self.sitemaps_hash = hash_generator(sitemap_list)
    ...
    def test_url(self, url, token):
        ...
        if selected_block:
            ...
            for rule in self.rule_blocks[selected_block]:
                if re.search(unquote(rule[3]), url_path_to_test):
                    if rule[0] == 'Disallow':
                        return {'results': 'disallowed', 'matching_rule': rule[2]}

                    else:
                            return {'results': 'allowed', 'matching_rule': rule[2]}

            else:
                return {'results': 'allowed', 'matching_rule': None}
        else:
            return {'results': 'allowed', 'matching_rule': None}
