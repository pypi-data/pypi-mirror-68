...
from pyjamas.rimage.rimclassifier.rimnn import LeNet
from pyjamas.rimage.rimclassifier.nn.cnn import Network
...
class RCBIO(RCallback):
    ...
    def cbLoadAnnotations(self, filename: str = None, image_file: str = None) -> bool:  # Handle IO errors
        """
        Load a PyJAMAS annotation file.

        :param filename: path to the file containing the annotations.
    ...
    def cbSaveTimeSeries(self, filename: str = None) -> bool:  # function cancel = cbLoadTimeSeries(fig, filename, in)
        '''
    ...
    def cbSaveROI(self, filename: str = None, x_range: Tuple[int, int] = False, y_range: Tuple[int, int] = False, z_range: Tuple[int, int] = False) -> bool:
        ...
        :param filename: '' for automated naming based on coordinates. If None, a dialog will open.
        ...
        if filename is None or filename is False:
            ...
            filename = os.path.join(self.pjs.cwd, fname + '_X' + str(x_range[0]) + '_' + str(x_range[1]-1) + '_Y' + str(y_range[0]) + \
    ...
    def cbSaveAnnotationsXML(self, filename: str = None) -> bool:  # Handle IO errors.
        """
        Save annotations in XML format. These annotations CANNOT be opened with PyJAMAS.
    ...
    def cbSaveClassifier(self, filename: str = None, theclassifier: rimclassifier = None) -> bool:  # Handle IO errors.
        ...
        if filename[-4:] != rimclassifier.CLASSIFIER_EXTENSION:
            filename = RUtils.set_extension(filename, rimclassifier.CLASSIFIER_EXTENSION)
    ...
    def cbLoadClassifier(self, filename: str = None) -> bool:  # Handle IO errors
        ...
        elif type(theclassifier) is Network:
            self.pjs.batch_classifier.image_classifier = LeNet(theparameters)
        else:
    ...
    def cbExportAllPolylineAnnotations(self, folder_name: str = None) -> bool:  # Handle IO errors.
        """
    ...
    def cbImportSIESTAAnnotations(self, filename: str = None, image_file: str = None) -> bool:
        ...
        :param filename: path to the file that will contain the Matlab-based annotations.
        ...
        for z, slice_polylines in enumerate(polyline_list):
            for apolyline in slice_polylines:
                ...
                if len(apolyline[0]) > 1:
                    ...
                    self.pjs.addPolyline(apolyline_list.append([apolyline_list[0][0], apolyline_list[0][1]]), z)
