...
def remote_log_piper(node_number, level, host, port=22, user=None, path=None, event=None, trigger=None):
    ...
    dest_file = (path / r'vilya_logs' / (os.uname().nodename +
    ...
    remote_writer = Popen(["ssh", "-p", str(port), authority, "split -d -a 3 -l 100000", "-", dest_file], stdin=PIPE)# , stdout=PIPE)
...
def remote_log_piper_daemon(node_number):
    ...
    level = '*:DEBUG'
    host = 'ns331615.ip-37-187-122.eu'
    port = 22
    user = 'vilya'
    path = '/home/vilya'
...
################ START WORKING ################
    ...
    path = pathlib.Path(path)
    authority = '@'.join((user, host))
    dest_file = (path / r'vilya_records' / (os.uname().nodename +
                                            '_%i_records_' % int(time.time()))
                 ).as_posix()
    remote_writer = Popen(["ssh", "-p", str(port), authority, "split -d -a 3 -l 10000", "-", dest_file], stdin=PIPE)# , stdout=PIPE)
    sink_pipe = remote_writer.stdin
