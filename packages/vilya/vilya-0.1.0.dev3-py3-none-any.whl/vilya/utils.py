...
NA = 'n/a'
# NNA = NumberNotAvailable()  # 'not a number' placeholder
# SNA = StringNotAvailable()  # 'not available' placeholder

def timestamp(unix_time=None, add_usec=True):
    unix_time = unix_time or time.time()
    hh_mm_ss = '%s' % time.strftime('%H:%M:%S', time.gmtime(unix_time))
...
def elrond_path(check_exists=True):
    """
...
def number_of_nodes(check_exist=False):
    """
...
def node_path(node_number=0, check_exists=False):
    """
...
def node_binary(node_number=0, check_exists=False):
    """
...
def logviewer_binary(node_number=0, check_exists=False):
    """
    Returns the expected pathlib.Path to the node binary.
...
def config_dir(node_number=0, check_exists=False):
    """
