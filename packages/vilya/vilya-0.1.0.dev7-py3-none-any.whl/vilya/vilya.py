...
def remote_log_piper(node_number, level, host, port=22, user=None, path=None, event=None, trigger=None):
    # (name, pipe, output_queue, trigger_queue, trigger_func=None):
    # t = threading.Thread(target=enqueue_from_pipe, args=( '1', output_queue, waiting_queue,p.stdout, trigger_func))
    ...
    remote_writer = Popen(["ssh", "-p", str(port), authority, "split -d -a 6 -l 100000", "-", dest_file], stdin=PIPE)# at some point we'll run out of numbers and split errors out, but before that point Iøll have found a smarter way
...
def remote_log_piper_daemon(node_number):
    ...
    level = '*:DEBUG'
    host = 'ns347823.ip-91-121-134.eu'# 'ns331615.ip-37-187-122.eu'
    port = 22
    user = 'vilya'
...
def remote_record_writer(record, sink_pipe, sink_pipe_lock):# , host, port=22, user=None, path=None):# , output_queue=None):
    #TODO: get the remote_writer process as a parameter to share it with the other records
    # and also a lock to use when writing to the sink_pipe
    ...
    while 'forever and ever':
        ...
        output_csv = rec(rec.measure_period)
        # print(output_csv)
...
def remote_record_writer_daemon(record, sink_pipe, sink_pipe_lock):
    ...
    t = threading.Thread(target=remote_record_writer, args=args)# , host, port, user, path))# , output_queue))
    t.daemon = True
...
################ START WORKING ################
    ...
    def resurrect_logger(node_number):
        # FIXME: the remote file is overwritten, either find a way or add changing info
        # to the filename to make them unique
    ...
    remote_writer = Popen(["ssh", "-p", str(port), authority, "split -d -a 6 -l 10000", "-", dest_file], stdin=PIPE)# at some point we'll run out of numbers and split errors out, but before that point Iøll have found a smarter way
    sink_pipe = remote_writer.stdin
