...
* Complete new look on covidindia server to make the experience better.
* You can download data as json or csv from local server.
* New dashboard added that shows important statistics for Covid-19 in India.
* Now user can also get data on No. of covid-19 test count for every states.

## Note

There might be some issues to start the server for most probably Mac users.

     Files can't be made due to some error.

if above line is seen in the terminal please let me know by opening an issue in [github](https://github.com/kalyaniuniversity/covidindia).If this happens server will start normally but download function will not work.
