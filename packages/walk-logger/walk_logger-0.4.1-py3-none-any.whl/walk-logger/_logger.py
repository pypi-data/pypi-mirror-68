...
context = ContextVar("WALK-LOGGER_context", default={})
...
class Core:
    def __init__(self):
        levels = [
            Level(
                "TRACE",
                _defaults.WALK-LOGGER_TRACE_NO,
                _defaults.WALK-LOGGER_TRACE_COLOR,
                _defaults.WALK-LOGGER_TRACE_ICON,
            ),
            Level(
                "DEBUG",
                _defaults.WALK-LOGGER_DEBUG_NO,
                _defaults.WALK-LOGGER_DEBUG_COLOR,
                _defaults.WALK-LOGGER_DEBUG_ICON,
            ),
            Level(
                "INFO",
                _defaults.WALK-LOGGER_INFO_NO,
                _defaults.WALK-LOGGER_INFO_COLOR,
                _defaults.WALK-LOGGER_INFO_ICON,
            ),
            Level(
                "SUCCESS",
                _defaults.WALK-LOGGER_SUCCESS_NO,
                _defaults.WALK-LOGGER_SUCCESS_COLOR,
                _defaults.WALK-LOGGER_SUCCESS_ICON,
            ),
            Level(
                "WARNING",
                _defaults.WALK-LOGGER_WARNING_NO,
                _defaults.WALK-LOGGER_WARNING_COLOR,
                _defaults.WALK-LOGGER_WARNING_ICON,
            ),
            Level(
                "ERROR",
                _defaults.WALK-LOGGER_ERROR_NO,
                _defaults.WALK-LOGGER_ERROR_COLOR,
                _defaults.WALK-LOGGER_ERROR_ICON,
            ),
            Level(
                "CRITICAL",
                _defaults.WALK-LOGGER_CRITICAL_NO,
                _defaults.WALK-LOGGER_CRITICAL_COLOR,
                _defaults.WALK-LOGGER_CRITICAL_ICON,
            ),
        ]
...
class Logger:
    ...
    The |Logger| is the core object of ``WALK-LOGGER``, every logging configuration and usage pass
    ...
    You should not instantiate a |Logger| by yourself, use ``from WALK-LOGGER import logger`` instead.
    """
    ...
    def __repr__(self):
        return "<WALK-LOGGER.logger handlers=%r>" % list(self._core.handlers.values())

    def add(
        self,
        sink,
        *,
        level=_defaults.WALK-LOGGER_LEVEL,
        format=_defaults.WALK-LOGGER_FORMAT,
        filter=_defaults.WALK-LOGGER_FILTER,
        colorize=_defaults.WALK-LOGGER_COLORIZE,
        serialize=_defaults.WALK-LOGGER_SERIALIZE,
        backtrace=_defaults.WALK-LOGGER_BACKTRACE,
        diagnose=_defaults.WALK-LOGGER_DIAGNOSE,
        enqueue=_defaults.WALK-LOGGER_ENQUEUE,
        catch=_defaults.WALK-LOGGER_CATCH,
        **kwargs
    ):
        ...
        **kwargs
        ...
        - A asynchronous |coroutine function|_ defined with the ``async def`` statement. The
          ...
          |complete|.
        - A built-in |Handler| like ``logging.StreamHandler``. In such a case, the `WALK-LOGGER` records
        ...
        Each of the |add| default parameter can be modified by setting the ``WALK-LOGGER_[PARAM]``
        environment variable. For example on Linux: ``export WALK-LOGGER_FORMAT="{time} - {message}"``
        or ``export WALK-LOGGER_DIAGNOSE=NO``.

        The default levels' attributes can also be modified by setting the ``WALK-LOGGER_[LEVEL]_[ATTR]``
        environment variable. For example, on Windows: ``setx WALK-LOGGER_DEBUG_COLOR "<blue>"``
        or ``setx WALK-LOGGER_TRACE_ICON "🚀"``. If you use the ``set`` command, do not include quotes
        but escape special symbol as needed, e.g. ``set WALK-LOGGER_DEBUG_COLOR=^<blue^>``.

        If you want to disable the pre-configured sink, you can set the ``WALK-LOGGER_AUTOINIT``
        variable to ``False``.
    ...
    def opt(
        ...
        *,
        ...
        depth : |int|, optional
            ...
            information.
        ansi : |bool|, optional
            Deprecated since version 0.4.1: the ``ansi`` parameter will be removed in WALK-LOGGER 1.0.0,
    ...
    def patch(self, patcher):
        ...
        ``record`` dict itself, as some values are used internally by WALK-LOGGER, and modify them may
    ...
    def disable(self, name):
        ...
        Developers of library using `WALK-LOGGER` should absolutely disable it to avoid disrupting
        users with unrelated logs messages.

        Note that in some rare circumstances, it is not possible for `WALK-LOGGER` to
    ...
    def enable(self, name):
        ...
        Logging is generally disabled by imported library using `WALK-LOGGER`, hence this function
    ...
    def start(self, *args, **kwargs):
        ...
        --------
        .. deprecated:: 0.2.2
          ``start()`` will be removed in WALK-LOGGER 1.0.0, it is replaced by ``add()`` which is a less
          confusing name.
        """
        warnings.warn(
    ...
    def stop(self, *args, **kwargs):
        ...
        --------
        .. deprecated:: 0.2.2
          ``stop()`` will be removed in WALK-LOGGER 1.0.0, it is replaced by ``remove()`` which is a less
          confusing name.
        """
        warnings.warn(
