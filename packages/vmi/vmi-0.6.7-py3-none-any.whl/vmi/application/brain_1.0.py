...
def LeftButtonPressMove(**kwargs):
    ...
    if kwargs['picked'] is original_slice:
        ...
        if voi_view_textboxes[1].text() == '请勾画切割范围':
            ...
            return True
        elif voi_view_textboxes[3].text() == '请勾画载流动脉':
            pt = voi_view.pickPt_FocalPlane()
        ...
        if voi_view_textboxes[5].text() == '正在测量载流动脉':
            ...
            # print(vessel_centers[index[0][0]])
...
def LeftButtonPressMoveRelease(**kwargs):
    ...
    if kwargs['picked'] is original_slice:
        ...
        if voi_view_textboxes[1].text() == '请勾画切割范围':
            ...
            voi_view_textboxes[3].draw_text('勾画载流动脉')
            ...
            # polygon.GetPointIDs.Se
            # para_dict['tumor_area'] = np.round(tumor_area/6,2)
...
def LeftButtonPressRelease(**kwargs):
    ...
    global pipe_props, pipe_pts, brace_pts, brace_props, \
        ...
        line, line_pts, line_props, line_measure, radius_arr, section_pts, section_prop
            ...
            if voi_view_textboxes[4].text() == '正在测量动脉瘤':
                if len(tumor_f_l_props) < 3:
                    ...
                    if first_pt != [0, 0, 0] and end_pt != [0, 0, 0]:
                        ...
                        update_para_text_1_2()
                    return True
            elif voi_view_textboxes[5].text() == '正在测量载流动脉':
                pt = voi_view.pickPt_FocalPlane()
                look = voi_view.cameraVt_Look()
                pt_f = pt - 2000 * look
                pt_b = pt + 2000 * look
    ...
    # 勾画载流动脉
        ...
        if voi_view_textboxes[3].text() == '勾画载流动脉':
            voi_view_textboxes[3].draw_text('请勾画载流动脉')
            voi_view_textboxes[1].draw_text('线切割')
            voi_view_textboxes[2].draw_text('勾画动脉瘤')
            return True
        elif voi_view_textboxes[3].text() == '请勾画载流动脉':
    ...
    # 测量动脉瘤
        ...
        if voi_view_textboxes[4].text() == '测量动脉瘤':
            ...
            voi_view_textboxes[5].draw_text('测量载流动脉')
    ...
    # 测量载流动脉
        ...
        if voi_view_textboxes[5].text() == '测量载流动脉':
            voi_view_textboxes[5].draw_text('正在测量载流动脉')
            voi_view_textboxes[4].draw_text('测量动脉瘤')
            return True
        elif voi_view_textboxes[5].text() == '正在测量载流动脉':
    ...
    # 选择导管路径
        ...
        if voi_view_textboxes[6].text() == '路径规划':
            pipe_pts, pipe_props, pipe_wires, pipe_wires_props = [], [], [], []
    ...
    # 选择支撑路径
        ...
        if voi_view_textboxes[8].text() == '截面测量':
            section_pts = []
            section_prop = vmi.PolyActor(voi_view, color=[1, 0.4, 0.4], line_width=3)
        ...
        if voi_view_textboxes[9].text() == '长度测量':
            ...
            line_measure = vmi.PolyActor(voi_view, color=[0.2, 1, 0.2], line_width=3)
            make_centers_on_top()
            voi_view_textboxes[10].draw_text('选择角一')
            voi_view_textboxes[6].draw_text('路径规划')
            voi_view_textboxes[7].draw_text('支撑设计')
        ...
        if voi_view_textboxes[10].text() == '选择角一':
            ...
            if len(angle_1_pts) == 3:
                ...
                update_para_text_1_2()
            voi_view_textboxes[10].draw_text('选择角一')
            for i in angle_1_props:
                i.setVisible(False)
            angle_1_line_1.setVisible(False)
            angle_1_line_2.setVisible(False)
        return True
        ...
        if voi_view_textboxes[11].text() == '选择角二顶点':
            ...
            if len(angle_2_pts) == 3:
                ...
                update_para_text_1_2()
            voi_view_textboxes[11].draw_text('选择角二')
            for i in angle_2_props:
                i.setVisible(False)
            angle_2_line_1.setVisible(False)
            angle_2_line_2.setVisible(False)
            return True
    ...
    # 隐藏载流动脉
        ...
        if voi_view_textboxes[14].text() == '隐藏载流动脉':
            vessel_pd.setVisible(False)
            voi_view_textboxes[14].draw_text('显示载流动脉')
            return True

        else:
            vessel_pd.setVisible(True)
            voi_view_textboxes[14].draw_text('隐藏载流动脉')
            return True
...
if __name__ == '__main__':
    ...
    voi_view_textboxes = [vmi.TextBox(voi_view, text='阈值：{} HU'.format(threshold),
                          ...
                          vmi.TextBox(voi_view, text='勾画动脉瘤'.format(threshold),
                                      ...
                                      pickable=True),
                          vmi.TextBox(voi_view, text='勾画载流动脉'.format(threshold),
                          ...
                          vmi.TextBox(voi_view, text='测量动脉瘤'.format(threshold),
                                      ...
                                      pickable=True),
                          vmi.TextBox(voi_view, text='测量载流动脉'.format(threshold),
                          ...
                          vmi.TextBox(voi_view, text='隐藏动脉瘤',
                                      ...
                                      pickable=True),
                          vmi.TextBox(voi_view, text='隐藏载流动脉',
                          ...
                          vmi.TextBox(voi_view, text='弹簧圈直径:{}in'.format(pring_coil),
    ...
    # 中心点和中心线 载瘤动脉和动脉瘤
    vessel_f_l, vessel_f_l_props, vessel_line_props, vessel_centers, vessel_center_props = [], [], [], [], []
    ...
    angle_1_line_1 = vmi.PolyActor(voi_view, color=[0.2, 1, 0.2], line_width=3)
    angle_1_line_2 = vmi.PolyActor(voi_view, color=[0.2, 1, 0.2], line_width=3)

    angle_2, angle_2_pts, angle_2_props = 0, [], []
    angle_2_line_1 = vmi.PolyActor(voi_view, color=[0.2, 1, 0.2], line_width=3)
    angle_2_line_2 = vmi.PolyActor(voi_view, color=[0.2, 1, 0.2], line_width=3)
