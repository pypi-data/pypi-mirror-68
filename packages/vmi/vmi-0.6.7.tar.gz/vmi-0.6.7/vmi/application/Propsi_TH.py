...
def case_snapshot(video=False):
    ...
    with tempfile.TemporaryDirectory() as p:
        ...
        pelvis_view.setCamera_FPlane(pelvis_APP_camera_cs, 300)
        if video:
        ...
        pelvis_view.setCamera_FPlane(pelvis_MPP_camera_cs, 300)
        if video:
            pelvis_view.camera().Roll(-LR * 15)
            pelvis_view.camera().OrthogonalizeViewUp()
            for _ in range(15):
                ...
                origin = pelvis_view.pickPt_FocalPlane([0.5 * pelvis_view.width(), 0.5 * pelvis_view.height()])
                cs = vmi.CS4x4().reflect(pelvis_view.cameraCS().axis(0), origin)
                pd = vmi.pdMatrix(pelvis_prop.data(), cs.matrix4x4())
                pelvis_MPP_prop.setData(pd)
...
def LeftButtonPressRelease(**kwargs):
    ...
    if kwargs['picked'] is dicom_box:
        ...
        if pelvis_MPP_box.text() == '标记对称面':
            ...
            pelvis_MPP_camera_cs = pelvis_view.cameraCS()
            pelvis_MPP_prop.setVisible(False)
            pelvis_cs_indicator_prop.setVisible(False)
        ...
        if pelvis_APP_box.text() == '标记前平面':
            ...
            pelvis_APP_camera_cs = pelvis_view.cameraCS()
            pelvis_cs_indicator_prop.setVisible(False)
        ...
        if psi_match_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(psi_match_prop.data(), str(pathlib.Path(p) / '匹配板.stl'))
        if psi_mold_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(psi_mold_prop.data(), str(pathlib.Path(p) / '配模.stl'))
        if psi_arm_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(psi_arm_prop.data(), str(pathlib.Path(p) / '旋臂.stl'))
        if aceta_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(aceta_prop.data(), str(pathlib.Path(p) / '髋臼.stl'))
        if pelvis_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(pelvis_prop.data(), str(pathlib.Path(p) / '骨盆.stl'))
        if metal_prop.data().GetNumberOfCells():
            vmi.pdSaveFile_STL(metal_prop.data(), str(pathlib.Path(p) / '金属.stl'))
        ...
        vmi.pdSaveFile_STL(vmi.ccPd_Sh(kirs), str(pathlib.Path(p) / '导向针.stl'))

        c = head_cs.mpt([0, 0, head_z])
        kirs = vmi.ccPrism(vmi.ccFace(vmi.ccWire(vmi.ccCircle(
            kirs_hole_radius, c, head_cs.axis(2)))), -head_cs.axis(2), 200)
        vmi.pdSaveFile_STL(vmi.ccPd_Sh(kirs), str(pathlib.Path(p) / '固定针.stl'))

        cs = pelvis_cs.copy()
        (pathlib.Path(p) / '骨盆坐标系.json').write_text(json.dumps(cs.array4x4().tolist()))
        cs = body_cs.copy()
        cs.setOrigin(body_cs.mpt([0, 0, head_z]))
        (pathlib.Path(p) / '髋臼坐标系.json').write_text(json.dumps(cs.array4x4().tolist()))
        cs = head_cs.copy()
        cs.setOrigin(head_cs.mpt([0, 0, head_z]))
        (pathlib.Path(p) / '旋臂坐标系.json').write_text(json.dumps(cs.array4x4().tolist()))

        i, snapshots, filenames = 0, case_snapshot(True), []
        for by in snapshots:
            filenames.append(str(pathlib.Path(p) / '{}.png'.format(i)))
            pathlib.Path(filenames[-1]).write_bytes(by)
            i += 1

        image = cv2.imread(str(pathlib.Path(p) / '0.png'))
        xvid_writer = cv2.VideoWriter(str(pathlib.Path(p) / '{} {}.mp4'.format(patient_name, 'L' if LR else 'R')),
                                      cv2.VideoWriter_fourcc(*'XVID'),
                                      20, (image.shape[1], image.shape[0]))
        for f in filenames:
            xvid_writer.write(cv2.imread(f))
        xvid_writer.release()
...
if __name__ == '__main__':
    ...
    if select == '打开':
        ...
        pelvis_prop = vmi.PolyActor(pelvis_view)
        ...
        cup_RA_box = vmi.ValueBox(pelvis_view, value_format='前倾角：{:.1f}°',
                                  ...
                                  size=[0.2, 0.04], pos=[0, 0.1], anchor=[0, 0], pickable=True)
        ...
        aceta_prop = vmi.PolyActor(guide_view, color=[250 / 255, 246 / 255, 222 / 255])
        ...
        def on_guide():
            global body_angle, cup_RA, cup_RI
