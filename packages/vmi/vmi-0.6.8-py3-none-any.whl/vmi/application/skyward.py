...
def case_sync(case: dict, ukw: list) -> Union[dict, bool]:
    ...
    if find and 'case_uid' in find:
        if find['order_released']:
            ...
            vmi.askInfo('顾客已确认')
            r = vmi.askButtons(['上传案例', '下载案例', '发布工单'])
        else:
...
def order_review_docx(reviewer_id: str, order_uid: str, file: str):
    ...
    with tempfile.TemporaryDirectory() as p:
        ...
        add_row_text(body_table, ['评审结果', '（通过/不通过）' + '\n' * 15],
                     [s_body_center, s_body_left], [1, 3])

        reviewer = role_quality_inspector_db.find_one({'_id': reviewer_id})['name']
        add_row_text(body_table, ['评审人员/日期', '（{}）'.format(reviewer) + '\n' * 2],
                     [s_body_center, s_body_left], [1, 3])
...
def order_review():
    ...
    r = vmi.askButtons(['下载工单', '接受工单', '驳回工单'])
    if r == '下载工单':
        user = ask_user()
        if not user:
            return False
        ...
        f = pathlib.Path(p) / f
        order_review_docx(role_quality_inspector_id, order_uid, str(f))
        vmi.askInfo('下载完成')
    elif r == '接受工单':
        user = ask_user()
        if not user:
...
if __name__ == '__main__':
    ...
    def csv_update_role_quality_inspector():
    ...
    admin_menu.addAction('更新评审人员').triggered.connect(csv_update_role_quality_inspector)
    admin_menu.addAction('更新生产人员').triggered.connect(csv_update_role_machine_operator)
