...
def ccPlane(origin: Union[np.ndarray, List[float]], normal: Union[np.ndarray, List[float]]):
    """
    构造平面
...
def ccVertex(pt: Union[np.ndarray, List[float]]):
    """
    构造顶点拓扑
...
def ccEdge(curve: Handle_Geom_Curve):
    """
    构造边拓扑
...
def ccEdges(curves: List[Handle_Geom_Curve]):
    """
    构造边拓扑列表
...
def ccWire(edges: List[Union[TopoDS_Edge, Handle_Geom_Curve]]):
    """
    构造线
...
def ccWires(edges: List[TopoDS_Edge]):
    """
    构造线列表
...
def ccFace_OnPlane(wire: TopoDS_Wire,
                   origin: Union[np.ndarray, List[float]],
                   normal: Union[np.ndarray, List[float]]):
    """
    构造面，约束投影到平面
...
def ccLoft(sections: List[TopoDS_Wire], ruled=False, redirect=False, solid=False) -> Union[TopoDS_Shell, TopoDS_Solid]:
    """
    构造放样面，指定截面轮廓
...
def ccSolid(shells: Union[TopoDS_Shell, List[TopoDS_Shell]]) -> TopoDS_Solid:
    """
    构造实体，指定相互连接的壳表示封闭空间
...
def ccMatchSolid_Rect(target_poly: vtk.vtkPolyData, cs: vmi.CS4x4, nx: int, ny: int, res: float,
    ...
    :param smooth: z半方向均值滤波
    :param return_mold_inv: 返回匹配实体的反向实体
    :param return_z_array: 返回网格节点的z坐标
    :return: 与模型相匹配的实体
    """
    if not nx >= 1:
