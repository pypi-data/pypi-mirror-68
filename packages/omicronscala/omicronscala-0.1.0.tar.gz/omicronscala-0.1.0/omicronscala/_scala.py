...
class OMICRONscala:
    ...
    Args:
        filename: the name of the .par file to be opened
    """
...
class OMICRONchannel:
    ...
    def __init__(self, data, attrs):
        ...
        self.name = self.attrs['TopographicChannel']+"_"+self.attrs['Direction']

        xsize = self.attrs['ImageSizeinX']
        xres = self.attrs['IncrementX']
        ysize = self.attrs['ImageSizeinY']
        yres = self.attrs['IncrementY']
        self.coords = [('x', xres * np.arange(xsize, dtype=np.float)), ('y', yres * np.arange(ysize, dtype=np.float))]
