    ...
    def adjust_run_commands(self, job_id, job_runs, using_hp, experiment, service_type, snapshot_dir, args):
        ...
        for i, box_runs in enumerate(job_runs):
            ...
            if not wrapped_parts:
                ...
                sh_parts = super().wrap_user_command(cmd_parts, snapshot_dir, store_data_dir, data_action, 
                    ...
                    install_blobfuse=True, setup=setup, run_name=run_name, change_dir=False, args=args)

                # AML wants a python script, so use our tiny python shim to run wrapped.sh 
                fn_shim = "aml_shim.py"
                fn_from = file_utils.get_xtlib_dir() + "/" + fn_shim
