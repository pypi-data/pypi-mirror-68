...
class ImplUtilities(ImplBase):
    ...
    @flag(name="default", help="specifies that the XT DEFAULT config file should be viewed )readonly)")
    ...
    def config_cmd(self, response, default, create):
        '''
        ...
        if default:
            fn = os.path.join(file_utils.get_xtlib_dir(), "helpers", "default_config.yaml")
            if not os.path.exists(fn):
