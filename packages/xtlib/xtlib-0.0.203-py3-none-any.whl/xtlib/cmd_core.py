...
class CmdCore():
    ...
    def get_client_context(self, exper_name, run_name, app_info, box_info, job_id, node_index, run_specs, resume_name=None, 
        ...
        # transfer the set of properties
        context.update_job_stats = self.config.get("mongo", "update-job-stats")
        context.update_run_stats = self.config.get("mongo", "update-run-stats")
        context.add_log_records = self.config.get("mongo", "add-log-records")
        context.use_simple_scheduling = self.config.get("mongo", "use-simple-scheduling")
        ...
        context.first_run_index = runs_per_node*node_index
        context.last_run_index = context.first_run_index + node_run_count - 1
