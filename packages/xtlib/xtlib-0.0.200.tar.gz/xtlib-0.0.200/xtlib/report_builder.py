...
class ReportBuilder():
    ...
    def build_report(self, records, report_type, args):
        ...
        if fn_export:
            sep_char = "\t"
            count = self.export_records(fn_export, records, avail_list, actual_cols, sep_char)
            line = "report exported to: {} ({} rows)".format(fn_export, count-1)
            lines = [line]
        else:
    ...
    def export_records(self, fn_report, records, avail_cols, col_list, sep_char):
        if not col_list:
            col_list = avail_cols
