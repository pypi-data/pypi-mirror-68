...
class MetricReport(object):
    def __init__(self, metric_dict, primary_metric):
        ...
        if USE_SUCCESS_RATE:
            self.score = float(metric_dict["RSR"])
            self.sample_efficiency = float(metric_dict[primary_metric])   # "SE"])
        else:
            self.score = float(metric_dict[primary_metric])   # "reward"])
...
class Run(object):
    def __init__(self, run_data, primary_metric):
        ...
        if "hparams" in run_data:
            hparams = run_data["hparams"]     #  event_dict["data"]
...
class DGD(object):
    ...
    def define_hyperparameters(self, records):
        ...
        global USE_SUCCESS_RATE, SUCCESS_RATE_THRESHOLD, HELDOUT_TESTING
    ...
    def create_run_objects(self, run_reports, primary_metric):
        ...
        # if "TOTAL_STEPS" in self.name_hparam_dict:
        #     TOTAL_STEPS = self.name_hparam_dict["TOTAL_STEPS"].single_value
        # if "REPORTING_INTERVAL" in self.name_hparam_dict:
        #     REPORTING_INTERVAL = self.name_hparam_dict["REPORTING_INTERVAL"].single_value
        # self.max_reports_per_run = TOTAL_STEPS // REPORTING_INTERVAL
    ...
    def compute_sample_efficiency(self, runs):
        ...
        if HELDOUT_TESTING:
            return self.compute_sample_efficiency_by_median(runs)
        else:
    ...
    def compute_sample_efficiency_by_median(self, runs):
        ...
        for i_run, run in enumerate(runs):
            ...
            ses[i_run] = se
        median = np.median(ses)
        tie_broken_median = median - score_sum / num_scores
        return tie_broken_median

    def compute_sample_efficiency_by_mean_success_rate(self, runs):
        # First aggregate the success rates of the runs.
        
        num_runs = len(runs)
        #num_reports = self.max_reports_per_run
        max_reports = max(len(r.metric_reports) for r in runs)

        score_sum = 0.
        xs = np.zeros((max_reports))
        ys = np.zeros((max_reports))

        for run in runs:
            for i in len(run.metric_reports):
                score = run.metric_reports[i].score
                ys[i] += score
                score_sum += score
        ...
        se = self.x_at_y(SUCCESS_RATE_THRESHOLD, xs, ys)
        tie_broken_se = se - score_sum / (num_runs * max_reports)

        return tie_broken_se
