...
class BackendBase(implements(BackendInterface)):
    ...
    def wrap_user_command(self, user_parts, snapshot_dir, store_data_dir, data_action, data_writable, \
        ...
        setup=None, post_setup_cmds=None, run_name=None, args=None, nonempty=False, change_dir=True):
        ...
        if not self.is_windows:
            ...
            store_path = "runs/{}/output".format(run_name)
        ...
        if is_windows:
            ...
            wrapped_parts = [os.path.basename(fn_wrapped)]
        else:
            fn_wrapped = snapshot_dir + "/" + constants.FN_WRAPPED_CMDS
            scriptor.write_script_file(cmds, fn_wrapped, is_windows)
            wrapped_parts = ['/bin/bash', '--login', os.path.basename(fn_wrapped)]
