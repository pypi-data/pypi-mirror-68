...
class ImplStorage(ImplBase):
    ...
    def view_metrics(self, runs, workspace, steps, merge, metrics, hparams, export):
        ...
        for rr in run_log_records:
            ...
            for i, ms in enumerate(metric_sets):
                ...
                if export:
                    sep_char = "\t"
                    count = lb.export_records(export, ms["records"], ms["keys"], ms["keys"], sep_char)
    ...
    def extract(self, run_name, dest_dir, workspace, response, browse):
        is_aml = self.is_aml_ws(workspace)
        ws, run_name, full_run_name = run_helper.validate_run_name(self.store, workspace, run_name, parse_only=is_aml)
        ...
        if extract:
            ...
            files = capture.download_run(self.store, ws, run_name, dest_dir)
            console.print("{} files downloaded to: {}".format(len(files), dest_dir))
    ...
    def explore(self, aggregate_name, workspace, cache_dir, steps_name, log_interval_name, 
        ...
        fn_hp_config = constants.HP_CONFIG_DIR + "/" + filenames[0]
        ...
        if dest_name.startswith("job"):
            ...
            if job_ws:
                workspace = job_ws
                console.diag("{} found in ws={}".format(dest_name, workspace))
            console.diag("after get_job_workspace call (mongo-db)")

        # ws, hx_metric, score_rollup, cache_dir, aggregate_dest, dest_name, fn_hp_config
        # , steps_name=steps_name, step_name=step_name, time_name=time_name, sr_name=sr_name, se_name=se_name)
        hx = HyperparameterExplorer(self.store, workspace, primary_metric, cache_dir, aggregate_dest, 
            dest_name, fn_hp_config, steps_name, log_interval_name, time_name, step_name, success_rate_name, sample_efficiency_name)
        hx.run(timeout)
    ...
    @see_also("XT Filters", "filters")
    @see_also("XT Columns", "columns")
