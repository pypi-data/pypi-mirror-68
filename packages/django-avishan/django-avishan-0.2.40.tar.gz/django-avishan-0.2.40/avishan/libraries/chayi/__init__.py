...
class ChayiWriter:
    ...
    @staticmethod
    def compress_files(files: Dict[str, str]):
        archive = BytesIO()
        ...
        with open('static/chayi_models.zip', 'wb') as f:
    ...
    def model_file_write_direct_callable_methods(self, model: Type[AvishanModel]) -> str:
        data = ''
        if len(model.direct_non_authenticated_callable_methods() + model.direct_callable_methods()) == 0:
            return data
        ...
        for method_name in model.direct_callable_methods():
            data += f'    public static final boolean {method_name}_token = true;\n'

        data += '\n\n'

        for method_name in model.direct_non_authenticated_callable_methods() + model.direct_callable_methods():
            ...
            # response
            if inspect.signature(method).return_annotation is inspect._empty or inspect.signature(
                    method).return_annotation is None:
                returns = 'void'
            else:
                returns = inspect.signature(method).return_annotation
            data += f'    public static {returns} {method_name}_response(Response<ResponseBody> response) ' + '{\n'
            if returns == 'void':
                data += '\n    }\n\n'
            else:
                data += '        try {\n'
                data += '            JSONObject responseObject = new JSONObject(response.body().string());\n'
                data += f'            return RetrofitSingleTone.getInstance().getGson().fromJson(responseObject.get("{model.class_snake_case_name()}").toString(), {returns}.class);\n'
                data += '        } catch (JSONException | IOException e) {\n'
    ...
    def write_with_type_check(self, annotation, key, text, null_checked: bool = False):
        ...
        if annotation is str:
            ...
            text += f'        for (int i = 0; i < {key}.size(); i++) ' + "{\n"
            text += f'            {key}_array.add(RetrofitSingleTone.getInstance().getGson().toJson({key}.get(i)));\n'
            text += '        }\n'
