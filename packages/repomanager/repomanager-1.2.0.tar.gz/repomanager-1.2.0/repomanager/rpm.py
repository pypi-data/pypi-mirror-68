...
def repoman(yaml_file, clean, update, patch, unpatch, work_dir):

    repo_yaml = utils.load_yaml(yaml_file)
    ...
    for name,field in repo_yaml.items():
        ...
        if update:
            if os.path.isdir(repo_path):
                ...
                utils.shellCommand(r_clone.format(repo_url, name, recurse_cmd))\
                    .run(cwd=workdir)
                logger.info('Checking out "{0}" for repo "{1}"'.format(repo_checkout, name))
                utils.shellCommand(r_checkout.format(repo_checkout)).run(cwd=repo_path) 
            else:
