...
from Slam.GPSUtils               import *

from Slam.GoogleMaps             import *
from Slam.MetadataReader         import *
from Slam.ExtractKeyPoints       import *
from Slam.FindMatches            import *
from Slam.MultiSensorAlignment   import *
...
class ImageProvider:
	...
	def guessPlane(self):
		...
		if True:
			...
			elevations=GetTerrainElevations(lats,lons)
...
def CreateProvider(cache_dir,progress_bar=None):
	...
	provider_names=[os.path.splitext(os.path.basename(it))[0] for it in glob.glob(os.path.dirname(__file__) + "/ImageProvider*.py")]
	provider_names.remove("ImageProvider")
	provider_names.remove("ImageProvider3d")
	provider_names.remove("ImageProviderGeneric") # must be the last one
	provider_names.append("ImageProviderGeneric")
	print("Provider names",provider_names)

	provider_modules=[importlib.import_module("Slam."+it) for it in provider_names]
