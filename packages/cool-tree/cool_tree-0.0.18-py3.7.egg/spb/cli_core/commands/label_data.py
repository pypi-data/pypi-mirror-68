...
def _update_label(args):
    ...
    try:
        ...
        console.print(err)
        console.print(f"Error while uploading label '{label['data_key']}.json'. Try again.")


# def _create_label_worker(asset_images, start_num, end_num, project_id, is_force, assets, results):
#     for idx in range(start_num, end_num):
#         toUpload = True
#         asset_image = asset_images[idx]
#         try:
#             command = spb.Command(type='create_data')
#             asset = spb.run(command=command, option=asset_image, optional={'projectId': project_id})
#             assets[idx] = asset.id
...
#                     json_data = json.load(json_file)
#                     label.result = json_data['result']
#                     if label.result == None:
#                         continue
#                     if not is_force:
#                         toUpload = click.confirm(f"'{asset_image['data_key']}'  already exists in the project. Update label only?")
#                         if not toUpload:
#                             results[idx] = True
#                             continue
#                     command = spb.Command(type='update_label')
...
#         except Exception as err:
#             console.print(err)
#             console.print(f"Error while uploading label '{asset_image['data_key']}.json'. Try again.")
#             results[idx] = False
