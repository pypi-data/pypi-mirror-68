...
from .doc import Doc
from .decorators import wrap_reader, wrap_writer
from .exceptions import TableNotOpen, TrainingDataExists
...
class Compression:
    ...
    def _get(self, dat: bytes) -> dict:
        ...
        then convert it to 'str', then run it through 'loads'

        dat - the incoming binary data
        """
        return loads(self._decompress(dat).decode())
        ...
        if self._meta:
            ...
            if 'compression' in config:
                ...
                if ctype == CompressionType.ZSTD:
                    self._cbyte = self._meta.fetch_tag(self.name, 'zstd_cdict', txn=txn)
                    if not self._cbyte:
                        self._cbyte = b''
                    self._meta.store_tag(self.name, 'zstd_cdict', self._cbyte, txn=txn)
                    cdict = ZstdCompressionDict(self._cbyte)
