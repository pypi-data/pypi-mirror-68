...
from typing import TYPE_CHECKING, Any, Union, Optional, KeysView, ItemsView, ValuesView, Dict
...
class Doc:
    ...
    def __init__(
            self,
            doc: Optional[Union[Dict, Cursor]]=None,
            oid: Optional[bytes, str]=None) -> None:
        """
        ...
        """
        if doc and isinstance(doc, Cursor):
            self.oid, val = doc.item()
            self.upd = loads(val.decode())
        else:
            self.oid = oid.encode() if isinstance(oid, str) else oid
            self.upd = doc or {}
    ...
    def changed(self) -> bool:
        ...
        """
        return len(self.upd) is not 0 or len(self.rem) is not 0
    ...
    def delete(self, table: Table, txn: TXN) -> None:
        """
        Delete an entry from this index

        db - the database associated with this record
        txn - active database transaction
        """
        txn.delete(self.oid, db=table._db)
    ...
    def out(self) -> bytes:
        """
        Return a dumped and encoded version of our current buffer state ready for
        writing to the database. (Internal)
        """
        return dumps(dict(self.dat, **self.upd)).encode()
