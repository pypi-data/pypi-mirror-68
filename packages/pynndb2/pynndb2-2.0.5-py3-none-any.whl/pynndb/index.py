...
from .doc import Doc
from .decorators import wrap_reader
...
class Index(UserDict):
    ...
    def __init__(self, database: Database, index: str, conf: Config) -> None:
        """
        ...
        index - the name of the index we're creating a reference to
        ...
        self._duplicates = conf.get('dupsort', False)
        del conf['func']
    ...
    def open(self, txn: TXN) -> None:
        ...
        self._db = self._database.env.open_db(**options, txn=txn)
    ...
    def put(self, doc: Doc, txn: TXN) -> None:
        ...
        if not isinstance(keys, list):
            keys = [keys]
        for key in keys:
            if not txn.put(key, doc.oid, db=self._db, overwrite=self._duplicates):
    ...
    def put_cursor(self, cursor: Cursor, txn: TXN) -> None:
        ...
        txn - an optional transaction
        """
        self.put(Doc(loads(cursor.value().decode()), cursor.key()), txn=txn)
    ...
    def delete(self, doc: Doc, txn: TXN) -> None:
        ...
        txn - an optional transaction
        """
        txn.delete(self._func(doc.doc), doc.oid, self._db)
