# -*- coding: utf-8 -*-
'''
Data and functions related to a chemical's environmental properties.
'''
...
read = CASDataReader(__file__, 'Environment')
GWP_data = read('Official Global Warming Potentials.tsv')
ODP_data = read('Ozone Depletion Potentials.tsv')
CRClogPDict = read('CRC logP table.tsv')
SyrresDict2 = read('Syrres logP data.csv.gz', compression='gzip')
...
def GWP(CASRN, AvailableMethods=False, Method=None):
    r'''
    This function handles the retrieval of a chemical's Global Warming
