...
DDBST_MODIFIED_UNIFAC_assignments = {}
DDBST_PSRK_assignments = {}
class UNIFACGroupAssigner(GroupAssigner):
    assignments = DDBST_UNIFAC_assignments
    GroupCounts = UNIFACGroupCounts
class DortmundGroupAssigner(GroupAssigner):
    assignments = DDBST_MODIFIED_UNIFAC_assignments
    GroupCounts = DortmundGroupCounts
class PSRKGroupAssigner(GroupAssigner):
    assignments = DDBST_PSRK_assignments
    GroupCounts = PSRKGroupCounts

_assigners = (UNIFACGroupAssigner,
              DortmundGroupAssigner,
              PSRKGroupAssigner)

with open(os.path.join(folder, 'DDBST UNIFAC assignments.tsv')) as f:
    for line in f.readlines():
        InChI_key, valids, *original_modified_PSRK = line.split('\t')
        # list of whether or not each method was correctly identified or not
        valids = [i == '1' for i in valids.split(' ')]
        for Assigner, groups, valid in zip(_assigners, original_modified_PSRK, valids):
            if valid:
                groups = groups.rstrip().split(' ')
                assigner = Assigner(InChI_key)
                for i in range(int(len(groups)/2)):
                    index = i*2
                    assigner[int(groups[index])] = int(groups[index+1])
