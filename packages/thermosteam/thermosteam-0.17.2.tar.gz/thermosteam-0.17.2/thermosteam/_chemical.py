...
class Chemical:
    ...
    **data : float or str
    ...
    def __new__(cls, ID, *, search_ID=None,
                ...
                sigma=None, kappa=None, epsilon=None,
                **data):
        cache = cls._cache
            ...
            if CAS in cache:
                ...
                if ID != self._ID:
                    if ID in cache:
                        ...
                        cache[ID] = self = self.copy(ID=ID, CAS=self._CAS)
                setfield = setattr
                for i,j in data.items(): 
                    if j is not None: setfield(self, i, j)
            else:
        ...
        if phase: self = self.at_state(phase, copy=True)
        ...
        if epsilon: self.sigma.add_model(epsilon, top_priority=True)
    ...
    def new(cls, ID, CAS, eos=PR, phase_ref=None, **data):
        ...
        cache = cls._cache
        if CAS in cache:
            raise RuntimeError(f'chemical with CAS {repr(CAS)} already created')                    
        else:
            self = super().__new__(cls)
            self._ID = ID
            self.reset(CAS, eos, phase_ref, **data)
    ...
    def at_state(self, phase, copy=False):
        ...
        """
        if copy:
            new = self.copy(self._ID, self._CAS)
            new.at_state(phase)
            return new
