# -*- coding: utf-8 -*-
'''
All UNIFAC groups are loaded here.
'''
...
class UNIFAC_subgroup:
    ...
    def __repr__(self):
        return f"{type(self).__name__}{self.group, self.main_group_id, self.main_group, self.R, self.Q}"
...
UFIP = utils.load_json(folder, 'UNIFAC original interaction parameters.json', keys2int)
DOUFIP2016 = utils.load_json(folder, 'UNIFAC modified Dortmund interaction parameters.json', keys2int)
NISTUFIP = utils.load_json(folder, 'UNIFAC modified NIST 2015 interaction parameters.json', keys2int)
PSRKIP = utils.load_json(folder, 'PSRK interaction parameters.json', keys2int)
del keys2int
...
class GroupCounts(dict):
    ...
    def __init_subclass__(cls):
        subgroups_by_id = cls.subgroups_by_id
        cls.ids_by_group_name = {subgroup.group: key
                                 for key, subgroup in subgroups_by_id.items()}
    
    def __init__(self, dict=None):
        if dict: 
            to_int = int
            super().__init__({to_int(i): j for i,j in dict.items()})
...
class GroupAssigner:
    __slots__ = ('group_counts',)
    def __init__(self, InChI_key):
        self.group_counts = group_counts = self.GroupCounts()
        self.assignments[InChI_key] = group_counts
    
    def __setitem__(self, key, count):
        self.group_counts[key] = count

data = utils.load_json(folder, 'DDBST UNIFAC-original assignments.json')
DDBST_UNIFAC_assignments = {i: UNIFACGroupCounts.from_json_dict(j) for i,j in data.items()}
data = utils.load_json(folder, 'DDBST UNIFAC-Dortmund assignments.json')
DDBST_MODIFIED_UNIFAC_assignments = {i: DortmundGroupCounts.from_json_dict(j) for i,j in data.items()}
data = utils.load_json(folder, 'DDBST UNIFAC-original assignments.json')
DDBST_PSRK_assignments = {i: PSRKGroupCounts.from_json_dict(j) for i,j in data.items()}
del data
