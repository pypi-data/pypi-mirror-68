...
__all__ = ('checkCAS', 'chemical_metadata_from_any', 'PubChem', 'MW',
           ...
           'name', 'synonyms', 'cryogenics', 'pubchem_db')
import re
import os
...
def to_searchable_format(ID):    
    return searchable_format.sub(r" \1", ID).replace('_', ' ')
    ...
    def load(self, file_name):
        ...
        for line in f:
            ...
            if CAS in self.CAS_index:
                ...
                self.formula_index[formula] = obj
...
def chemical_metadata_from_any(ID):
    ...
    * Atomic number (as a string)
    ...
    ID_search = to_searchable_format(ID)
    for name in (ID_lower, ID, ID_search.capitalize(), ID_search.lower()):
