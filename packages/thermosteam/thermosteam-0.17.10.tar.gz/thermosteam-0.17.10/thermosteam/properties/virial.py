...
.. [2] Smith, H. C. Van Ness Joseph M. Introduction to Chemical Engineering
       Thermodynamics 4E 1987.
.. [3] Tsonopoulos, Constantine. "An Empirical Correlation of Second Virial
       Coefficients." AIChE Journal 20, no. 2 (March 1, 1974): 263-72.
       doi:10.1002/aic.690200209.       
.. [4] Tsonopoulos, C., and J. L. Heidman. "From the Virial to the Cubic
       Equation of State." Fluid Phase Equilibria 57, no. 3 (1990): 261-76.
       doi:10.1016/0378-3812(90)85126-U
.. [5] Tsonopoulos, Constantine, and John H. Dymond. "Second Virial
...
def BVirial_Pitzer_Curl(T, Tc, Pc, omega, order=0):
    ...
    order : int, optional
        ...
        temperature. No other integrals or derivatives are implemented, and a
        ValueError is raised if any other order is given.
...
def BVirial_Abbott(T, Tc, Pc, omega, order=0):
    r'''Calculates the second virial coefficient using the model in [2]_.
    ...
    Examples
    --------
    Example is from [2]_, p. 93, and matches the result exactly, for isobutane.
...
def BVirial_Tsonopoulos(T, Tc, Pc, omega, order=0):
    r'''Calculates the second virial coefficient using the model in [3]_.
...
@njitable
def coefficients_Tsonopoulos_extended(T, Tc, order):
    Tr = T/Tc
    if order == 0:
        B0 = 0.1445 - 0.33/Tr - 0.1385/Tr**2 - 0.0121/Tr**3 - 0.000607/Tr**8
        B1 = 0.0637 + 0.331/Tr**2 - 0.423/Tr**3 - 0.008/Tr**8
...
def BVirial_Tsonopoulos_extended(T, Tc, Pc, omega, a, b, species_type, dipole, order):
    ...
    comprehensive model in [4]_ [5]_. See the notes for the calculation of `a` and
    `b`.
    ...
    '''
    B0, B1, B2, B3 = coefficients_Tsonopoulos_extended(T, Tc, order)
    if a == 0 and b == 0 and species_type != '':
        if species_type in {'simple', 'normal'}:
            ...
            if species_type in {'ketone', 'aldehyde', 'alkyl nitrile', 'ether',
