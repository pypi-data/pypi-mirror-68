...
class ConfigCache:
    ...
    def config_files_glob(self):
        return glob.glob(os.path.join(self.configs_path, '*.yml'))
    ...
    def read_cache(self, environment='dev'):
        ...
        return [Config(config_dict=config, environment=environment) for config in configs]


class DagLoader:
    """Parses pipeline configurations, creates Airflow DAGs and exposes them for Airflow to pickup"""

    def __init__(self, project, configs_path, event_handlers=None, alerters=None, environment='dev'):
        """
    ...
    def load(self, global_symbol_table):
        configs = self.configs_cache.read_cache(environment=self.environment)
        dags = []
    ...
    def __build_sub_dag(self, sensor_configs, params):
        ...
        for sensor_config in sensor_configs:
            task = DagLoader.build_task(
                ...
                expected_class=BaseSensorOperator,
                params=params,
