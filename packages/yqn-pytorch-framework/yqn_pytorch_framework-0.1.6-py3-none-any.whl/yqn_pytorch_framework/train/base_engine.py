    ...
    def train_one_epoch(self,
        ...
        try:
            ...
            for dataset, index in metric_logger.log_every(data_loader, self.config.progress_freq, header):
                input_features, labels = self.load_inputs(dataset).to(device), self.load_labels(dataset).to(device)
                outputs = model(input_features)
                focal_loss = self.criterion(outputs, labels)
                focal_acc = self.accuracy(outputs, labels)
