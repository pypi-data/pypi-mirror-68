...
"""Tests for WindowedTimeSeries class."""
...
class Test_WindowedTimeSeries(TestCase):

    def setUp(self):
        self.full_path = get_full_path(__file__)

    def test_check(self):
        for value in ["values", ["a", "b", "c"], [1, 2, 3], (1, 2, 3)]:
            self.assertRaises(TypeError,
                              sigpropy.WindowedTimeSeries._check_input,
                              name="blah",
                              values=value)
        for value in [[[1, 2, 3], [4, 5, 6]]]:
            value = sigpropy.WindowedTimeSeries._check_input(name="blah",
                                                             values=value)
    ...
    def test_init(self):
        dt = 1
        amp = [[0, 1, 0, -1], [0, 1, 0, -1]]
        test = sigpropy.WindowedTimeSeries(amp, dt)
    ...
    def test_time(self):
        ...
        returned = sigpropy.WindowedTimeSeries.from_timeseries(tseries, 3).time
        ...
        tseries = sigpropy.TimeSeries(amp, dt)
        returned = sigpropy.WindowedTimeSeries.from_timeseries(tseries, 4).time
    ...
    def test_from_timeseries(self):
        ...
        dt = 1
        tseries = sigpropy.TimeSeries(amp, dt)
        returned = sigpropy.WindowedTimeSeries.from_timeseries(tseries, 2).amp
        ...
        dt = 0.5
        tseries = sigpropy.TimeSeries(amp, dt)
        returned = sigpropy.WindowedTimeSeries.from_timeseries(tseries, 1).amp
        expected = np.array([[1, 2, 3],
                             [3, 4, 5],
                             [5, 6, 7],
                             [7, 8, 9]])
        ...
        dt = 1
        tseries = sigpropy.TimeSeries(amp, dt)
        returned = sigpropy.WindowedTimeSeries.from_timeseries(tseries, 4).amp
