...
def interpolate_by_location(df: pandas.DataFrame, column_to_interpolate_on: str = 'day_of_the_year',
        ...
        for day in range(first_day_in_location, max_day + 1):
            if day not in days_that_we_have_data_for:
                ...
                for column in row_to_cat.keys():
                    if (column in ['confirmed_count', 'death_count']) or column.startswith('state_weekly_covid_hospitalization_rate_per'):
                        row_to_cat[column] = 0
    ...
    df = pandas.concat([df.reset_index(drop=True).copy(), pandas.concat(rows_to_concat, axis=1, ignore_index=True).T], ignore_index=True).reset_index(drop=True)
...
def add_cumsums_to_cases_table(
    ...
    cases.drop(columns=['location'], inplace=True)
