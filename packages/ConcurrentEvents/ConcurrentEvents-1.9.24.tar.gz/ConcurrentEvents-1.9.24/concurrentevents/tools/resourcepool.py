...
from concurrentevents.tools.threadmonitor import ThreadMonitor
...
class ResourcePool:
    ...
    If it is specified to be thread_save additional resources are created to match the number of threads running
    and resource use it checked when requesting a resource
    ...
    def __init__(self, resource, amount, thread_safe=False):
        ...
        if not (isinstance(resource, Resource) or issubclass(resource.__class__, Resource)):
            raise TypeError(
                f"ResourceManager() resource argument must be a Resource of subclass of Resource, not {resource}")
...
class Resource(EventHandler):
    ...
    def copy(self):
        ...
        """
        return self.__class__(self._f) if self._f else self.__class__()
    ...
    def open(self):
        """Empty function to override if functionality wants to be added to the __enter__ operation"""

    def close(self):
        """Function can either be override to add function to the with statement or
        if no with statement is used this signals that the object is no longer in use __exit__ operation"""
        self._in_use = False
