...
class EventManager:
    ...
    def __init__(self,
                 ...
                 debug=False):
        """Constructor Method"""
        _base.log = logger or logging.getLogger("__main__")
        ...
        if threads:
            _base.threads = threads
        else:
            _base.threads = self.__thread_pool._max_workers
        ...
        # Set a reference to the instantiated eventManager so other parts
        #   of this library can access it without it being passed around
        _base.event_manager = self
    ...
    def __handle(self, event):
        ...
        if event_name in _base.handlers.keys():
            handlers = sorted(_base.handlers[event_name], key=lambda h: h.priority)
            _base.log.info(f" {[h.__name__ for h in handlers]} handling {event.__str__()}")
    ...
    def fire(self, event):
        ...
        if event.__class__.__name__ not in _base.handlers.keys() and not isinstance(event, Exit):
            raise KeyError(f"No Handlers For {event.__class__.__name__}")

        # DEBUG
        if not self.exit:
            _base.log.debug(f"Firing {event}")
    ...
    def start(self):
        ...
        while not (self.exit or len(self.__futures.keys()) == 0):
            try:
                if self.debug:
                    ...
                    monitored = _base.thread_monitor

                    process_info = f"EventManager Loop (futures={futures}, " \
                                   f"running={running}, " \
                                   f"monitored={monitored})"
                    _base.log.info(process_info)
                    ...
                    if f.done():
                        ...
                        try:
                            if self.progress_tracking:
                                ...
                                _base.log.debug(f"{future_info} ({self.finished}/{self.fired})")

                            # Raises errors
                            f.result()
                        except Exception:
                            _base.log.exception(fn)
                            if self.progress_tracking:
                                self.errors += 1

                time.sleep(0.25)
            except Exception:
                _base.log.exception("Event Manager Main Loop Error")
        ...
        try:
            ...
            _base.log.exception("Final Cleaning Timed Out")

        _base.log.debug(f"Runtime {timedelta(seconds=time.time() - s)}")
        if self.progress_tracking:
            _base.log.debug(f"Events Fired ({self.fired})")

            _base.log.debug(f"Events Finished ({self.finished})")
            _base.log.debug(f"Events Errors ({self.errors})")

            _base.log.debug(f"Events Canceled ({self.canceled})")
