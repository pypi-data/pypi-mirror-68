...
class Master:
    ...
    def push_task(self, worker_id, task):
        ...
        success = self.task_queue_manager.put(worker_id, task)
        if not success:
            # qsize 在macos下会报错
            logger.error(
                'put item fail. worker_id: %s, queue_size: %s / %s',
                worker_id, self.task_queue_manager.qsize(worker_id),
                self.task_queue_manager.max_size
            )
