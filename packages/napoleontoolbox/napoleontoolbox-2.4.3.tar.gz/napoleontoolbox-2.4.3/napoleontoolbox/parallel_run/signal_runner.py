...
class AbstractRunner(ABC):
    def __init__(self, starting_date = None, running_date = None, drop_token=None, dropbox_backup = True, underlying = None, freqly_pkl_file_name_sufffix='freqly_candels.pkl', local_root_directory='../data/', user = 'napoleon',period=252):
        super().__init__()
        self.freqly_pkl_file_name_sufffix=freqly_pkl_file_name_sufffix
        self.underlying = underlying
        self.local_root_directory=local_root_directory
        self.saving_return_path = self.local_root_directory+self.underlying+self.freqly_pkl_file_name_sufffix
        self.user=user
...
class SimpleSignalRunner(AbstractRunner):
    def runTrial(self, saver, seed, trigger, signal_type, idios_string, transaction_costs, normalization):
        ...
        freqly_df = signal_utility.reconstitute_signal_perf(data = freqly_df, transaction_cost = transaction_costs , normalization = normalization)
