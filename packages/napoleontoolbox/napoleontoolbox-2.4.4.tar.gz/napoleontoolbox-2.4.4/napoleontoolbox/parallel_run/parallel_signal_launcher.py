...
class SignalParalleLauncher():
    def __init__(self,drop_token='', dropbox_backup=True, local_root_directory='../data',user='napoleon',  db_path_suffix = '_run.sqlite', triggers = [True], transaction_costs = [True], normalizations = [False], signal_idiosyncratic_paramaters=[None], table_max_size = 2000):
        self.args = []
        self.key_args = []
        self.seed = 0
        self.mapping_keys={}
            ...
            for transaction_cost in transaction_costs:
                for normalization in normalizations:
                    for signal_type, params  in signal_idiosyncratic_paramaters.items():
                        for param in params:
                            self.args.append((self,self.seed,trigger,signal_type, json.dumps(param, sort_keys=True), transaction_cost, normalization))
                            self.key_args.append((self.seed, trigger, signal_type, json.dumps(param, sort_keys=True),transaction_cost, normalization))
    ...
    def create_saving_key(self,seed, trigger, signal_type, idios_string, transaction_costs, normalization):
