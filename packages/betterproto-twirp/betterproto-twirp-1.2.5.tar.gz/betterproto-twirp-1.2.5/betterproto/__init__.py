...
class ServiceStub(ABC):
    ...
    """
    session.host = None
    session.suffix_url = None
    session.common_args = None

    def __init__(self, target: str = None, common_args: Dict[str, str] = {}):
        self._common_args = common_args
        if session.suffix_url is None:
            session.suffix_url = "?" + urlencode(self._common_args)
        self.suffix_url = session.suffix_url
    ...
    def _do_twirp_rpc(self, api, request: Message, response: Message):
        # if len(self._common_args) > 0:
        #     api = api + "?" + urlencode(self._common_args)
        ...
        r = session.request(
            method="POST",
            url=url,
            data=bytes(request.to_json(), "utf8"),
            headers={"Content-Type": "application/json"},
            timeout=60 * 5
        )
        logger.info(f'[请求结果]:{r.json()}')
        logger.info(f'[请求http_code]:{r.status_code}')
        logger.info(f'[请求耗时]:{r.elapsed.total_seconds()}秒')
        # try:
        #     resp = req.
