...
async def update_data_points_data_quality(dbpool, dataquality, datatypeid, freq):
    query = 'WITH n_samples AS (' \
            'SELECT GREATEST(EXTRACT(EPOCH FROM CURRENT_TIME - \'06:00\'::time)::INTEGER / (60 * $1), 1) AS n' \
            ')' \
            'UPDATE data_point_data_quality SET data_quality = ' \
...
async def update_roads_data_quality(dbpool, dataquality):
    query = 'WITH n_samples AS (' \
            'SELECT GREATEST(EXTRACT(EPOCH FROM CURRENT_TIME - \'06:00\'::time)::INTEGER / 60, 1) AS n' \
            ')' \
            'UPDATE road_data_quality SET data_quality = ' \
            '(SELECT ($1 + (data_quality * (n - 1))) / n FROM n_samples) ' \
            'WHERE day=CURRENT_DATE;'
    await _run_execute_request(dbpool, query, (dataquality,))
...
async def select_data_points_data_quality(dbpool, datatypeid):
    query = 'SELECT jsonb_object_agg(trim(both \'"\' from to_json(dpdq.day)::text), dpdq.data_quality::int) AS res ' \
            'FROM data_point_data_quality AS dpdq '
    whereclause = ''
    whereparams = ()
    if datatypeid is not None:
        whereclause, whereparams = _add_where_condition('WHERE ', whereparams,
                                                        _build_where_clause_for_eids_fields,
                                                        (datatypeid, 'dpdq.data_type'))
    query += whereclause + ';'
    return (await _run_fetch_request(dbpool, query, whereparams))[0]['res']


async def select_roads_data_quality(dbpool):
    query = 'SELECT jsonb_object_agg(trim(both \'"\' from to_json(rdq.day)::text), rdq.data_quality::int) AS res ' \
            'FROM road_data_quality AS rdq;'
    return (await _run_fetch_request(dbpool, query, ()))[0]['res']
