...
class DbStorage:
    ...
    async def insert_traffic_data(self, samples, datatypeeid, timestamp):
        if datatypeeid == METROPMEDATATYPE or datatypeeid == TOMTOMFCDDATATYPE:
            await self._insert_traffic_indicators(samples, timestamp, self.datapointseidtoid,
                                                  settings.DATA_POINTS_INDICATORS,
                                                  dbrequest.insert_data_points_indicators)
        elif datatypeeid == ROADSDATATYPE:
            await self._insert_traffic_indicators(samples, timestamp, self.roadseidtoid, settings.ROADS_INDICATORS,
                                                  dbrequest.insert_roads_indicators)

    async def update_data_quality(self, dataquality, datatypeeid, freq=1):
        if datatypeeid == METROPMEDATATYPE or datatypeeid == TOMTOMFCDDATATYPE:
            if datatypeeid not in self.datatypeseidtoid:
                return
            await dbrequest.update_data_points_data_quality(self.dbpool, dataquality,
                                                            self.datatypeseidtoid[datatypeeid], freq)
        elif datatypeeid == ROADSDATATYPE:
            await dbrequest.update_roads_data_quality(self.dbpool, dataquality)
    ...
    async def get_data_quality(self, datatypeeid):
        ...
        if datatypeeid == METROPMEDATATYPE or datatypeeid == TOMTOMFCDDATATYPE:
            if datatypeeid in self.datatypeseidtoid:
                result = await dbrequest.select_data_points_data_quality(self.dbpool, self.datatypeseidtoid[datatypeeid])
        elif datatypeeid == ROADSDATATYPE:
            result = await dbrequest.select_roads_data_quality(self.dbpool)
        return result
