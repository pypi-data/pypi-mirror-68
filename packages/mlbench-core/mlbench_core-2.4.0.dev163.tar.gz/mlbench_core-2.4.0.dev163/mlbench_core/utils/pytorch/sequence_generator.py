...
class SequenceGenerator(object):
    def __init__(
            self, models, src_dict, trg_dict, beam_size=1, minlen=1, maxlen=None, stop_early=True,
    ...
    def generate_batched_itr(
        ...
        for sample in data_itr:
            ...
            for i, id in enumerate(s['id'].data):
                ...
                ref = strip_pad(s['target'].data[i, :], self.pad) if s['target'] is not None else None
    ...
    def get_translations(self, loader, maxlen_a, maxlen_b, prefix_size=0, use_cuda=False):
        translation_gen = self.generate_batched_itr(loader, beam_size=self.beam_size, maxlen_a=maxlen_a, maxlen_b=maxlen_b, prefix_size=prefix_size,
                                                    cuda=use_cuda)

        ref_toks = []
        sys_toks = []
        for sample_id, src_tokens, target_tokens, hypos in translation_gen:
            has_target = target_tokens is not None
            target_tokens = target_tokens.int() if has_target else None

            src_str = self.src_dict.string(src_tokens, None)
            if has_target:
                target_str = self.trg_dict.string(target_tokens, None)

            for i, hypo in enumerate(hypos[:min(len(hypos), 1)]):
                hypo_tokens, hypo_str, alignment = post_process_prediction(
                    hypo_tokens=hypo['tokens'].int().cpu(),
                    alignment=hypo['alignment'].int().cpu() if hypo['alignment'] is not None else None,
                    align_dict=None,
                    tgt_dict=self.src_dict,
                    remove_bpe=None
                )
                ...
                if has_target and i == 0:
                    ...
                    # sys_tok = bleu_tokenize(hypo_str.lower())
                    # ref_tok = bleu_tokenize(target_str.lower())
                    sys_toks.append(hypo_str.lower())
                    ref_toks.append(target_str.lower())

        return ref_toks, sys_toks
