...
class SequenceGenerator(object):
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        with torch.no_grad():
            if incremental_states[model] is not None:
                decoder_out = list(
                    model.decoder(
                        tokens, encoder_out, incremental_state=incremental_states[model]
                    )
                )
            else:
    ...
    def get_translations(
        self, loader, maxlen_a, maxlen_b, prefix_size=0, use_cuda=False
    ):
        translation_gen = self.generate_batched_itr(
            loader,
