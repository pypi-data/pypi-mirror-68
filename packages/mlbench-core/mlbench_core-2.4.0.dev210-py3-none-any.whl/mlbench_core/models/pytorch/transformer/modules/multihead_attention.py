...
class QueryLinear(torch.autograd.Function):
    ...
    def forward(ctx, input, weights_q, scale):
        ...
        q = torch.addmm(input.view(input.size(0) * input.size(1), input.size(2)), input.view(input.size(0) * input.size(1), input.size(2)), weights_q,
                        beta=0.0, alpha=s[0])
    ...
    def backward(ctx, q_grad):
        ...
        q = torch.addmm(q_grad.view(q_grad.size(0) * q_grad.size(1), q_grad.size(2)), q_grad.view(q_grad.size(0) * q_grad.size(1), q_grad.size(2)),
...
class KeyValueLinears(torch.autograd.Function):
    ...
    def forward(ctx, input, weights_k, weights_v):
        ...
        k = torch.addmm(input.view(input.size(0) * input.size(1), input.size(2)), input.view(input.size(0) * input.size(1), input.size(2)), weights_k,
                        beta=0.0, alpha=1.0)
        k = k.view(input.size(0), input.size(1), input.size(2))
        v = torch.addmm(input.view(input.size(0) * input.size(1), input.size(2)), input.view(input.size(0) * input.size(1), input.size(2)), weights_v,
                        beta=0.0, alpha=1.0)
    ...
    def backward(ctx, k_grad, v_grad):
        ...
        input = input.view(input.size(0) * input.size(1), input.size(2)).transpose(0, 1)
        k = torch.addmm(k_grad.view(k_grad.size(0) * k_grad.size(1), k_grad.size(2)), k_grad.view(k_grad.size(0) * k_grad.size(1), k_grad.size(2)),
...
class FastSoftmaxFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        output = torch.empty(input.size(), dtype=input.dtype, device=torch.device('cuda'))
        output = padded_softmax.fast_forward(output, input)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.empty(grad_output.size(), dtype=grad_output.dtype, device=torch.device('cuda'))
        grad_input = padded_softmax.fast_backward(grad_input, grad_output, output)

        return grad_input


class MaskedFastSoftmaxWithBwdFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, mask):
        output = torch.empty(input.size(), dtype=input.dtype, device=torch.device('cuda'))
        output = padded_softmax.masked_fast_forward(output, input, mask)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input = torch.empty(grad_output.size(), dtype=grad_output.dtype, device=torch.device('cuda'))
        grad_input = padded_softmax.masked_fast_backward(grad_input, grad_output, output, mask)

        return grad_input, None


class MaskedFastSoftmaxFunc(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, mask):
        output = torch.empty(input.size(), dtype=input.dtype, device=torch.device('cuda'))
        output = padded_softmax.masked_fast_forward(output, input, mask)
...
fast_softmax_func = FastSoftmaxFunc.apply
masked_fast_softmax_func = MaskedFastSoftmaxFunc.apply
masked_fast_softmax_with_bwd_func = MaskedFastSoftmaxWithBwdFunc.apply
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask_future_timesteps=False,
        ...
        if qkv_same:
            ...
            q = torch.addmm(query.view(query.size(0) * query.size(1), query.size(2)), query.view(query.size(0) * query.size(1), query.size(2)),
                            self.in_proj_weight_q, beta=0.0, alpha=self.scaling)
            if key is None:
                assert value is None
                # this will allow us to concat it with previous value and get
                # just get the previous value
        ...
        ### DEBUG
        # print("BMM1", q.size(), q.stride(), k.size(), q.stride())
        ...
        if self.softmax_type == 'masked_fast':
            if key_padding_mask is not None:
                attn_weights = masked_fast_softmax_func(attn_weights, key_padding_mask)
            else:
                attn_weights = fast_softmax_func(attn_weights)
        elif self.softmax_type == 'masked_fast_bwd':
            if key_padding_mask is not None:
                attn_weights = masked_fast_softmax_with_bwd_func(attn_weights, key_padding_mask)
            else:
                attn_weights = fast_softmax_func(attn_weights)
        elif self.softmax_type == 'fast':
            if key_padding_mask is not None:
                attn_weights = attn_weights.view(bsz, self.num_heads, tgt_len, src_len)
                attn_weights = attn_weights.masked_fill(key_padding_mask.unsqueeze(1).unsqueeze(2), float('-inf'))
                attn_weights = attn_weights.view(bsz * self.num_heads, tgt_len, src_len)
            attn_weights = fast_softmax_func(attn_weights)
        elif self.softmax_type == 'fast_fill':
            if key_padding_mask is not None:
