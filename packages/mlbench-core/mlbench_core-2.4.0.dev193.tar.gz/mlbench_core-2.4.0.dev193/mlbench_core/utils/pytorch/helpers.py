...
def item(tensor):
    """Given a tensor, returns the single item.
    Returns the whole tensor if has more than one item.

    Args:
        tensor (`obj`:torch.Tensor): The item tensor

    Returns:
        (`obj`:torch.Tensor, float): Either the whole tensor, or the single item
    """
