...
def _get_grads(model, has_grad=True):
    """Returns a list of tensors in the module
    ...
    for p in model.parameters():
        if p.requires_grad and p.grad is None:
            if has_grad:
                raise RuntimeError("Model has non gradient")
            else:
...
class Aggregation(object):
    ...
    def _agg_gradients_by_model(self, model, op, denom=None):
        ...
        # Pack all layers
        packed, indices, sizes = pack_tensors(
            _get_grads(model, True), use_cuda=self.use_cuda
        )
        aggregated = self._agg(packed, op=op, denom=denom)
...
class AllReduceAggregation(Aggregation):
    ...
    def _divide(self, data, op, denom=None):
        ...
        if op == "avg":
            ...
            if denom is None or len(denom.size()) > 1 or denom.size(0) > 1:
