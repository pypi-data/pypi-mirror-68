...
from mlbench_core.utils.pytorch.helpers import item
...
class FP16Optimizer:
    ...
    def initialize_flat_fp32_weight(self):
        ...
        nelem = sum(p.numel() for p in self.fp16_model.parameters())
    ...
    def step(self, closure=None, multiplier=1, denom=None):
        ...
        has_nan = any(torch.isnan(p.data).sum() > 0 for p in self.fp16_model.parameters())
        has_nan_grad = any(torch.isnan(p.grad.data).sum() > 0 for p in self.fp16_model.parameters())
        # Aggregate gradients
        self.agg(self.fp16_model, self.agg_mode, denom=denom)
        has_nan_grad_2 = any(torch.isnan(p.grad.data).sum() > 0 for p in self.fp16_model.parameters())

        print("Hasnan={}, nan_grad={}, after_red={}".format(has_nan, has_nan_grad, has_nan_grad_2))
...
class FP32Optimizer:
    ...
    def step(self, closure=None, denom=None):
        ...
        """
        # Aggregate gradients
        has_nan = any(torch.isnan(p.data).sum() > 0 for p in self.model.parameters())
        has_nan_grad = any(torch.isnan(p.grad.data).sum() > 0 for p in self.model.parameters())
        # Aggregate gradients
        self.agg(self.model, self.agg_mode, denom=denom)
        has_nan_grad_2 = any(torch.isnan(p.grad.data).sum() > 0 for p in self.model.parameters())
