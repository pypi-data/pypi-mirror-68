...
class FP16Optimizer:
    ...
    def step(self, closure=None, multiplier=1, denom=None):
        ...
        # min_grad = min(p.grad.data.norm() for p in model.parameters())
        # max_grad = max(p.grad.data.norm() for p in model.parameters())
        # logger.info(" max_grad={}, min_grad={}".format(max_grad, min_grad))
        # min_ = self.fp32_params.grad.data.abs().min()
        # max_ = self.fp32_params.grad.data.max()
        # print("FP32 grad min={}, max={} (before scaling)".format(min_, max_))
        ...
        # min_ = self.fp32_params.grad.data.abs().min()
        # max_ = self.fp32_params.grad.data.max()
        # print("FP32 grad min={}, max={}".format(min_, max_))
