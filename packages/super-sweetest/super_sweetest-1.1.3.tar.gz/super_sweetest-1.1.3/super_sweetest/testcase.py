...
from super_sweetest.keywords import web, common, mobile, http, files, air_mobile
from super_sweetest.config import web_keywords, mobile_keywords, http_keywords, windows_keywords, files_keywords, air_mobile_keywords
    ...
    def run(self):
        ...
        for index, step in enumerate(self.testcase['steps']):
            ...
            try:
                ...
                elif g.platform.lower() in ('air_android', 'air_ios') and step['keyword'].upper() in air_mobile_keywords:
                    # 处理截图数据
                    snap.pre(step, label)
                ...
                if g.platform.lower() in ('desktop',) and step['keyword'].upper() in web_keywords:
                    ...
                    try:
                        ...
                        #  失败步骤图片名称转 base64
                        base64Failure = base64_photo(step['snapshot']['Failure'])
