...
class DynamixelBus(BaseBus):
    ...
    def scan(self, range=list(range(254))):
        if not self.is_open:
...
class MockPacketHandler():
    ...
    def __common_writeTxRx(self, ph, dev_id, address, value):
        if random.random() < self.__err:
            ...
            for dev in list(self.__robot.devices.values()):
                if dev.dev_id == dev_id:
                    break
            for reg in list(dev.registers.values()):
    ...
    def ping(self, ph, dxl_id):
        ...
        for device in list(self.__robot.devices.values()):
