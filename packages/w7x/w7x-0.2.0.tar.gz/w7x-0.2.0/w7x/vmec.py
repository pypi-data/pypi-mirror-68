...
class SumAtan(Profile):
    ...
    def __call__(self, value):
        return self.coefficients[
            0] + 2 / np.pi * self.coefficients[1] * np.arctan(
                np.divide(self.coefficients[2] * value**self.coefficients[3],
...
class FourierCoefficients(VMECBase):
    ...
    def __call__(self, radial, phi, theta, component):
        values = []

        if component not in ['cos', 'sin']:
            raise ValueError(
                "Type of Fourier Coefficients not supported")

        if radial >= self.numRadialPoints:
            raise ValueError(
                "Radial location {r} is outside coefficients boundary".format(
                    r=radial))

        i = radial * len(self.poloidalModeNumbers) * len(
            self.toroidalModeNumbers)
        for m in self.poloidalModeNumbers:
            for n in self.toroidalModeNumbers:
                if component == 'cos':
                    part = self.coefficients[i] * np.cos(m * theta -
                                                         5 * n * phi)
                else:
                    part = self.coefficients[i] * np.sin(m * theta -
                                                         5 * n * phi)
                values.append(part)
                i += 1
    ...
    def reshape(self, shape, keepdims=False):
        ...
        indices = [indices[r, m, n + self.toroidalModeNumbers[-1]]
                   for r in nRad
                   for m in mPol
                   for n in nTor]
        ...
        if keepdims:
            ...
            return FourierCoefficients(coefficients=coeffs,
        ...
        return FourierCoefficients(coefficients=coeffs,
                                   ...
                                   numRadialPoints=len(rRad))

    @classmethod
    def get_indices(cls, poloidal_modes, toroidal_modes, modes=12):
        ...
        coeffs = np.arange(modes**2 * 2)
        coeffs = np.insert(coeffs, 0, [0] * modes)
        coeffs = coeffs.reshape(modes, modes * 2 + 1)

        indeces = [
            coeffs[m, n] for m in poloidal_modes
            for n in [mode + modes for mode in toroidal_modes]
        ]
    ...
    @classmethod
    def from_array(cls, coefficients):
        """Return FourierCoefficients from array.

        We assume that coefficients have the same number of poloidal and
        toroidal modes.

        Args:
            coefficients (numpy.ndarray): input array.

        Returns:
            w7x.vmec.FourierCoefficients: output fourier coefficients.
        """

        radial_points = coefficients.shape[0]
        poloidal_modes = range(int(np.sqrt(coefficients.shape[1] / 2)))
        toroidal_modes = range(-poloidal_modes[-1] - 1, poloidal_modes[-1] + 2)

        #  Extend coefficients for m=0, n<0
        coefficients = np.apply_along_axis(
            lambda x: np.insert(x, 0, [0] * len(poloidal_modes)), 1, coefficients)

        return cls(coefficients=coefficients.flatten(),
...
class SurfaceCoefficients(VMECBase):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.RCos.numRadialPoints != self.ZSin.numRadialPoints:
            raise ValueError(
                "Coefficients radial resolutions are not the same")
    ...
    def __call__(self, radial, phi, theta):
        partsR = []
        partsZ = []

        if radial >= self.RCos.numRadialPoints:
            raise ValueError(
                "Radial location {r} is outside coefficients boundary".format(
                    r=radial))

        i = radial * len(self.RCos.poloidalModeNumbers) * len(
            self.RCos.toroidalModeNumbers)
        ...
        j = radial * len(self.RCos.poloidalModeNumbers) * len(
            self.RCos.toroidalModeNumbers)
    ...
    def plot(self, *args, **kwargs):
        ...
        w7x.plotting.vmec.plot_surface(self, *args, **kwargs)
...
def get_initial_from_w7xfp(coil_currents, pressure, current):
    ...
    magneticAxis_ZSin = w7x.vmec.FourierCoefficients(
        ...
        toroidalModeNumbers=[0, 1],
        numRadialPoints=1)
    magneticAxis = w7x.vmec.SurfaceCoefficients(RCos=magneticAxis_RCos,
                                                ZSin=magneticAxis_ZSin)
    ...
    boundary_ZSin = w7x.vmec.FourierCoefficients(
        ...
        toroidalModeNumbers=[-1, 0, 1],
        numRadialPoints=1)
    boundary = w7x.vmec.SurfaceCoefficients(RCos=boundary_RCos,
                                            ZSin=boundary_ZSin)
...
def vmec_input(**kwargs):
    ...
    numGridPointsPoloidal = kwargs.pop('numGridPointsPoloidal',
                                       2 * numModesPoloidal + 6)
...
class Run(object):
    ...
    def closest_vmec_run(self, **kwargs):
        ...
        vmec_ids = [
            vmec_id for vmec_id in vmec_ids if vmec_id_filter in vmec_id
        ]
        ...
        log.info("List of close vmec runs in term of coil currents  "
                 "{runs}".format(runs=close_runs_in_currents[min(
                     close_runs_in_currents.keys())]))
        close_runs = list(
            map(lambda x: Run(x),
    ...
    def _start_hpc(self):
        ...
        if not os.path.isfile(threed1_file):
            while jobs_in_queue > _max_jobs:
                log.info("{j} in the queue, waiting {s} seconds".format(
    ...
    def _parse_input(self, **kwargs):
        ...
        path = rna.path.resolve(
            self._folder + "{self.vmec_id}.input.json".format(**locals()))
        input_path = rna.path.resolve(
    ...
    def _gen_folder(self):
        ...
        folder = rna.path.resolve(self._folder + self.vmec_id)
    ...
    def _build_input(self):
        ...
        input_dict['MGRID_FILE'] = "'{mgrid}'".format(
            mgrid=input_data.mgridFile)
        ...
        if input_data.pressureProfile.ProfileType == 'cubic_spline':
            ...
            input_dict['AM_AUX_F'] = ' '.join(
                [str(c) for c in profile(radial)])
        ...
        for m in r_cos.poloidalModeNumbers:
            for n in r_cos.toroidalModeNumbers:
                input_dict['RBC({n},{m})'.format(
                    **locals())] = r_cos.coefficients[i]
                input_dict['ZBS({n},{m})'.format(
                    **locals())] = z_sin.coefficients[i]
                i += 1
    ...
    def _gen_input(self):
        ...
        path = rna.path.resolve(
            self._folder + self.vmec_id +
            "/input.{self.vmec_id}".format(**locals()))

        run_content = self._build_input()
        with open(path, 'w') as f:
            f.write('&INDATA ')
            for key, value in run_content.items():
                value = " ".join([str(x) for x in value]) if isinstance(
    ...
    def _dump_slurm(self, **kwargs):
        ...
        path = os.path.expanduser(self._folder + self.vmec_id +
                                  "/slurm_vmec")
    ...
    def _parse_wout(self, **kwargs):
        ...
        if not local:
            if not self.is_ready():
                raise ValueError(
                    "Calculation with vmec_id {self.vmec_id} has not finishe yet."
                    .format(**locals()))
            return {}

        wout_path = rna.path.resolve(
            self._folder + self.vmec_id +
        ...
        #  Look for wout_{vmec_id}.nc file
        ds = netCDF4.Dataset(wout_path)
        wout = ds.variables
        self._wout = wout
    ...
    def _parse_threed1(self, **kwargs):
        ...
        filePath = rna.path.resolve(
            self._folder + "{self.vmec_id}.threed1.json".format(**locals()))
        threed1_path = rna.path.resolve(
            self._folder + self.vmec_id +
            "/threed1.{self.vmec_id}".format(**locals()))
        ...
        #  Look up on the server
            ...
            vmec_server = w7x.get_server(w7x.Server.addr_vmec_server)
            content = vmec_server.service.getVmecRunData(
                self.vmec_id, 'threed1')
            iterable = content.split('\n')
            content = threed1_transcoding.read(iterable)
            if cached:
    ...
    def to_dict(self, features):
        ...
        _features_in_input = ['pressure', 'coilCurrents', 'phiedge',
                              'curtor', 'toroidalCurrent', 'ntheta',
                              'mpol', 'ns', 'pressureScale',
                              'pressureProfileCoefficients',
                              'toroidalCurrentProfileCoefficients']
        _features_in_wout = ['raxis', 'zaxis', 'BCos', 'RCos', 'ZSin',
                             'LambdaSin', 'gCos', 'BsubUCos', 'BsubVCos',
                             'BsubSSin', 'BsupUCos', 'BsupVCos',
                             'CurrUCos', 'CurrVCos']
        _features_in_threed1 = ['totalTime', 'PlasmaVolume', 'MinorRadius']

        self._parse_input()
        if not set(features).isdisjoint(set(_features_in_wout)):
            try:
                self._parse_wout()
            except ValueError as e:
                log.warning(e)

        if not set(features).isdisjoint(set(_features_in_threed1)):
            try:
                self._parse_threed1()
            except ValueError as e:
                log.warning(e)
        ...
        for f in features:
            ...
            if f in _features_in_wout and 'wout' not in self._parsed:
                value = None
            elif f in _features_in_threed1 and f not in self._threed1.keys():
                value = None
            elif f == "pressure":
                value = self.pressure_profile(radial_grid)
            elif f == "pressureProfileCoefficients":
                value = np.array(self.pressure_profile.coefficients).astype(dtype)
            elif f == "coilCurrents":
                value = np.array(self._input['coilCurrents'][
                    :num_coil_currents]).astype(dtype)
            elif f == "phiedge":
                ...
                value = self.current_profile(radial_grid)
                ...
                value = int(self._input['numGridPointsRadial'][-1])
            elif f == 'raxis':
                value = np.array(self._wout['raxis_cc']).astype(dtype)
            elif f == 'zaxis':
                value = np.array(self._wout['zaxis_cs']).astype(dtype)
            elif f == 'BCos':
                value = np.array(self._wout['bmnc']).astype(dtype)
            elif f == 'RCos':
                value = np.array(self._wout['rmnc']).astype(dtype)
            elif f == 'ZSin':
                value = np.array(self._wout['zmns']).astype(dtype)
            elif f == 'LambdaSin':
                value = np.array(self._wout['lmns']).astype(dtype)
            elif f == 'gCos':
                value = np.array(self._wout['gmnc']).astype(dtype)
            elif f == 'BsubUCos':
                value = np.array(self._wout['bsubumnc']).astype(dtype)
            elif f == 'BsubVCos':
                value = np.array(self._wout['bsubvmnc']).astype(dtype)
            elif f == 'BsubSSin':
                value = np.array(self._wout['bsubsmns']).astype(dtype)
            elif f == 'BsupUCos':
                value = np.array(self._wout['bsupumnc']).astype(dtype)
            elif f == 'BsupVCos':
                value = np.array(self._wout['bsupvmnc']).astype(dtype)
            elif f == 'CurrUCos':
                value = np.array(self._wout['currumnc']).astype(dtype)
            elif f == 'CurrVCos':
                value = np.array(self._wout['currvmnc']).astype(dtype)
            elif f == 'totalTime':
                value = self._threed1['totalTime']
            elif f == 'PlasmaVolume':
                value = self._threed1['PlasmaVolume']
            elif f == 'MinorRadius':
                value = self._threed1['MinorRadius']
            else:
                raise RuntimeError("Feature {f} is not supported".format(
                    **locals()))
