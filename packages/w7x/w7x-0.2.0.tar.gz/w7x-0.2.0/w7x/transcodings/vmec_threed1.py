...
def get_transcoding():
    ...
    t_pressureProfile = tc.Trigger(lambda x: " MASS PROFILE COEFFICIENTS" in x,
                                   delay=0)
    ...
    b_pressureProfile = tc.List("{pressureProfileCoefficients:e}",
                                separator='   ',
                                rigid=False,
                                prefix='  ')

    #  Force Iterations
    b_forceIterHead = tc.Block(" BEGIN FORCE ITERATIONS",
                               " -----------------------",
                               "",
                               "",
                               start=tc.Trigger(
                                   lambda x: " BEGIN FORCE ITERATIONS" in x,
                                   delay=0))
    ...
    t_forceIterTableStart = tc.Trigger(lambda x: "ITER    FSQR      FSQZ" in x,
                                       delay=1)
    ...
    t_profileHeadStart = tc.Trigger(lambda x: "JSUPU" in x and "BSUBU" in x,
                                    delay=0)
    ...
    b_magField = tc.Block(
        ...
        " bpol**2 /(2 mu0) =   {PoloidalMagneticEnergyIntegral:.6e}  {PoloidalMagneticEnergyAverage:.6e}",
        " btor**2/(2 mu0)  =   {ToroidalMagneticEnergyIntegral:.6e}  {ToroidalMagneticEnergyAverage:.6e}",
        " b**2/(2 mu0)     =   {MagneticEnergyIntegral:.6e}  {MagneticEnergyAverage:.6e}",
        " EKIN (3/2p)      =   {KineticEnergyIntegral:.6e}  {KineticEnergyAverage:.6e}",
        start=tc.Trigger(lambda x: "Magnetic Fields and Pressure" in x,
                         delay=0))

    #  Magnetic Axis Coefficients
    t_axis_start = tc.Trigger(lambda x: "MAGNETIC AXIS COEFFICIENTS" in x,
                              delay=4)
    t_axis_stop = tc.Trigger(lambda x: not x, delay=0, skip=1)
    b_axis = tc.Table(" {na:4d} {rac:11.4e} {zas:11.4e}",
                      start=t_axis_start,
                      stop=t_axis_stop)

    t_boundary_start = tc.Trigger(
        lambda x:
        "nb  mb      rbc         zbs      vacpot_s  |B|_c(s=.5) |B|_c(s=1.)" in
        x,
        delay=2)
    ...
    #  VMEC file
    b_file = tc.Block(" FILE : {vmecFile}",
                      start=tc.Trigger(lambda x: 'FILE :' in x))

    #  Computational time
    b_time = tc.Block("    TOTAL COMPUTATIONAL TIME (SEC) {totalTime:12.2f}",
                      start=tc.Trigger(lambda x: 'TOTAL COMPUTATIONAL' in x))

    inst = tc.Transcoding(b_header, b_pressureProfileHead, b_pressureProfile,
                          l_forceIterLoop, b_profileHead, b_profileTable,
                          b_geoMagnetic, b_magField, b_axis, b_boundary, b_file,
                          b_time)
    return inst
