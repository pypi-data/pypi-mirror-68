...
class ConnectionLength(Points3D):
    ...
    __slots__ = ['coord_sys', 'name',
                 'parts', 'elements', 'lengths', 'mm_ids']
    __slot_setters__ = [tfields.bases.get_coord_system_name, None,
                        None, None, None, '_set_mm_ids']

    def __new__(cls, tensors, **kwargs):
        # TODO(@dboe): this should be unneccesarry if u just use a field, idiot
        if isinstance(tensors, list):
            if w7x.is_w7x_instance(tensors, cls.get_ws_class()) or \
...
class ComponentLoad(object):
    ...
    def mm_ids(self):
        ...
        try:
            ...
            logging.error(" No mm_ids specified")
...
class Grid(Base):
    ...
    """
    prop_defaults = {'cylindrical': None,
                     'hybrid': None,
                     'afsFileName': None,
                     'gridField': None,
...
class Machine(Base):
    ...
    """
    prop_defaults = {'meshedModels': None,
    ...
    def __init__(self, *args, **kwargs):
        ...
        if len(args) == 1 and w7x.is_w7x_instance(args[0], self.get_ws_class()):
            ...
            # meshed_models_ids = kwargs.pop('meshedModelsIds', [])
            # mm_ids = []
            # for arg in args:
            #     if issubclass(type(arg), tfields.Mesh3D):
            #         meshed_models.append(MeshedModel(arg))
            #     elif isinstance(arg, string_types):
            #         meshed_models.append(tfields.Mesh3D.load(arg))
            #     elif isinstance(arg, int):
            #         mm_ids.append(arg)
            #     else:
            #         arg_type = type(arg)
            #         raise NotImplementedError("No valid argument type "
            #                                   "'{arg_type}'.".format(**locals()))

            # for mm_id in mm_ids:
            #     ws_mm_id, own_mm_id = interprete_mm_id(mm_id)
            #     if own_mm_id is None:
            #         meshed_models_ids.append(mm_id)
            #     else:
            #         meshed_models.append(MeshedModel.from_mm_id(mm_id))

            # kwargs['meshedModels'] = meshed_models
            # kwargs['meshedModelsIds'] = meshed_models_ids
        ...
        for i in range(len(meshed_models)):
            ...
            if not isinstance(meshed_models[i], MeshedModel):
                raise ValueError("meshed model of wrong type {0}"
                                 .format(type(mm)))
        mm_ids_db = []
        mm_ids_user = []
    ...
    def meshed_models(self,
        ...
        for mm_id in self.get_mm_ids(user_defined=user_defined, pre_defined=False):
            ...
            meshed_models.append(meshedModel)
    ...
    def intersect_mesh_phi_planes(self, phi_list_rad):
        ...
        if not type(phi_list_rad) is list:
            raise TypeError(
                "phi_list_rad has to be of type list but is of type {}"
                .format(type(phi_list_rad)))
        ...
        phi_container = []
        for iPhi, phi in rna.log.progressbar(zip(range(len(phi_list_rad)),
                                                 phi_list_rad)):
            mm_id_container = []
            for mm_id in self.mm_ids:
                ...
                if type(res) is list:
                    if res[0] is None:
                        ...
                        for intersection in res:
                            ...
                            vertex_points = Points3D(intersection.vertices,
                                                     coord_sys=tfields.bases.CARTESIAN)
                    ...
                    logging.debug("Result was None. Probably there was no intersection of the mesh with this plane.")
                else:
...
class MagneticConfig(Base):
    ...
    Examples:
        ...
        (MagneticConfig){
            ...
            grid = (Grid){
                       ...
                       hybrid = None (CylindricalGrid)
                       afsFileName = //share.ipp-hgw.mpg.de/mp/fieldline/dboe/test.dat
                       gridField = None (Points3D)
                       fieldSymmetry = 5
                   }
    ...
    def from_currents(cls,
        ...
        if len(currents) < 14:
            if len(currents) < 5:
                if unit == 'rw':
                    currents = [1.] * 5  # standard case
                else:
                    raise ValueError("For all units except 'rw' we expect"
                                     " at least the planar coil currents")
            # TODO The following is dangerous in case default is changed
            currents = list(currents)
            currents = currents + [0.] * (14 - len(currents))
        ...
        if not len(kwargs['coilsIds']) == len(currents) == n_total_expected:
            raise ValueError("#Coils: {0}, \t#Currents: {1}, \texpected: {2}"
                             .format(len(kwargs['coilsIds']),
                                     len(currents),
                                     n_total_expected)
                             )
    ...
    def from_afs_file(cls, file_path, **kwargs):
        ...
        if file_path.startswith(w7x.Defaults.Paths.tracer_network_share):
            file_path = file_path.lstrip(
                w7x.Defaults.Paths.tracer_network_share)
    ...
    @classmethod
    def from_dat_file(cls, file_path_dat,
                      network_share=w7x.Defaults.Paths.tracer_network_share,
                      network_share_mount=\
                        w7x.Defaults.Paths.tracer_network_share_mount,
                      network_share_workgroup=\
                        w7x.Defaults.Paths.tracer_network_share_workgroup,
                      network_share_relative_path=\
                          w7x.Defaults.Paths.tracer_network_share_relative_path,
                      **kwargs):
        """
        ...
        if os.name == 'posix':
            if os.path.exists(network_share_mount):
                local_mount_file_path_dat = rna.path.resolve(
                    network_share_mount, network_share_relative_path,
                    os.path.basename(file_path_dat))
        ...
        if os.path.exists(local_mount_file_path_dat):
            ...
            # Copy file to nas
            logging.info('Copy {file_path_dat} to {local_mount_file_path_dat}'
                     .format(**locals()))
            ...
            import subprocess
            logging.info('Copy {file_path_dat} to {network_file_path_dat}'
                     .format(**locals()))
            remote_path = os.path.join(network_share_relative_path,
                                       os.path.basename(file_path_dat))
            remote_path = remote_path.replace('/', '\\')  # nt compatible
            smbclient_cmd = [
                "smbclient", network_share.rstrip('/'),
                "-k",  # kerberos!
            ]
            smbclient_cmd = [x.encode('utf8') for x in smbclient_cmd]
            command_list = ['put']
            command_list.extend(u'"%s"' % arg
                                for arg in [file_path_dat, remote_path])
            ...
            while attempts < n_tries:
                ...
                if p.returncode != 0:
                    logging.warning(u"Attempting to copy by smbclient failed. "
                                u"Number of attemps: {0} (cmd: {1})"
                                .format(attempts,
                ...
                raise OSError(u"Error on %r: %r"
                              % (b' '.join(cmd).decode('utf8'), result))
    ...
    def coil_currents(self, unit='rw'):
        ...
        if unit == 'r':
            ...
            currents = currents * 1. / currents[0]
            return currents
        ...
        tcs = np.array(tcs)
        I_n = npcs[0]
        if (len(set(scs[[i for i in range(10) if i % 2 == 0]])) == 1
        ...
        if unit == 'A':
            return (
                list(npcs * 1. /
                     w7x.Defaults.MagneticConfig.n_windings_npcs)
                + list(plcs * 1. /
                       w7x.Defaults.MagneticConfig.n_windings_plcs)
                + list(scs * 1. /
                       w7x.Defaults.MagneticConfig.n_windings_scs)
                + list(tcs[:1] * 1. /
                       w7x.Defaults.MagneticConfig.n_windings_tcs_a)
                + list(tcs[1:2] * 1. /
                       w7x.Defaults.MagneticConfig.n_windings_tcs_b)
                + list(tcs[2:] * 1. /
                       w7x.Defaults.MagneticConfig.n_windings_tcs_a)
            )
        if unit == 'rw':
    ...
    def geiger_string(self):
        ...
        """
        return "{0:-04d}_{1:-04d}_{2:-04d}_{3:-04d}_{4:0=+5d}_{5:0=+5d}" \
                .format(*[int(x * 1000) for x in self.coil_currents()[1:8]])
    ...
    def poincare_in_phi_plane(self, phi_list_rad,
        ...
        nFailed = 0
        logging.info(
            "Running through Seeds. Tracing each seed for all given phi."
        )
        for i_seed in range(len(seeds)):
            seed_point = seeds[i_seed:i_seed + 1]
            res = w7x.run_service(field_line_server.service.trace,
                                  seed_point.as_input(),
                                  self.as_input(),
                                  task,
                                  errors=errors)
            ...
            if res is None:
                logging.warning("Turn {0} of {1}: fail"
                                .format(i_seed + 1, len(seeds)))
                nFailed += 1
                continue
            else:
                nSuccess += 1
        ...
        if nFailed > 0:
            logging.warning(
                "Trace finished with {0} succeded and {1} failed."
                .format(nSuccess, nFailed))
        else:
            logging.info(
                "Trace finished with {0} succeded and {1} failed."
                .format(nSuccess, nFailed))
    ...
    def magnetic_characteristics(self, points3D,
                                 taskStepSize=0.2, return_type=list):
        """
        Args:
        ...
        task.characteristics.axisSettings = field_line_server.types.AxisSettings()  # NOQA

        # run web service
        logging.info("Retrieving MagneticCharacteristics with the points given.")
        # the None after 'task' can be machine boundary.
        result = w7x.run_service(field_line_server.service.trace,
                                 points3D.as_input(),
                                 self.as_input(),
                                 task, None, None)
    ...
    def iota(self, points3D, taskStepSize=0.2, return_type=list):
        ...
        Examples:
            ...
            >>> assert tf.fields[0].equal([0.9633], atol=1e-4)

        """
        mag_char = self.magnetic_characteristics(
            points3D, taskStepSize, return_type)
        if return_type is list:
    ...
    def find_axis_at_phi(self, phi=0, size=w7x.Defaults.Poincare.stepSize, settings=None):
        ...
        result = w7x.run_service(field_line_server.service.findAxisAtPhi,
                                phi,
                                size,
                                self.as_input(),
                                settings)
    ...
    def magnetic_field(self, points3D):
        ...
        result = w7x.run_service(field_line_server.service.magneticField,
                                 points3D.as_input(),
                                 self.as_input())
    ...
    def plot_poincare(self, phi=0, seeds=None, numPoints=300, errors=None, **kwargs):
        ...
        surfs = self.poincare_in_phi_plane([phi], seeds=seeds,
                                           numPoints=numPoints, errors=errors)
...
class Run(object):
    ...
    def find_lcfs(self, step=0.001, settings=None, max_time=None):
        ...
        lCFSPoint = w7x.run_service(field_line_server.service.findLCFS,
                                   step,
                                   self.magnetic_config.as_input(),
                                   self.machine.as_input(),
                                   settings,
                                   max_time=max_time)  # find last closed flux surface
    ...
    def line_tracing(self, points, step=0.01, numSteps=300):
        ...
        res = field_line_server.service.trace(points.as_input(),
                                              config,
                                              task,
                                              None)
        logging.info("... done")
    ...
    def line_diffusion(
        ...
        fldTask = field_line_server.types.Task()
        fldTask.step = size
        ...
        fldTask.diffusion = line_diffusion
        fldTask.connection = field_line_server.types.ConnectionLength()
        ...
        if start_points is None and launchPointsPath is None:
            logging.info("Creating random start lcfs_init_points on last closed flux surface")
        ...
        for iShift, additionalShift in enumerate(additiona_shifts):
            if iShift == 0 and start_points is not None:
                ...
                logging.info(" Startpoint: {lcfs_init_points}".format(**locals()))

                traceTask = field_line_server.types.Task()
                traceTask.step = 0.5
                line = field_line_server.types.LineTracing()
                line.numSteps = nStart - 1
                traceTask.lines = line

                logging.info("Trace this point " + str(line.numSteps) + "-times.")
                res = w7x.run_service(field_line_server.service.trace,
                                     lcfs_init_points.as_input(), config, traceTask, machine, max_time=None)
            ...
            resForward = w7x.run_service(field_line_server.service.trace,
            ...
            if len(resForwardClass) == nStart:
                logging.info(
                    "Length {} accepted!".format(len(resForwardClass)))
                ...
                logging.error(
                    "Also after shift of {} number of H".format(
                        sum(additiona_shifts)))
                raise ValueError("Also after shift of %s number of H" %
                                 sum(additiona_shifts))
            else:
                logging.warning(
                    "Restart the forward tracing with a new lCFSInitPoint "
                    "offset of {0} since the number of output points ({1}) "
                    "does not meet opts.nRevolutions({2})"
                    .format(sum(additiona_shifts[:iShift + 2]),
                            len(resForwardClass), nStart))

        logging.info("Line diffusion backwards.")
        config.inverseField = True
        # max_time estimate: int(round(line.numSteps / 4000. * 2000)))
        resInvers = w7x.run_service(field_line_server.service.trace,
                                    start_points.as_input(),
                                    config,
                                    fldTask,
                                    machine,
...
if __name__ == "__main__":
    import doctest
    # doctest.run_docstring_examples(MagneticConfig.iota, globals())
    # quit()
    doctest.testmod()
