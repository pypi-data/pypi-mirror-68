...
class Cms:
    ...
    class SigAlg:
        ...
        DEFAULT = 0  #: Use default signature algorithm (``rsa-sha1``/``sha1WithRSAEncryption``)
        RSA_SHA1 = 0  #: Use sha1WithRSAEncryption (rsa-sha1) signature algorithm (default)
        RSA_SHA224 = 6  #: Use sha224WithRSAEncryption (rsa-sha224) signature algorithm
        RSA_SHA256 = 3  #: Use sha256WithRSAEncryption (rsa-sha256) signature algorithm
        RSA_SHA384 = 4  #: Use sha384WithRSAEncryption (rsa-sha384) signature algorithm
        RSA_SHA512 = 5  #: Use sha512WithRSAEncryption (rsa-sha512) signature algorithm
        RSA_MD5 = 1  #: Use md5WithRSAEncryption (rsa-md5) signature algorithm (for legacy applications - not recommended for new implementations)
        RSA_PSS_SHA1 = 0xB0    #: Use RSA-PSS signature algorithm with SHA-1
        RSA_PSS_SHA224 = 0xB6  #: Use RSA-PSS signature algorithm with SHA-224
        RSA_PSS_SHA256 = 0xB3  #: Use RSA-PSS signature algorithm with SHA-256
        RSA_PSS_SHA384 = 0xB4  #: Use RSA-PSS signature algorithm with SHA-384
        RSA_PSS_SHA512 = 0xB5  #: Use RSA-PSS signature algorithm with SHA-512
...
class Ecc:
    ...
    def read_key_by_curve(keyhex, curvename):
        ...
        Returns:
            ...
            An EC private key ``w`` is represented as ``HEX(w)``
            and a public key ``(x,y)`` in the uncompressed X9.63 form ``04||HEX(x)||HEX(y)``.
        """
        nc = _dipki.ECC_ReadKeyByCurve(None, 0, keyhex.encode(), curvename.encode(), 0)
        if (nc < 0): raise PKIError(-nc)
        buf = create_string_buffer(nc + 1)
        nc = _dipki.ECC_ReadKeyByCurve(buf, nc, keyhex.encode(), curvename.encode(), 0)
        return buf.value.decode()[:nc]
...
class X509:
    ...
    class SigAlg:
        ...
        RSA_SHA1   = 0  #: Sign with sha1WithRSAEncryption (rsa-sha1) (default)
        RSA_SHA224 = 6  #: Sign with sha224WithRSAEncryption (rsa-sha224)
        RSA_SHA256 = 3  #: Sign with sha256WithRSAEncryption (rsa-sha256)
        RSA_SHA384 = 4  #: Sign with sha384WithRSAEncryption (rsa-sha384)
        RSA_SHA512 = 5  #: Sign with sha512WithRSAEncryption (rsa-sha512)
        RSA_MD5    = 1  #: Sign with md5WithRSAEncryption (rsa-md5)
