...
def init(agree=None, src_path=None, dst_path=None):
    ...
    pip_conf = utils.input_('please pip.conf path (default: ~/.pip/pip.conf): ') # not tested
    if len(pip_conf) == 0: # not tested
    ...
    with open(f'{dst_path_}info.py', 'w') as fd:
        ...
        fd.write(lines)
    shutil.copyfile(pip_conf, './deployments/.secrets/pip.conf')  # not tested
