...
def init(agree=None, src_path=None, dst_path=None):
    ...
    author_name = utils.input_('please enter author name: ') # not tested
    author_email = utils.input_('please enter author\'s email name: ') # not tested
    description = utils.input_('please enter description of service: ') # not tested
    pip_conf_path = utils.input_('please pip.conf path (default: ~/.pip/pip.conf). You can read about pip conf here (https://github.com/U-Company/python-service-layout/blob/master/deployments/README.md): ') # not tested
    print(len(pip_conf_path), pip_conf_path)
    ...
    with open(f'{dst_path_}info.py', 'w') as fd:
        lines = lines.replace('{{prj_name}}', prj_name)
        lines = lines.replace('{{author}}', author_name)
        lines = lines.replace('{{author_email}}', author_email)
        lines = lines.replace('{{description}}', description)
        fd.write(lines)
    os.listdir(os.path.expanduser('~') + pip_conf_path)
    shutil.copyfile(os.path.expanduser('~') + pip_conf_path, './deployments/.secrets/pip.conf') # not tested
    shutil.rmtree('.git') # not tested
    for root, dirs, files in os.walk(src_path): # not tested
    ...
    with open('README.md', 'w') as fd: # not tested
