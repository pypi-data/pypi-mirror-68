...
if __name__ == '__main__':
    ...
    res = pickle.load(open('tests/res_2020-05-05_02-14-53.pickle', 'rb'))

    rvecs, tvecs, checkerboard_points, points_2D = dict(np.load(parameters['ba_initialization_file'])).values()

    checkerboard_points = checkerboard_points.reshape(checkerboard_points.shape[0], checkerboard_points.shape[2], checkerboard_points.shape[3], 1)

    image_size = parameters['image_size']

    cm_shape = parameters['cm_shape']

    n_images = len(rvecs)

    start_grid = old_grid_creation()

    spline_grid = res['spline_grid']
    rvecs = res['rvecs']
    tvecs = res['tvecs']

    ba_params = np.hstack((np.array(spline_grid).ravel(), np.array(rvecs).ravel(), np.array(tvecs).ravel()))

    residuals_2D = residuals_2d(ba_params, parameters, n_images, points_2D, image_size, cm_shape, checkerboard_points)

    ssq = np.sum(residuals_2D**2)

    rms = np.sqrt(residuals_2D.dot(residuals_2D)/residuals_2D.size)

    print('ssq: ', ssq)

    print('Least squares for 2D residuals: ', rms)
