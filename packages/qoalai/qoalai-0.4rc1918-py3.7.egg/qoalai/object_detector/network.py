...
class YoloNetwork(object): 
    ...
    def build_yolov3_net2(self, inputs, network_type, is_training):
        ...
        def yolo_layer(inputs, n_classes, anchors, img_size, data_format):
            ...
            print ("================ Yolo Layer ==================")
            print ("box center: ", box_centers)
            print ("box_shapes: ", box_shapes)
        ...
        def build_boxes(inputs):
            ...
            if self.is_multilabel: 
                ...
                for i in self.multilabel_dict.values(): 
                    additional_class.append(classes_all[:, :, base:base + i])
                    base = base + i
            else: 
                classes = classes_all
            ...
            if self.lite:
                ...
                for idx, i in additional_class: 
                    tmp = tf.boolean_mask(i, tf.squeeze(selected_mask, 2), axis=0)
                    additional_class[idx] = tf.reshape(tensor=tmp, shape=(-1, total_num, self.multilabel_dict.values()[idx]))
                    print (additional_class[idx])
