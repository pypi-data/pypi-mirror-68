...
import tlclient.linker.comm_pb2 as comm_pb2 
...
class Fist:
    ...
    def create_fist(self):
        ...
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_REQ_FIST_CREATE)
        f.set_nano(Timer.nano())
        req = comm_pb2.ReqFistCreate()
        ...
        ret_obj = ret.get_obj(comm_pb2.RspFistCreate)
        if ret_obj.source <= 0:
    ...
    def _set_curve_server_option(self, s):
        if self._curve_server_secret_key:
            ...
            self.logger.debug(f'[_set_curve_server_option] succeed')
    ...
    def _set_curve_client_option(self, s):
        if self._curve_server_key:
            ...
            self.logger.debug(f'[_set_curve_client_option] succeed')
    ...
    def _reg_req(self, other_fist_name, addr: str):
        ...
        if not IS_NEW_VERSION:
            if other_fist_name == MASTER_FIST_NAME:
                sock.setsockopt(zmq.ROUTING_ID, bytify(self.fist_name))
        else:
            # routing_id: fist_name + ' ' + fist_type
            routing_id = self.fist_name + ' ' + str(FistType.parse(self.fist_type))
    ...
    def _req_fist_infos(self):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_GET_STATUS)
        ...
        with self.req_sock_locks[fist_name]:
            ...
            try:
                ...
                if e.errno == 156384763 and retries > 0:
                    self.logger.warning('[req] invalid sock state, to recreate sock and resend frame (mt){}'.format(frame.get_msg_type()))
    ...
    def notify(self, title, content, notification_type):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_REQ_NOTIFY)
        f.set_source(self.get_source_id())
        f.set_nano(Timer.nano())
        req = comm_pb2.ReqNotify()
        req.title = title
        req.message = content
        ...
        if ret is None:
            ...
            return False
        rsp_obj = ret.get_obj(comm_pb2.RspNotify)
    ...
    def send_req_command(self, target_fist_name, content):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_CMD_REQUEST)
        f.set_source(self.get_source_id())
        f.set_nano(Timer.nano())
    ...
    def send_rsp_command(self, request_id, content):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_CMD_RESPONSE)
        f.set_source(self.get_source_id())
        f.set_nano(Timer.nano())
    ...
    def on_pub_master(self, f):
        ...
        if msg_type == frame_pb2.MsgType.MSG_TYPE_CMD_SUICIDE:
            suicide_req = f.get_obj(comm_pb2.ReqFistSuicide)
            fist_name = suicide_req.fist_name
            if fist_name in [ANY_FIST_NAME, self.fist_name]:
                self.stop()
        elif msg_type == frame_pb2.MsgType.MSG_TYPE_CMD_REQUEST:
            s = f.get_string()
        ...
        elif msg_type == frame_pb2.MsgType.MSG_TYPE_CMD_RESPONSE:
            s = f.get_string()
        ...
        elif msg_type == frame_pb2.MsgType.MSG_TYPE_FIST_ADDR_UPDATE:
            info = FistAddrUpdate(**json.loads(f.get_string()))
            if info.comm_type == comm_pb2.CommType.COMM_TYPE_ZMQ_PUB and \
            ...
            elif info.comm_type == comm_pb2.CommType.COMM_TYPE_ZMQ_REP and \
    ...
    def get_bind_addr(self, comm_type):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_REQ_FIST_SET)
        f.set_source(self.get_source_id())
        f.set_nano(Timer.nano())
        req = comm_pb2.ReqFistSet()
        req.comm_type = comm_type
        f.set_data(req)
        ret = self.req_master(f)
        rsp_obj = ret.get_obj(comm_pb2.RspFistSet)
    ...
    def get_connect_addr(self, fist_name, comm_type):
        f = Event.new_pb_frame()
        f.set_msg_type(frame_pb2.MsgType.MSG_TYPE_REQ_FIST_REG)
        f.set_source(self.get_source_id())
        f.set_nano(Timer.nano())
        req = comm_pb2.ReqFistReg()
        req.fist_name = fist_name
        req.comm_type = comm_type
        f.set_data(req)
        ret = self.req_master(f)
        rsp_obj = ret.get_obj(comm_pb2.RspFistReg)
