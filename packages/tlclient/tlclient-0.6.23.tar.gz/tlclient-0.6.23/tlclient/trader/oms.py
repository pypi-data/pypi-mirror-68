    ...
    def _proc_msg_queue(self):
        while not self.is_stopped() or not self.msg_queue.empty():
            ...
            if isinstance(obj, message_pb.ReqOrderInsert):
                order_info = self.oms_helper.on_req_order_insert(obj)
            elif isinstance(obj, message_pb.RspOrderInsert):
                order_info = self.oms_helper.on_rsp_order_insert(obj)
                ...
                order_info = self.oms_helper.on_rsp_order_cancel(obj)
            elif isinstance(obj, message_pb.RtnOrder):
                order_info = self.oms_helper.on_rtn_order(obj)
            ...
            if order_info is not None:
                self.db_helper.save(order_info)
