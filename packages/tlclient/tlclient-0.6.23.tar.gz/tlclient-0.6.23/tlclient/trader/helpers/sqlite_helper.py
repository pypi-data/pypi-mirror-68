...
from ..database.sqlite_models import order_info_db, rtn_order_db, rtn_trade_db, position_db, capital_db
from ..database.sqlite_models import OrderInfo, RtnOrder, RtnTrade, Position, Capital
...
class OrderHelper(BaseHelper):
    ...
    def __init__(self, db_path):
        BaseHelper.__init__(self, db_path, order_info_db, OrderInfo, message_pb.RspOrderInfo)

    def save_obj(self, obj: message_pb.RspOrderInfo):
        obj_dict = MessageToDict(obj, preserving_proto_field_name=True, use_integers_for_enums=True)
...
class DBHelper:
    ...
    def __init__(self, db_path):
        ...
        self._order_info_helper = OrderHelper(os.path.join(db_path, 'order_info.db'))
        ...
        self.thread_pool.submit(self._proc_queue, self._order_info_queue, self._order_info_helper)
    ...
    def save(self, obj):
        if isinstance(obj, message_pb.RspOrderInfo):
            if obj.order_id:
                self._order_info_queue.put(obj)
    ...
    def get_order_info(self, order_id):
        obj = self._order_info_helper.get_objs(order_id=order_id)
        return obj[0] if obj else None

    def get_history_order_info(self, account_id=None, sub_account=None):
        return self._order_info_helper.get_objs(account_id=account_id, sub_account=sub_account)
