...
class OceanAgreements:
    ...
    @deprecated("Use `create` instead of this.")
    ...
    def create(self, did, index, agreement_id,
               ...
               account, provider_address=None, auto_consume=False):
        """
        ...
        """
        assert consumer_address and Web3Provider.get_web3().isChecksumAddress(consumer_address), \
            f'Invalid consumer address {consumer_address}'

        asset = self._asset_resolver.resolve(did)
        asset_id = asset.asset_id
        service_agreement = asset.get_service_by_index(index)
        if service_agreement.type not in (ServiceTypes.ASSET_ACCESS, ServiceTypes.CLOUD_COMPUTE):
            raise Exception('The agreement could not be created. Review the index of your service.')

        template_id = self._keeper.get_agreement_template_id(service_agreement.type)
        try:
            agreement_template_approved = self._keeper.template_manager.is_template_approved(template_id)
            if not agreement_template_approved:
                msg = (f'The agreement template for Service type {service_agreement.type} and templateId '
                       f'{template_id} is not approved and cannot be used for creating service agreements.')
                logger.debug(msg)
        except Exception as err:
            msg = f'Checking approval of agreement template with id {template_id} and ' \
                  f'name {self._keeper.template_manager.SERVICE_TO_TEMPLATE_NAME[service_agreement.type]} ' \
                  f'failed: {err}. \nCreate agreement will continue anyway.'
            logger.debug(msg)

        existing_agreement = self._keeper.agreement_manager.get_agreement(agreement_id)
        if existing_agreement and existing_agreement.owner:
        ...
        if not provider_address:
            providers = self._keeper.did_registry.get_did_providers(asset_id)
            provider_address = providers[0] if providers else publisher_address

        assert provider_address != consumer_address, f'provider and consumer accounts are the same, this will cause createAgreement to fail.'
        template_mgr = self._keeper.template_manager
        ...
        actor_type_map = {'provider': provider_address, 'consumer': consumer_address}
        actors = [actor_type_map[actor_type] for actor_type in actor_types]
        success = self._keeper.agreement_manager.create_agreement(
            agreement_id,
            asset_id,
            template_id,
            condition_ids,
        ...
        if not success:
            ...
            did_owner = self._keeper.agreement_manager.get_agreement_did_owner(agreement_id)
            if did_owner:
                success = True
            else:
                event_log = self._keeper.agreement_manager.subscribe_agreement_created(
    ...
    def _process_consumer_agreement_events(
        ...
        self._keeper.agreement_manager.subscribe_agreement_created(
            agreement_id,
            300,
            fulfillLockRewardCondition,
            (agreement_id, service_agreement.get_price(), account, condition_ids[0]),
    ...
    def is_access_granted(self, agreement_id, did, consumer_address):
        ...
        """
        agreement_consumer = self._keeper.get_agreement_consumer(agreement_id)
