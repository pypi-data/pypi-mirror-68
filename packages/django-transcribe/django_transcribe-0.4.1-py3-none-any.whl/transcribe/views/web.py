        ...
        if len(context['projects']) < max_tasks:
            ...
            for project in projects_available_review:
                ...
                if pending_task:
                    continue
                # add the task
                # if project.available_review_task(user):
        ...
        if len(context['projects']) < max_tasks:
            ...
            for project in projects_available_transcription:
                ...
                if pending_task:
                    continue
                # add the task
                # if project.available_transcription_task(user):
...
class ProjectClaimTaskView(mixins.ActiveUsersOnlyMixin, View):
    def get(self, request, pk=None, type=None):
        ...
        if not failed and user_task:
            ...
            messages.error(request, 'Could not claim task.')
            return redirect('dashboard')
...
class ProjectListView(
    ...
    def get_context_data(self, **kwargs):
        ...
        for project in context['projects']:
            project = _get_project_context(project, self.request.user)
        return context


class ProjectDetailView(mixins.ActiveUsersOnlyMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        project = _get_project_context(self.object, self.request.user)
