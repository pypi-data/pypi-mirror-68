...
def get_version():
    ...
    if repo.tags:
        # get the latest tag
        latest_tag = sorted(
            repo.tags, key=lambda t: t.commit.committed_date, reverse=True)[0]
        ...
        if repo.head.commit.hexsha == latest_tag.commit.hexsha:
            ...
            commit_count_since_latest_tag = len(
                list(repo.iter_commits(max_age=latest_tag.commit.authored_date))) - 1
...
# the actual setup
setup(install_requires=['simple-classproperty==1.1'],
