...
class ACIProcessorQueueControllerABC(metaclass=ABCMeta):
    ...
    def __init__(self, ormSessionCreator,
        ...
        self._nextDedupeTime = datetime.now(pytz.utc) \
                               + self._randomiseTimeDelta(self.DEDUPE_PERIOD_SECONDS)
        ...
        assert self.DEDUPE_PERIOD_SECONDS, "ACI, DEDUPE_PERIOD_SECONDS is missing"
    ...
    def _poll(self):
        ...
        # Wait for the dedupe to complete
    ...
    def _fetchBlocksInPg(cls, plpy,
                         ...
                         queueCount: int) -> List[ACIProcessorQueueBlockItem]:
        # ---------------
        ...
        for start in range(0, len(queueItems), cls.QUEUE_ITEMS_PER_TASK):
            ...
            itemUniqueIds = list(latestUpdates.keys())
    ...
    def _dedupeQueue(self):
        if datetime.now(pytz.utc) < self._nextDedupeTime:
            return
    ...
    def _dedupeQueueWrapped(self):
        ...
        try:
            ...
            self._logger.error("Deduplication failed")
